...
- Historical normal Steam-client startup/log verification at that time confirmed `Spire Plus (EZMicroBalance)`, config registration, 16 previous saved-state registrations, and a clean main-menu load. Current beta.93 startup/log evidence is RitsuLib-only loader proof and does not replace the still-pending live Mod Settings page refresh.
