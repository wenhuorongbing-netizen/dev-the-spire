...
Current boundary: Spire Plus is RitsuLib-only for beta.125 on Slay the Spire 2
`v0.107.1` with `STS2.RitsuLib` / `STS2-RitsuLib` `0.4.34`. beta.125 package
parity, runtime preflight, and source-workspace validation are current
no-launch evidence; beta.125 still needs game-launch/runtime patch-count and
