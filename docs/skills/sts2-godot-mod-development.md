...
- Prefer local game command APIs, RitsuLib APIs, template APIs, and package references before Harmony patches. BaseLib is historical or other-mod context only for current Spire Plus work unless the owner explicitly approves a new dependency decision.
...
2. Inspect local RitsuLib/template APIs or package references when available; use BaseLib references only as historical or other-mod context unless a new owner-approved dependency decision exists.
