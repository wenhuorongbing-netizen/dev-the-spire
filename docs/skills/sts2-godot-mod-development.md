...
Use this repository-local skill/reference guide when working on Spire Plus (`EZMicroBalance` manifest id) or future Slay the Spire 2 Godot/.NET mods in this workspace.
...
- Prefer local game command APIs, RitsuLib APIs, template APIs, and package references before Harmony patches. Do not add another shared runtime framework dependency for current Spire Plus work unless the owner explicitly approves a new dependency decision.
...
- Preserve `Spire Plus` as the current player-facing display name while keeping `EZMicroBalance` as the stable manifest id/package id unless a separate migration plan is approved.
...
2. Inspect local RitsuLib/template APIs or package references when available; do not use another shared runtime framework unless a new owner-approved dependency decision exists.
...
- A prior smoke log is stale after source changes that alter previous saved-state registrations, hook registration, resources, or package contents.
