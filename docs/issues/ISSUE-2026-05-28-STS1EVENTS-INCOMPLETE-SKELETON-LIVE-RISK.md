...
2026-06-20 correction: historical loader proof exists for Off, CanaryOnly, and AdditiveBatch1. Retained beta.85 `v0.107.0` proof covers default-Off plus CanaryOnly loader registration only as previous-package/game-version context, while current beta.91 RitsuLib-only proof covers Off plus AdditiveBatch1 loader/registration shape only. Event gameplay, save-load, screenshot, replacement functional proof, and two-client co-op/fail-closed proof remain pending or blocked.
...
## Current State (updated 2026-06-20 from beta.91 RitsuLib-only loader-gate evidence)
