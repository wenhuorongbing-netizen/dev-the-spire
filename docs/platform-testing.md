...
```powershell
Get-FileHash -LiteralPath .\publish\SpirePlus-v0.1.0-private-beta.92.zip -Algorithm SHA256
```

Unpack a package for inspection:

```powershell
Expand-Archive -LiteralPath .\publish\SpirePlus-v0.1.0-private-beta.92.zip -DestinationPath .\publish\inspect -Force
```
...
```sh
shasum -a 256 publish/SpirePlus-v0.1.0-private-beta.92.zip
```

Unpack a package for inspection:

```sh
unzip -q publish/SpirePlus-v0.1.0-private-beta.92.zip -d publish/inspect
```
