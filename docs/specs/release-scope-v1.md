...
| --- | --- | --- |
| Spire Plus | Manual-test build now; release candidate only after live evidence | Automation is current for beta.135 and previous beta.128 forced Ancient clicked UI smoke passed, but gameplay, save/load, failure/death, hover/readability follow-through, gated Vakuu fight-option, and co-op rows remain pending. |
...
| Ancient Urda/Morvi/Lotha | Manual-test candidates | Source-backed and visible reward markers exist; previous beta.128 forced clicked UI smoke exists, while gameplay, hover/readability follow-through, save/load, and co-op proof remain pending. |
