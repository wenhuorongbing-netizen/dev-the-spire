...
| Morvi/Lotha state | `MorviBlessingService.State.cs`, `MorviBlessingService.CombatState.cs`, `LothaBlessingService.State.cs`, `LothaBlessingService.DeathReprieveState.cs`, `LothaBlessingService.DeathReprieveTurn.cs` | Deck/player mirrors mitigate persistent state loss; transient combat state and Death Reprieve restore state remain lifecycle-owned and live save/load still required. |
