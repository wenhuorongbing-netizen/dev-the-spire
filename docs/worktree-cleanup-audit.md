...
| Current worktree batch pathspecs | `.tools/worktree-batches/current/` | Ignored local staging aids generated by `scripts/report-worktree-batches.ps1`; latest 2026-05-26 snapshot has 50 dirty entries and 0 unclassified paths after beta.32 status/docs/website surface changes, Soul Tide localization/source-design text, release-evidence package-path/hash, and current-package test helper cleanup. |
...
| Current-package test expectations | `TestRepo.cs` exposes shared current-package helpers, and guard tests now use them instead of repeating the current beta.32 path and ZIP hash in package, platform, release-hash, handoff, and verifier assertions. | `ReleaseEvidenceGateTests`, `AncientBehaviorGuardTests`, `CrossPlatformTestingGuardTests`, `ReleaseHashGuardTests`, and `TestInfrastructureGuardTests` |
