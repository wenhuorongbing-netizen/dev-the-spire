...
| Current worktree batch pathspecs | `.tools/worktree-batches/current/` | Ignored local staging aids generated by `scripts/report-worktree-batches.ps1`; latest 2026-05-26 snapshot has 47 dirty entries and 0 unclassified paths after release-evidence package-path helper cleanup. |
...
| Release-evidence package artifact paths | Collector and verifier scripts now derive versioned package artifact paths from `EZMicroBalance.json` through `scripts/spire-plus-package-evidence.ps1` instead of repeating the same `publish\SpirePlus-v...` folder paths in every script. | `ReleaseEvidenceGateTests.ReleaseEvidenceScriptsDeriveVersionedPackageArtifactPathsFromManifest` |
...
- Release-evidence package-path helper validation: `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\spire-plus-package-evidence.ps1`, focused helper guard, full `ReleaseEvidenceGateTests` with `--no-build`, and the scripts README governance guard passed.
- Current batch classification: `scripts/report-worktree-batches.ps1 -FailOnUnclassified` passed with 47 dirty entries and 0 unclassified paths.
