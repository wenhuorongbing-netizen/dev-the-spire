...
| `.gitignore` | Keeps local/generated drafts out of release-candidate diffs. |
...
| Current manual-test display-name wording | Active manual verification and waiting-test instructions now say Spire Plus / `EZMicroBalance`; remaining `EZ Micro Balance` references are historical logs or explicitly historical evidence. | Default tests and targeted `rg` scan. |
...
| `website/` and `.github/workflows/spire-plus-site.yml` | Current `.gitignore` intentionally excludes both; `docs/PROJECT_MAP.md` and `docs/doc-inventory.md` record a website snapshot under `.tools/archive/local-website-preview-20260516/`. Active root copies have been deleted. | Keep deleted unless the owner promotes the website from the snapshot. | Keep deleted; restore from snapshot and promote after repair. | If promoting later, remove the ignore rules, restore from `.tools/archive/local-website-preview-20260516/`, repair the Pages workflow, add docs ownership, and run a local website build/smoke check. |
...
| `website/` and `.github/workflows/spire-plus-site.yml` | Deleted local draft after verifying `.tools/archive/local-website-preview-20260516/` snapshot; keep deleted unless deliberately promoted later. |
...
- `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build --filter FullyQualifiedName~ReleaseSafetyExpandedGuardTests`: pass, 18 passed / 0 skipped.
- `dotnet test EZMicroBalance.sln --no-build --filter FullyQualifiedName~ReleaseArtifactTests`: pass, 9 passed / 7 skipped.
- `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build --filter FullyQualifiedName~ReleaseArtifactTests`: pass, 16 passed / 0 skipped.
...
- Website draft deletion check: `Test-Path website` and `Test-Path .github\workflows\spire-plus-site.yml` return `False`; `.tools\archive\local-website-preview-20260516\website\index.html` and `.tools\archive\local-website-preview-20260516\.github\workflows\spire-plus-site.yml` return `True`; `git check-ignore -v website/ .github/workflows/spire-plus-site.yml` confirms both ignore rules remain.
