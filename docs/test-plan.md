...
- Dependencies include only `STS2-RitsuLib >= 0.4.31` for the shared runtime
  framework.
...
2. Open Settings.
3. Open `Mod Settings (RitsuLib)`.
4. Confirm `STS2-RitsuLib` is enabled.
5. Confirm Spire Plus appears under its release mod id `EZMicroBalance`.
6. Confirm Spire Plus can be enabled.
7. Confirm the enabled mod set for this lane contains only `STS2-RitsuLib`
   plus `EZMicroBalance`.
...
10. Inspect `godot.log` for `EZMicroBalance`, `STS2-RitsuLib`, old scaffold
    mod names, unexpected extra dependency requirements, `error`, and
    `exception`; specifically confirm no
...
The prior legacy `EzDailyContent` setup passed build, publish, and Mod
Settings verification on public beta `v0.104.0` (`2026.04.23`) under the old
scaffold dependency setup. That row is historical and is not a current Spire
Plus prerequisite.
...
Installed beta.97 package parity is the current package evidence. Earlier
...
AdditiveBatch1 13/14 mismatch remains root-cause history only.
...
- Historical package smoke/log/resource evidence under
  `.tools/runtime-evidence/current-package-smoke-20260514-015901` covers the
  earlier 22-field package, installed-PCK loading for Urda/Morvi/Lotha scenes
  plus 43 Ancient textures, and a clean normal Steam helper startup for that
  historical package line.
- Historical Steam-client loader evidence under
  `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` reports
  `v0.1.0-private-beta.19`, startup completion, clean log audit, stopped game,
  and restored mod isolation for the beta.19 package. Earlier beta.93
  RitsuLib-only Off/AdditiveBatch1 loader evidence is under
  `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/`
  and
  `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`;
  retained beta.85 `v0.107.0` evidence is older dependency/game-version
  context. Older beta.17, beta.13, `20260523-current`, 16-field, and 22-field
  startup/log passes are historical.
- Historical plug-off evidence under
  `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020`
  loaded without initializing Spire Plus. Earlier beta.93 startup/log shape is
  RitsuLib-only; actual disable-mod gameplay remains pending. The earlier
  settings-only disabled attempt is invalid because Spire Plus still
  initialized.
...
- No-launch validation confirmed helper restore behavior under
  `.tools/runtime-evidence/live-helper-preserve-current-run-smoke-20260513-133431`;
  the old plug-off helper path was no-launch checked under
  `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142957`
  as historical tooling context.
