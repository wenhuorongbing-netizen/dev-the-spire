...
- Latest Steam-client loader evidence under `.tools/runtime-evidence/manual-test-handoff-20260524-161744/release/fresh-current-package-loader-smoke` reports `Found 30 SavedSpireFields` with only BaseLib plus Spire Plus loaded and a clean log audit for a prior package hash. The current package still needs a fresh loader row. Older `20260523-current`, 16-field, and 22-field startup/log passes are historical.
