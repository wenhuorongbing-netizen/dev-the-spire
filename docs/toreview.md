...
Latest no-game validation snapshot: build passed, default tests `272 passed / 20 skipped`, opt-in artifact tests `292 passed / 0 skipped`, website syntax checks passed, format passed, and `git diff --check` passed. After the README wording refresh, the current-package loader row is pending again because the ZIP/README hash changed without opening the game. The release verifier fails closed with 19 pending live/manual rows.
...
| MANUAL-20260519-COUNTERS-PEEK-TAGS | UI/tooltips | Vakuu combat counters are visible. Vakuu's Sere Talon / 瓦库原初之爪 is the 2 random Curses + 3 Wish pickup; Tanx Claws / 坦克斯利爪 transforms cards into Maul+ / 撕咬+. | Inspect Vakuu's Sere Talon pickup, Tanx Claws pickup, relic-bar art, Crystal Sphere preview, Quiet Echo, and Deferred Verdict text. |
...
| MANUAL-20260524-SERE-TALON-TANX-CLAWS-REPORT | Ancient rewards / relic art | Treat any green Tanx Claws art on Vakuu's Sere Talon as our display/package route problem until live proof says otherwise. | If effect is 2 Curses + 3 Wish but art/title is Tanx Claws, capture `godot.log` route lines and the surface that bypassed the patch. |
