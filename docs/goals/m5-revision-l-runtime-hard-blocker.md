# M5 Revision L Runtime Hard-Blocker

Date: 2026-06-10
Status: historical beta.84/beta.85 blocker packet; archived out of the active reading path.

Full archived record:

- `docs/archive/legacy-planning/m5-revision-l-runtime-hard-blocker-20260610.md`

Revision L is superseded by beta.91 RitsuLib-only setup and loader evidence in
`PROJECT_STATE.md`, `docs/goals/migration.md`, and `docs/test-ready-development-goal.md`.

Retained historical AdditiveBatch1 count boundary: historical log used the then-current 10 event types / 11 registration-call shape; current source expects 10 event types / 14 calls.

Retained non-claim boundary: Do not use the historical `v0.106.1` loader smokes or the red beta.84 smoke as current runtime proof.
