...
Current beta.108 package truth is RitsuLib-only on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.33`: package parity, runtime preflight, and source-workspace validation are current. Beta.107 clicked UI proof at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/` is smoke-level UI evidence only. Beta.99 settings/Off proof, previous beta.96 Off proof, and previous beta.93 AdditiveBatch1 proof remain previous-package context only.
