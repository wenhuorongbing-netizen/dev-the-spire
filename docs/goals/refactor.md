...
| --- | --- | --- |
| Runtime dependency blocker | Closed for package/dependency; runtime recapture pending | STS2-RitsuLib `v0.4.32` direct NuGet runtime layout is installed for Slay the Spire 2 `v0.107.1`; beta.99 package parity is current, while previous beta.96 Off direct proof is previous-package startup/loading evidence only. |
...
| AdditiveBatch1 proof | Previous-package loader/registration pass | beta.93 direct proof records 10 event types / 14 registration calls, 25/25 Spire Plus patches, clean audit, enabled-mode verifier 31 / 0, and packet verifier 61 / 0. Current beta.99 enabled-mode proof still needs recapture before gameplay claims. |
