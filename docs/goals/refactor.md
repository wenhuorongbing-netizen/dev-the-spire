...
Runtime dependency target: STS2-RitsuLib `v0.4.31` / `lib\0.107.1`, Slay the Spire 2 `v0.107.1`. BaseLib is not a current Spire Plus dependency.
...
Beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context, beta.88 remains previous BaseLib-backed context, and beta.93 is the current RitsuLib-only loader/registration proof.
...
| --- | --- | --- |
| Runtime dependency blocker | Closed for loader/registration | STS2-RitsuLib `v0.4.31` with `lib\0.107.1` is installed for Slay the Spire 2 `v0.107.1`; beta.93 Off and AdditiveBatch1 direct proof reached main menu with clean retained verifiers and no Spire Plus BaseLib dependency. |
