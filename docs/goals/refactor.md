...
Runtime dependency target: STS2-RitsuLib `v0.4.32` direct NuGet runtime layout, Slay the Spire 2 `v0.107.1`. previous package is not a current Spire Plus dependency.
...
dependency floor is now aligned on STS2-RitsuLib `v0.4.32`, and Spire Plus no longer has a project,
...
Previous beta.96 RitsuLib-only Off proof exists under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`, and previous beta.93 RitsuLib-only AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`.
...
| --- | --- | --- |
| Runtime dependency blocker | Closed for package/dependency; runtime recapture pending | STS2-RitsuLib `v0.4.32` direct NuGet runtime layout is installed for Slay the Spire 2 `v0.107.1`; beta.98 package parity is current, while previous beta.96 Off direct proof is previous-package startup/loading evidence only. |
...
| AdditiveBatch1 proof | Previous-package loader/registration pass | beta.93 direct proof records 10 event types / 14 registration calls, 25/25 Spire Plus patches, clean audit, enabled-mode verifier 31 / 0, and packet verifier 61 / 0. Current beta.98 enabled-mode proof still needs recapture before gameplay claims. |
