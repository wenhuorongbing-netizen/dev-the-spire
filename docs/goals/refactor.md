...
Runtime dependency target: STS2-RitsuLib `v0.4.29` / `lib\0.107.1`, Slay the Spire 2 `v0.107.1`. BaseLib is not a current Spire Plus dependency.
...
clean-audit blocker was BaseLib `v3.2.1` patch drift for the older dependency lane; the current
dependency floor is now aligned on STS2-RitsuLib `v0.4.29`, and Spire Plus no longer has a project,
manifest, package, or current runtime dependency on BaseLib.

Beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context, beta.88 remains previous BaseLib-backed context, and beta.92 is the current RitsuLib-only loader/registration proof.
Current beta.92 RitsuLib-only Off proof exists under `.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/`, and current beta.92 RitsuLib-only AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/`.
...
| --- | --- | --- |
| Runtime dependency blocker | Closed for loader/registration | STS2-RitsuLib `v0.4.29` with `lib\0.107.1` is installed for Slay the Spire 2 `v0.107.1`; beta.92 Off and AdditiveBatch1 direct proof reached main menu with clean retained verifiers and no Spire Plus BaseLib dependency. |
...
| AdditiveBatch1 proof | Current loader/registration pass | beta.92 direct proof records 10 event types / 14 registration calls, 25/25 Spire Plus patches, clean audit, enabled-mode verifier 31 / 0, and packet verifier 61 / 0. |
