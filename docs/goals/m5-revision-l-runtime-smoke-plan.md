# M5 Revision L Runtime Smoke Plan

Date: 2026-06-10
Status: historical beta.84/beta.85 smoke plan; archived out of the active reading path.

Full archived record:

- `docs/archive/legacy-planning/m5-revision-l-runtime-smoke-plan-20260610.md`

Revision L is superseded by beta.93 RitsuLib-only Off and AdditiveBatch1 packet
checks in `PROJECT_STATE.md` and `docs/goals/migration.md`.

Retained registration-shape boundary: current source expects 10 event types / 14 calls.

Retained proof boundary: Runtime smoke does not prove gameplay.
