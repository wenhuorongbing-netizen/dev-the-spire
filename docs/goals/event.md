...
Current beta.93 RitsuLib-only Off/AdditiveBatch1 loader/registration: pass for v0.107.1 with STS2-RitsuLib v0.4.31 and only STS2-RitsuLib as the shared runtime dependency
...
### 0.2 Current beta.93 override

As of 2026-06-21, current loader truth is beta.93 on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus `v0.1.0-private-beta.93` loaded, only STS2-RitsuLib as the shared runtime dependency, 25/25 Spire Plus patches applied, clean audits, Off packet verifier 43 / 0, AdditiveBatch1 enabled-mode verifier 31 / 0, and AdditiveBatch1 packet verifier 61 / 0. Treat beta.85, beta.86, beta.87, beta.88, and beta.90 loader lines below as retained previous-package or previous-package context unless a line explicitly names beta.93 evidence.
...
Current beta.93 RitsuLib-only AdditiveBatch1 enabled-mode loader proof：
...
- Current beta.93 RitsuLib-only AdditiveBatch1 loader registration proof can be treated as current-pass for `O33`.
...
```text
Retained beta.85 Off, retained beta.85 CanaryOnly, retained beta.87 AdditiveBatch1, previous package beta.88 AdditiveBatch1, and current beta.93 AdditiveBatch1 loader proof must not be extended to:
gameplay
save-load
replacement
multiplayer
...
| AdditiveBatch1 enabled-mode | Current beta.93 RitsuLib-only direct proof: 10 event types / 14 calls | Loader/registration proof only |
...
- preserve current beta.93 AdditiveBatch1 loader/registration proof
...
这些都有当前 validation 记录支持：beta.93 Off proof 和 beta.93 AdditiveBatch1 proof 显示 `v0.1.0-private-beta.93`、STS2-RitsuLib `0.4.31`、compat branch `0.107.1`、25/25 patches、main menu reached、audit clean；beta.93 AdditiveBatch1 proof 显示 10 event types / 14 registration calls、enabled-mode verifier 31 / 0、packet verifier 61 / 0。当前文档也明确这些 proof 只覆盖 loader/registration 层面，不覆盖 gameplay、save-load、replacement、multiplayer、image/render 或 QA。
