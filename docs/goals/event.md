# StS1 Event Port Strict Audit v20 - beta.93 / v0.107.1 RitsuLib-Only Current Loader Truth, June Dev Spec, Subagent, and Overnight Run
...
当前项目相比 v19 又有实质进展：beta.93 在 StS2 `v0.107.1` + STS2-RitsuLib `v0.4.31` 下的 **RitsuLib-only Off loader proof 和 AdditiveBatch1 enabled-mode loader/registration proof 已 clean**，并且当前 Spire Plus 交付面不再依赖 previous package。beta.85/beta.87/beta.88/beta.90 证据只保留为 previous-package、previous-game-version 或 previous-package context。
...
Previous beta.93 RitsuLib-only Off/AdditiveBatch1 loader/registration: pass for v0.107.1 with STS2-RitsuLib v0.4.31 and only STS2-RitsuLib as the shared runtime dependency
...
### 0.2 Current beta.97 package override

As of 2026-06-21, current package truth is beta.99 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.32`. Beta.99 clicked settings UI proof is captured under `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`; beta.99 direct Off loader proof remains pending after the RitsuLib settings-page I18N resource migration. Previous beta.96 Off proof under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus `v0.1.0-private-beta.96` loaded, only STS2-RitsuLib as the shared runtime dependency, 25/25 Spire Plus patches applied, clean audit, StS1Events disabled with 0 registration lines, and Off packet verifier 43 / 0. Previous beta.93 AdditiveBatch1 proof under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` remains previous-package enabled-mode registration context with verifier 31 / 0 and packet verifier 61 / 0. Treat beta.85, beta.86, beta.87, beta.88, beta.90, beta.93, and beta.96 loader lines below as retained previous-package or previous-game-version context.

### 0.3 Historical beta.88 previous-package context

As of 2026-06-19, beta.88 was the previous package loader truth on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta88-previous-package330-additive-batch1-direct-cleanlog-20260619-103937/` reached main menu with previous package `v3.3.0`, STS2-RitsuLib `v0.4.24`, Spire Plus `v0.1.0-private-beta.88`, 25/25 Spire Plus patches applied, AdditiveBatch1 10 event types / 14 registration calls, clean audit, enabled-mode verifier 31 / 0, and runtime packet verifier 0 mismatches. It is now superseded by the beta.93 RitsuLib-only active loader truth above. Treat beta.85, beta.86, beta.87, beta.88, and beta.90 loader lines below as retained previous-package, previous-game-version, or previous-package context unless a line explicitly names beta.93 evidence.
...
Latest beta.99 governance checkpoint after the RitsuLib settings-page I18N resource migration: static/source docs must be recaptured for the current package before any handoff. Retained beta.96 source-workspace and Off proof is previous-package or no-launch evidence only and does not close beta.97 enabled-mode registration, gameplay, save-load, replacement, multiplayer, QA, game-native AutoSlay batch proof, release, or handoff gates.
...
Current retained split coverage after beta.93 migration validation: 475 passed / 0 failed / 21 skipped / 496 total
Opt-in installed-artifact lane: 67 passed / 0 failed / 2 skipped / 69 total
Static suite: 15 static steps / 0 suite failures
current-doc-claims: 1331 checks / 0 mismatches
runtime-preflight: 27 checks / 0 mismatches (local v0.107.1 / beta.93 target; read-only source/prereq only)
...
source workspace: 57 checks / 0 mismatches against installed v0.107.1, Spire Plus v0.1.0-private-beta.99, and STS2-RitsuLib 0.4.32 direct NuGet runtime layout, with two retained GDRE warnings
...
- `0 warnings / 0 errors` 已保留到 beta.93 RitsuLib-only migration 证据。
...
- beta.93 RitsuLib-only Off loader proof 是当前通过的 Off loader 证据；beta.85 default-Off proof 只作为 previous-package/game-version context 保留。
...
Previous beta.93 RitsuLib-only AdditiveBatch1 enabled-mode loader proof：

```text
v0.107.1
STS2-RitsuLib 0.4.31 / compat branch 0.107.1
Spire Plus v0.1.0-private-beta.93
...
- Previous beta.93 RitsuLib-only AdditiveBatch1 loader registration proof can be treated as current-pass for `O33`.
...
```text
Retained beta.85 Off, retained beta.85 CanaryOnly, retained beta.87 AdditiveBatch1, previous package beta.88 AdditiveBatch1, and previous beta.93 AdditiveBatch1 loader proof must not be extended to:
gameplay
save-load
replacement
multiplayer
...
| AdditiveBatch1 enabled-mode | Previous beta.93 RitsuLib-only direct proof: 10 event types / 14 calls | Loader/registration proof only |
...
- preserve previous beta.93 AdditiveBatch1 loader/registration proof
...
```text
StS1 Event Port Prototype Batch 1 - beta.93 RitsuLib-Only Loader Foundation
```
...
8. beta.85 CanaryOnly loader proof captured:
   - 4 event types / 6 registration calls.
9. beta.93 RitsuLib-only AdditiveBatch1 loader proof captured:
   - 10 event types / 14 registration calls.
...
| O26 | beta.93 package parity proof |
| O27 | beta.93 package SHA recorded |
...
| O34 | beta.93 AdditiveBatch1 loader audit clean |
...
优先级：
1. 保持 beta.93 RitsuLib-only Off clean proof。
2. 保留 beta.85 CanaryOnly loader proof as previous-package/game-version context：4 event types / 6 registration calls。
3. 保持 beta.93 RitsuLib-only AdditiveBatch1 loader proof：10 event types / 14 registration calls。
...
- 不要把 beta.93 Off/AdditiveBatch1 loader proof 或 retained beta.85 CanaryOnly loader proof 外推成 gameplay proof。
...
结论：**当前仍未完成**，但审查口径要更新到最新 beta.93 状态：**v0.107.1 的 RitsuLib-only Off 和 AdditiveBatch1 loader/registration proof 已通过，当前 Spire Plus 交付面不再依赖 previous package；现在真正阻塞点转移到 gameplay、save/load、replacement、image/render、multiplayer、独立 QA 和 handoff。**
...
| Test matrix             | beta.93 retained split coverage 记录为 `475 passed / 0 failed / 21 skipped / 496 total`                                                  |
| Installed artifact lane | `67 passed / 0 failed / 2 skipped / 69 total`                                                                                           |
| Static suite            | `15 static steps / 0 suite failures`                                                                                                    |
| Current doc claims      | 后续静态检查最高记录为 `1331 checks / 0 mismatches` |
| beta.93 Off loader      | **clean**：v0.107.1 + STS2-RitsuLib 0.4.31，25/25 patches，RitsuLib-only package                                                               |
| beta.85 CanaryOnly loader | **clean**：retained verifiers 证明 4 event types / 6 registration calls                                                                    |
| beta.93 AdditiveBatch1 loader | **clean**：enabled-mode verifier 31 / 0，packet verifier 61 / 0，10 event types / 14 registration calls                                                                   |
...
这些都有 beta.97 package parity / previous beta.96 / previous beta.93 validation 记录支持：beta.97 是当前包；previous beta.96 Off proof 显示 `v0.1.0-private-beta.96`、STS2-RitsuLib `0.4.31`、compat branch `0.107.1`、25/25 patches、main menu reached、audit clean、StS1Events disabled with 0 registration lines、packet verifier 43 / 0；previous beta.93 AdditiveBatch1 proof 显示 10 event types / 14 registration calls、enabled-mode verifier 31 / 0、packet verifier 61 / 0。当前文档也明确这些 proof 只覆盖 loader/registration 层面，不覆盖 current enabled-mode gameplay、save-load、replacement、multiplayer、image/render 或 QA。
...
当前 validation 明确说：没有 gameplay、clicked UI、save-load、co-op、event encounter、replacement、independent QA、release handoff proof；并且 beta.93 Off/AdditiveBatch1 loader proof 和 retained beta.85 CanaryOnly loader proof 不得扩展到 gameplay、save-load、replacement、multiplayer 或 QA gates。
...
```text
- 保持 beta.93 RitsuLib-only Off clean proof
- 保留 beta.85 CanaryOnly enabled-mode proof as previous-package/game-version context
- 保持 beta.93 RitsuLib-only AdditiveBatch1 enabled-mode proof
...
6. beta.93 RitsuLib-only Off loader clean proof 保留。
7. beta.85 CanaryOnly loader proof 捕获：4 event types / 6 registration calls。
8. beta.93 RitsuLib-only AdditiveBatch1 loader proof 捕获：10 event types / 14 registration calls。
...
| O26-O35 | beta.93 package parity/SHA、RitsuLib-only paths、godot.log、Off/Canary/Additive clean loader audits                                               |
...
2. **Runtime Environment Bootstrap**：beta.93 package、STS2-RitsuLib v0.4.31、EZMicroBalance install、godot.log、loader audit。
...
最新证据显示 beta.93 RitsuLib-only Off 与 AdditiveBatch1 enabled-mode loader proof 都已经通过：clean audit，25/25 patches applied，AdditiveBatch1 10 event types / 14 registration calls，enabled-mode verifier 31 / 0，packet verifier 61 / 0。Retained beta.85 CanaryOnly 仍只作为 previous-package/game-version loader context。source/test/static 也有强进展：build 0/0，retained split coverage 475 passed / 0 failed / 21 skipped / 496 total，static suite 15 steps / 0 failures，current-doc claims 1331 / 0。

但是 beta.93 Off/AdditiveBatch1 loader proof 和 retained beta.85 CanaryOnly loader proof 都不能外推到 gameplay、save-load、replacement、multiplayer、image/render 或 QA。AdditiveBatch1 的 beta.85 13/14 mismatch 只保留为历史 drift 诊断；当前 runtime parity 仍未完成，因为 gameplay/save-load/render/replacement/multiplayer/QA proof 都缺失。
...
管理红线：**不要把 source/test/static pass 当 gameplay completion；不要把 beta.93 Off/AdditiveBatch1 loader proof 或 retained beta.85 CanaryOnly loader proof 外推成 gameplay proof；不要把 registry count 当 StS1 experience；不要把 hard-stop report 当 completion；不要在相关 gameplay 证据 clean 前推进 gameplay claims。**
