...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, and focused runtime preflight pass for the current beta.87 package baseline. `scripts/verify-spire-plus-release-evidence.ps1` hashes the zip, rejects duplicate rows, wrong row kinds, empty/invalid files, invalid notes, undersized PNGs, evidence dirs outside the evidence root, and required-file/screenshot paths that escape their row evidence dir. Fresh path-containment negative smokes failed as designed under `.tools/runtime-evidence/release-ready-path-containment-smoke*`; current beta.87 AdditiveBatch1 direct smoke is clean for loader registration proof, but release audit still needs formal gameplay/UI/save-load/co-op proof and fails closed with 20 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.87.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
| Build and publish | `dotnet build EZMicroBalance.sln -m:1 --no-incremental` passed with 0 warnings / 0 errors, and `dotnet publish EZMicroBalance.sln -m:1` passed after the beta.86 package refresh; installed/staging/versioned/package hashes are in parity. | Pass for build/publish/package |
| Latest source/package refresh | Source review confirms visible Ancient marker relics, current EN/zhs text guards, separate art/resource routes, dedicated gated Vakuu fight source, Root Eyes preview hardening, A11-A20 source guards, integrated preview-tool guards, and the short manual-test package README. Current zip SHA256 is `97C65F040F7269738778368878E7946D1563F622D0D8959644C54DBC6806A0B1`; detailed pass history lives in `docs/review.md` and `docs/archive/**`. | Pass for source/build/package artifact parity; gameplay verification pending |
| Current automated tests | `dotnet build EZMicroBalance.sln -m:1 --no-incremental` passes with 0 errors / 0 warnings. The documented beta.85 split baseline remains 475 passed / 21 skipped / 0 failed / 496 total; current beta.86 split no-build validation passed with 489 passed / 39 skipped / 0 failed / 528 total, and the focused opt-in artifact subset passed with 67 passed / 2 skipped / 0 failed / 69 total. `dotnet test EZMicroBalance.sln -c Release` previously passed with 81 passed / 18 skipped. | Automated build/package/split/focused artifact pass |
...
| Steam loader evidence | Current direct AdditiveBatch1 startup/log evidence at `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531` loaded BaseLib, RitsuLib, and Spire Plus under technical id `EZMicroBalance`, selected RitsuLib compat branch `0.107.0`, reported `v0.1.0-private-beta.87`, applied 25/25 Spire Plus ModPatcher patches, reached main menu, restored settings and 25 moved mod entries, stopped the game, and audited clean with 0 release-blocking hits. The first beta.86 already-running-Steam attempt is diagnostic only because StS1 stayed disabled. | Pass for beta.87 AdditiveBatch1 loader registration only; gameplay rows pending |
...
Not achieved. The current tree has the latest Urda/Morvi/Lotha/Vakuu source hardening build/publish/package-refreshed, beta.87 AdditiveBatch1 loader registration proof, historical loader/resource smoke for context, and the restore-safe live-session helper is ready for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
