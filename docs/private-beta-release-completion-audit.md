...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, and focused runtime preflight pass target the current beta.91 RitsuLib-only package baseline. The release verifier covers zip hashes, duplicate rows, row kinds, invalid files/notes, screenshot size, and path containment; the `release-ready-path-containment-smoke` fixture rejects evidence dirs outside the evidence root and required-file/screenshot paths that escape their row evidence dir. Current beta.91 Off and AdditiveBatch1 direct smokes are loader/registration proof only; release audit still needs gameplay/UI/save-load/co-op proof and fails closed with 21 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
