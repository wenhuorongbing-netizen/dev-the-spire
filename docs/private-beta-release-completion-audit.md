...
Original audit date: 2026-05-15; latest beta.99 RitsuLib-only package refresh: 2026-06-21; previous beta.96 Off loader proof and previous beta.93 AdditiveBatch1 evidence remain historical loader/registration proof only.
...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.99 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Beta.99 clicked settings UI proof and beta.99 direct Off smoke are captured; gameplay/UI/save-load/co-op proof and current release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.99.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
| Build and publish | `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` passed with 0 warnings / 0 errors, and `dotnet publish EZMicroBalance.sln -m:1` passed after the beta.99 RitsuLib settings page I18N resource migration with only the known Godot ignored-folder warning; installed/staging/versioned/package hashes are in parity. | Pass for build/publish/package |
| Latest source/package refresh | Source review covers visible Ancient marker relics, current EN/zhs text guards, separate art/resource routes, gated Vakuu fight source, Root Eyes preview hardening, A11-A20 guards, preview-tool guards, and the manual-test README. Current zip SHA256 is `F775CA27F55CAB7D62A529BA45FE307FF0FEB5530861F03A4194F3931294092A`; beta.99 settings UI proof is `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`; beta.99 Off proof is `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/`; detailed pass history lives in `docs/review.md` and `docs/archive/**`. | Pass for source/build/package artifact parity, settings UI visibility, and startup/default-Off loader proof; gameplay verification pending |
...
| Steam loader evidence | Beta.99 direct RitsuLib-only Off startup/log evidence exists at `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/` with main-menu load, clean audit, 25/25 Spire Plus patches, StS1Events disabled with 0 registration lines, Off verifier 21 / 0, and packet verifier 43 / 0. Current beta.99 settings UI proof exists at `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`; previous beta.96 Off evidence and previous beta.93 AdditiveBatch1 direct evidence remain previous-package context only. | Pass for beta.99 startup/default-Off loader; gameplay evidence pending |
| Plug-off loader evidence | Current beta.99 RitsuLib-only Off direct proof at `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/` loads only `STS2-RitsuLib` plus Spire Plus for startup/log and packet parity, with StS1Events disabled. The older previous package-only normal Steam startup/log pass at `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020` is historical disabled-mod loader-isolation context only. The settings-only disabled attempt `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142835` is invalid because Spire Plus still initialized. | Pass for beta.99 startup/log loader shape; disable-mod gameplay pending |
| Mod Settings evidence | Current beta.99 RitsuLib Mod Settings evidence `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/` shows `RitsuLib` and `Spire Plus`, opens the Spire Plus page, renders Migration Status, `STS2-RitsuLib >= 0.4.32`, evidence-boundary, technical-id, and Preview Tools controls, and has clean same-session log audit plus StS1 Off runtime shape verification 21 / 0. | Pass for settings UI visibility only; gameplay remains pending |
...
Not achieved. The current tree has beta.99 RitsuLib-only source/build/publish/package refresh, beta.99 settings UI proof, beta.99 direct Off loader proof, previous beta.96 Off loader proof as previous-package context, previous beta.93 AdditiveBatch1 loader/registration proof as previous-package context, historical loader/resource smoke for context, and the restore-safe live-session helper ready for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
