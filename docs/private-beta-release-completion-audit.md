...
| Current automated tests | `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` passes with 0 errors / 0 warnings. The current beta.88 pass also has installed-package parity, runtime preflight 27 / 0, current-doc claims 1119 / 0, static suite 15 / 0, and static-file hygiene 12 / 0. Earlier split no-build and opt-in artifact lanes remain retained context until rerun for the final handoff. | Automated build/package/static/runtime-preflight pass |
...
| Steam loader evidence | Current direct AdditiveBatch1 startup/log evidence at `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937` loaded BaseLib `v3.3.0`, RitsuLib `0.4.24`, and Spire Plus under technical id `EZMicroBalance`, selected RitsuLib compat branch `0.107.0`, reported `v0.1.0-private-beta.88`, applied 25/25 Spire Plus ModPatcher patches, reached main menu, registered 10 StS1 event types through 14 calls, and audited clean with 0 release-blocking hits. Beta.87 direct proof remains previous-game-version context only. | Pass for beta.88 AdditiveBatch1 loader registration only; gameplay rows pending |
