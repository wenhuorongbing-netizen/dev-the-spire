...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, and focused runtime preflight pass for the current beta.86 package baseline. `scripts/verify-spire-plus-release-evidence.ps1` hashes the zip, rejects duplicate rows, wrong row kinds, empty/invalid files, invalid notes, undersized PNGs, evidence dirs outside the evidence root, and required-file/screenshot paths that escape their row evidence dir. Fresh path-containment negative smokes failed as designed under `.tools/runtime-evidence/release-ready-path-containment-smoke*`; current beta.86 AdditiveBatch1 direct smoke is clean for loader registration proof, but release audit still needs formal gameplay/UI/save-load/co-op proof and fails closed with 20 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.86.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
...
| Steam loader evidence | Current direct AdditiveBatch1 startup/log evidence at `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254` loaded BaseLib, RitsuLib, and Spire Plus under technical id `EZMicroBalance`, selected RitsuLib compat branch `0.107.0`, reported `v0.1.0-private-beta.86`, applied 25/25 Spire Plus ModPatcher patches, reached main menu, restored settings and 25 moved mod entries, stopped the game, and audited clean with 0 release-blocking hits. The first beta.86 already-running-Steam attempt is diagnostic only because StS1 stayed disabled. | Pass for beta.86 AdditiveBatch1 loader registration only; gameplay rows pending |
