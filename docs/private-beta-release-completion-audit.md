...
| Automation and release evidence | Build, test, format, diff-check, opt-in artifact tests, publish, and package refresh pass for the current source/package baseline. `scripts/verify-spire-plus-release-evidence.ps1` hashes the zip, rejects duplicate rows, wrong row kinds, empty/invalid files, invalid notes, undersized PNGs, evidence dirs outside the evidence root, and required-file/screenshot paths that escape their row evidence dir. Fresh path-containment negative smokes failed as designed under `.tools/runtime-evidence/release-ready-path-containment-smoke*`; current beta.85 runtime smoke is clean for loader proof, but release audit still needs formal loader-row packaging plus gameplay/UI/save-load/co-op proof and fails closed with 20 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.85.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
| Build and publish | `dotnet build EZMicroBalance.sln` and `dotnet publish EZMicroBalance.sln` passed after the beta.85 runtime-fix package refresh; installed/staging/versioned/package hashes are in parity. | Pass for build/publish/package |
| Latest source/package refresh | Source review confirms visible Ancient marker relics, current EN/zhs text guards, separate art/resource routes, dedicated gated Vakuu fight source, Root Eyes preview hardening, A11-A20 source guards, integrated preview-tool guards, and the short manual-test package README. Current zip SHA256 is `B182F6DCD8E88D9209C28997901C9EF5E9947F79E1CA93FD47E7F38625140CEE`; detailed pass history lives in `docs/review.md` and `docs/archive/**`. | Pass for source/build/package artifact parity; gameplay verification pending |
| Current automated tests | `dotnet build EZMicroBalance.sln` passes with 0 errors / 0 warnings; current rebuilt split no-build test lanes pass with 475 passed / 21 skipped / 0 failed / 496 total; current beta.85 opt-in package/artifact validation passed with 67 passed / 0 skipped / 0 failed; `dotnet test EZMicroBalance.sln -c Release` previously passed with 81 passed / 18 skipped. | Automated build/test/artifact pass; live/manual evidence pending |
...
| Steam loader evidence | Current normal Steam helper startup/log evidence at `.tools/runtime-evidence/v01070-beta85-current-package-runtime-fix-20260611-0510` loaded BaseLib, RitsuLib, and Spire Plus under technical id `EZMicroBalance`, selected RitsuLib compat branch `0.107.0`, reported `v0.1.0-private-beta.85`, applied 25/25 Spire Plus ModPatcher patches, reached main menu, restored settings and 25 moved mod entries, stopped the game, and audited clean with 0 release-blocking hits. | Pass for beta.85 loader only; gameplay rows pending |
