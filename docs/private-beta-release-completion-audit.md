...
Original audit date: 2026-05-15; latest beta.131 RitsuLib-only package refresh/no-launch validation: 2026-06-23; previous beta.128 clicked UI smoke: 2026-06-23; previous beta.123, beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.
...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, runtime preflight, and source-workspace validation target the current beta.131 RitsuLib-only package baseline. Previous beta.128 forced clicked Ancient UI smoke is retained separately. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Gameplay/save-load/co-op proof and full release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.131.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
| Build and publish | `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` passed with 0 warnings / 0 errors, and `dotnet publish EZMicroBalance.sln -m:1 -p:UseSharedCompilation=false` passed for beta.131 with only the known Godot ignored-folder warning; installed/staging/versioned/package hashes are in parity. | Pass for build/publish/package |
| Latest source/package refresh | Source review covers visible Ancient marker relics, current EN/zhs text guards, separate art/resource routes, gated Vakuu fight source, Root Eyes preview hardening, A11-A20 guards, preview-tool guards, and the manual-test README. Current zip SHA256 is `40A867AF57BE9EEA74D3A0E425391F52C159FA0F4BF4DDBDD4E9BFFEC7F017F5`; previous beta.128 clicked UI smoke is `.tools/runtime-evidence/monkey-stability-20260623-062913/`; beta.99 settings/Off proof remains previous-package context; detailed pass history lives in `docs/review.md` and `docs/archive/**`. | Pass for source/build/package artifact parity and previous-package smoke-level clicked UI proof; gameplay verification pending |
...
| Steam loader evidence | Previous beta.128 clicked UI smoke exists at `.tools/runtime-evidence/monkey-stability-20260623-062913/` with main-menu load, clean audit, 152/152 default runtime Spire Plus patches applied, StS1Events disabled, Off verifier pass, and packet verifier 1621 / 0. Beta.99 settings/Off proof, previous beta.96 Off evidence, and previous beta.93 AdditiveBatch1 direct evidence remain previous-package context only. | Pass for previous beta.128 startup/UI smoke; beta.131 runtime smoke and gameplay evidence pending |
| Plug-off loader evidence | Previous beta.128 clicked Ancient UI smoke at `.tools/runtime-evidence/monkey-stability-20260623-062913/` loads only `STS2-RitsuLib` plus Spire Plus for startup/log and packet parity, with StS1Events disabled. Beta.99 Off proof and the older previous package-only normal Steam startup/log pass are historical disabled-mod loader-isolation context only. The settings-only disabled attempt `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142835` is invalid because Spire Plus still initialized. | Pass for previous beta.128 startup/UI smoke; beta.131 runtime smoke and disable-mod gameplay pending |
...
Not achieved. The current tree has beta.131 RitsuLib-only source/build/publish/package refresh, runtime preflight, and source-workspace validation; previous beta.128 clicked Ancient UI smoke remains previous-package smoke-level proof. Beta.99 settings UI proof, beta.99 direct Off loader proof, previous beta.96 Off loader proof, previous beta.93 AdditiveBatch1 loader/registration proof, historical loader/resource smoke, and the restore-safe live-session helper remain context for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
