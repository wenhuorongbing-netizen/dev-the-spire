...
Original audit date: 2026-05-15; latest beta.115 RitsuLib-only package refresh: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.
...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.115 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Beta.108 clicked UI smoke is captured; gameplay/save-load/co-op proof and current release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.115.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
