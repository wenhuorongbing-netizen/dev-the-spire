...
| Automation and release evidence | `dotnet build`, normal tests, format, diff-check, and opt-in artifact tests pass. `scripts/verify-spire-plus-release-evidence.ps1` hashes the zip under test, rejects duplicate rows, wrong row kinds, empty/invalid files, invalid notes, undersized PNGs, manifest/evidence dirs outside the evidence root, and required-file/screenshot paths that escape their row evidence dir; default row evidence files, including per-row `command.txt`, cannot be removed by editing `RequiredFiles`, and unknown or blank rows are reported in `Warnings`. Fresh path-containment negative smokes failed as designed under `.tools/runtime-evidence/release-ready-path-containment-smoke*`; the fresh template audit under `.tools/runtime-evidence/release-ready-manual-current-audit` failed as designed with 16 pending manual rows. | Automation gates work; release evidence is not complete. |
...
| Build and publish | `dotnet build EZMicroBalance.sln` and `dotnet publish EZMicroBalance.sln` passed after the Vakuu/Lotha reward and zhs encoding polish pass; installed/staging/versioned/package hashes are in parity. | Pass for build/publish/package |
| Latest source/package refresh | Local source review confirmed Ancient reward selections now grant visible marker relics, Vakuu victory choices include custom Lotha visible relics when eligible, active Simplified Chinese localization no longer contains known mojibake fragments, and package review confirmed the short manual-test README. Package zip `EA0EC3611DC21FD33C9B87E592326A9000ECE593512554D720843D7490CC589C` includes this mod/resource state plus the browser GPTimage2 small-art rebuild, custom card portraits, Rootblight/Blight Sprout state-hardening, A11 optional-route source proof, clicked-Ancient UI hardening, Vakuu/Lotha save-risk reduction, Lotha Power replacement polish, Public Evidence debuff-policy hardening, Morvi reward/state lifecycle hardening, art-manifest coverage, player-facing localization scrub, Root-Sight/Vakuu text simplification, and release-evidence verifier hardening. | Pass for source/build/package artifact parity; gameplay verification pending |
| Current automated tests | `dotnet build EZMicroBalance.sln` passes with 0 warnings/errors after the player-facing text and release-evidence verifier polish pass; `dotnet test EZMicroBalance.sln --no-build` passes with 170 passed / 18 skipped; `dotnet test EZMicroBalance.sln -c Release` previously passed with 81 passed / 18 skipped; opt-in artifact tests pass with 188 passed / 0 skipped after the current docs/guard update. | Pass |
| Formatting and diff hygiene | `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` passes after the release-evidence path-containment guard update; `git diff --check` passes with CRLF warnings only. | Pass |
...
| Steam loader evidence | Current normal Steam helper startup/log pass at `.tools/runtime-evidence/live-spire-plus-session-20260515-211414` loads exactly BaseLib plus Spire Plus / `EZMicroBalance`, reports `Found 22 SavedSpireFields`, reaches main menu in `13,539ms`, restores settings, 24 moved mod entries, and 2 current-run files, leaves 0 `SlayTheSpire2` processes, and audits clean with 0 `ERROR` / release-blocking hits. | Pass for loader |
...
Not achieved. The current tree has the latest Urda/Morvi/Lotha/Vakuu source hardening build/publish/package-refreshed, current-package loader/resource smoke is clean, and the previous loader evidence plus restore-safe live-session helper remain useful. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
