...
| Automation and release evidence | `dotnet build`, normal tests, format, diff-check, opt-in artifact tests, and current-package loader smoke pass. `scripts/verify-spire-plus-release-evidence.ps1` hashes the zip, rejects duplicate rows, wrong row kinds, empty/invalid files, invalid notes, undersized PNGs, evidence dirs outside the evidence root, and required-file/screenshot paths that escape their row evidence dir. Fresh path-containment negative smokes failed as designed under `.tools/runtime-evidence/release-ready-path-containment-smoke*`; the current evidence audit still fails closed with 18 manual rows until gameplay/UI/save-load/co-op proof is captured. | Automation gates work; release evidence is not complete. |
...
| Latest source/package refresh | Source review confirms visible Ancient marker relics, current EN/zhs text guards, separate art/resource routes, dedicated gated Vakuu fight source, Root Eyes preview hardening, A11-A20 source guards, integrated preview-tool guards, and the short manual-test package README. Current zip SHA256 is `D8C7BB29B45FDA669BC2345B133EF0277E97004B1B6B53992DFF4FAC55F466F4`; detailed pass history lives in `docs/review.md` and `docs/archive/**`. | Pass for source/build/package artifact parity; gameplay verification pending |
| Current automated tests | `dotnet build EZMicroBalance.sln` passes with 0 warnings/errors; `dotnet test EZMicroBalance.sln --no-build` passes with 271 passed / 20 skipped; `dotnet test EZMicroBalance.sln -c Release` previously passed with 81 passed / 18 skipped; latest package refresh evidence includes `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` with 291 passed / 0 skipped after the player-visible localization naming guard. | Automated build/test/artifact pass; live/manual evidence pending |
...
| Steam loader evidence | Current normal Steam helper startup/log evidence at `.tools/runtime-evidence/manual-test-handoff-20260524-161744/release/fresh-current-package-loader-smoke` loaded exactly BaseLib plus Spire Plus under technical id `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, restored settings and 25 moved mod entries, left 0 `SlayTheSpire2` processes, and audited clean with 0 `ERROR` / release-blocking hits. | Pass for current loader; gameplay rows pending |
