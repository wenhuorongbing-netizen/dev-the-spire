...
| Build and publish | `dotnet build EZMicroBalance.sln --no-restore` and `dotnet publish EZMicroBalance.sln --no-restore` passed after the Urda UI/art repair, default-on Lotha, and source-complete/live-pending Vakuu fight source slices; installed/staging/versioned/package hashes are in parity. | Pass for build/publish/package |
| Latest source/package refresh | Local source and resource review confirmed the Ancient art promotion package refresh. Package zip `61BA0EB09A01E3AF849A6AFA78C1F6EE6D9395CB8ACF2CCF962B5BC4AA8B58D0` includes this state plus 52 promoted source-local Ancient art assets, unique custom card portraits, Rootblight/Blight Sprout state-hardening, A11 optional-route source proof, clicked-Ancient UI hardening, Vakuu/Lotha save-risk reduction, Lotha Power replacement polish, Lotha hand-order bugfix, Public Evidence debuff-policy hardening, Morvi reward/state lifecycle hardening, art-manifest coverage, player-facing localization scrub, and package hash refresh. | Pass for source/build/package artifact parity; gameplay verification pending |
| Current automated tests | `dotnet test EZMicroBalance.sln --no-build` passes with 151 passed / 18 skipped; `dotnet test EZMicroBalance.sln -c Release` previously passed with 81 passed / 18 skipped; opt-in artifact tests pass with 169 passed / 0 skipped after the latest `SpirePlus` package refresh. | Pass |
