...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.99 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Beta.99 clicked settings UI proof is captured; beta.99 direct Off smoke, gameplay/UI/save-load/co-op proof, and current release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
| Steam loader evidence | Beta.99 direct RitsuLib-only Off startup/log evidence remains pending. Current beta.99 settings UI proof exists at `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/` and previous beta.96 Off evidence exists at `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`. Previous beta.93 AdditiveBatch1 direct evidence and older beta.88/beta.87 direct proof remain previous-package or previous-game-version context only. | Pending for beta.99 direct Off loader; gameplay evidence pending |
| Plug-off loader evidence | Previous beta.96 RitsuLib-only Off direct proof at `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/` loads only `STS2-RitsuLib` plus Spire Plus for startup/log and packet parity, with StS1Events disabled. The older previous package-only normal Steam startup/log pass at `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020` is historical disabled-mod loader-isolation context only. The settings-only disabled attempt `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142835` is invalid because Spire Plus still initialized. | Pending for beta.99 startup/log loader shape; disable-mod gameplay pending |
...
Not achieved. The current tree has beta.99 RitsuLib-only source/build/publish/package refresh, beta.99 settings UI proof, beta.99 direct Off loader proof pending, previous beta.96 Off loader proof, previous beta.93 AdditiveBatch1 loader/registration proof, historical loader/resource smoke for context, and the restore-safe live-session helper ready for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
