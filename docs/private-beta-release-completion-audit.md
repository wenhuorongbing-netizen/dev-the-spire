...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.99 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. Beta.99 clicked settings UI proof is captured; beta.99 direct Off smoke, gameplay/UI/save-load/co-op proof, and current release evidence remain pending. | Automation gates work; release evidence is not complete. |
