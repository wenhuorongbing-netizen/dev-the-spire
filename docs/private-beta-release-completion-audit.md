...
Original audit date: 2026-05-15; latest beta.96 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.
...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, and focused runtime preflight pass target the previous beta.93 RitsuLib-only package baseline. The release verifier covers zip hashes, duplicate rows, row kinds, invalid files/notes, screenshot size, and path containment; the `release-ready-path-containment-smoke` fixture rejects evidence dirs outside the evidence root and required-file/screenshot paths that escape their row evidence dir. Previous beta.93 Off and AdditiveBatch1 direct smokes are loader/registration proof only; release audit still needs gameplay/UI/save-load/co-op proof and fails closed with 21 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
...
Not achieved. The current tree has beta.96 RitsuLib-only source/build/publish/package refresh, previous beta.93 Off and AdditiveBatch1 loader/registration proof, historical loader/resource smoke for context, and the restore-safe live-session helper ready for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
