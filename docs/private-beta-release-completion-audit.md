...
| Single package | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, and `publish/SpirePlus-v0.1.0-private-beta.17.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
...
| Steam loader evidence | Current normal Steam helper startup/log evidence at `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` loaded exactly BaseLib plus Spire Plus under technical id `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, restored settings and 24 moved mod entries, left 0 `SlayTheSpire2` processes, matched the current beta.17 package hash, and audited clean with 0 `ERROR` / release-blocking hits. | Pass for current loader; gameplay rows pending |
