...
Original audit date: 2026-05-15; latest beta.98 RitsuLib-only package refresh: 2026-06-21; previous beta.96 Off loader proof and previous beta.93 AdditiveBatch1 evidence remain historical loader/registration proof only.
...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.98 RitsuLib-only package baseline. The release verifier covers zip hashes, duplicate rows, row kinds, invalid files/notes, screenshot size, and path containment; the `release-ready-path-containment-smoke` fixture rejects evidence dirs outside the evidence root and required-file/screenshot paths that escape their row evidence dir. Beta.97 Off smoke and clicked settings UI proof remain pending; previous beta.96 Off smoke and previous beta.93 AdditiveBatch1 direct smoke are previous-package loader/registration proof only; release audit still needs gameplay/UI/save-load/co-op proof and fails closed with 21 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
...
Not achieved. The current tree has beta.98 RitsuLib-only source/build/publish/package refresh, beta.98 loader/settings proof pending, previous beta.96 Off loader proof, previous beta.93 AdditiveBatch1 loader/registration proof, historical loader/resource smoke for context, and the restore-safe live-session helper ready for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
