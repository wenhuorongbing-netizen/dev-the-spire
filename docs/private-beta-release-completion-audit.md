...
| Steam loader evidence | Current normal Steam helper startup/log evidence at `.tools/runtime-evidence/beta16-loader-smoke-20260525-184743` loaded exactly BaseLib plus Spire Plus under technical id `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, restored settings and 24 moved mod entries, left 0 `SlayTheSpire2` processes, matched the current beta.16 package hash, and audited clean with 0 `ERROR` / release-blocking hits. | Pass for current loader; gameplay rows pending |
