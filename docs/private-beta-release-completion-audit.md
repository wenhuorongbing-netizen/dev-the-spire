...
Original audit date: 2026-05-15; latest beta.128 RitsuLib-only package refresh and clicked UI smoke: 2026-06-23; previous beta.123, beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.
...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, runtime preflight, source-workspace validation, and forced clicked Ancient UI smoke target the current beta.128 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Gameplay/save-load/co-op proof and full release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
Not achieved. The current tree has beta.128 RitsuLib-only source/build/publish/package refresh, runtime preflight, source-workspace validation, and smoke-level clicked Ancient UI proof. Beta.99 settings UI proof, beta.99 direct Off loader proof, previous beta.96 Off loader proof, previous beta.93 AdditiveBatch1 loader/registration proof, historical loader/resource smoke, and the restore-safe live-session helper remain context for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
