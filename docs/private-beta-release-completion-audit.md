...
Original audit date: 2026-05-15; latest beta.107 RitsuLib-only package refresh and clicked UI smoke: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.
...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.107 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Beta.107 clicked UI smoke is captured; gameplay/save-load/co-op proof and current release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
| Steam loader evidence | Beta.107 clicked UI smoke exists at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/` with main-menu load, clean audit, all then-current 46 Spire Plus patches applied, StS1Events disabled, Off verifier pass, and packet verifier 1620 / 0. Current source now has 52 migrated patches after the source-only Batch 4c localization migration; recapture runtime proof before claiming that source state in-game. Beta.99 settings/Off proof, previous beta.96 Off evidence, and previous beta.93 AdditiveBatch1 direct evidence remain previous-package context only. | Pass for beta.107 startup/UI smoke; gameplay evidence pending |
| Plug-off loader evidence | Current beta.107 clicked Ancient UI smoke at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/` loads only `STS2-RitsuLib` plus Spire Plus for startup/log and packet parity, with StS1Events disabled. Beta.99 Off proof and the older previous package-only normal Steam startup/log pass are historical disabled-mod loader-isolation context only. The settings-only disabled attempt `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142835` is invalid because Spire Plus still initialized. | Pass for beta.107 startup/UI smoke shape; disable-mod gameplay pending |
| Mod Settings evidence | Previous beta.99 RitsuLib Mod Settings evidence `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/` shows `RitsuLib` and `Spire Plus`, opens the Spire Plus page, renders Migration Status, `STS2-RitsuLib >= 0.4.33`, evidence-boundary, technical-id, and Preview Tools controls, and has clean same-session log audit plus StS1 Off runtime shape verification 21 / 0. | Previous-package settings UI visibility only; gameplay remains pending |
...
Not achieved. The current tree has beta.107 RitsuLib-only source/build/publish/package refresh, runtime preflight, source-workspace validation, and clicked Ancient UI smoke proof. Beta.99 settings UI proof, beta.99 direct Off loader proof, previous beta.96 Off loader proof, previous beta.93 AdditiveBatch1 loader/registration proof, historical loader/resource smoke, and the restore-safe live-session helper remain context for the next manual pass. It is not private-beta release-ready. The current Codex target is test-ready handoff; this release audit stays open until the live/manual blockers above are executed or explicitly release-noted as deferred by the project owner.
