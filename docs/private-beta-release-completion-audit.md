...
Original audit date: 2026-05-15; latest beta.105 RitsuLib-only package refresh and clicked UI smoke: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.
...
| Automation and release evidence | Build, publish, package refresh, and installed-package checks target the current beta.105 RitsuLib-only package baseline. The release verifier covers package/evidence hashes, row kinds, screenshots, and path containment. It fails closed with manual rows until live evidence is present, and the `release-ready-path-containment-smoke` fixtures reject evidence dirs outside the evidence root plus required-file/screenshot paths that escape their row evidence dir. Beta.105 clicked UI smoke is captured; gameplay/save-load/co-op proof and current release evidence remain pending. | Automation gates work; release evidence is not complete. |
...
| Steam loader evidence | Beta.105 clicked UI smoke exists at `.tools/runtime-evidence/monkey-stability-20260622-025733/` with main-menu load, clean audit, 25/25 Spire Plus patches, StS1Events disabled, Off verifier pass, and packet verifier 1621 / 0. Beta.99 settings/Off proof, previous beta.96 Off evidence, and previous beta.93 AdditiveBatch1 direct evidence remain previous-package context only. | Pass for beta.105 startup/UI smoke; gameplay evidence pending |
| Plug-off loader evidence | Current beta.105 clicked Ancient UI smoke at `.tools/runtime-evidence/monkey-stability-20260622-025733/` loads only `STS2-RitsuLib` plus Spire Plus for startup/log and packet parity, with StS1Events disabled. Beta.99 Off proof and the older previous package-only normal Steam startup/log pass are historical disabled-mod loader-isolation context only. The settings-only disabled attempt `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142835` is invalid because Spire Plus still initialized. | Pass for beta.105 startup/UI smoke shape; disable-mod gameplay pending |
