...
Original audit date: 2026-05-15; latest beta.93 RitsuLib-only evidence refresh: 2026-06-21.
...
- Prove the installed package builds, publishes, hashes, loads through Steam with only STS2-RitsuLib plus Spire Plus, and has no release-blocking loader errors.
...
| Automation and release evidence | Build, publish, package refresh, installed-package checks, and focused runtime preflight pass target the current beta.93 RitsuLib-only package baseline. The release verifier covers zip hashes, duplicate rows, row kinds, invalid files/notes, screenshot size, and path containment; the `release-ready-path-containment-smoke` fixture rejects evidence dirs outside the evidence root and required-file/screenshot paths that escape their row evidence dir. Current beta.93 Off and AdditiveBatch1 direct smokes are loader/registration proof only; release audit still needs gameplay/UI/save-load/co-op proof and fails closed with 21 manual rows until evidence is added. | Automation gates work; release evidence is not complete. |
...
| Steam loader evidence | Current beta.93 RitsuLib-only startup/log evidence exists for Off and AdditiveBatch1 direct smokes. Previous direct AdditiveBatch1 startup/log evidence at `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937` loaded BaseLib `v3.3.0`, RitsuLib `0.4.24`, and Spire Plus, selected RitsuLib compat branch `0.107.0`, reported `v0.1.0-private-beta.88`, applied 25/25 Spire Plus patches, reached main menu, registered 10 StS1 event types through 14 calls, and audited clean. Beta.87 direct proof remains previous-game-version context only. | Pass for beta.93 loader/registration; gameplay evidence pending |
| Plug-off loader evidence | Current beta.93 RitsuLib-only Off direct proof at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` loads only `STS2-RitsuLib` plus Spire Plus for startup/log registration and packet parity. The older BaseLib-only normal Steam startup/log pass at `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020` is historical disabled-mod loader-isolation context only. The settings-only disabled attempt `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-142835` is invalid because Spire Plus still initialized. | Pass for current startup/log loader shape; disable-mod gameplay pending |
