...
Date: 2026-06-23 beta.127 handoff summary; older May/June package notes below are retained only as historical context.
...
- Latest source/package validation on 2026-06-23: `dotnet build`, `dotnet publish`,
  ...
  validation passed for beta.127 after the RitsuLib `0.4.34` dependency refresh,
  SavedAttachedState initialization hardening, RitsuLib default localization aliases,
  and low-risk Ancient reward hook migration. beta.127 clicked UI proof is pending. Previous beta.96 RitsuLib-only Off loader
...
The beta.127 pass updates the package and must use the validation section below as the current package truth.
...
- Package: `publish\SpirePlus-v0.1.0-private-beta.127.zip`
...
- README_INSTALL SHA256: `F613617DE67249F7721A7AEF4DDB22BCEB25D37E9317F1BA8540AC486B70D298`

## Known Automated Evidence

- Current automated snapshot: beta.127 passed `dotnet build` with 0 warnings / 0 errors, `dotnet publish` with only the known Godot ignored `source code` project warning, package refresh, installed-package parity, runtime preflight 28 / 0, and current source-workspace validation 58 checks / 0 mismatches with retained GDRE warnings only and local RitsuLib XML/API marker coverage. beta.127 clicked UI proof is pending.
...
- Previous beta.93 AdditiveBatch1 evidence registered 10 event types through 14 calls, audited clean, passed enabled-mode verifier 31 / 0, and passed packet verification with 61 / 0 checks for the previous package only. Current beta.127 enabled-mode proof and gameplay proof are still pending.
...
- Current package parity is represented by the hashes in **Package Under Test** and by `scripts/check-installed-spire-plus-package.ps1 -ModDirectory "E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance" -ExpectedPackageVersion "v0.1.0-private-beta.127"`, which also checks the packaged PCK's Sere Talon imported textures and Sere Talon / Tanx Claws content split.
