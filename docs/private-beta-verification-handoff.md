...
Date: 2026-06-21 beta.96 handoff summary; older May notes below are retained only as historical context.
...
Current beta.96 RitsuLib-only Off loader proof is startup/loading context only; previous beta.93 AdditiveBatch1 registration proof remains previous-package loader/registration context only.
...
- Current beta.96 Off evidence reached main menu with exactly `RitsuLib [STS2-RitsuLib] (0.4.31)` and `Spire Plus [EZMicroBalance] (v0.1.0-private-beta.96)` loaded, audited clean, logged StS1Events disabled with 0 registration lines, and passed packet verification with 43 / 0 checks.
- Previous beta.93 AdditiveBatch1 evidence registered 10 event types through 14 calls, audited clean, passed enabled-mode verifier 31 / 0, and passed packet verification with 61 / 0 checks for the previous package only. Current beta.96 enabled-mode proof and gameplay proof are still pending.
...
- Beta.96 Mod Settings clicked UI proof is current for settings-page visibility only, and beta.96 Off loader proof is current for startup/loading only.
