...
RitsuLib 能帮你解决其中一部分，尤其是 **注册、生命周期、持久化、设置、诊断、资源路径/本地化规范**。但它不能自动解决 Boss flow、extra-play 递归、Death Reprieve 死亡中断、multiplayer ownership 这类游戏逻辑问题。

---

# 1. 当前项目为什么容易出 bug

当前 `PROJECT_STATE.md` 已经说明项目是一个大型玩法包：Spire Plus、Ancient reward rebalance、Ascension 11–20、Rootblight、Urda、Morvi、Lotha、隐藏 Vakuu fight、Preview tools、网站/美术/发布证据都在一个仓库里，而且很多 live gameplay / clicked UI / save-load / co-op verification 仍 pending。

`MainFile.Initialize()` 现在直接初始化 Lotha、Morvi、Urda、VakuuFight、Ascension。 这说明项目仍然是：

```text
MainFile -> 直接调用所有 initializer
每个 initializer -> 自己注册 hook / patch / service
每个 feature -> 自己管理 gate / state / diagnostics
```
...
[ ] feature gate 命名不统一
[ ] disable/force 行为不统一
[ ] reward / combat / death / map / save-load 多处 patch 互相踩
[ ] source guard 很多，但行为级测试少
...
## 3.1 生命周期事件：替代散落 Harmony patch / run hook

RitsuLib 的 lifecycle docs 推荐：如果 mod 需要游戏时机但不需要自己拥有 Harmony patch，可以订阅 lifecycle event。它列出的常见事件包括 RunStarted、RunLoaded、RoomEntering/Entered/Exited、ActEntering/Entered、CombatStarting/Ended/Victory、SideTurnStarting/Started、CardPlaying/Played、CardMovedBetweenPiles、CardDrawn、CardExhausted、RewardTaken 等。([RitsuLib][6])

这和你现在的问题高度对应：

```text
Urda reward / room entry
Morvi combat start / card played / card drawn
Lotha death / turn start / reward suppression
Rootblight card moved / combat end
Ascension boss flow / map entry
```

如果用 RitsuLib lifecycle 把“时机”统一起来，就可以减少一堆散落 patch 和 static service 之间的顺序不明问题。

## 3.2 持久化：替代超长 semicolon string state

RitsuLib persistence 文档建议把每个要保存的概念定义成 class，通过 `BeginModDataRegistration` 和 `GetDataStore` 注册，并明确说“使用 class，而不是裸 primitive value，这样未来新增字段不需要换存储槽”。([RitsuLib][7])

它还支持不同 scope：Global、Profile、InMemory、RunSidecar，并说明普通 settings、progression、feature data 优先使用 ModDataStore。([RitsuLib][7])

这正好解决你当前的痛点：

```text
UrdaStateKey = string
MorviStateKey = string
LothaStateKey = string
各种 state 用 ; 分割
字段越加越长
旧存档迁移困难
```

RitsuLib 的 data store + migrations 很适合做：

```text
UrdaRunStateV1 / V2
MorviRunStateV1
LothaRunStateV1
AscensionRunMetadataV1
RootblightStateV1
PreviewToolsSettingsV1
```

## 3.3 Registry-first 内容管理：减少 MainFile 直接初始化所有模块

RitsuLib README 的 Main Entry Points 包括：

```text
RitsuLibFramework.CreateContentPack(modId)
RitsuLibFramework.CreatePatcher(modId, patcherName)
RitsuLibFramework.SubscribeLifecycle<TEvent>
RitsuLibFramework.GetDataStore(modId)
RitsuLibFramework.RegisterModSettings(modId, configure)
```



这说明它很适合承载我们之前提到的 FeatureRegistry / module registry 思路。不是说你必须完全采用它的 registry，而是可以把它当成：
...
## 好处 A：减少 static service 的无边界扩张

现在很多逻辑是：

```text
UrdaBlessingService.*
MorviBlessingService.*
LothaBlessingService.*
AscensionCombatModifierService
RootDeckService
```

这类 service 一开始方便，但越写越像“万能工具类”。RitsuLib 的 registry / lifecycle / data store 可以逼你把模块拆成：
...
RitsuLib persistence 文档明确支持 migrations，并建议保存 class 而不是 primitive。([RitsuLib][7]) 你的项目现在最需要的就是状态 migration：

```text
Urda string state -> UrdaRunStateV1
Morvi debt/openbook/borrowed state -> MorviRunStateV1
Lotha death reprieve / mirror state -> LothaRunStateV1
Ascension metadata -> AscensionRunStateV1
```

## 好处 C：生命周期 hooks 更统一

RitsuLib lifecycle events 覆盖 run flow、combat、cards、rewards、inventory、unlock 等常见事件。([RitsuLib][6]) 你当前很多 Harmony patch 其实是为了拿这些 timing。如果 RitsuLib 的事件足够覆盖，能减少 patch surfaces。
...
## 风险 D：RitsuLib API 本身也要实测

RitsuLib 文档发布时间很新，docs 里版本显示 v0.2.31 / v0.2.32。([RitsuLib][5]) 你的项目是高复杂 mod，不应该一次性把所有核心流程改到 RitsuLib。必须先做 proof-of-concept。
...
## Layer 2：引入 RitsuLib 做“架构层”

优先用 RitsuLib 处理：

```text
Feature registry / content pack
Lifecycle events
Data store / migrations
Settings grouping
Diagnostics / compatibility logging
Top-bar / UI helpers if needed
```

## Layer 3：自建 domain pipeline

RitsuLib 不能直接替你解决所有 domain 问题。你仍然需要自己建立：

```text
SpirePlusFeatureRegistry
SpirePlusRewardPipeline
SpirePlusCardPlayContext
SpirePlusDeathProtectionService
SpirePlusMultiplayerPolicy
SpirePlusStateCodec
```

这些是你项目自己的玩法复杂性，不是任何库能自动解决的。
...
```text
[ ] 添加 STS2.RitsuLib NuGet reference
[ ] manifest 添加 STS2-RitsuLib dependency
[ ] 不改任何 gameplay
[ ] 注册一个只打印日志的 lifecycle event
[ ] 注册一个 InMemory data store
[ ] 注册一个测试 settings section
[ ] 正常 Steam 启动
[ ] multiplayer 双端都装 RitsuLib 后能进 lobby
```
...
## Milestone 1：FeatureRegistry 迁移

目标：MainFile 不再直接调用所有 Initializer。

用 RitsuLib lifecycle / registry 思路实现：

```text
SpirePlusFeatureModule
SpirePlusFeatureRegistry
```
...
```text
[ ] MainFile 变短
[ ] 每个 feature 输出 enabled reason
[ ] Urda/Morvi/Lotha 默认开启不变
[ ] Vakuu hidden by default 不变
[ ] Ascension 初始化不变
```

## Milestone 2：DataStore / StateCodec POC

先选一个最小高收益对象：**Urda state**。

当前 Urda string state 是典型重构点。
...
注意：RitsuLib 文档说 RunSidecar 需要显式 StorageContext，不适合简单 Register 重载。([RitsuLib][7]) 所以这一步必须先查 API，不要凭空写。

## Milestone 3：Lifecycle 事件替换部分 Harmony patch
...
RitsuLib lifecycle docs 明确列了这些事件。([RitsuLib][6])

不要先碰 death hook / A20 boss transition / reward alternatives，因为这些更危险。
...
## Milestone 5：RewardPipeline / CardPlayContext / DeathProtection

这些是项目内部架构，不是“迁 RitsuLib”本身，但可以在 RitsuLib lifecycle 事件上运行。
...
```text
引入 RitsuLib 做架构层，分阶段迁移。
```

优先级：

```text
P0：POC：RitsuLib 加载 + lifecycle log + data store smoke
P1：FeatureRegistry / init decoupling
P1：Urda/Morvi/Lotha state codec
P1：lifecycle events 替换低风险 hook
P2：settings 分组
P2：diagnostics / compatibility
P3：逐步迁移内容注册
```
...
```text
ISSUE-2026-05-26-RITSULIB-MIGRATION-DECISION
ISSUE-2026-05-26-RITSULIB-POC-LOAD-LIFECYCLE-DATASTORE
ISSUE-2026-05-26-FEATURE-REGISTRY-DECOUPLING
ISSUE-2026-05-26-STATE-CODEC-MIGRATION
ISSUE-2026-05-26-REWARD-PIPELINE-ARCHITECTURE
ISSUE-2026-05-26-CARDPLAY-CONTEXT-ARCHITECTURE
ISSUE-2026-05-26-DEATH-PROTECTION-SERVICE
ISSUE-2026-05-26-MULTIPLAYER-POLICY-ANNOTATION
```
...
EZMicroBalanceCode/Core/Features/
  ...
  FeatureRuntimeStatus.cs

EZMicroBalanceCode/Core/State/
  IStateCodec.cs
  StateDecodeResult.cs

EZMicroBalanceCode/Core/Rewards/
  RewardPipeline.cs
  RewardPipelineContext.cs
  IRewardHandler.cs

EZMicroBalanceCode/Core/CardPlay/
  EzmbCardPlayContext.cs
  ExtraPlayPolicy.cs

EZMicroBalanceCode/Core/Death/
  EzmbDeathProtectionService.cs

EZMicroBalanceCode/Core/Multiplayer/
  MultiplayerPolicy.cs
  MultiplayerPolicyAttribute.cs
```

RitsuLib 可放在：

```text
EZMicroBalanceCode/Core/Ritsu/
  RitsuBootstrap.cs
  RitsuDataStores.cs
  RitsuLifecycleBridge.cs
```

---

# 10. 给 Codex 的迁移调研 prompt
...
必须先读：
1. PROJECT_STATE.md
2. EZMicroBalance.csproj
3. EZMicroBalanceCode/MainFile.cs
4. EZMicroBalanceCode/Ancients/Common/AncientSavedStateFields.cs
5. EZMicroBalanceCode/Ancients/Expansion/Urda/UrdaBlessingService.State.cs
6. EZMicroBalanceCode/Ancients/Expansion/Morvi/**
7. EZMicroBalanceCode/Ancients/Expansion/Lotha/**
8. docs/features/ancient-expansion-v2.2/**
...
Phase 1：研究并写文档
创建 docs/features/ritsulib-migration/：
- migration-decision.md
- base-vs-ritsulib-comparison.md
- migration-spec.md
- migration-plan.md
...
- 是否使用 RitsuLib content pack？

Phase 2：POC only
如果 NuGet 可用且风险低：
- 添加 STS2.RitsuLib 包引用。
- manifest 添加 STS2-RitsuLib dependency。
- 不改 gameplay。
- 注册一个 lifecycle log，比如 GameReadyEvent 或 RunStartedEvent。
- 注册一个 InMemory data store 或 Global test data store。
- 不迁移任何真实 state。
- POC 必须可关闭/可回滚。

Phase 3：FeatureRegistry spec
不一定实现，但写清：
- IFeatureModule
- FeatureRegistry
- FeatureGateResult
- how MainFile changes
- how existing initializers plug in

Phase 4：State codec spec
写清：
- UrdaStateV1
- MorviStateV1
- LothaStateV1
...
- data store file created / not created as expected
- disable Ritsu POC fallback
...
- git diff --check
- 不 publish，除非 package content changed

最终回复：
- 是否建议迁移：yes/no/partial
- 推荐迁移范围
- 不推荐迁移范围
- POC 是否完成
- 风险
...
```text
reward pipeline、extra-play context、death protection、multiplayer policy、state codec。
```
