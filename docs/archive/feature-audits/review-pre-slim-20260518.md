...
- P2: `scripts/bootstrap-windows.ps1` still introduced itself as the old `EzDailyContent` bootstrap, still warned for BaseLib v3.1.0, and told testers to confirm `EzDailyContent` in Mod Settings. Fix: retarget script text to Spire Plus / `EZMicroBalance` and BaseLib v3.1.2.
...
Current source defines 25 SavedSpireFields after Seedbed marker work, while the latest loader smoke evidence still reports 22. Some current docs called the 22-field smoke "current" without making the stale field-count boundary obvious.
...
- State carriers and save/load source carriers: checked `AncientPlayerState`, Urda/Morvi/Lotha run hooks, SavedSpireField declarations, and mirror guard tests; no direct state-field bypass or missing source carrier found beyond live persistence proof still pending.
