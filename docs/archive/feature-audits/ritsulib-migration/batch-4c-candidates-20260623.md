...
- beta.135 package parity, runtime preflight, and source-workspace validation passed for the 169/0 source state and 168 default compiled patch classes; previous beta.128 clicked Ancient UI smoke applied all 152 default runtime patch classes from that package.
- This source pass moved the six localization fallback patches to RitsuLib `IPatchMethod` / `ModPatcher`; later visual-hover UI, rest-site UI, Act Ancient unlock-list UI, Vakuu event-state UI, Ancient reward hook, low-risk reward hook, intent UI, Enemy Damage getter, RitsuLib compatibility, Urda Root Sight room-routing, Ascension map generation, and final A20 boss-flow migration passes raised the current source count to 169 migrated patches with 0 raw Harmony declarations remaining. The beta.128 clicked UI smoke covers the packaged default-runtime state for forced Ancient UI only.
...
- Before citing the current source migration as runtime coverage, capture a fresh package/runtime packet that proves the current source inventory and the 168 default compiled Spire Plus patch classes apply in the installed game.
