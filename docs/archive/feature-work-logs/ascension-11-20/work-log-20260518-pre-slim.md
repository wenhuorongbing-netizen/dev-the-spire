...
- Added gated multiplayer diagnostics (`EZMB_ASCENSION_MULTIPLAYER_DIAGNOSTICS=1`) with Harmony patches on:
  - `StartRunLobby.BeginRunForAllPlayers` 閳?lobby state before run start
  - `StartRunLobby.BeginRunLocally` 閳?ascension/player count at local launch
  - `StartRunLobby.UpdateMaxMultiplayerAscension` 閳?ascension cap computation
  - `NGame.StartNewMultiplayerRun` 閳?RunState player HP post-creation
  - `RunManager.EnterAct` 閳?player HP before and after act entry
  - `AncientEventModel.BeforeEventStarted` 閳?player HP before/after Neow healing
  - `SaveManager.SaveRun`, `NGame.ReturnToMainMenu`, `NGame.Quit` 閳?save/quit/disconnect logging
...
1. **Neow HP healing flow** (`AncientEventModel.BeforeEventStarted`, line 143-156):
   ...
   - `CreatureCmd.Heal` calls `creature.HealInternal(amount)` directly 閳?no queue dependency.
...
2. **Vanilla AscensionManager** (`AscensionManager.cs`):
   ...
   - Constructor accepts `int level` directly 閳?no clamping.
   - `HasLevel(AscensionLevel)` checks `_level >= (int)level` 閳?works for values > 10.
...
3. **Run start flow** (`NGame.StartNewMultiplayerRun`):
   - `RunState.CreateForNewRun()` with `ascensionLevel` from lobby 閳?`Player.CreateForNewRun()` uses `character.StartingHp` for both current and max HP.
   - `RunManager.SetUpNewMultiPlayer()` 閳?`InitializeNewRun()` 閳?`ApplyAscensionEffects()` (no HP change).
   - `StartRun()` 閳?`RunManager.Instance.EnterAct(0, doTransition: false)` 閳?Neow event starts 閳?`BeforeEventStarted` fires.
...
4. **Save/quit** (`NPauseMenu.cs`, refreshed v0.105.0 source):
   ...
   - `NGame.Quit()` saves settings/progress and calls `GetTree().Quit()` 閳?does not send network disconnect.
...
- English/ZHS Ascension text was refreshed; ZHS Fission wording uses "閼版鍏? and no longer uses "鐠愬湱鏁? for this mechanic.
...
- Confirmed `CardModel.HoverTips` appends keyword hover tips from `CanonicalKeywords`, so Rootblight and Blight Sprout should not repeat Exhaust / 濞戝牐鈧?in their descriptions when their models already expose `CardKeyword.Exhaust`.
...
- Updated Rootblight I/II/III and Blight Sprout descriptions in English and Simplified Chinese to remove duplicate `Play: Exhaust` / `閹垫挸鍤敍姘Х閼版, add `[gold]` card/pile terms, and match the current play/unplayed outcomes.
