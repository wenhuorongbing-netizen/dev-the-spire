...
- Date: 2026-06-22; latest addendum 2026-06-23: source now has 162 RitsuLib `IPatchMethod` classes and 8 raw Harmony declarations after the Batch 4c ascension-localization fallback, visual-hover UI, A20 reward proceed, Meat Cleaver rest-site UI, Preview transform prediction source/lifetime, Ascension selection/lobby UI, Neow/Vakuu event-option UI, A20 courtyard portrait, inline-localization fallback, Ancient reward getter/relic hook, low-risk reward hook, Aeonglass intent UI, Enemy Damage polish getter, RitsuLib settings-button compatibility, Urda Root Sight room-routing, Ascension map generation, multiplayer run-state diagnostics, and debug-only StS1 replacement-prototype migrations. beta.132 packages this 162/8 source state. Previous beta.128 runtime proof registers 152 patches because the StS1 replacement prototype is compile-symbol gated behind `REPLACEMENT_PROTOTYPE_ENABLED`; recapture beta.132 runtime proof before claiming current in-game coverage.
- Current package target is Spire Plus `v0.1.0-private-beta.132` on Slay the Spire 2 `v0.107.1`.
...
- beta.132 package refresh passed `dotnet build` with 0 warnings / 0 errors, `dotnet publish` with only the known Godot ignored `source code` project warning, and `scripts/package-spire-plus.ps1`.
- Installed-package parity passed for `publish/SpirePlus-v0.1.0-private-beta.132.zip`, the game-root zip, installed DLL, manifest, PCK, README, and Sere Talon/Tanx Claws package content. beta.132 artifact hashes are recorded in `docs/private-beta-verification-handoff.md`.
- 2026-06-23 beta.132 migration validation passed `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` with 0 warnings / 0 errors.
...
- Runtime preflight passed 28 checks / 0 mismatches for beta.132. RitsuLib latest-package guard passed 9 checks / 0 mismatches, keeping `STS2.RitsuLib` at `0.4.34`.
- Source-workspace validation passed 57 checks / 0 mismatches against installed `v0.107.1`, package `v0.1.0-private-beta.132`, and STS2-RitsuLib `0.4.34`, with two retained GDRE warnings only. The checker also verifies local `STS2-RitsuLib.xml` API-doc markers for `RegisterModSettings`, `BeginModDataRegistration`, `ModDataStore.Register`, `CreateContentPack`, `CreatePatcher`, and `SavedAttachedState`.
...
- beta.132 is validated through build, publish, package creation, installed-package parity, runtime preflight, and source-workspace checks.
- The previous beta.128 clicked Ancient UI proof covers forced Urda, Morvi, Lotha, and normal Vakuu UI only for that package; it does not prove beta.132 gameplay, save-load, gated Vakuu fight-option/victory return, co-op, or release readiness.
