...
- Current beta.128 clicked Ancient UI smoke proof passed at `.tools/runtime-evidence/monkey-stability-20260623-062913/`: 4 / 4 `AncientUiSmoke` iterations for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`, each with command ACK, screenshot, clean log audit, StS1 Off verifier pass, exact game/Ritsu/package markers, 152/152 default runtime Spire Plus ModPatcher patches applied, and packet verification 1621 / 0.

## Evidence Boundary

- beta.128 is validated through build, publish, package creation, installed-package parity, runtime preflight, source-workspace checks, and smoke-level forced clicked Ancient UI.
- The beta.128 clicked Ancient UI proof covers forced Urda, Morvi, Lotha, and normal Vakuu UI only; it does not prove gameplay, save-load, gated Vakuu fight-option/victory return, co-op, or release readiness.
