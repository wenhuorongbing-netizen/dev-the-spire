...
- Date: 2026-06-22; latest addendum: source now has 99 RitsuLib `IPatchMethod` classes and 70 raw Harmony declarations after the Batch 4c ascension-localization fallback, visual-hover UI, A20 reward proceed, Meat Cleaver rest-site UI, Preview transform prediction source/lifetime, Ascension selection/lobby UI, Neow/Vakuu event-option UI, A20 courtyard portrait, and inline-localization fallback migrations. beta.117 build, publish, package refresh, installed-package parity, runtime preflight, focused migration/preview/release metadata tests, and current-doc claims are current for this source pass; beta.108 clicked UI smoke is retained as previous-package evidence only.
- Current package target is Spire Plus `v0.1.0-private-beta.117` on Slay the Spire 2 `v0.107.1`.
...
- beta.117 package refresh passed `dotnet build` with 0 warnings / 0 errors, `dotnet publish` with only the known Godot ignored `source code` project warning, and `scripts/package-spire-plus.ps1`.
- Installed-package parity passed for `publish/SpirePlus-v0.1.0-private-beta.117.zip`, the game-root zip, installed DLL, manifest, PCK, README, and Sere Talon/Tanx Claws package content. beta.117 artifact hashes are recorded in `docs/private-beta-verification-handoff.md`.
...
- Source-workspace validation passed 58 checks / 0 mismatches against installed `v0.107.1`, package `v0.1.0-private-beta.117`, and STS2-RitsuLib `0.4.34`, with retained GDRE warnings only. The checker also verifies local `STS2-RitsuLib.xml` API-doc markers for `RegisterModSettings`, `BeginModDataRegistration`, `ModDataStore.Register`, `CreateContentPack`, `CreatePatcher`, and `SavedAttachedState`.
...
- This is previous-package smoke-level clicked Ancient UI migration proof; recapture it for beta.117 before tester handoff or release claims.
