...
- Date: 2026-06-22; latest addendum 2026-06-23: source now has 151 RitsuLib `IPatchMethod` classes and 18 raw Harmony declarations after the Batch 4c ascension-localization fallback, visual-hover UI, A20 reward proceed, Meat Cleaver rest-site UI, Preview transform prediction source/lifetime, Ascension selection/lobby UI, Neow/Vakuu event-option UI, A20 courtyard portrait, inline-localization fallback, Ancient reward getter/relic hook, low-risk reward hook, Aeonglass intent UI, Enemy Damage polish getter migrations, and the RitsuLib settings-button compatibility patch. beta.127 packages this 152/18 source state and has no-launch validation; beta.127 game-launch/runtime patch-count proof is still pending.
- Current package target is Spire Plus `v0.1.0-private-beta.127` on Slay the Spire 2 `v0.107.1`.
...
- beta.127 package refresh passed `dotnet build` with 0 warnings / 0 errors, `dotnet publish` with only the known Godot ignored `source code` project warning, and `scripts/package-spire-plus.ps1`.
- Installed-package parity passed for `publish/SpirePlus-v0.1.0-private-beta.127.zip`, the game-root zip, installed DLL, manifest, PCK, README, and Sere Talon/Tanx Claws package content. beta.127 artifact hashes are recorded in `docs/private-beta-verification-handoff.md`.
- 2026-06-23 no-game validation for the dependency/doc/guard addendum passed `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` with 0 warnings / 0 errors. Split test-project validation passed `ReleaseEvidenceGateTests` 9 / 0 / 0 / 9 and the complementary `FullyQualifiedName!~ReleaseEvidenceGateTests` lane 652 / 0 / 40 / 692 after the RitsuLib migration-shape and doc-encoding guard updates.
...
- 2026-06-23 current-doc claims passed 1323 / 0; repository hygiene, `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore`, full tracked-text retired-dependency scan, active old-GitHub-lag wording scan, and `git diff --check` passed. `git diff --check` reported only CRLF normalization warnings.
...
- Source-workspace validation passed 58 checks / 0 mismatches against installed `v0.107.1`, package `v0.1.0-private-beta.127`, and STS2-RitsuLib `0.4.34`, with two retained GDRE warnings only. The checker also verifies local `STS2-RitsuLib.xml` API-doc markers for `RegisterModSettings`, `BeginModDataRegistration`, `ModDataStore.Register`, `CreateContentPack`, `CreatePatcher`, and `SavedAttachedState`.
...
- beta.127 is validated through build, publish, package creation, installed-package parity, runtime preflight, and source-workspace checks only. It has not been game-launched or clicked-UI-smoked.
- The retained beta.123 clicked Ancient UI proof covers the earlier packaged 127-patch state only; it does not prove beta.127 runtime patch registration.
