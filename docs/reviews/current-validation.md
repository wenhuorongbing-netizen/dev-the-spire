...
- Date: 2026-06-22; latest addendum 2026-06-23: source now has 127 RitsuLib `IPatchMethod` classes and 43 raw Harmony declarations after the Batch 4c ascension-localization fallback, visual-hover UI, A20 reward proceed, Meat Cleaver rest-site UI, Preview transform prediction source/lifetime, Ascension selection/lobby UI, Neow/Vakuu event-option UI, A20 courtyard portrait, inline-localization fallback, Ancient reward getter/relic hook, Aeonglass intent UI, Enemy Damage polish getter migrations, and the RitsuLib settings-button compatibility patch. beta.123 package/runtime proof remains the current package target; the 2026-06-23 addendum refreshed dependency evidence, RitsuLib-only migration guard assertions, active-source manifest coverage, and active-doc mojibake cleanup without publishing a new tester package.
- Current package target is Spire Plus `v0.1.0-private-beta.123` on Slay the Spire 2 `v0.107.1`.
...
- beta.123 package refresh passed `dotnet build` with 0 warnings / 0 errors, `dotnet publish` with only the known Godot ignored `source code` project warning, and `scripts/package-spire-plus.ps1`.
- Installed-package parity passed for `publish/SpirePlus-v0.1.0-private-beta.123.zip`, the game-root zip, installed DLL, manifest, PCK, README, and Sere Talon/Tanx Claws package content. beta.123 artifact hashes are recorded in `docs/private-beta-verification-handoff.md`.
...
- Source-workspace validation passed 57 checks / 0 mismatches against installed `v0.107.1`, package `v0.1.0-private-beta.123`, and STS2-RitsuLib `0.4.34`, with retained GDRE warnings only. The checker also verifies local `STS2-RitsuLib.xml` API-doc markers for `RegisterModSettings`, `BeginModDataRegistration`, `ModDataStore.Register`, `CreateContentPack`, `CreatePatcher`, and `SavedAttachedState`.
