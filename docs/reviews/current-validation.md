...
- Date: 2026-06-22; latest addendum 2026-06-23: source now has 169 RitsuLib `IPatchMethod` classes and 0 raw Harmony declarations after the ascension-localization fallback, visual-hover UI, A20 reward proceed, Meat Cleaver rest-site UI, Preview transform prediction source/lifetime, Ascension selection/lobby UI, Neow/Vakuu event-option UI, A20 courtyard portrait, inline-localization fallback, Ancient reward getter/relic hook, low-risk reward hook, Aeonglass intent UI, Enemy Damage polish getter, RitsuLib settings-button compatibility, Urda Root Sight room-routing, Ascension map generation, multiplayer run-state/save-quit diagnostics, and debug-only StS1 replacement-prototype migrations. beta.135's default DLL compiles 168 patch classes because `Sts1ReplacementPrototype` is compile-symbol gated behind `REPLACEMENT_PROTOTYPE_ENABLED`; recapture beta.135 runtime proof before claiming current in-game coverage.
...
- 2026-06-23 RitsuLib-first docs/bootstrap cleanup passed `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` with 0 warnings / 0 errors; focused DocumentationCompactness, RitsuLibMigration, and EngineeringGovernance tests passed 87 / 0 / 0 / 87; `dotnet format`, `git diff --check`, RitsuLib latest-package guard 9 / 0, repository hygiene, current-doc claims 1331 / 0, static-file hygiene 13 / 0, patch-inventory freshness, worktree batch classification, full tracked-text retired-dependency scan, and active raw `[HarmonyPatch]` source scan all passed. No publish/package refresh or live game proof was run because this pass changed only source comments/log wording, active docs, and tests.
- 2026-06-23 RitsuLib registry naming cleanup retired the old migration-named runtime patch registry, standardized the runtime entry as `SpirePlusRitsuLibPatchRegistry`, renamed the bootstrap entry to `ApplyRitsuLibPatches`, and refreshed docs/tests/scripts/patch inventory to match the RitsuLib-owned registration surface. Validation passed `dotnet build EZMicroBalance.sln -m:1 --no-incremental -p:UseSharedCompilation=false` with 0 warnings / 0 errors; focused RitsuLibMigration, DocumentationCompactness, EngineeringGovernance, and ActiveSourceManifest guards passed 89 / 0 / 0 / 89; `dotnet format`, `git diff --check`, RitsuLib latest-package guard 9 / 0, repository hygiene, current-doc claims 1331 / 0, static-file hygiene 13 / 0, patch-inventory freshness, worktree batch classification, full tracked-text retired-dependency scan, and active raw `[HarmonyPatch]` source scan all passed. No publish/package refresh or live game proof was run because this pass changed only runtime names, active docs, scripts, generated inventory, and tests.
