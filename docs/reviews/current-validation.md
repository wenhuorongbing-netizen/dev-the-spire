...
- Current ReleaseEvidence no-launch template split moved the no-launch collector/verifier contract test and versioned package-hash assertions into `ReleaseEvidenceGateTests.NoLaunchTemplates.cs`, leaving `ReleaseEvidenceGateTests.cs` focused on package path derivation and stale package-hash rejection. Validation passed after a stale current-repo `testhost` lock exited: parser check passed for `scripts/analyze-spire-plus-runtime-failure.ps1`; `dotnet build EZMicroBalance.sln -m:1 --no-incremental` returned 0 warnings / 0 errors; focused `ReleaseEvidenceGateTests` plus `RuntimeMonkeyStabilityGuardTests` passed 52 / 0 / 0 / 52. No publish, package/release-evidence validation, runtime smoke, gameplay, QA, release, or handoff gate was closed.
