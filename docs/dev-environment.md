...
- The recovered local source snapshot and prior clean `v0.106.1` loader evidence remain historical. Current `v0.107.0` beta.86 direct smokes are clean for Off, CanaryOnly, and AdditiveBatch1: `.tools\runtime-evidence\v01070-beta86-off-direct-20260618-115305`, `.tools\runtime-evidence\v01070-beta86-canary-direct-cleanlog-20260618-115723`, and `.tools\runtime-evidence\v01070-beta86-additive-batch1-direct-cleanlog-20260618-115723` all reached main menu, loaded BaseLib/RitsuLib/Spire Plus, selected RitsuLib compat branch `0.107.0`, applied 25/25 Spire Plus ModPatcher patches, audited clean, and passed retained mode/packet verifiers with 0 mismatches.
- Current refactor/runtime pass evidence: solution build and publish passed with 0 errors and 0 warnings during the beta.86 package pass, installed package checker passed for beta.86, runtime preflight passed 27 / 0, and the fresh loader smokes above closed the current beta.86 Off/CanaryOnly/AdditiveBatch1 loader-registration blocker. The direct-smoke false failure was helper settings drift: direct `--force-steam off` reads default profile settings, while the helper prepared only Steam-user settings and wrote `mods_enabled` at the wrong JSON level. Earlier cross-thread aborts are runner-contamination evidence only.
...
- Current source defines 30 SavedSpireFields. Historical 22-field and 16-field startup rows remain useful records only; beta.86 Off, CanaryOnly, and AdditiveBatch1 loader parity has fresh live evidence, while gameplay proof still needs manual verification.
...
- Historical RitsuLib diagnostic Off, CanaryOnly, and AdditiveBatch1 loader gates have clean `v0.106.1` evidence with BaseLib, RitsuLib, and Spire Plus loaded and 25/25 Spire Plus ModPatcher patches applied. Current beta.86 `v0.107.0` proof now covers Off with 0 StS1 registrations, CanaryOnly with 4 event types / 6 registration calls, and AdditiveBatch1 with 10 event types / 14 registration calls and exact tuple parity. Live gameplay, save-load, failure/death-path, clicked Ancient UI, preview-tools, and co-op verification remain pending.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.86.zip`, `E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `E:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.86.zip` were refreshed and hash-checked on 2026-06-18 for local manual testing.
  ...
  The current `v0.107.0` beta.86 direct loader rows are clean under `.tools\runtime-evidence\v01070-beta86-off-direct-20260618-115305`, `.tools\runtime-evidence\v01070-beta86-canary-direct-cleanlog-20260618-115723`, and `.tools\runtime-evidence\v01070-beta86-additive-batch1-direct-cleanlog-20260618-115723`, with Off 0 StS1 registrations, CanaryOnly 4 event types / 6 registration calls, and AdditiveBatch1 10 event types / 14 registration calls with exact tuple parity.
