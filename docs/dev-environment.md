...
- BaseLib `v3.1.2` compatibility with v0.105.1:
  - Historical 22-field loader evidence: `.tools\runtime-evidence\live-spire-plus-session-20260515-211414` reached main menu with only BaseLib plus Spire Plus / `EZMicroBalance`, BaseLib `177 patches successfully, 0 failed`, config registration, `Finished mod initialization for 'Spire Plus' (EZMicroBalance)`, `Found 22 SavedSpireFields`, `Time to main menu: 13,539ms`, and no release-blocking log signatures.
  - Current source defines 25 SavedSpireFields after the 2026-05-17 static fixes; this is not refreshed 25-field package parity. Fresh live loader smoke remains pending for the refreshed package.
  - Current display-name list evidence shows `Spire Plus`; older Mod Settings page evidence still uses the historical `EZ Micro Balance` display name.
...
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-20 after the governance/package refresh. Result: passed, 202 passed, 18 skipped release artifact/runtime evidence tests, 0 failed.
...
- Latest opt-in artifact-validation run: `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-20 after the governance/package refresh. Result: passed, 220 passed, 0 skipped, 0 failed.
- Latest full local CI-script run: `scripts\ci-full-validation.ps1 -IncludeFuturePeek` on 2026-05-20 with explicit `STS2_PATH` and `GODOT_PATH`. Result: passed. This is local no-game validation, not live loader or gameplay proof.
...
- Last attempted publish: `dotnet publish EZMicroBalance.sln` on 2026-05-20 for the governance/package refresh; Future Peek source was not republished in this package pass.
...
- Last PCK hash check: the 2026-05-20 local manual-test package refresh left the installed, staging, versioned, and zip-entry PCK at SHA256 `073CAF976C91D9E6CEA39FA90FB5A6417E66CD5E12DED5EDD8169C892A0F0538`.
- Last staging/versioned DLL hash check: SHA256 `A1D86D01E57E0F58617ACA23EA8094B1AF35F525E3254007DE3675A1289B8159`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.0.zip` was rebuilt from the configured installed artifacts on 2026-05-20.
  ...
  - README SHA256: `C9F19363848AEECD4B763BFF7BB2B75980A90BFE22358ACEC8FF5E9E5C129CE4`
...
- Target EZMicroBalance Mod Settings verification:
  ...
  - Earlier package startup/log evidence under `.tools\runtime-evidence\current-package-smoke-20260514-015901` logged `Finished mod initialization for 'Spire Plus' (EZMicroBalance)`, `Loaded 2 mods (2 total)`, `Found 22 SavedSpireFields`, and 0 release-blocking hits. This is historical 22-field loader evidence, not refreshed 25-field package parity.
...
- 2026-05-08: fresh RC1 normal Steam-client isolated startup log after the structured-dependency manifest refresh:
  - Evidence: temporarily moved 23 non-BaseLib/EZMB mod entries, launched through `D:\Steam\steam.exe -applaunch 2868840`, reached main menu in `13,470ms`, saved `.tools\runtime-evidence\rc1-normal-steam-clean-godot-20260508-090122.log`, then restored moved entries and `settings.save`.
  - Positive signals: only `BaseLib\BaseLib.json` and `EZMicroBalance\EZMicroBalance.json` were discovered, `Loaded 2 mods (2 total)`, BaseLib applied `177 patches successfully, 0 failed`, EZ Micro Balance initialized, and BaseLib reported `Found 13 SavedSpireFields`.
...
- 2026-05-14: historical package smoke/log/resource verification under `.tools\runtime-evidence\current-package-smoke-20260514-015901`:
  ...
  - Boundary: Current source defines 25 SavedSpireFields after the 2026-05-17 static fixes, so this smoke is historical loader/log/resource evidence only. Gameplay/manual gates remain pending.
...
- Ascension 11-20 implementation track:
  ...
  - Gates: `EZMB_ASCENSION_DISABLE_PUBLIC_SELECTION=1`, `EZMB_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1`, and legacy-compatible `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1`.
  - Live Ascension and live co-op verification remain pending. Host A20 multiplayer logs describe development-test status and downgraded Dual King Brand / second-boss Brand behavior.
  ...
  - A20 uses the vanilla double-boss path for Boss 2 creation/reveal, Boss 2 Brand metadata, Boss 1 recovery, one Boss card reward, narrow Boss 1 reward-screen intermission wording, and a fixed courtyard event before Boss 2. A full custom intermission screen remains deferred.
