...
- The recovered local source snapshot and prior clean `v0.106.1` loader evidence remain historical. Current `v0.107.0` beta.85 Off smoke is clean under `.tools\runtime-evidence\v01070-beta85-current-package-runtime-fix-20260611-0510`: main menu reached, BaseLib/RitsuLib loaded, RitsuLib selected compat branch `0.107.0`, Spire Plus applied 25/25 ModPatcher patches, and `godot-log-audit.json` is clean.
- Current refactor/runtime pass evidence: solution build and publish passed with 0 errors and 0 warnings, rebuilt split no-build test lanes passed with 475 passed / 0 failed / 21 skipped / 496 total, installed package checker passed for beta.85, and the fresh loader smoke above closed the package runtime blocker. Earlier cross-thread aborts are runner-contamination evidence only.
...
- Current source defines 30 SavedSpireFields. Historical 22-field and 16-field startup rows remain useful records only; beta.85 loader parity has fresh live evidence, while gameplay proof still needs manual verification.
...
- Historical RitsuLib diagnostic Off, CanaryOnly, and AdditiveBatch1 loader gates have clean `v0.106.1` evidence with BaseLib, RitsuLib, and Spire Plus loaded and 25/25 Spire Plus ModPatcher patches applied. Current `v0.107.0` beta.85 Off smoke with installed RitsuLib `v0.4.16` reached main menu and audited clean; current StS1 CanaryOnly/AdditiveBatch1 proof is still pending and must show CanaryOnly 4 event types / 6 registration calls plus AdditiveBatch1 10 event types / 14 registration calls before gameplay or handoff claims. Live gameplay, save-load, failure/death-path, clicked Ancient UI, preview-tools, and co-op verification remain pending.

## Last known commands
- Last attempted build: `dotnet build EZMicroBalance.sln -m:1 --no-incremental` on 2026-06-17 during the migration validation-lane helper continuation. Result: succeeded with 0 warnings and 0 errors.
- Last successful build: `dotnet build EZMicroBalance.sln -m:1 --no-incremental` on 2026-06-17 during the migration validation-lane helper continuation. Result: succeeded with 0 warnings and 0 errors.
...
- Latest rebuilt split no-build test lanes on 2026-06-17 after the validation-lane helper/docs refresh: the isolated `ReleaseEvidenceGateTests` class passed 9 passed, 0 skipped, 0 failed, 9 total in the clean `migration-ci-helper-release-evidence-final-tree-diag.log` run; the complementary no-build test-project lane excluding `ReleaseEvidenceGateTests` passed 448 passed, 39 skipped release artifact/runtime/local-source tests, 0 failed, 487 total.
...
- Latest opt-in artifact-validation run: `STS2_PATH=E:\Steam\steamapps\common\Slay the Spire 2` and `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test tests\EZMicroBalance.Tests\EZMicroBalance.Tests.csproj --no-build --filter "FullyQualifiedName~ReleaseArtifactTests|FullyQualifiedName~ReleasePackageArtifactGuardTests|FullyQualifiedName~ReleaseArtifactParityGuardTests|FullyQualifiedName~AscensionV2MilestoneGuardTests|FullyQualifiedName~AncientBehaviorGuardTests"` on 2026-06-11 after the beta.85 package refresh. Result: passed, 67 passed, 0 skipped, 0 failed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` remains accepted.
- Latest full local CI-script run: `scripts\ci-full-validation.ps1` on 2026-05-20 with explicit `STS2_PATH` and `GODOT_PATH`. Result: passed. This is local no-game validation, not live loader or gameplay proof. As of 2026-06-17, the script defaults to the beta.85 split `ReleaseEvidenceGateTests` strategy, but that updated full script has not been rerun end-to-end with publish/package.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.85.zip`, `E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `E:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.85.zip` were refreshed and hash-checked on 2026-06-11 for local manual testing. The zip uses the player-facing `SpirePlus` archive name while the install folder remains `EZMicroBalance`. The current `v0.107.0` beta.85 Off loader row is clean under `.tools\runtime-evidence\v01070-beta85-current-package-runtime-fix-20260611-0510`; current StS1 enabled-mode smoke, live gameplay, save-load, failure/death-path, clicked Ancient UI, and co-op verification remain pending.
- Last PCK hash check: the 2026-06-11 local package-hash refresh found the staging, versioned, and zip-entry PCK at SHA256 `9F1595C7401CACE4E7A4345CCC3CA1E256EBD2615B56A32E5908D61682E024B9`.
- Last staging/versioned DLL hash check: SHA256 `F35E2706140AC7B0560FDB950AC479529D4DBA3CA08DA5341FBE987B4106E062`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.85.zip` was rebuilt from the real installed mod artifacts, copied to the game root, and synced into the real installed mod folder on 2026-06-11. The staging, versioned, installed, game-root zip, and zip-entry artifacts match the hashes below.
  ...
  - README SHA256: `004F7E2086C336F7804336FFC3FCD4061B0694F06C2B9B55459993C6737A498D`
