...
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-08 after the RC1 issues-file live-evidence guard was added. Result: passed, 66 passed, 16 skipped release artifact/runtime evidence tests, 0 failed.
- Last attempted test run: `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-08 after the RC1 issues-file live-evidence guard was added. Result: passed, 82 passed, 0 skipped, 0 failed.
...
- Last attempted publish: `dotnet publish EZMicroBalance.sln` on 2026-05-08 during the RC1 documentation/live-validation hygiene pass.
...
- Last PCK audit: automated release tests parsed `D:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance\EZMicroBalance.pck` after publish/package refresh. Result: 0 entries from `EzDailyContent`, `EzDailyContentCode`, `EZMicroBalanceCode`, `docs`, `art_pipeline`, `asset`, `source code`, or `legacy`; SHA256 `435D55B14FAD38F611C550F4ACAF604EE1A2C3E63E75C52FC3FA9FCE52D064CA`.
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `1AEE7CD1C6EB945F022CB85997ADC709D930C3E6FC318E7E0EFE1A13436C589F`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt after the 2026-05-08 RC1 Mod Settings config-page pass. SHA256 `BE05559B4EA1180FB88129235A980978B1E2498187F1CB665882EC7DCC1CD314`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
...
- Target EZMicroBalance Mod Settings verification: passed for RC1. A no-op BaseLib `ModConfig` page is registered for EZ Micro Balance, and the normal Steam-client UI probe opened `模组配置` after temporarily isolating non-BaseLib/EZMB mods. Evidence `.tools\runtime-evidence\rc1-modsettings-page-20260508-095137-modconfig-list.png` shows the EZ Micro Balance page entry as `微平衡`; `.tools\runtime-evidence\rc1-modsettings-page-20260508-095137-ezmb-page.png` shows its `无可配置选项。` page; `.tools\runtime-evidence\rc1-normal-steam-modsettings-page-godot-20260508-095137.log` shows `Registered config for mod EZMicroBalance`, `Loaded 2 mods (2 total)`, and 0 `ERROR` / release-blocking signature hits.
...
- Runtime verification still requires live gameplay/manual feature matrix checks; normal Steam-client Mod Settings, the Act 1 A11 map/save-load spot check, and Act 2/3 A11 map-surface observation are now covered by RC1 evidence.
...
- Remaining before private beta: manual feature matrix, save/load, multiplayer disposition, author decision, clean commit, and user-approved push.
