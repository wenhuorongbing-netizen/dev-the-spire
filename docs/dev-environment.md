...
- The recovered local source snapshot has been refreshed from the installed `v0.107.1` game PCK and `sts2.dll` using GDRE Tools `v2.5.0`. The 2026-06-22 `scripts\check-local-godot-source-workspace.ps1 -RequireCurrentSourceSnapshot -ExpectedPackageVersion v0.1.0-private-beta.104 -ExpectedRitsuLibVersion 0.4.33 -ExpectedRitsuCompatBranch 0.107.1 -FailOnMismatch` recheck passed 57 checks / 0 mismatches for the current package. Retained warnings are the GDRE 18 failed scripts and one debug-scene parse warning, not source identity mismatches. Retained `v0.107.0` beta.85/beta.86/beta.87 direct smokes remain previous-package/game-version loader context.
...
- Current RitsuLib-only migration evidence: solution build, focused guards, publish, package refresh, installed package checker, runtime preflight, source-workspace checker, and clicked Ancient UI smoke passed for beta.104 after the STS2-RitsuLib `v0.4.33` update. The retained beta.88/beta.90/beta.93/beta.96/beta.99 smokes are previous-package contexts only. The direct-smoke verifier now accepts both append-style current-log slices and Godot-rewritten logs only when `godot.log.current-iteration` matches the retained after-launch log. Earlier cross-thread aborts are runner-contamination evidence only.
...
- Historical display-name list evidence under `.tools\runtime-evidence\current-spire-plus-modsettings-20260513-111342` shows `Spire Plus`; older page-level Mod Settings evidence predates the display-name refresh.
  ...
  Treat it as previous-package settings-page context after beta.104.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.104.zip`, `E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `E:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.104.zip` were refreshed and hash-checked on 2026-06-22 for local manual testing.
  ...
  - beta.104 package parity, source-workspace validation, and clicked Ancient UI smoke are current package context.
  - beta.99 Mod Settings proof is under `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210` with clean log audit and StS1 Off runtime shape verification 21 / 0; it is previous-package settings-page context after beta.104.
  ...
  Live gameplay, save-load, failure/death-path, gated Vakuu fight-option/victory return, and co-op verification remain pending; beta.104 clicked Ancient UI smoke exists under `.tools\runtime-evidence\monkey-stability-20260622-025733`.
- Last PCK hash check: the 2026-06-22 local package-hash refresh found the staging, versioned, and zip-entry PCK at SHA256 `5DE44D25785B5250EFB755691895F168DA6448E4D6B9086A39DD34298BBAEBBC`.
- Last staging/versioned DLL hash check: SHA256 `9AE370B89C1A74D278F78424EC1ABF0DAB72942C665469ACDE6E488BF289DBF1`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.104.zip` was rebuilt from the real installed mod artifacts, copied to the game root, and synced into the real installed mod folder on 2026-06-22. The staging, versioned, installed, game-root zip, and zip-entry artifacts match the hashes below.
  ...
  - README SHA256: `8124C4EAEFBBC46EC3DB5F752C9F3E06161CE208D4059885617AA4E1ACF3D0FD`
...
- Current target Mod Settings verification: beta.99 screenshot/page proof is retained at `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210` after the settings-page I18N resource migration.
  ...
  It is previous-package settings-page context after beta.104; rerun only if settings UI code/resources or RitsuLib version changes again.
