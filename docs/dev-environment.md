...
- Current source defines 30 SavedAttachedState fields. Historical 22-field and 16-field startup rows remain useful records only. Beta.85/beta.86/beta.87 `v0.107.0`, beta.88 previous package `v0.107.1`, and beta.90 RitsuLib-only loader rows are previous-context evidence; beta.93 Off and AdditiveBatch1 loader/registration proof is current RitsuLib-only evidence, and gameplay proof still needs manual verification.
...
- Beta.93 live gameplay verification remains pending.
