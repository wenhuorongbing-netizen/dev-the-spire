...
- BaseLib `v3.1.2` compatibility with v0.105.0: controlled main-menu smoke passed with only BaseLib + EZMicroBalance enabled. The RC1 normal Steam-client launch/log probe reached main menu with only BaseLib + EZMicroBalance loaded, BaseLib `177 patches successfully, 0 failed`, and no v0.105 removed-API signatures; it still has unrelated local invalid-manifest/dependency `ERROR` lines, so the clean normal Steam-client log gate and Mod Settings UI pass remain pending. The earlier `v3.1.0` / 17-mod live log (`godot2026-05-08T05.06.30.log`) is superseded for loader evidence, but live gameplay/combat verification is still pending.
...
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `70E7D2FF06C067A139027E2B64DFAA76E9638C990E40B0A504CCD34EE6FE9174`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt after the 2026-05-08 RC1 documentation/live-validation hygiene pass. SHA256 `C928B50616109FF198405F3990A1F4DA40FA9460E8CC6DFE69CC95784DBEEAE2`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
...
- Target EZMicroBalance Mod Settings verification: pending UI pass. The RC1 normal Steam-client launch/log probe reached main menu but did not open Mod Settings.
...
- 2026-05-08: RC1 normal Steam-client launch/log probe used `D:\Steam\steam.exe -applaunch 2868840` and reached main menu in `14,444ms`. `godot.log` showed `Loaded 2 mods (19 total)`, BaseLib `Version=3.1.2.0`, `[BaseLib] Applied 177 patches successfully, 0 failed`, BaseLib and EZ Micro Balance initialization, and `Found 12 SavedSpireFields`. Strict scan found 0 `Creature.get_ShowsInfiniteHp`, 0 `BaseLib.Patches.UI.HealthBarForecastPatch`, 0 BaseLib undefined-target patch failures, 0 `TypeLoadException`, 0 `MissingMethodException`, and 0 EZMB error/exception pattern hits. This log still contains unrelated local invalid-manifest/dependency `ERROR` lines for discovered disabled mods (`RouteSuggestConfig.json`, `sts2-heybox-support`) and old-style dependency warnings, so it is not the clean normal Steam-client log gate.
