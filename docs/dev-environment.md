...
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-08 after publish/package refresh. Result: passed, 62 passed, 15 skipped release artifact/runtime evidence tests, 0 failed.
- Last attempted test run: `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-08 after package/hash/smoke refresh. Result: passed, 77 passed, 0 skipped, 0 failed.
- Last formatting check: `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` on 2026-05-08 after the A20 host-only warning/current smoke pass. Result: exit code 0.
- Last required diff check: `git diff --check` on 2026-05-08 after the A20 host-only warning/current smoke pass. Result: exit code 0 with CRLF normalization warnings for touched files.
...
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `BC4CB9AC61C2F59172F79B96A1996782A15FC247449EB02B5D9AE62A65C0F27D`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt after the 2026-05-08 A20 host-only warning/current smoke pass. SHA256 `F3F601C1ED304203D269A905F48A2E6F66F468F2D4C5D7E34E1080A79BE74B2C`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
...
- 2026-05-08: current-package bounded `--force-steam off` smoke after the A20 host-only multiplayer warning, publish, and package refresh temporarily enabled only `BaseLib` and `EZMicroBalance`, explicitly disabled other discovered local mods, loaded exactly 2 mods (`Loaded 2 mods (19 total)`), initialized BaseLib and EZ Micro Balance, loaded the current installed `EZMicroBalance.dll` and `.pck`, confirmed the current source defines 12 SavedSpireFields and runtime reported `Found 12 SavedSpireFields`, reached main menu in `12,886ms`, found 0 EZ Micro Balance error/exception lines, and restored both `default\1\settings.save` and `settings.save.backup` byte-for-byte.
