...
- Current RitsuLib-only migration evidence: solution build, publish, package refresh, installed package checker, runtime preflight, and source-workspace checker passed for beta.98 after the hash-doc refresh and STS2-RitsuLib `v0.4.32` update. The retained beta.88/beta.90/beta.93/beta.96 smokes are previous-package contexts only. The direct-smoke verifier now accepts both append-style current-log slices and Godot-rewritten logs only when `godot.log.current-iteration` matches the retained after-launch log. Earlier cross-thread aborts are runner-contamination evidence only.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.98.zip`, `E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `E:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.98.zip` were refreshed and hash-checked on 2026-06-21 for local manual testing.
  ...
  - beta.98 package parity is current package context.
...
- Last PCK hash check: the 2026-06-21 local package-hash refresh found the staging, versioned, and zip-entry PCK at SHA256 `626C1671247E4C6DCC6CD18355E4E3855690C8EF943366CBA89BC9ACD75B1E01`.
- Last staging/versioned DLL hash check: SHA256 `A16EAA4E60006DCF6776A93B384C62183BFB6324FC1710A52E51E9AC0812BEFF`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.98.zip` was rebuilt from the real installed mod artifacts, copied to the game root, and synced into the real installed mod folder on 2026-06-21. The staging, versioned, installed, game-root zip, and zip-entry artifacts match the hashes below.
  ...
  - README SHA256: `14A6FEDA48F2928C89014ED811BFA886B5EE572C78D2A1CEFAFA7A8FD58B52D1`
...
- Current target Mod Settings verification: beta.98 screenshot/page proof remains pending after the settings-page I18N resource migration. Previous beta.96 screenshot/page proof was captured with only STS2-RitsuLib and `EZMicroBalance` enabled at `.tools\runtime-evidence\beta96-ritsulib-mod-settings-clicked-ui-20260621-160701`. Historical list evidence under `.tools\runtime-evidence\current-spire-plus-modsettings-20260513-111342` showed `Spire Plus` with previous package plus `EZMicroBalance` in an older package context.
