...
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-10. Result: passed, 68 passed, 16 skipped release artifact/runtime evidence tests, 0 failed.
...
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `EC0FF02E1AF625B0C3DA63F68D1BDBE37A85BF0E5EDBDF021299472DE4D1489F`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt from the current installed artifacts after the latest artifact parity refresh. The package `README_INSTALL.txt` records the resolved author/generated-art status. SHA256 `EDA9A59FD8F028287EE5EED558D8CEFFF6FE310AEC07F428B4C53310EBED3EBA`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
