...
## Last known commands
- Last attempted default build: `dotnet build EZMicroBalance.sln` on 2026-05-26 after the beta.27 Seedbed planting rules refresh. Result: succeeded with 0 warnings and 0 errors.
- Last successful build: `dotnet build EZMicroBalance.sln` on 2026-05-26 after the beta.27 Seedbed planting rules refresh. Result: succeeded with 0 warnings and 0 errors.
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-26 after the beta.27 Seedbed planting rules refresh. Result: passed, 283 passed, 20 skipped release artifact/runtime evidence tests, 0 failed.
...
- Latest opt-in artifact-validation run: `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-26 after the beta.27 Seedbed planting rules refresh. Result: passed, 303 passed, 0 skipped, 0 failed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` remains accepted.
...
- Last formatting check: `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` on 2026-05-26 after the beta.27 Seedbed planting rules refresh. Result: exit code 0.
- Last required diff check: `git diff --check` on 2026-05-26 after the beta.27 Seedbed planting rules refresh. Result: exit code 0 with CRLF/LF warnings only.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.27.zip`, the installed `mods\EZMicroBalance` folder, and `D:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.27.zip` were refreshed and hash-checked on 2026-05-26 for local manual testing. The zip uses the player-facing `SpirePlus` archive name while the install folder remains `EZMicroBalance`. The beta.19 loader row is historical startup/log proof only; beta.27 loader, live gameplay, save-load, failure/death-path, clicked Ancient UI, and co-op verification remain pending.
- Last PCK hash check: the 2026-05-26 local manual-test package refresh left the staging, versioned, and zip-entry PCK at SHA256 `C62CBC8B9DAC1C4522FA1A30B4BA35B95C53EDF4628FD6B8135F4B9BBD83281A`.
- Last staging/versioned DLL hash check: SHA256 `B6F91BC3079CAA342C27690E7DCC056E958CF927D41147EE78207EF717553F04`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.27.zip` was rebuilt from package staging, copied to the game root, and matched the installed mod artifacts on 2026-05-26.
  ...
  - README SHA256: `DCAB3CE1B276CC9093C9AEA86C19D900081E097A7A821F2E0AD6101F113C390D`
