...
- Current RitsuLib-only migration evidence: solution build, focused guards, publish, package refresh, installed package checker, runtime preflight, source-workspace checker, and clicked Ancient UI smoke passed for beta.105 after the STS2-RitsuLib `v0.4.33` update. The retained beta.88/beta.90/beta.93/beta.96/beta.99 smokes are previous-package contexts only. The direct-smoke verifier now accepts both append-style current-log slices and Godot-rewritten logs only when `godot.log.current-iteration` matches the retained after-launch log. Earlier cross-thread aborts are runner-contamination evidence only.
...
- Historical display-name list evidence under `.tools\runtime-evidence\current-spire-plus-modsettings-20260513-111342` shows `Spire Plus`; older page-level Mod Settings evidence predates the display-name refresh.
  ...
  Treat it as previous-package settings-page context after beta.105.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.105.zip`, `E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `E:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.105.zip` were refreshed and hash-checked on 2026-06-22 for local manual testing.
  ...
  - beta.105 package parity, source-workspace validation, and clicked Ancient UI smoke are current package context.
  - beta.99 Mod Settings proof is under `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210` with clean log audit and StS1 Off runtime shape verification 21 / 0; it is previous-package settings-page context after beta.105.
  ...
  Live gameplay, save-load, failure/death-path, gated Vakuu fight-option/victory return, and co-op verification remain pending; beta.105 clicked Ancient UI smoke exists under `.tools\runtime-evidence\monkey-stability-20260622-025733`.
- Last PCK hash check: the 2026-06-22 local package-hash refresh found the staging, versioned, and zip-entry PCK at SHA256 `E965454C0B3DE096459FFFCF80677E8BC10F9D3B41239B8504114F26DE348913`.
- Last staging/versioned DLL hash check: SHA256 `DB4340FDF9CAF5E35DAABDD9CCABFFFBCCA0BA4E9CF1617291C4A6846958B1DA`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.105.zip` was rebuilt from the real installed mod artifacts, copied to the game root, and synced into the real installed mod folder on 2026-06-22. The staging, versioned, installed, game-root zip, and zip-entry artifacts match the hashes below.
  ...
  - README SHA256: `FA3312EDE6E1E32D3A5301A02310E62078A079E6F588FB88AAA230B0331D667A`
...
- Current target Mod Settings verification: beta.99 screenshot/page proof is retained at `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210` after the settings-page I18N resource migration.
  ...
  It is previous-package settings-page context after beta.105; rerun only if settings UI code/resources or RitsuLib version changes again.
