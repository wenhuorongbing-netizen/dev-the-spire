...
- Installed `STS2-RitsuLib` `v0.4.34` uses the direct NuGet runtime layout at `<GameRoot>\mods\STS2-RitsuLib`; the previous `v0.4.32` runtime install is backed up under `%TEMP%\codex-sts2-ritsulib-backup-0.4.32-20260622-025408`.
- The recovered local source snapshot has been refreshed from the installed `v0.107.1` game PCK and `sts2.dll` using GDRE Tools `v2.5.0`. The 2026-06-23 `scripts\check-local-godot-source-workspace.ps1 -RequireCurrentSourceSnapshot -ExpectedPackageVersion v0.1.0-private-beta.133 -ExpectedRitsuLibVersion 0.4.34 -ExpectedRitsuCompatBranch 0.107.1 -FailOnMismatch` recheck passed 57 checks / 0 mismatches for the current package, including local `STS2-RitsuLib.xml` API marker coverage. Retained warnings are the GDRE 18 failed scripts and one debug-scene parse warning, not source identity mismatches. Retained `v0.107.0` beta.85/beta.86/beta.87 direct smokes remain previous-package/game-version loader context.
...
- Current RitsuLib-only migration evidence: solution build, focused guards,
  publish, package refresh, installed package checker, runtime preflight,
  source-workspace checker, and forced clicked Ancient UI smoke passed for
  beta.128 after the STS2-RitsuLib `v0.4.34` update.
  Latest clicked Ancient UI smoke is
  `.tools\runtime-evidence\monkey-stability-20260623-062913`: 4 / 4
  iterations, clean audits, exact package/game/Ritsu markers, 152/152 default
  runtime Spire Plus patches, and packet verification 1621 / 0.
  The retained beta.88/beta.90/beta.93/beta.96/beta.99 smokes are
  previous-package contexts only. The direct-smoke verifier now accepts both
  append-style current-log slices and Godot-rewritten logs only when
  `godot.log.current-iteration` matches the retained after-launch log. Earlier
  cross-thread aborts are runner-contamination evidence only.
...
## Dependencies
- Current Spire Plus runtime dependency: `STS2-RitsuLib` `v0.4.34` under `<GameRoot>\mods\STS2-RitsuLib`.
- Current Spire Plus compile dependency: `STS2.RitsuLib` `0.4.34` in `EZMicroBalance.csproj`.
- Analyzer-only compile package: `Alchyr.Sts2.ModAnalyzers` `0.1.9`; this is not a runtime dependency, manifest dependency, tester setup step, or current development API surface.
- Historical scaffold template notes live only in `docs/archive/superseded/setup-spec-original-scaffold.md`; do not use them for current setup or implementation.
- Current setup and validation must use STS2-RitsuLib docs/XML plus the unpacked local game source under `source code/src/Core/`.
...
- Current source defines 30 SavedAttachedState fields. Historical 22-field and 16-field startup rows remain useful records only. Beta.85/beta.86/beta.87 `v0.107.0`, beta.88 previous package `v0.107.1`, beta.90 RitsuLib-only loader rows, beta.93 AdditiveBatch1 loader/registration rows, beta.96 Off/Mod Settings rows, and beta.99 package/settings/off rows are previous-context evidence; beta.133 package parity and source-workspace validation are the current package evidence; beta.128 clicked Ancient UI smoke remains previous-package evidence, and gameplay proof still needs manual verification.
...
- Historical display-name list evidence under `.tools\runtime-evidence\current-spire-plus-modsettings-20260513-111342` shows `Spire Plus`; older page-level Mod Settings evidence predates the display-name refresh.
  Previous beta.99 RitsuLib Mod Settings page proof is captured under `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210`.
  It opened Settings -> `Mod Settings (RitsuLib)` in a normal Steam-client isolated session, showed only `RitsuLib` and `Spire Plus`, rendered the Spire Plus Migration Status / `STS2-RitsuLib >= 0.4.34` / evidence-boundary / Preview Tools page, retained same-session `godot.log`, audited clean, and passed StS1 Off runtime shape verification 21 / 0.
  Treat it as previous-package settings-page context after beta.133.
...
- Historical RitsuLib diagnostic Off, CanaryOnly, and AdditiveBatch1 loader gates have clean `v0.106.1` evidence with previous package, RitsuLib, and Spire Plus loaded and 25/34 Spire Plus ModPatcher patches applied. Retained beta.85/beta.86/beta.87 `v0.107.0` proof remains previous-package/game-version context, beta.88 `v0.107.1` AdditiveBatch1 proof is previous-package context, beta.90/beta.93/beta.96/beta.99 rows are previous RitsuLib-only contexts. Previous beta.128 clicked Ancient UI smoke now exists; live gameplay, save-load, failure/death-path, gated Vakuu fight-option/victory return, preview-tools, and co-op verification remain pending.
...
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.133.zip`, `E:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `E:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.133.zip` were refreshed and hash-checked on 2026-06-23 for local manual testing.
  ...
  - beta.133 package parity, source-workspace validation, and runtime preflight are current package context; beta.128 clicked UI smoke is previous-package context.
  - beta.99 Mod Settings proof is under `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210` with clean log audit and StS1 Off runtime shape verification 21 / 0; it is previous-package settings-page context after beta.133.
  - beta.96 RitsuLib-only Off proof is previous-package startup/loading context under `.tools\runtime-evidence\v01071-beta96-ritsulib0431-off-direct-20260621-185056`.
  ...
  Live gameplay, save-load, failure/death-path, gated Vakuu fight-option/victory return, and co-op verification remain pending; previous beta.128 clicked Ancient UI smoke exists under `.tools\runtime-evidence\monkey-stability-20260623-062913`.
- Last PCK hash check: the 2026-06-23 local package-hash refresh found the staging, versioned, and zip-entry PCK at SHA256 `2472578F14D4572A2888F41F10D449F0331CB057A112939C7EC43196F501F87B`.
- Last staging/versioned DLL hash check: SHA256 `6F257CCF31280FA002AF75C3507307FA288867BDFF13825CC9289BBB9825F53B`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.133.zip` was rebuilt from the real installed mod artifacts, copied to the game root, and synced into the real installed mod folder on 2026-06-23. The staging, versioned, installed, game-root zip, and zip-entry artifacts match the hashes below.
  ...
  - Zip SHA256: `658AAB5F5FE0793253513F472B2BD5DC6C93BA2FF9EA6911E3130DFC6719115E`
  - DLL SHA256: `6F257CCF31280FA002AF75C3507307FA288867BDFF13825CC9289BBB9825F53B`
  - README SHA256: `7607A59DBA274931641B041028B5A06E96EE4DBF9E12EE4D21215B16CDEEAABB`
  - Manifest SHA256: `2FA3920BC1EC84485A7FBA43E5446F09E8E5AB6A575BBD2FB4C114DA9E34879C`
  - PCK SHA256: `2472578F14D4572A2888F41F10D449F0331CB057A112939C7EC43196F501F87B`
...
- Current target Mod Settings verification: beta.99 screenshot/page proof is retained at `.tools\runtime-evidence\mod-settings-beta99-ritsulib-click-20260621-223210` after the settings-page I18N resource migration.
  The proof used only STS2-RitsuLib and `EZMicroBalance`, showed `Spire Plus` in the RitsuLib Mods list, rendered the Spire Plus settings page, retained same-session `godot.log`, audited clean, and passed StS1 Off runtime shape verification 21 / 0.
  It is previous-package settings-page context after beta.133; rerun only if settings UI code/resources or RitsuLib version changes again.
  Previous beta.96 screenshot/page proof is previous-package context only.
  Historical list evidence under `.tools\runtime-evidence\current-spire-plus-modsettings-20260513-111342` showed `Spire Plus` with previous package plus `EZMicroBalance` in an older package context.
...
- Beta.97 live gameplay verification remains pending.
