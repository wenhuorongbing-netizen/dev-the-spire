...
## Last known commands
- Last attempted default build: `dotnet build EZMicroBalance.sln` on 2026-05-07 after the A20 multiplayer warning, A11 localization, release-artifact test gate, and current smoke pass. Default Debug builds no longer overwrite installed release artifacts; Release build/publish remains the installed-mod copy path. Result: succeeded with 0 warnings and 0 errors.
- Last successful build: `dotnet build EZMicroBalance.sln` on 2026-05-07 after the A20 multiplayer warning, A11 localization, release-artifact test gate, and current smoke pass. Result: succeeded with 0 warnings and 0 errors.
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-07 after publish/package refresh. Result: passed, 62 passed, 15 skipped release artifact/runtime evidence tests, 0 failed.
- Last attempted test run: `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-07 after package/hash/smoke refresh. Result: passed, 77 passed, 0 skipped, 0 failed.
- Last formatting check: `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` on 2026-05-07 after the A20 warning/current smoke pass. Result: exit code 0.
- Last required diff check: `git diff --check` on 2026-05-07 after the A20 warning/current smoke pass. Result: exit code 0 with CRLF normalization warnings for touched files.
- Last attempted publish: `dotnet publish EZMicroBalance.sln` on 2026-05-07 after the A20 multiplayer warning, A11 localization, and release-artifact test gate pass.
- Last successful publish: `dotnet publish EZMicroBalance.sln` on 2026-05-07. Result: command returned exit code 0, built `EZMicroBalance` in Release, copied `EZMicroBalance.dll` and `EZMicroBalance.json`, exported the selected-resource PCK, and did not publish the test project. Godot printed the known non-fatal `sts2` assembly lookup exception during headless export.
...
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `8A6B9696F81B39DF5C04FF306687381629BDD33970A1C3D685A5C2E7CA517DA8`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt after the 2026-05-07 A20 warning/current smoke pass. SHA256 `4BC21B37C9C2D90FC4991F353245DEBB47555026975647341281480BB492F089`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
...
- 2026-05-07: current-package bounded `--force-steam off` smoke after the A20 multiplayer warning, A11 localization, publish, and package refresh temporarily enabled only `BaseLib` and `EZMicroBalance`, explicitly disabled other discovered local mods, loaded exactly 2 mods (`Loaded 2 mods (19 total)`), initialized BaseLib and EZ Micro Balance, loaded the current installed `EZMicroBalance.dll` and `.pck`, confirmed the current source defines 12 SavedSpireFields and runtime reported `Found 12 SavedSpireFields`, reached main menu in `13,423ms`, found 0 EZ Micro Balance error/exception lines, and restored both `default\1\settings.save` and `settings.save.backup` byte-for-byte.
