...
- Current source defines 30 SavedSpireFields. Historical 22-field and 16-field startup rows remain useful records only; beta.81 loader parity and gameplay proof still need fresh live evidence.
...
- Current beta.81 loader, live gameplay, save-load, failure/death-path, clicked Ancient UI, preview-tools, and co-op verification remain pending.

## Last known commands
- Last attempted default build: `dotnet build EZMicroBalance.sln` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: succeeded with 0 warnings and 0 errors.
- Last successful build: `dotnet build EZMicroBalance.sln` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: succeeded with 0 warnings and 0 errors.
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: passed, 296 passed, 20 skipped release artifact/runtime evidence tests, 0 failed.
...
- Latest opt-in artifact-validation run: `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: passed, 316 passed, 0 skipped, 0 failed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` remains accepted.
...
- Last formatting check: `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: exit code 0.
- Last required diff check: `git diff --check` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: exit code 0 with no warnings.
- Last attempted default publish: `dotnet publish EZMicroBalance.sln` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: succeeded against the real installed mods root.
- Last successful isolated publish: `dotnet publish EZMicroBalance.sln -p:ModsPath=.tools\publish-game-root\mods\` on 2026-05-27 after the beta.81 AscensionFeature rootblight guard split package sync. Result: succeeded against an isolated temporary mods root; the isolated root is tooling context only and is not the current package-parity source.
- Publish/package note: package staging, the versioned package folder, `publish\SpirePlus-v0.1.0-private-beta.81.zip`, `D:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and `D:\Steam\steamapps\common\Slay the Spire 2\SpirePlus-v0.1.0-private-beta.81.zip` were refreshed and hash-checked on 2026-05-27 for local manual testing. The zip uses the player-facing `SpirePlus` archive name while the install folder remains `EZMicroBalance`. The beta.19 loader row is historical startup/log proof only; beta.81 loader, live gameplay, save-load, failure/death-path, clicked Ancient UI, and co-op verification remain pending.
- Last PCK hash check: the 2026-05-27 local manual-test package refresh left the staging, versioned, and zip-entry PCK at SHA256 `11617E8BC7774FB4B4EBA397DA21A8214444931D63BC8DBF2DB5459CE21BEB48`.
- Last staging/versioned DLL hash check: SHA256 `073914AF51B5160404F0C24768A893F21130045C1B49BA8CB167A704C67A430F`.
...
- Last private beta package: `publish\SpirePlus-v0.1.0-private-beta.81.zip` was rebuilt from the real installed mod artifacts, copied to the game root, and synced into the real installed mod folder on 2026-05-27. The staging, versioned, installed, game-root zip, and zip-entry artifacts match the hashes below.
  ...
  - Zip SHA256: `0352CFB69B7DC13A5B044D3AF4C978AEDB6AC43FDDAB38BB1BBC8B2EAF8B968B`
  - DLL SHA256: `073914AF51B5160404F0C24768A893F21130045C1B49BA8CB167A704C67A430F`
  - README SHA256: `59A5A8D2DD0B7D1F91E1B20A2A3FBD9E889447B24853F1C483ECEAF23CA86F4A`
  - Manifest SHA256: `F064BC29377DF64A8644C5420B4F1911FF1DD6FD070A5C0B7DC07F4316899268`
  - PCK SHA256: `11617E8BC7774FB4B4EBA397DA21A8214444931D63BC8DBF2DB5459CE21BEB48`
