...
- Last successful normal test run: `dotnet test EZMicroBalance.sln --no-build` on 2026-05-09 before the Rootblight top-level notice hardening, optional portrait fallback, and generated-art/author refresh. Result: passed, 67 passed, 16 skipped release artifact/runtime evidence tests, 0 failed. Tests were not rerun after the latest source/resource refresh because the current implementation pass explicitly excluded testing.
- Last attempted test run: `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-09 before the Rootblight top-level notice hardening, optional portrait fallback, and generated-art/author refresh. Result: passed, 83 passed, 0 skipped, 0 failed. Opt-in release tests are pending rerun for the latest DLL/package.
- Last formatting check: `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` on 2026-05-09 before the Rootblight top-level notice hardening, optional portrait fallback, and generated-art/author refresh. Result: exit code 0; not rerun after the latest source/resource refresh.
- Last required diff check: `git diff --check` on 2026-05-09 before the Rootblight top-level notice hardening, optional portrait fallback, and generated-art/author refresh. Result: exit code 0; not rerun after the latest source/resource refresh.
- Last attempted publish: `dotnet publish EZMicroBalance.sln` on 2026-05-09 during the Rootblight generated-art/author refresh.
- Last successful publish: `dotnet publish EZMicroBalance.sln` on 2026-05-09. Result: command returned exit code 0, built `EZMicroBalance` in Release, copied `EZMicroBalance.dll` and `EZMicroBalance.json`, exported the selected-resource `EZMicroBalance.pck`, and did not publish the test project.
- Last PCK audit: publish/package refresh left the installed, staging, versioned, and zip PCK at SHA256 `253E1310D8357EEB4D099F34BFA8785A66FEE77576BDA59A4D34277874696C25`. The latest opt-in automated PCK-content audit predates the final notice/portrait/generated-art refresh and should be rerun before release handoff.
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `9A0E750122D3AEBE449D2D95A20AED84657AFF6D169079E0F0184CC7084A70DF`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt after the 2026-05-09 Rootblight optional portrait fallback, generated portrait art, and manifest author refresh. The package `README_INSTALL.txt` records the resolved author/generated-art status. SHA256 `1699D7BEC6C1A0BD02223E45E4B90399C7BFBB20D4E95236F9ED1E08A795AF8F`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
