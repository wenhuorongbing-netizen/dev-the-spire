...
- Latest opt-in artifact-validation run: `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1 dotnet test EZMicroBalance.sln --no-build` on 2026-05-12 after the A11-A20 v2.0 package refresh. Result: passed, 89 passed, 0 skipped, 0 failed.
- Last formatting check: `dotnet format EZMicroBalance.sln --verify-no-changes --no-restore` on 2026-05-12 after the A11-A20 v2.0 source/doc refresh. Result: exit code 0.
...
- Last successful publish: `dotnet publish EZMicroBalance.sln` on 2026-05-12. Result: command returned exit code 0, built `EZMicroBalance` in Release, copied `EZMicroBalance.dll` and `EZMicroBalance.json`, and exported the selected-resource PCK with the current A11-A20 localization/resources included.
- Last PCK audit: publish/package refresh left the installed, staging, versioned, and zip PCK at SHA256 `0417E3402A026C4B4D0597A3183108F88C22391499C20886BAF28BFD28BBF292`. The latest opt-in automated PCK-content audit matches this full artifact set and is current through the latest package refresh.
- Last installed DLL audit: installed, staging, versioned package, and extracted zip DLL SHA256 all match `05C9E0430EB219BB1B7B3E830DC7286A7CE2879427939D0DEDC6DDF2E50A5E34`.
- Last Harmony patch audit: standalone .NET 9 audit called `Harmony.PatchAll(...)` on `EZMicroBalance.dll` and returned `PatchAll OK`.
- Last private beta package: `publish\EZMicroBalance-v0.1.0-private-beta.0.zip` was rebuilt from the current installed artifacts after the A11-A20 v2.0 hardening pass. The package `README_INSTALL.txt` records the resolved author/generated-art status, the multiplayer diagnostics note, Urda live-verification limits, current Fission rates, and the one-Rootblight cap. SHA256 `F09686BA68A8A63FEC716713C2B7A5D3A184F1FEE5EE1EB749BBD1AEDEB142FC`; entries are `EZMicroBalance/EZMicroBalance.dll`, `.json`, `.pck`, and `README_INSTALL.txt`.
