...
- [ ] Release artifact, installed DLL/PCK, package hash, and runtime-smoke evidence tests have been rerun for the current tree with `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1`. Latest pass predates the Rootblight top-level notice hardening, optional portrait fallback, and generated-art/author refresh.
- [x] `publish/EZMicroBalance-v0.1.0-private-beta.0.zip` is rebuilt from the current installed artifacts. Current zip SHA256 `1699D7BEC6C1A0BD02223E45E4B90399C7BFBB20D4E95236F9ED1E08A795AF8F`.
...
- RC1 live gates still pending: Rootblight full combat-end behavior and clean non-paused notice timing, Blight Sprout combat-end notices, live visual check of the generated Rootblight-family card art, broader A11 geometry/traversal diagnostics beyond the Act 1/2/3 width/row spot checks, Ancient reward gameplay spot checks, multiplayer matrix, Steam-client Mod Settings rerun if package/mod state changes, and Ancient/co-op save/load verification. Targeted Rootblight English/ZHS hover/text and A14 Neow starter notice spot checks passed on 2026-05-09; Rootblight combat-end notices are source-hardened but still need full manual verification.
...
- Targeted Rootblight normal Steam-client evidence passed for English and Simplified Chinese hover/text and the A14 Neow starter add notice. `.tools\runtime-evidence\rootblight-a14-hover-eng-20260509-044010` shows the English Rootblight-added bubble at Neow with the starter deck at 11 cards, plus Rootblight I/II/III and Blight Sprout hovers with one visible Exhaust keyword, no raw `[gold]` tags, and expected previews. `.tools\runtime-evidence\rootblight-a14-ui-eng-20260509-033516` shows the same hover/text checks in Simplified Chinese. `.tools\runtime-evidence\rootblight-a14-notice-zhs-step-20260509-040455\07-run-start-06.png` shows the localized ZHS Rootblight-added bubble at Neow with the starter deck at 11 cards. Combat-end add notices, full Rootblight/Blight Sprout behavior, generated-art visual verification, and co-op ownership/desync remain pending.
...
- `AUTHOR_NAME_REPLACE_ME` is no longer present in the active `EZMicroBalance` manifest; legacy scaffold/historical docs may still mention it for setup history.
