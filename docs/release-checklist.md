...
Current status note (2026-06-18): the `v0.1.0-private-beta.87` package/hash rows below describe the current packaged tester artifact after the package/source alignment pass. Fresh direct `v0.107.0` AdditiveBatch1 smoke is clean under `.tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531`; live-ready and release-ready claims still require gameplay, UI, save-load, co-op, and independent QA evidence.

- [x] `dotnet build` succeeds.
- [x] Latest source/package build check passed with `dotnet build EZMicroBalance.sln -m:1 --no-incremental` after the beta.87 dependency-floor refresh.
...
- [x] Manifest declares structured `BaseLib` dependency with `min_version: v3.2.1`.
...
- [x] `publish/SpirePlus-v0.1.0-private-beta.87.zip` was rebuilt from the beta.87 dependency-floor refresh and copied to the local game root for manual testing.
- Current package hashes: DLL `35575A2651882B4C2D8EDD082A8FD7C6B6534CEB6433F846AEC59259353B19B4`; manifest `15C677876A056D0C695DAB05BB7A18A5CB9B13FDDEFD0E595746ADF36D652059`; PCK `C4EFDCA19F5F98F6A945599110AAF13A89452E3E1B8069326385BD45575218F2`; README `E968521929ECA272A5C5FA62DA4D90FB37C5307922FD2893ABDA154C3A12DFBC`; zip `97C65F040F7269738778368878E7946D1563F622D0D8959644C54DBC6806A0B1`.
...
- [ ] Current beta.87 Mod Settings list plus Spire Plus config page screenshots are captured under release-evidence row `mod-settings-current-display`.
- [x] Historical pre-display-name-refresh Mod Settings evidence exists for the same technical manifest id.
- [x] Fresh loader smoke for the current beta.87 ZIP hash is clean under `.tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531`. The beta.19 smoke under `.tools\runtime-evidence\beta19-loader-smoke-20260525-213336` remains historical startup evidence only.
- [x] Historical RitsuLib diagnostic loader gates exist for Off, CanaryOnly, and AdditiveBatch1 modes with clean audits and 25/25 Spire Plus ModPatcher patches; beta.85 Off/CanaryOnly and beta.86 AdditiveBatch1 are previous-package `v0.107.0` loader proof, while current beta.87 AdditiveBatch1 proof has 10 event types / 14 registration calls with exact tuple parity.
...
- [x] Latest loader smoke for the current beta.87 package hash reached main menu, loaded BaseLib/RitsuLib/Spire Plus, applied 25/25 Spire Plus ModPatcher patches, registered AdditiveBatch1 as 10 event types / 14 calls, and audited clean.
...
- [x] `godot.log` reviewed after fresh beta.87 direct enabled-mode isolated startup/log verification.
...
- Fresh loader smoke for the current beta.87 package hash is clean under `.tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531`: it loaded BaseLib, RitsuLib, and `EZMicroBalance`, selected RitsuLib compat branch `0.107.0`, reported `v0.1.0-private-beta.87`, applied 25/25 Spire Plus ModPatcher patches, registered AdditiveBatch1 as 10 event types / 14 calls, reached main menu, audited clean, stopped the game, and restored 25 isolated mod entries. This is loader/startup evidence, not gameplay proof.
...
Current beta.87 proof covers AdditiveBatch1 loader registration only, not event gameplay proof.
