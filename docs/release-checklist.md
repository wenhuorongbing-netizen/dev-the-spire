...
Current status note (2026-06-23): the active manifest is `v0.1.0-private-beta.128` after the RitsuLib compatibility and package refresh pass.
...
- [x] STS2-RitsuLib appears in Mod Settings for the beta.99 RitsuLib-only package; this is previous-package context after beta.128 and should be recaptured if the settings row becomes release-blocking.
...
- [x] Current beta.128 RitsuLib-only clicked Ancient UI smoke is retained as smoke-level UI proof after the latest RitsuLib package refresh.
...
- [x] Latest retained clicked Ancient UI smoke is beta.128 current-package smoke-level UI proof on Slay the Spire 2 `v0.107.1`; retained beta.87/beta.88/beta.90/beta.96/beta.99 loader evidence is historical or previous-package context only.
- [x] Historical normal Steam helper startup/log verification under `.tools\runtime-evidence\live-spire-plus-session-20260515-211414` reached main menu with only previous package and Spire Plus, reported `Found 22 previous saved-state registrations`, restored settings, 24 moved mod entries, and 2 current-run files, left 0 `SlayTheSpire2` processes, and audited clean. This is historical context; beta.128 clicked UI smoke supersedes it for current startup/default-Off smoke shape.
