...
- [x] Latest source/package build check passed with `dotnet build EZMicroBalance.sln` after the beta.81 AscensionFeature rootblight guard split package sync.
...
- [ ] Fresh loader smoke for the current beta.81 ZIP hash is pending. The beta.19 smoke under `.tools\runtime-evidence\beta19-loader-smoke-20260525-213336` remains historical startup evidence only.
...
- [ ] Latest loader smoke for the current beta.81 package hash has not been recaptured yet.
...
- [ ] `godot.log` reviewed after fresh beta.81 normal Steam-client isolated startup/log verification.
...
- Fresh loader smoke for the current beta.81 package hash is pending. The beta.19 smoke under `.tools\runtime-evidence\beta19-loader-smoke-20260525-213336` loaded exactly BaseLib and `EZMicroBalance`, registered config, reported `v0.1.0-private-beta.19`, `Found 30 SavedSpireFields`, reached startup completion, and had 0 release-blocking hits. The helper stopped the game and restored 24 isolated mod entries. This is loader/startup evidence, not gameplay proof; it remains historical context for beta.81.
