...
- RitsuLib migrated patch bootstrap now uses `ApplyRequiredPatcher(...)` and fails closed before feature initialization if required migrated patches cannot apply. Migrated registrations are split by source ownership: the root registry preserves order, `SpirePlusMigratedPatchRegistry.Ui.cs` owns clicked/hover/settings/selection patches, `SpirePlusMigratedPatchRegistry.PreviewUi.cs` owns local-only preview-tool UI patches, and sibling partials own reward, localization/compatibility, gameplay, and diagnostic groups.
