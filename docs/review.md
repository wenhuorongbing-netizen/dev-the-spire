...
- RitsuLib migrated patch bootstrap now uses `ApplyRequiredPatcher(...)` and fails closed before feature initialization if required migrated patches cannot apply. Migrated registrations are split by source ownership: the root registry preserves order, `SpirePlusMigratedPatchRegistry.AncientUi.cs` owns Ancient event-option/marker UI patches, `SpirePlusMigratedPatchRegistry.MapUi.cs` owns map-click/hover UI patches, `SpirePlusMigratedPatchRegistry.HostUi.cs` owns host-screen UI patches, `SpirePlusMigratedPatchRegistry.PreviewUi.cs` owns local-only preview-tool UI patches, `SpirePlusMigratedPatchRegistry.DisplayUi.cs` owns display-only icon/hover/intent/damage-number patches, and sibling partials own reward, localization/compatibility, gameplay, and diagnostic groups.
