...
- Current source has completed the clicked/input UI migration, visual-hover UI getter migration, rest-site Meat Cleaver UI migration, Neow/Vakuu event-option UI migration, A20 courtyard portrait migration, ascension-localization fallback migration, core inline-localization fallback migration, Ancient reward getter/relic hook migration, low-risk Ancient reward hook migration, Aeonglass intent UI migration, Enemy Damage polish getter migration, and Urda Root Sight room-routing migration to RitsuLib `IPatchMethod` / `ModPatcher`: 169 migrated patch classes and 0 raw Harmony declarations remain in `docs/patch-inventory.md`.
- RitsuLib patch bootstrap uses `ApplyRequiredPatcher(...)` and fails closed before feature initialization if required patches cannot apply. Registrations are split by source ownership: the root registry preserves order, `SpirePlusRitsuLibPatchRegistry.AncientUi.cs` owns Ancient event-option/marker UI patches, `SpirePlusRitsuLibPatchRegistry.MapUi.cs` owns map-click/hover UI patches, `SpirePlusRitsuLibPatchRegistry.HostUi.cs` owns host-screen UI patches, `SpirePlusRitsuLibPatchRegistry.PreviewUi.cs` owns local-only preview-tool UI patches, `SpirePlusRitsuLibPatchRegistry.DisplayUi.cs` owns display-only icon/hover/intent/damage-number patches, and sibling partials own reward, localization/compatibility, gameplay, and diagnostic groups.
...
- The beta.135 package proof covers the refreshed package with the Urda Root Sight room-routing migration included. Previous beta.128 clicked UI smoke applied all 152 default runtime Spire Plus ModPatcher patches in that older package; gameplay and handoff remain separate pending gates.
- Previous enabled-mode packets remain historical or previous-package context and do not close current beta.135 enabled-mode gameplay proof.
- AutoSlay and runtime-monkey verifier hardening is methodology evidence only. Proof-mode packets still require `-ExpectedAncientIds` to match retained plan/summary/traversal state, and those verifier rules do not close current beta.135 gameplay or handoff proof.
