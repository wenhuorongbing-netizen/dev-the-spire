...
- The beta.133 package proof covers the refreshed package with the Urda Root Sight room-routing migration included. Previous beta.128 clicked UI smoke applied all 152 default runtime Spire Plus ModPatcher patches in that older package; gameplay and handoff remain separate pending gates.
- Previous enabled-mode packets remain historical or previous-package context and do not close current beta.133 enabled-mode gameplay proof.
- AutoSlay and runtime-monkey verifier hardening is methodology evidence only. Proof-mode packets still require `-ExpectedAncientIds` to match retained plan/summary/traversal state, and those verifier rules do not close current beta.133 gameplay or handoff proof.
