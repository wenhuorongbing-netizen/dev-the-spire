...
- RitsuLib migrated patch bootstrap now uses `ApplyRequiredPatcher(...)` and fails closed before feature initialization if required migrated patches cannot apply. Migrated registrations are split by source ownership: the root registry preserves order, `SpirePlusMigratedPatchRegistry.Ui.cs` owns event/clicked/map/settings/selection patches, `SpirePlusMigratedPatchRegistry.PreviewUi.cs` owns local-only preview-tool UI patches, `SpirePlusMigratedPatchRegistry.DisplayUi.cs` owns display-only icon/hover/intent/damage-number patches, and sibling partials own reward, localization/compatibility, gameplay, and diagnostic groups.
