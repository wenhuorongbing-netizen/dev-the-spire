...
- Dependency decision is recorded: beta.93 intentionally aligns the compile package and manifest minimum on STS2-RitsuLib `v0.4.31`, uses `lib\0.107.1`, removes BaseLib from current Spire Plus requirements, and uses package version `v0.1.0-private-beta.93`.
...
- [x] Final dependency migration decision recorded: beta.93 compile/manifest floor is `0.4.31`, and BaseLib is no longer a Spire Plus project, manifest, package, or current runtime dependency.
