...
- Latest beta.104 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, and clicked Ancient UI smoke passed for the current dependency target. Previous beta.99 settings/Off proof, beta.96 Off proof, and beta.93 AdditiveBatch1 packet verification remain previous-package or historical loader/registration evidence only.
- Current HEAD/worktree validation refreshed after the beta.104 RitsuLib default-key localization and clicked UI pass; recheck again before handoff if any later edits appear.
...
- Before any AdditiveBatch1 gameplay claim, confirm the beta.104 package and STS2-RitsuLib `v0.4.33` direct NuGet runtime are installed and recapture current-package enabled-mode proof.
...
3. Capture beta.104 enabled-mode loader/registration proof separately before any current enabled-mode claim.
...
- [x] Current beta.104 clicked Ancient UI smoke captured under `.tools/runtime-evidence/monkey-stability-20260622-025733/`; previous beta.99 Off loader proof remains previous-package context under `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/`.
