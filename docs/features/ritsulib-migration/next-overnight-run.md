...
The old hard blocker was missing or non-clean RitsuLib runtime smoke. That blocker was closed for `v0.107.0` loader/registration proof: historical Slay the Spire 2 `v0.106.1` Off, CanaryOnly, and AdditiveBatch1 diagnostic evidence is clean, `v0.107.0` beta.85 Off smoke under `.tools/runtime-evidence/v01070-beta85-current-package-runtime-fix-20260611-0510/` reached main menu and audited clean after the Spire Plus API-target drift fix, beta.85 CanaryOnly smoke under `.tools/runtime-evidence/v01070-beta85-canary-20260617-233621/` passed retained log/packet verifiers with 4 event types / 6 registration lines as previous-package context, and beta.87 AdditiveBatch1 direct smoke under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` passed retained log/packet verifiers with 10 event types / 14 registration lines. The local game moved to `v0.107.1`; the first current-version direct AdditiveBatch1 recapture under `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` reached main menu and matched the expected 10 event types / 14 calls, but failed clean audit because BaseLib `v3.2.1` logged 2 patch failures. The beta.88 recapture after installing BaseLib `v3.3.0` under `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937/` passed clean audit, retained enabled-mode verifier 31 / 0, and packet verifier 0 mismatches. The next run should move to gameplay, render, save-load, replacement, multiplayer, QA, and handoff proof while keeping beta.88/BaseLib `v3.3.0` package parity intact. It is not approval to migrate more patches.
...
- Latest beta.87 no-game validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: 0 build errors and 0 warnings; split no-build lanes covered 139 passed / 0 failed / 15 skipped / 154 total for the migration-focused surface; opt-in artifact/package coverage passed; the latest follow-up validation on the same runtime/analyzer/governance surface passed build 0 warnings / 0 errors plus focused `RuntimeMonkeyStabilityGuardTests|EngineeringGovernanceGuardTests` 86 / 0 / 0 / 86 after the AutoSlay artifact-trust and repository-hygiene guard split, with current-doc claims still at 1090 / 0 in the prior static sweep. Revalidate current HEAD in a clean single validation lane again before package handoff if the worktree changes.
...
- Current AdditiveBatch1 enabled-mode proof is clean under `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937/`: 10 event types / 14 registration lines, retained `sts1-enabled-mode-report.json` with 31 / 0 and `sts1-runtime-evidence-packet.json` with 0 mismatches. The earlier `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` failure is BaseLib `v3.2.1` root-cause history.
- Dependency decision is recorded: beta.88 intentionally aligns the compile package and manifest minimum on BaseLib `v3.3.0` and keeps `STS2.RitsuLib` / `STS2-RitsuLib` at `0.4.24`, with package version `v0.1.0-private-beta.88`.
...
- Gameplay, Mod Settings UI page refresh, event screenshots, save-load, image/render, replacement functional proof, multiplayer fail-closed proof, independent QA, and tester-package handoff remain pending. Current-version clean loader proof is available for beta.88/BaseLib `v3.3.0` only; future handoff must recapture HEAD and worktree status after any later edits.
...
6. Before any AdditiveBatch1 gameplay claim, confirm the beta.88 package and BaseLib `v3.3.0` runtime are still installed and current; `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` is retained `v0.107.0` context only, and `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` is failed blocker evidence only.
