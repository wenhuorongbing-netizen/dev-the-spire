...
The old hard blocker was missing or non-clean RitsuLib runtime smoke. That blocker is now closed for the previous `v0.107.1` / beta.93 RitsuLib-only loader/registration target: historical Slay the Spire 2 `v0.106.1` Off, CanaryOnly, and AdditiveBatch1 diagnostic evidence is clean; retained beta.85/beta.87 `v0.107.0` evidence remains previous-package context; the first `v0.107.1` beta.87 recapture failed clean audit because previous package `v3.2.1` logged 2 patch failures; beta.88 later passed only as previous-package context. Previous beta.93 proof under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus loaded, 25/25 patches, clean audits, Off packet 43 / 0, AdditiveBatch1 enabled-mode verifier 31 / 0, and AdditiveBatch1 packet 61 / 0. The next run should move to gameplay, render, save-load, replacement, multiplayer, QA, and handoff proof while keeping beta.95 RitsuLib-only package parity intact. It is not approval to migrate more patches.
...
- Latest beta.95 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace check passed for the current dependency target; previous beta.93 RitsuLib-only Off/AdditiveBatch1 packet verification remains historical loader/registration evidence only. Revalidate current HEAD in a clean single validation lane again before package handoff if the worktree changes.
...
- Historical `v0.106.1` loader-gate runtime proof exists:
  ...
  - Recorded smokes loaded previous package, RitsuLib, and Spire Plus, applied 25/25 migrated patches, and observed 30 previous saved-state registrations.
...
- Previous beta.93 AdditiveBatch1 enabled-mode proof is clean under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`: 10 event types / 14 registration lines, retained enabled-mode report 31 / 0 and runtime packet verifier 61 / 0. The earlier `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` failure is previous package `v3.2.1` root-cause history, and beta.88 is previous-package context.
- Dependency decision is recorded: beta.95 intentionally aligns the compile package and manifest minimum on STS2-RitsuLib `v0.4.31`, uses `lib\0.107.1`, removes previous package from current Spire Plus requirements, and uses package version `v0.1.0-private-beta.95`.
...
- A historical no-launch Mod Settings UI scaffold exists at `.tools/runtime-evidence/mod-settings-current-display-20260618-223145/` with package `v0.1.0-private-beta.87`. Treat it as a template only; refresh or replace it for beta.95 before using it as current UI evidence. It is not screenshot, log/audit, gameplay, or handoff proof.
- A historical no-launch manual-test handoff scaffold exists at `.tools/runtime-evidence/manual-test-handoff-20260619-120202/` on pushed HEAD `2400ec4b`, with 21 required live rows, 21 expected pending failures, 0 warnings, package ZIP `D547847874919EE923E2281A495D5389BAB22BBDB9F1090DC57B77033668A36D`, and `GitHeadMatchesUpstream=true`. Use it as a template only; current beta.95 handoff must use the package hashes in `docs/private-beta-verification-handoff.md` / `docs/release-evidence-status.md` and recapture HEAD/worktree status. No game was launched for that scaffold.
- Gameplay, Mod Settings UI page refresh, event screenshots, save-load, image/render, replacement functional proof, multiplayer fail-closed proof, independent QA, and tester-package handoff remain pending. Previous clean loader proof is available for beta.93/RitsuLib-only only; future handoff must recapture HEAD and worktree status after any later edits.
...
2. Verify the current game version still has a matching RitsuLib variant and that the installed package under test matches the intended beta.95 artifact; if either check fails, record the blocker instead of launching the game. Previous beta.93 clean `v0.107.1` RitsuLib-only loader proof remains under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`.
...
6. Before any AdditiveBatch1 gameplay claim, confirm the beta.95 package and STS2-RitsuLib `v0.4.31` / `lib\0.107.1` runtime are installed and recapture current-package enabled-mode proof; `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` is retained `v0.107.0` context only, beta.88 is previous-package context, beta.93 is previous-package loader/registration context, and `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` is failed blocker evidence only.
...
1. Capture current Spire Plus Mod Settings UI evidence for beta.95. Prefer a fresh `collect-mod-settings-evidence.ps1 -NoLaunch` folder, then rerun that evidence dir with `-Capture List -RequireSpireForeground` and `-Capture Page -RequireSpireForeground` after manually opening the Mods list and Spire Plus page. The Spire Plus page screenshot must show the Migration Status section, RitsuLib dependency card, evidence-boundary card, and Preview Tools controls.
...
- [x] Current STS2-RitsuLib installed locally (`v0.4.31`, `lib\0.107.1`).
...
- [x] Current `v0.107.1` game install has STS2-RitsuLib `v0.4.31`; the selected compatibility branch remains `lib\0.107.1`.
- [x] Final dependency migration decision recorded: current compile/manifest floor is `0.4.31`, and previous package is no longer a Spire Plus project, manifest, package, or current runtime dependency.
...
- [x] Installed beta.95 package parity verified after the 2026-06-21 RitsuLib-only settings UI package refresh.
...
- [x] Current HEAD validation refreshed after the beta.95 RitsuLib-only settings UI pass and runtime packet-checker hardening; recheck again before handoff if any later edits appear.
...
- [x] Current `v0.107.1` clean Off proof recaptured for beta.93/RitsuLib-only at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/`.
- [x] Current `v0.107.1` clean AdditiveBatch1 proof recaptured for beta.93/RitsuLib-only at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`.
