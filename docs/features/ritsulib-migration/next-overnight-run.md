...
- Latest beta.96 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace check, and beta.96 RitsuLib-only Off packet verification passed for the current dependency target; previous beta.93 RitsuLib-only AdditiveBatch1 packet verification remains historical loader/registration evidence only.
...
3. Recapture beta.96 Off loader proof only if the package, source, game, or RitsuLib runtime changed.
...
- [x] Current beta.96 Off loader proof captured.
