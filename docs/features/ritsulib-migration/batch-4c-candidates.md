...
The historical `v0.106.1` RitsuLib loader gate is good enough to propose low-risk candidates, but not to approve a migration or claim current `v0.107.0` runtime proof:
...
8. Current `v0.107.0` beta.87 AdditiveBatch1 loader/registration proof is clean, but this proposal is not a substitute for any new loader smoke required by changed patch registration paths or for gameplay proof.
