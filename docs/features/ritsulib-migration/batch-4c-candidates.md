...
- beta.134 package parity, runtime preflight, and source-workspace validation passed for the packaged 168/1 source state; previous beta.128 clicked Ancient UI smoke applied all 152 default runtime patch classes from that package.
- This source pass moved the six localization fallback patches to RitsuLib `IPatchMethod` / `ModPatcher`; later visual-hover UI, rest-site UI, Act Ancient unlock-list UI, Vakuu event-state UI, Ancient reward hook, low-risk reward hook, intent UI, Enemy Damage getter, RitsuLib compatibility, Urda Root Sight room-routing, and Ascension map generation passes raised the current source count to 168 migrated patches with 1 raw Harmony declaration remaining. The beta.128 clicked UI smoke covers the packaged default-runtime state for forced Ancient UI only.
...
- Before citing the current source migration as runtime coverage, capture a fresh package/runtime packet that proves the current 168 migrated Spire Plus patch classes apply in the installed game.
