...
- STS2-RitsuLib `v0.4.31` is installed locally with the `0.107.1` runtime variant; the prior `v0.4.24` install was backed up before the beta.93 dependency pass.
- The current local game install is Slay the Spire 2 `v0.107.1`; installed beta.95 package parity is recorded. Previous beta.93 Off plus AdditiveBatch1 direct smokes under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` audited clean for loader/registration only. Earlier beta.84/beta.85/beta.87/beta.88/beta.90 smokes are previous-package, previous package, or root-cause history only.
- Spire Plus now compiles against NuGet `STS2.RitsuLib` `0.4.31`; the manifest dependency floor is `STS2-RitsuLib >= 0.4.31`, and no previous package or separate `STS2.RitsuLib.Compat.*` package is used.
...
- Off, CanaryOnly, and AdditiveBatch1 diagnostic smokes have historical `v0.106.1` loader-gate evidence with 25/25 migrated patches applied and 30 previous saved-state registrations observed.
...
3. Keep the current dependency floor aligned at `STS2.RitsuLib` / `STS2-RitsuLib` `0.4.31` unless a future owner-approved versioned package deliberately changes it again.
...
6. Use the retained AdditiveBatch1 enabled-mode smoke at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` only as beta.93 / `v0.107.1` 10 event types / 14 registration-call proof; recapture it after any package/source change before using it for a new claim.
