...
- The current local game install is Slay the Spire 2 `v0.107.1`; installed beta.93 package parity is recorded, and fresh Off plus AdditiveBatch1 direct smokes under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` audited clean. Earlier beta.84/beta.85/beta.87/beta.88/beta.90 smokes are previous-package, previous package, or root-cause history only.
...
3. Keep the beta.93 dependency floor aligned at `STS2.RitsuLib` / `STS2-RitsuLib` `0.4.31` unless a future owner-approved versioned package deliberately changes it again.
...
6. Use the current AdditiveBatch1 enabled-mode smoke at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` as the beta.93 / `v0.107.1` 10 event types / 14 registration-call proof; recapture it after any package/source change before using it for a new claim.
