...
- STS2-RitsuLib `v0.4.29` is installed locally with the `0.107.1` runtime variant; the prior `v0.4.24` install was backed up before the beta.92 dependency pass.
- The current local game install is Slay the Spire 2 `v0.107.1`; installed beta.92 package parity is recorded, and fresh Off plus AdditiveBatch1 direct smokes under `.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` audited clean. Earlier beta.84/beta.85/beta.87/beta.88/beta.90 smokes are previous-package, previous-dependency, or root-cause history only.
- Spire Plus now compiles against NuGet `STS2.RitsuLib` `0.4.29`; the manifest dependency floor is `STS2-RitsuLib >= 0.4.29`, and no BaseLib or separate `STS2.RitsuLib.Compat.*` package is used.
...
3. Keep the beta.92 dependency floor aligned at `STS2.RitsuLib` / `STS2-RitsuLib` `0.4.29` unless a future owner-approved versioned package deliberately changes it again.
...
6. Use the current AdditiveBatch1 enabled-mode smoke at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` as the beta.92 / `v0.107.1` 10 event types / 14 registration-call proof; recapture it after any package/source change before using it for a new claim.
