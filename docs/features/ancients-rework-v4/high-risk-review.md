...
Scope: private beta hardening pass for implemented EZ Micro Balance patches. v4.3 is current.
...
| Meat Cleaver Cook option | Reviewed | Cook remains gated by current HP and removable-card count, removes exactly two cards, then loses 5 current HP through `CreatureCmd.SetCurrentHp`. Manual rest-site verification still required. |
