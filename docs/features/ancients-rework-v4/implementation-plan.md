...
6. `SereTalon` / 瓦库原初之爪: patch pickup to offer 4 generated Curses, add the selected Curse, then add 2 `Wish` and 1 upgraded `Wish+`. Keep `Claws` (`Tanx Claws` / 坦克斯利爪 in player text) on the Tanx Maul-transform path; selected cards now become upgraded Maul+ / 撕咬+.
