...
- Historical BaseLib `v3.1.4` XML/decompiled local package evidence exposed `CustomAncientModel.CustomScenePath` and patched the Ancient background scene path only for `CustomAncientModel`. Current beta.91 code no longer uses that path: Urda, Morvi, and Lotha derive from RitsuLib `ModAncientEventTemplate`, register through `SharedAncient<...>()`, and expose `CustomBackgroundScenePath`, map-icon, and run-history icon properties directly.
...
- RitsuLib `SavedAttachedState<TKey, TValue>` replaced the previous BaseLib saved-field API. `Player` persistence for the attached state remains not live-proven by this pass; the deck mirror is a safer carrier for live save/load testing, not a substitute for runtime proof.
...
Primary evidence remains local game source. Prefer native game command APIs, RitsuLib APIs, and template-supported APIs before Harmony. BaseLib material in this file is historical migration evidence only unless the owner explicitly approves reintroducing a BaseLib dependency. Tutorial material is secondary only: `https://glitchedreme.github.io/SlayTheSpire2ModdingTutorials/index.html`.
