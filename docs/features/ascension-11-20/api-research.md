...
Status: A11-A20 single-player and host-multiplayer selector expansion plus prototype slices added. The v0.106.0 source refresh and hook-signature migration are compile/test/package proven. Live co-op bisect and current-package loader parity remain pending.
...
- Ran `dotnet test EZMicroBalance.sln --no-build`; the latest normal guard pass passed with release artifact/runtime evidence tests skipped unless `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1`.
...
A11-A20 selection is now default-on in this private-beta multiplayer test candidate. `EZMB_ASCENSION_DISABLE_PUBLIC_SELECTION=1` restores vanilla A1-A10 selection for comparison, `EZMB_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1` disables only host-multiplayer A11-A20 selection, and `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1` is legacy-compatible and no longer required. Forced internal slice checks remain available with `EZMB_ASCENSION_DEBUG_LEVEL`. Full live Ascension and co-op verification is still pending.
...
- Release artifact tests now use `ReleaseArtifactFactAttribute` and are skipped unless `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` is set. Normal clean-clone `dotnet test` should not require ignored `publish/`, package-staging, versioned package, installed DLL/PCK, or local smoke-log artifacts; opt-in release checks remain strict after publish/package refresh.
- Historical runtime smoke exists: a 2026-05-15 normal Steam helper run under `.tools/runtime-evidence/live-spire-plus-session-20260515-211414` loaded only BaseLib and Spire Plus / EZMicroBalance, reported `Found 22 SavedSpireFields`, reached main menu in `13,539ms`, found 0 release-blocking signatures, restored settings/moved mods/current-run files, and left 0 `SlayTheSpire2` processes. current source defines 30 SavedSpireFields, so fresh loader parity remains pending. RC1 normal Steam-client recheck covers Mod Settings visibility for BaseLib and EZ Micro Balance under the historical display name; live gameplay verification remains pending.

## 2026-05-06 Implementation Evidence Update

| Area | Evidence | Status |
| --- | --- | --- |
| Selector and gates | `AscensionFeatureGate` defines A11-A20 constants, `MaxSupportedAscensionLevel = 20`, `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION`, `EZMB_ASCENSION_DISABLE_PUBLIC_SELECTION`, and `EZMB_ASCENSION_DISABLE_MULTIPLAYER_SELECTION`. `AscensionSelectionPatches` expands only non-daily single-player and host-multiplayer `StartRunLobby` paths. Host multiplayer is handled through `StartRunLobby.UpdateMaxMultiplayerAscension`, temporary `LobbyPlayer.maxMultiplayerAscensionUnlocked` snapshots, and a restore finalizer so vanilla multiplayer unlock progress is not mutated. A11-A20 selection is now default-on in this private-beta multiplayer test candidate; `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1` remains accepted but no longer required, `EZMB_ASCENSION_DISABLE_PUBLIC_SELECTION=1` disables all public A11-A20 selection, and `EZMB_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1` disables only host-multiplayer A11-A20 selection. | Compile-proven; live selection/start verification pending. |
...
| Runtime smoke | A bounded `--force-steam off` smoke first exposed `MissingMethodException` for `RootBudCombatHook`; after adding parameterless constructors to Ascension hook models, later A11/A12 run-start testing exposed `DuplicateModelException` from direct `RootRunHook(RunState)` and `AscensionMapQuestMarker` construction during map generation. The current hook registration and map markers use canonical `ModelDb` instances. The 2026-05-14 historical package normal Steam helper smoke loaded only BaseLib and Spire Plus / EZMicroBalance, registered config, reported `Found 22 SavedSpireFields`, found 0 Spire Plus / EZMicroBalance error signatures, and reached main menu. current source defines 30 SavedSpireFields after later static fixes, so fresh loader parity remains pending for the refreshed package. | Historical Steam startup/log evidence exists; live gameplay remains pending. |
...
- Source change: `AscensionFeatureGate.IsPublicSelectionEnabled` now returns true unless `EZMB_ASCENSION_DISABLE_PUBLIC_SELECTION=1` is set. `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1` remains a defined compatibility variable but is no longer required by the predicate.
- `EZMB_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1` still disables only host-multiplayer selector expansion; it does not disable single-player A11-A20 selection.
