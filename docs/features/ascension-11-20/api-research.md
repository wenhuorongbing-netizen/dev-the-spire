...
Status: A11-A20 single-player and host-multiplayer selector expansion plus prototype slices added. The v0.106.0 source refresh and hook-signature migration are compile/test/package proven. Current-package loader parity is covered by the 30-field Steam-client smoke; live co-op bisect and gameplay verification remain pending.
...
A11-A20 selection is now default-on in this private-beta multiplayer test candidate. `SPIREPLUS_ASCENSION_DISABLE_PUBLIC_SELECTION=1` restores vanilla A1-A10 selection for comparison, `SPIREPLUS_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1` disables only host-multiplayer A11-A20 selection, and `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1` is legacy-compatible and no longer required. Forced internal slice checks remain available with `SPIREPLUS_ASCENSION_DEBUG_LEVEL`. Full live Ascension and co-op verification is still pending.
...
| --- | --- | --- |
| Selector and gates | `AscensionFeatureGate` defines A11-A20 constants, `MaxSupportedAscensionLevel = 20`, `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION`, `SPIREPLUS_ASCENSION_DISABLE_PUBLIC_SELECTION`, and `SPIREPLUS_ASCENSION_DISABLE_MULTIPLAYER_SELECTION`. `AscensionSelectionPatches` expands only non-daily single-player and host-multiplayer `StartRunLobby` paths. Host multiplayer is handled through `StartRunLobby.UpdateMaxMultiplayerAscension`, temporary `LobbyPlayer.maxMultiplayerAscensionUnlocked` snapshots, and a restore finalizer so vanilla multiplayer unlock progress is not mutated. A11-A20 selection is now default-on in this private-beta multiplayer test candidate; `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1` remains accepted but no longer required, `SPIREPLUS_ASCENSION_DISABLE_PUBLIC_SELECTION=1` disables all public A11-A20 selection, and `SPIREPLUS_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1` disables only host-multiplayer A11-A20 selection. | Compile-proven; live selection/start verification pending. |
