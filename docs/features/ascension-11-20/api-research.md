...
- A20 Branded Form gameplay remains single-player gated through `AscensionFeatureGate.IsDualKingBrandsSinglePlayerEnabled(...)` and related A20/reward/combat patches. `AscensionSelectionPatches.WarnIfA20MultiplayerDowngraded(...)` now logs when host multiplayer selects A20, including before a client joins, and when a host multiplayer A20 run starts. The warning states that multiplayer A20 selection is enabled for development testing, A20 Branded Form / second-boss enhanced dedicated ability gameplay is disabled or downgraded in co-op pending live verification, and A11-A19 inherited systems may still apply if their gates are enabled.
...
| Runtime smoke | A bounded `--force-steam off` smoke first exposed `MissingMethodException` for `RootBudCombatHook`; after adding parameterless constructors to Ascension hook models, later A11/A12 run-start testing exposed `DuplicateModelException` from direct `RootRunHook(RunState)` and `AscensionMapQuestMarker` construction during map generation. The current hook registration and map markers use canonical `ModelDb` instances. The 2026-05-14 historical package normal Steam helper smoke loaded only BaseLib and Spire Plus / EZMicroBalance, registered config, reported `Found 22 SavedSpireFields`, found 0 Spire Plus / EZMicroBalance error signatures, and reached main menu. current source defines 26 SavedSpireFields after later static fixes, so fresh loader parity remains pending for the refreshed package. | Historical Steam startup/log evidence exists; live gameplay remains pending. |
