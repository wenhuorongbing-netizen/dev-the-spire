...
- A11 dedicated Long Road marker removal is source-patched: `AscensionNodeMetadata` has no `LongRoad` metadata, active localization no longer contains `LONG_ROAD_NODE`, and `AscensionMapUiPatches` generic normal-node hover only creates Deep Branch tips when `metadata.DeepBranch` exists. A11 geometry remains width +1, Act 1 +1 row, Act 2 +1 row, Act 3 +2 rows.
...
- Current package runtime smoke exists: a 2026-05-14 normal Steam helper run under `.tools/runtime-evidence/current-package-smoke-20260514-015901` loaded only BaseLib and Spire Plus / EZMicroBalance, reported `Found 22 SavedSpireFields`, reached main menu in `14,045ms`, found 0 Spire Plus / EZMicroBalance error signatures, found no missing-resource, Ancient scene-load, `Creature.get_ShowsInfiniteHp`, BaseLib patch-failure, or removed-API signatures, restored settings/moved mods/current-run files, and left 0 `SlayTheSpire2` processes. RC1 normal Steam-client recheck covers Mod Settings visibility for BaseLib and EZ Micro Balance under the historical display name; live gameplay verification remains pending.
...
| Runtime smoke | A bounded `--force-steam off` smoke first exposed `MissingMethodException` for `RootBudCombatHook`; after adding parameterless constructors to Ascension hook models, later A11/A12 run-start testing exposed `DuplicateModelException` from direct `RootRunHook(RunState)` and `AscensionMapQuestMarker` construction during map generation. The current hook registration and map markers use canonical `ModelDb` instances. The 2026-05-14 current-package normal Steam helper smoke loaded only BaseLib and Spire Plus / EZMicroBalance, registered config, reported `Found 22 SavedSpireFields`, found 0 Spire Plus / EZMicroBalance error signatures, and reached main menu. | Current Steam startup/log evidence exists; live gameplay remains pending. |
