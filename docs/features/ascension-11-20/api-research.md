...
- Current-package runtime smoke is refreshed: a 2026-05-08 controlled `--force-steam off` run after the default-on gate publish/package refresh loaded only BaseLib and EZ Micro Balance, reported `Found 12 SavedSpireFields`, logged the default-on Ascension initializer wording with 0 old `Default-off gate` lines, reached main menu in `13,201ms`, found 0 EZ Micro Balance error/exception lines, and restored settings byte-for-byte. This does not replace normal Steam-client Mod Settings or live gameplay verification.
