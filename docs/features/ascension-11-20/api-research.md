...
| BaseLib dependency | `EZMicroBalance.csproj` references `Alchyr.Sts2.BaseLib` 3.1.2; local runtime path is `<GameRoot>/mods/BaseLib`. | Fact | High | Missing runtime dependency causes mod-load failure. | Revalidate load with BaseLib v3.1.2 before release. |
...
- Local BaseLib runtime files exist under `<GameRoot>/mods/BaseLib` as DLL/JSON/PCK, and `EZMicroBalance.csproj` references NuGet `Alchyr.Sts2.BaseLib` v3.1.2. Local BaseLib/RitsuLib source trees were not found in this repository; future API claims that require their internals need either local source or a cited package/source inspection.
...
- A20 Dual King Brands gameplay remains single-player gated through `AscensionFeatureGate.IsDualKingBrandsSinglePlayerEnabled(...)` and related A20/reward/combat patches. `AscensionSelectionPatches.WarnIfA20MultiplayerDowngraded(...)` now logs when host multiplayer selects A20, including before a client joins, and when a host multiplayer A20 run starts. The warning states that multiplayer A20 selection is enabled for development testing, Dual King Brands / second-boss Brand gameplay is disabled or downgraded in co-op pending live verification, and A11-A19 inherited systems may still apply if their gates are enabled.
...
| A19 Boss reward and seals | `BossSealDefinition` / `BossSealCatalog` now map active boss encounters to the v2.0 Royal Seal set: Holy Daze, Martyr Oath, Ink Return, Startled Shell, Soul Tide, Boiling Critical, Misaligned Shell, Marginal Note, Struggle Bait, Aeonglass Strength, Chosen Decree, and Residual Sample. Door Wedge is removed from active EZMB source because v0.105.0 replaced Doormaker with Aeonglass. Local source proves `AeonglassBoss` has encounter id `AEONGLASS_BOSS` and generates one `AEONGLASS` monster, so the temporary Aeonglass seal targets `MONSTER.AEONGLASS` exactly and applies +5 Strength at combat start. The older generic Armor/Rage/Barrier/Chaos combat modifiers are no longer applied. Runtime mechanics are source-guarded through supported hooks and custom powers/cards/enchantments; live boss-by-boss verification remains pending. A19 boss card rewards still add a fourth rare option through `CardFactory.CreateForReward` with `NoModifyHooks`. | Build/source-guard proven; reward screen/reroll, Aeonglass +5 Strength, and boss trigger live verification pending. |
...
| Runtime smoke | A bounded `--force-steam off` smoke first exposed `MissingMethodException` for `RootBudCombatHook`; after adding parameterless constructors to Ascension hook models, later A11/A12 run-start testing exposed `DuplicateModelException` from direct `RootRunHook(RunState)` and `AscensionMapQuestMarker` construction during map generation. The current hook registration and map markers use canonical `ModelDb` instances. The 2026-05-14 historical package normal Steam helper smoke loaded only BaseLib and Spire Plus / EZMicroBalance, registered config, reported `Found 22 SavedSpireFields`, found 0 Spire Plus / EZMicroBalance error signatures, and reached main menu. current source defines 25 SavedSpireFields after later static fixes, so fresh loader parity remains pending for the refreshed package. | Historical Steam startup/log evidence exists; live gameplay remains pending. |
...
| Preview implementation conclusion | Firemarked Elite hover now shows the exact Firemark name and summary. Banner hover continues to show exact Banner name and summary. Boss hover shows Royal Seal or King Brand title plus the localized per-boss summary, with `BossSealDefinition.Summary` / `BrandSummary` as source-backed fallback text. Aeonglass hover explicitly states +5 Strength. |
...
| Area | Evidence | Status |
| --- | --- | --- |
| UI message | `source code/localization/zhs/main_menu_ui.json` maps `NETWORK_ERROR.VERSION_MISMATCH.body` to `你试图加入的游戏与您的杀戮尖塔2的版本不同。` | Source-evidenced |
