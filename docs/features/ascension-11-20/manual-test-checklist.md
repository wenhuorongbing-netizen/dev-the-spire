...
- Historical beta.17 normal Steam-client helper startup/log verification under `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` initialized only BaseLib and Spire Plus, registered config, reported `Found 30 SavedSpireFields`, reached startup completion, found 0 release-blocking signatures, restored settings/moved mods, and left 0 `SlayTheSpire2` processes. The current beta.30 package still needs fresh loader proof.
...
- [ ] Soul Tide triggers only for Soul Fysh: entering Intangible grants exactly 1 Artifact. Beckons left in hand grant Block at the next player turn start at 2 each with team caps solo 8, 2 players 12, 3-4 players 16. Branded Form uses 3 Block each with caps solo 12, 2 players 16, 3-4 players 20.
