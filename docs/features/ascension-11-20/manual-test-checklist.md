...
- Historical beta.17 normal Steam-client helper startup/log verification under `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` initialized only BaseLib and Spire Plus, registered config, reported `Found 30 SavedSpireFields`, reached startup completion, found 0 release-blocking signatures, restored settings/moved mods, and left 0 `SlayTheSpire2` processes. The current beta.48 package still needs fresh loader proof.
