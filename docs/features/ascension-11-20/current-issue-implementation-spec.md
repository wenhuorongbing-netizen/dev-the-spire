...
- `ISSUE-2026-05-07-A20-MULTIPLAYER-SELECTION-WARNING-MISSING`: add a log-facing warning when host multiplayer selects or starts A20 while Dual King Brands gameplay remains single-player gated.
...
- Current-package controlled `--force-steam off` smoke is pending unless completed after this publish.
- Live co-op A11-A20 matrix remains pending:
  ...
  - A20 warning/downgrade is visible/logged;
  - `godot.log` has no desync/checksum/ownership warnings.

## 7. Rollback Plan

- Revert only `AscensionSelectionPatches` warning additions and the related source guard snippets if the warning path causes issues.
...
- [x] Host multiplayer A20 selection/start logs a clear development-testing downgrade warning.
