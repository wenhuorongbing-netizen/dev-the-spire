...
1. 仓库为 public，或当前账号计划支持 private repository Pages。
2. 打开 `Settings -> Pages`。
...
```text
SpirePlus-v0.1.0-private-beta.125.zip
```
...
6. 回复该帖子。
7. 刷新页面，主帖和回复仍然存在。
8. 设 `SPIRE_PLUS_FORUM_READ_ONLY=1`，重新运行 Pages，确认发帖和回复被关闭。
9. 设 `SPIRE_PLUS_FORUM_READ_ONLY=0`，重新运行 Pages，确认发帖和回复恢复。
