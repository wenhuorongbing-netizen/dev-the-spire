...
Status: Urda is default-on for private-beta testing with eleven source-backed blessing ids: Seedbed, Humus Pact, Molting, Moss Map, Trial Branch, Shallow-Root Relic, Elite Root, Rooted Route, After the Rain, Root-Sight, and Seed Bank. Historical previous package resource evidence under `.tools/runtime-evidence/urda-pck-resource-load-20260513-123345` verified that the installed PCK resolved the custom scene/icon with 0 `ERROR` / `WARNING` lines; previous beta.93 loader proof is RitsuLib-only and does not by itself prove clicked Urda UI. Live gameplay, save/load, co-op, and current UI verification are still pending and tracked through live Urda issues in `docs/issues.md`.
