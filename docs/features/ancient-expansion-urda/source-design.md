...
The parser accepts the prior eight-field shape for migration. Historical May evidence did not prove previous package `previous saved-state API<Player,string>` persistence for player-owned Urda state; the current RitsuLib-only implementation uses `SavedAttachedState<Player,string>` plus the retained card-backed mirror. State must survive live save/load before this design can be marked release-ready.
