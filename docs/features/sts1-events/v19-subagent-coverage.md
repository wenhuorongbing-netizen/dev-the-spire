...
- 2026-06-15 subagent role coverage audit against `docs/goals/event.md` and `scripts/check-sts1-v19-subagent-coverage.ps1`.

These audits do not create CanaryOnly, AdditiveBatch1, gameplay, replacement, multiplayer, or QA proof. They only help keep static docs/scripts aligned while validation is paused.

## Required Role Ledger

| Role | Current disposition | Current evidence | Next action |
|------|---------------------|------------------|-------------|
| BuildGate / Repo Health | runtime-validation-paused | `PROJECT_STATE.md` records beta.85 historical/current validation; O0-O10 remain paused or pending in `v19-gate-ledger.csv`. | Rerun build/test/format/diff/package lanes after coordination pause. |
| Runtime Environment Bootstrap | runtime-validation-paused | `PROJECT_STATE.md` and `v19-gate-evidence-map.md` record beta.85 default-Off loader proof only. | Capture current CanaryOnly and AdditiveBatch1 smoke after coordination pause. |
| API Drift Fix Subagent | historical-pass-current-paused | beta.85 default-Off proof records the stale runtime target blocker fixed for Off mode. | Revalidate after any source/package change. |
...
| Feature Gate / Registration Engineer | static-only | Feature-gate, registry-shape, and enabled-log expected-shape checks guard Off, CanaryOnly, AdditiveBatch1, unsafe all-draft, and replacement gates. | Capture enabled-mode runtime proof after coordination pause. |
| Canary Gameplay Subagent | runtime-blocked | O25-O41 remain current-pending or blocked in `v19-gate-ledger.csv`. | Capture four canary runtime proofs after current CanaryOnly smoke passes. |
| Simple Batch Gameplay Subagent | runtime-blocked | O33 and O42-O52 remain current-pending or blocked in `v19-gate-ledger.csv`. | Capture AdditiveBatch1 smoke and six simple-batch gameplay proofs. |
| Asset + Localization Subagent | static-gap-known | Asset-safety and localization gap-baseline checkers guard zero tracked original StS1 images and the 33-key known localization gap. | Close localization/resource gaps only in a versioned, validated resource pass. |
...
These are future handoff targets for role owners after the coordination pause is lifted. They are not current evidence, and their absence keeps the mapped runtime gates open.

| Role owner | Required packet or proof | Gates guarded |
|------------|--------------------------|---------------|
| Runtime Environment Bootstrap + Feature Gate / Registration Engineer | CanaryOnly packet with `session-state.json`, `settings.save.before`, `game-release-info.json`, `godot.log.after-launch`, `godot-log-audit.json`, `enabled-mode-log-check.json`, `runtime-evidence-packet-check.json`, and `restore-state.json`; copied-log verifier must use `-ExpectedPackageVersion`, `-ExpectedRitsuCompatBranch`, `-ExpectedRitsuLibVersion`, `-ExpectedGameVersion`, `-OutFile`, and `-FailOnMismatch`; packet verifier must show matching `Sts1EventModeEnvironment`, explicit package/Ritsu-compat/RitsuLib-version/game-version targets, no unsafe-mode env leakage, no `-AllowMissingSessionState` / `-AllowMissingRestoreState` bypass, and CanaryOnly 4 event types / 6 registration calls. Copied-log proof covers registration-call count and class set; Act-target tuple proof remains source-derived until future logs or gameplay evidence prove those targets directly. | O25, O39 |
| Runtime Environment Bootstrap + Feature Gate / Registration Engineer | AdditiveBatch1 packet with the same required files, verifier switches, session metadata, package/Ritsu-compat/RitsuLib-version/game-version target checks, unsafe-mode rejection, missing-session/restore bypass rejection, and AdditiveBatch1 10 event types / 14 registration calls. Copied-log proof covers registration-call count and class set; Act-target tuple proof remains source-derived until future logs or gameplay evidence prove those targets directly. | O33, O51 |
...
| Simple Batch Gameplay Subagent | Purifier, Upgrade Shrine, Golden Shrine, The Cleric, Old Beggar, and Shining Light screenshots, result logs, save-load proof, EN/ZHS render screenshots, image/license disposition, and independent QA after AdditiveBatch1 proof exists. | O42-O52 |
| Asset + Localization Subagent | Versioned resource pass for `STS1_GOLDEN_IDOL.pages.LEAVE.description` first, then the remaining 32 source-referenced keys; update package version, package docs, handoff docs, and localization baseline guards in the same validated pass. Closing only the direct Golden Idol key is not O25/O33 proof. | O19, O35-O36, O49, O64 |
...
- Static subagent coverage does not close O25, O33, gameplay, save/load, replacement, multiplayer, image/render, localization-resource, or QA gates.
