...
| Custom saved fields | `SavedAttachedState` | Current Spire Plus beta.93 uses RitsuLib `SavedAttachedState` for custom persistence. Historical `previous saved-state registrations` rows are previous package-era evidence only. |
