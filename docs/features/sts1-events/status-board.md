...
> Last updated: 2026-06-18 (current beta.86 AdditiveBatch1 verifier packet captured and passed; latest CanaryOnly-mode packet remains beta.85 previous-package context; no gameplay or release-status change)
...
`historical-loader-verified` means the row has old enabled-mode loader-gate evidence only. Current beta.86 has clean `v0.107.0` AdditiveBatch1 direct loader proof with 10 event types / 14 registration calls and exact tuple parity. Beta.85 Off and CanaryOnly packets remain previous-package loader context. Gameplay, event rendering, save-load, image/license status, replacement-pool behavior, multiplayer disposition, and StS1 parity proof remain pending.
...
| Build | beta.86 validated 0 errors / 0 warnings | `PROJECT_STATE.md` and `docs/dev-environment.md` record the beta.86 package-alignment build as 0 errors / 0 warnings. This remains no-game build validation, not event gameplay proof. |
| Tests | beta.86 post-doc split lanes passed: 489 passed / 0 failed / 39 skipped / 528 total | `PROJECT_STATE.md` and `docs/dev-environment.md` record the latest trusted split test strategy and current package/artifact validation status. This proves automated guard/package coverage only, not enabled-mode or gameplay proof. |
...
|-------|--------|-------|----------|------------|-------|
| Big Fish | implemented, compiled, test-guarded, source/API verified, loader-verified | Encounter screenshot/result log, save-load, EN/ZHS render, image/license/render | true | Source registration targets Act 1 buckets; retained beta.85 CanaryOnly proof and current beta.86 AdditiveBatch1 proof cover loader registration only, while event-pool/UI proof remains pending. Source/localization now use wiki-aligned Box option identity; runtime UI proof remains pending | canary-source-api-proof.md, Sts1EventFeatureGuardTests.cs, beta.85 CanaryOnly verifier packet; beta.86 AdditiveBatch1 verifier packet |
| Golden Idol | implemented, compiled, test-guarded, source/API verified, loader-verified | Golden Idol relic parity decision, encounter screenshot/result log, save-load, EN/ZHS render, image/license/render | true | Source registration targets Act 1 buckets; retained beta.85 CanaryOnly proof and current beta.86 AdditiveBatch1 proof cover loader registration only, while event-pool/UI proof remains pending. Trap source/localization now use Outrun, Smash, and Hide branch identities; runtime UI/result proof remains pending. Take currently grants a random relic because no Golden Idol relic model is implemented | canary-source-api-proof.md, Sts1EventFeatureGuardTests.cs, beta.85 CanaryOnly verifier packet; beta.86 AdditiveBatch1 verifier packet |
| The Lab | implemented, compiled, test-guarded, source/API verified, loader-verified | Encounter screenshot/result log, save-load, EN/ZHS render, image/license/render | true | Retained beta.85 CanaryOnly proof and current beta.86 AdditiveBatch1 proof cover loader registration only. Source/localization now expose only Open and document 3 potions / 2 at A15+; runtime UI/result proof remains pending | canary-source-api-proof.md, Sts1EventFeatureGuardTests.cs, beta.85 CanaryOnly verifier packet; beta.86 AdditiveBatch1 verifier packet |
| Divine Fountain | implemented, compiled, test-guarded, source/API verified, loader-verified | Encounter screenshot/result log, save-load, EN/ZHS render, image/license/render | true | Retained beta.85 CanaryOnly proof and current beta.86 AdditiveBatch1 proof cover loader registration only. Curse prerequisite is now source-guarded through `IsAllowed(IRunState)` and the option identity is aligned to Drink; runtime selection/UI proof remains pending | canary-source-api-proof.md, Sts1EventFeatureGuardTests.cs, beta.85 CanaryOnly verifier packet; beta.86 AdditiveBatch1 verifier packet |
...
## Runtime Gates (historical v16 / `v0.106.1`; current beta.86 AdditiveBatch1 proof clean)

| Gate | Status | Blocker |
|------|--------|---------|
| Runtime path report (O21) | **current prerequisite pass** | E-drive game root, BaseLib v3.1.4, STS2-RitsuLib v0.4.16 with `lib\0.107.0`, and Spire Plus beta.86 package-parity install are present. |
...
| Active `godot.log` generated (O23) | **current pass** | Current beta.86 AdditiveBatch1 direct smoke generated `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/godot.log.after-launch`. |
| Loader proof (O24) | **current AdditiveBatch1 pass** | Current beta.86 AdditiveBatch1 direct smoke on `v0.107.0` / RitsuLib `v0.4.16` reached main menu, reported `v0.1.0-private-beta.86`, applied 25/25 Spire Plus ModPatcher patches, and has a clean `godot-log-audit.json` with 0 blocking signature hits. |
...
| AdditiveBatch1 source identity (O15) | **static-pass / current runtime pass** | Static registry-shape/current-doc guards expect 10 event types through 14 registration calls after Big Fish, Golden Idol, The Cleric, and Shining Light moved to both Act 1 buckets. Fresh beta.86 `v0.107.0` direct AdditiveBatch1 enabled-mode smoke is O33 and passes. |
...
| AdditiveBatch1 runtime launch (O33) | **current pass** | Fresh beta.86 AdditiveBatch1 direct smoke at `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/` reached main menu, audited clean, and retained verifiers passed with 10 event types / 14 registration calls and exact act/shared tuple parity. |
...
| **v21 beta.86 AdditiveBatch1 clean log** | `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/godot.log.after-launch` |
| **v21 beta.86 AdditiveBatch1 verifier reports** | `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/enabled-mode-log-check.json` and `runtime-evidence-packet-check.json` (retained enabled-mode log checks=21 / mismatches=0; packet checks=45 / mismatches=0; exact act/shared tuple parity) |
...
| **v20 2026-06-17/18 pause-safe static alignment** | `PROJECT_STATE.md`, `docs/goals/event.md`, `docs/reviews/current-validation.md`, `docs/features/sts1-events/test-plan.md`, `docs/features/sts1-events/v19-gate-evidence-map.md`, `docs/features/sts1-events/v20-final-gate-overlay.csv`, and `docs/features/sts1-events/hard-stop-blocker-report-v20-coordination-pause-20260617.md` record static suite 15 / 0, beta.86 runtime preflight 27 / 0, current-doc claims 956 / 0, static-file hygiene 11 / 0, v19 gate ledger 534 / 0, v20 final-gate overlay 29 / 0, subagent coverage 70 / 0, and focused `git diff --check --` exit 0 with CRLF warnings only for `AGENTS.md` and `scripts/check-sts1-event-current-doc-claims.ps1`; this is static evidence only and does not close gameplay gates. |
