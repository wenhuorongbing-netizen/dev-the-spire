...
> Last updated: 2026-06-19 (current beta.88 `v0.107.1` AdditiveBatch1 verifier packet captured and passed for loader/registration only; latest CanaryOnly-mode packet remains beta.85 previous-package context; gameplay and release proof remain pending)
...
`historical-loader-verified` means the row has old enabled-mode loader-gate evidence only. Retained beta.87 has clean `v0.107.0` AdditiveBatch1 direct loader proof with 10 event types / 14 registration calls and exact tuple parity. Current beta.88 has clean `v0.107.1` AdditiveBatch1 direct loader/registration proof with BaseLib `v3.3.0`, 25/25 Spire Plus patches applied, 10 event types / 14 registration calls, and exact tuple parity. Beta.85 Off and CanaryOnly packets remain previous-package loader context. Gameplay, event rendering, save-load, image/license status, replacement-pool behavior, multiplayer disposition, and StS1 parity proof remain pending.
...
## Runtime Gates (historical v16 / `v0.106.1`; retained `v0.107.0` beta.87 AdditiveBatch1 proof clean; current `v0.107.1` beta.88 loader clean)

| Gate | Status | Blocker |
|------|--------|---------|
| Runtime path report (O21) | **current prerequisite pass** | E-drive game root, BaseLib v3.3.0, STS2-RitsuLib v0.4.24 with `lib\0.107.0`, and Spire Plus beta.88 package-parity install are present. |
...
| Active `godot.log` generated (O23) | **current packet exists** | Beta.88 AdditiveBatch1 direct smoke generated `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937/godot.log.current-iteration` on `v0.107.1` and audited clean. Retained beta.87 `v0.107.0` proof and beta.87 `v0.107.1` BaseLib `v3.2.1` failure remain historical context. |
| Loader proof (O24) | **current pass for loader/registration** | Beta.88 AdditiveBatch1 direct smoke on `v0.107.1` / BaseLib `v3.3.0` / RitsuLib `v0.4.24` reached main menu, reported `v0.1.0-private-beta.88`, applied 25/25 Spire Plus ModPatcher patches, has a clean `godot-log-audit.json`, passed enabled-mode verifier 31 / 0, and passed packet verifier 0 mismatches. This is not gameplay proof. |
...
| AdditiveBatch1 source identity (O15) | **static-pass / current loader pass** | Static registry-shape/current-doc guards expect 10 event types through 14 registration calls after Big Fish, Golden Idol, The Cleric, and Shining Light moved to both Act 1 buckets. Beta.88 `v0.107.1` direct AdditiveBatch1 enabled-mode smoke is O33 and passes as current loader proof. |
...
| AdditiveBatch1 runtime launch (O33) | **current loader/registration pass** | Beta.88 AdditiveBatch1 direct smoke at `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937/` reached main menu on `v0.107.1`, audited clean, and retained verifiers passed with 10 event types / 14 registration calls and exact act/shared tuple parity. Beta.87 `v0.107.1` failure at `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` is BaseLib `v3.2.1` root-cause history. |
