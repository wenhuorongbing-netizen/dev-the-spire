...
> Last updated: 2026-06-17 (pause-safe v20 static alignment, status-board header guard, subagent role guard, and retained beta.85 Off packet verifier report; no enabled-mode, gameplay, or release-status change)
...
| **v20 2026-06-17 pause-safe static alignment** | `docs/goals/event.md`, `docs/reviews/current-validation.md`, `docs/features/sts1-events/test-plan.md`, and `docs/features/sts1-events/v19-gate-evidence-map.md` record static suite 14 / 0, runtime preflight 23 / 0, current-doc claims 896 / 0, static-file hygiene 11 / 0, v19 gate ledger 531 / 0, subagent coverage 66 / 0, and focused `git diff --check --` exit 0 with a CRLF warning only; this is static evidence only and does not close runtime gates. |
| **v20 2026-06-17 subagent role guard** | `docs/features/sts1-events/v19-subagent-coverage.md` records the 15-role v20 subagent coverage shape while retaining the v19 filename; `scripts/check-sts1-v19-subagent-coverage.ps1 -FailOnMismatch` returns 66 / 0 and remains static/non-runtime evidence only. |
...
| No-launch runtime preflight checker | `scripts/check-sts1-runtime-preflight.ps1 -FailOnMismatch` reads installed manifests and source-only expected shapes; it does not launch the game or close enabled-mode/runtime gates. |
