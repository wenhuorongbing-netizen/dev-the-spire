...
This file is source/API evidence only. It does not prove current `v0.107.0` loader/runtime behavior, encounter gameplay, save/load, EN/ZHS rendering, image/license status, replacement-pool behavior, multiplayer disposition, or full StS1 wiki parity. Current runtime smoke proof for these enabled modes remains pending under the shared validation pause.
