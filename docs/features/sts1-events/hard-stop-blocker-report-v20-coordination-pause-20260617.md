...
- `O25`: retained `v0.107.0` CanaryOnly enabled-mode smoke has since been captured by the shared validation lane and is loader proof only; this thread did not create it. Previous beta.96 Off loader proof exists on `v0.107.1` as previous-package startup proof after the beta.98 package refresh, but current beta.98 CanaryOnly enabled-mode proof still needs recapture.
- `O33`: retained `v0.107.0` AdditiveBatch1 enabled-mode smoke has since been captured by the shared validation lane and is loader/registration proof only; this thread did not create it. Previous beta.96 Off loader proof exists on `v0.107.1` as previous-package startup proof after the beta.98 package refresh, but current beta.98 AdditiveBatch1 enabled-mode proof still needs recapture.
