...
- Full historical validation ledger `docs/reviews/current-validation.md` -> `docs/archive/feature-audits/current-validation-full-20260622.md`; active validation file is a compact current beta.135/RitsuLib evidence summary.
...
- Completed RitsuLib migration process files moved from the active feature folder to `docs/archive/feature-audits/ritsulib-migration/`: localization fallback candidate record, next-runtime QA run plan, May 31 hard-block boundary, and monthly dev-spec stub. Current RitsuLib development starts from `docs/features/ritsulib-migration/README.md`, `docs/integrations/ritsulib.md`, and installed RitsuLib XML/API evidence.
