...
Compile and manifest dependency are active. `EZMicroBalance.csproj` references `STS2.RitsuLib` only, and `EZMicroBalance.json` declares only `STS2-RitsuLib` as the shared runtime framework dependency. Current beta.96 Off runtime loader-gate validation exists on Slay the Spire 2 `v0.107.1` with official `STS2-RitsuLib` `v0.4.31` and `lib\0.107.1`. Older beta.85/beta.86/beta.87/beta.88/beta.90 packets are previous-package or previous-game-version loader evidence only; beta.96 package parity is current package evidence, beta.96 RitsuLib Mod Settings clicked UI proof is captured under `.tools/runtime-evidence/beta96-ritsulib-mod-settings-clicked-ui-20260621-160701/`, and beta.96 Off loader proof is captured under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`.
...
| --- | --- | ---: | --- |
| `lib/0.107.1/` | Slay the Spire 2 `v0.107.1` | 5.3 MB | Selected compatibility branch for current local `v0.107.1`; beta.96 Off loader proof is clean current-package evidence, and beta.93 AdditiveBatch1 loader/registration proof is previous-package evidence |
...
- Batch 1: bootstrap and diagnostics scaffold complete; historical `v0.106.1` loader-gate validated by diagnostic smoke, beta.85/beta.86 Off/CanaryOnly loader proof remains previous-package context, beta.87 AdditiveBatch1 proof remains retained `v0.107.0` context, beta.96 Off loader proof is clean current-package evidence on `v0.107.1` with RitsuLib `v0.4.31`, and beta.93 AdditiveBatch1 loader/registration proof remains previous-package evidence.
