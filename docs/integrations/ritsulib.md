...
- `dotnet list ... package --outdated` and NuGet package search report
  `STS2.RitsuLib` `0.4.32` as the latest NuGet package.
...
- Batch 1: bootstrap and diagnostics scaffold complete; historical `v0.106.1` loader-gate validated by diagnostic smoke, beta.85/beta.86 Off/CanaryOnly loader proof remains previous-package context, beta.87 AdditiveBatch1 proof remains retained `v0.107.0` context, beta.98 package parity is current on `v0.107.1` with RitsuLib `v0.4.32`, beta.98 loader recapture is pending, and beta.96/beta.93 loader proof remains previous-package evidence.
