...
Compile and manifest dependency are active. Runtime loader-gate validation exists for the historical Slay the Spire 2 `v0.106.1` environment and current beta.86 Off/CanaryOnly/AdditiveBatch1 direct loader smokes on Slay the Spire 2 `v0.107.0`. Current-game RitsuLib is installed as official `v0.4.16` with `lib\0.107.0`.
...
Clean diagnostic loader evidence exists for historical Off, CanaryOnly, and AdditiveBatch1 modes with BaseLib, RitsuLib, and Spire Plus loaded, 25/25 migrated ModPatcher patches applied, and 30 SavedSpireFields observed. This remains historical `v0.106.1` loader-gate proof. The current `v0.107.0` game install has matching RitsuLib `lib\0.107.0` runtime files. Installed beta.86 package parity passed on 2026-06-18, and fresh current-package direct proof now covers Off `.tools/runtime-evidence/v01070-beta86-off-direct-20260618-115305/`, CanaryOnly `.tools/runtime-evidence/v01070-beta86-canary-direct-cleanlog-20260618-115723/`, and AdditiveBatch1 `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-cleanlog-20260618-115723/`. All three reached main menu, applied 25/25 Spire Plus ModPatcher patches, audited clean, and passed retained mode/packet verifiers with 0 mismatches. The prior beta.84 package-parity Off smoke under `.tools/runtime-evidence/v01070-off-package-parity-20260610-092045/` is retained as root-cause evidence for stale Spire Plus API targets.

Revision M source-fix context exists under `.tools/runtime-evidence/v01070-current-source-getter-targets-20260610-1000/`: it reached main menu on `v0.107.0`, selected RitsuLib compat branch `0.107.0`, applied 25/25 Spire Plus ModPatcher patches, and audited clean. Current beta.86 proof now includes `.tools/runtime-evidence/v01070-beta86-off-direct-20260618-115305/` (Off, 0 StS1 registrations), `.tools/runtime-evidence/v01070-beta86-canary-direct-cleanlog-20260618-115723/` (CanaryOnly, 4 event types / 6 registration calls), and `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-cleanlog-20260618-115723/` (AdditiveBatch1, 10 event types / 14 registration calls). The false direct-smoke failure was helper drift, not RitsuLib drift: `--force-steam off` uses default profile settings, and the helper prepared only Steam-user settings while writing `mods_enabled` at the wrong JSON level; `scripts/spire-plus-live-session.ps1` now writes `mod_settings.mods_enabled` and exposes `-PrepareDefaultSettings` for direct smoke. Gameplay, event screenshots, save-load, image/render, replacement functional proof, co-op/fail-closed proof, independent QA, and versioned tester-package handoff remain pending; the beta.86 migration validation was pushed at `eaaeb5a1`, but handoff must recapture HEAD and worktree status after any later edits. Post-baseline no-game/doc-governance recaptures include `bdc002d6`, `d0932097`, and `8aca2701`; they do not refresh the beta.86 package artifact or close manual gameplay gates.

Package metadata decision: the current source and beta.86 package line do not bump the compile package or manifest minimum. Keep `STS2.RitsuLib` and `STS2-RitsuLib` at `0.3.2` unless the owner chooses a dependency-version slice. A future owner-approved `v0.107.0` tester package may move both values to `0.4.16` in the same package-version increment, then refresh publish/package artifacts and runtime evidence.
...
- Batch 1: bootstrap and diagnostics scaffold complete; historical `v0.106.1` loader-gate validated by diagnostic smoke, and current beta.86 Off/CanaryOnly/AdditiveBatch1 loader/registration proof is clean on `v0.107.0` with RitsuLib `v0.4.16`.
