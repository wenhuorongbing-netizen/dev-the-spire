...
Package metadata decision: the beta.93 dependency-floor pass intentionally bumped both `STS2.RitsuLib` and the `STS2-RitsuLib` manifest minimum to `0.4.31`, removed BaseLib from current Spire Plus package requirements, and set the package version to `v0.1.0-private-beta.93` for the Slay the Spire 2 `v0.107.1` compatibility recapture.
