...
Compile and manifest dependency are active. Current Spire Plus no longer depends on BaseLib: `EZMicroBalance.csproj` references `STS2.RitsuLib` only, and `EZMicroBalance.json` declares only `STS2-RitsuLib` as the shared runtime framework dependency. Runtime loader-gate validation exists for the current Slay the Spire 2 `v0.107.1` environment with official `STS2-RitsuLib` `v0.4.29` and `lib\0.107.1`. Beta.85/beta.86/beta.87, beta.88 BaseLib-backed, and beta.90 RitsuLib-only packets are previous-package or previous-dependency-context loader evidence.

- Compile package: `STS2.RitsuLib` `0.4.29` from NuGet.
- Runtime dependency: manifest declares `STS2-RitsuLib` with `min_version: 0.4.29`.
- Installed runtime: official `STS2-RitsuLib` `v0.4.29` variant pack under the E-drive game root.
...
The installed official `STS2-RitsuLib.0.4.29.variant-pack.zip` contains game-version variants plus a root loader:
...
| `mod_manifest.json` | 414 B | Mod manifest, id `STS2-RitsuLib`, version `0.4.29`, min game `0.107.1` |
| `ritsulib-variants.manifest` | 240 B | Variant selection config, schema 1 |

| Variant Directory | Game Version | DLL Size | Compatibility |
| --- | --- | ---: | --- |
| `lib/0.107.1/` | Slay the Spire 2 `v0.107.1` | 5.3 MB | Selected compatibility branch for current local `v0.107.1`; beta.92 Off and AdditiveBatch1 loader/registration proof is clean |

Each variant directory contains `STS2-RitsuLib.dll`, `.pdb`, `.xml`, and `compat-target.txt`.

## NuGet Package Status

The installed runtime variant pack now covers the current local `v0.107.1` game target through the selected `lib\0.107.1` compatibility branch. Spire Plus references the main current-highest `STS2.RitsuLib` package `0.4.29`; no separate compat NuGet package is used.

| NuGet Package | Version | Status |
| --- | --- | --- |
| `STS2.RitsuLib` | `0.4.29` | Current repo compile package |
