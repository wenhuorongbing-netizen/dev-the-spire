...
The original Ancient-only private beta scope excluded Ascension 11-20-30 and custom-character work. The 2026-05-06 overnight sprint goal opened Ascension 11-20 as an active implementation track. The current test candidate exposes A11-A20 single-player selection through the original Ascension UI by default, keeps all selector expansion disableable with `SPIREPLUS_ASCENSION_DISABLE_PUBLIC_SELECTION=1`, keeps host-multiplayer selection separately disableable with `SPIREPLUS_ASCENSION_DISABLE_MULTIPLAYER_SELECTION=1`, accepts legacy `EZMB_ASCENSION_ALLOW_PUBLIC_ASCENSION=1` without requiring it, and fails host-multiplayer A11-A20 selection/gameplay closed by default after the 2026-05-25 crash logs unless `SPIREPLUS_ALLOW_UNVERIFIED_COOP_GAMEPLAY=1` is deliberately set for two-client debugging. It does not claim release readiness until live behavior is verified.
