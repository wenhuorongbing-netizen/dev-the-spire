...
Current beta.19 loader parity is pending. Current source defines 30 SavedSpireFields. The latest loader smoke under `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` is clean and reports `Found 30 SavedSpireFields`, but it is historical beta.17 startup evidence only. Installed-PCK resource smoke under `.tools/runtime-evidence/current-package-smoke-20260514-015901` remains resource-only evidence. Live gameplay, save-load, death/failure-path, co-op, and clicked Ancient UI remain pending.
...
Update note, 2026-05-14 package hash refresh: after the source red-team hardening pass, package staging, the versioned package folder, and `publish/SpirePlus-v0.1.0-private-beta.19.zip` were refreshed from installed artifacts. These hashes record automated source validation only; live gameplay, save-load, death/failure path, and co-op verification remain pending.
...
| Single package structure | `EZMicroBalance/`, `EZMicroBalanceCode/`, `EZMicroBalance.csproj`, `EZMicroBalance.json`, and `publish/SpirePlus-v0.1.0-private-beta.19.zip` with `EZMicroBalance/` as the install folder; duplicate root mod surfaces are removed from the active tree. | Pass |
...
| Phase 1 saved-state foundation | `AncientSavedStateFields.cs` defines 14 Ancient fields after the Morvi Open-Book sealed-card marker, `AscensionSavedStateFields.cs` defines 12 Ascension fields after Rootblight pending-downgrade persistence, and the historical beta.17 loader smoke reports `Found 30 SavedSpireFields`. `AncientPlayerState.cs` mirrors Urda/Morvi/Lotha player state onto deck card markers for reload-recovery testing. | Pass for source guards; current beta.19 loader/live save-load remains pending |
...
| Phase 8 required commands | Latest validation passed for source/package automation after the player-visible localization naming guard; current beta.19 still needs fresh loader smoke. Latest package refresh evidence still includes artifact tests and package/PCK text checks for Sere Talon / Tanx Claws plus Trial Branch. Gameplay, save-load, clicked UI, route-click, death/failure, and co-op remain pending. | Pass for source/package validation; current loader and live manual gameplay pending |
| Package artifacts | `publish/SpirePlus-v0.1.0-private-beta.19.zip` SHA256 `631837AF16F00C83F00954446A14E6B610E1B251B0D994E321C84C166016898E`; DLL `5994FBEFDE7B21B358D9072D9CBDF7E1254F6F0C8F23B190687B307BBC30BBD7`; manifest `C43CF5A9981A187C4B9F5D653C31892546A71257A9A6C4673FDCBADBC5B0C436`; PCK `337B6A2940094AF28C9D8431E34FDB4E4D467016FE1ADAD50969DA1965023856`; README `A51753782BADD5FEEF325910AFB5189A0FE1C7A58B9752FA2C6DD46B6B89198C`. The README is now a short manual-test install note and says Ancient selections grant visible marker relics. | Pass for installed/staging/versioned/zip artifact parity |
| Runtime smoke and Mod Settings UI | The beta.17 smoke at `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` loaded BaseLib plus Spire Plus under technical id `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, and audited clean. It is historical startup evidence for beta.17. The refreshed Mod Settings UI list screenshot at `.tools/runtime-evidence/current-spire-plus-modsettings-20260513-111342/02-mod-config-list.png` shows `Spire Plus`. | Pass for historical startup and visible mod list; current beta.19 loader/gameplay/UI-click verification pending |
...
- Historical beta.17 normal Steam-client startup/log verification reports `Found 30 SavedSpireFields`, and the refreshed Mod Settings UI list capture now shows `Spire Plus`; current beta.19 loader, gameplay, clicked UI, save-load, and co-op rows remain pending.
...
- Urda, Morvi, Lotha, and Vakuu source/art/localization are implemented, and historical loader/resource smoke exists, but current beta.19 loader, live gameplay, clicked Ancient UI, save-load, death/failure-path, and co-op checks are pending.
