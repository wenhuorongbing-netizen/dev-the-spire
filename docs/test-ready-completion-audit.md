...
Current loader parity is covered by the beta.16 Steam-client smoke. Current source defines 30 SavedSpireFields. The latest loader smoke under `.tools/runtime-evidence/beta16-loader-smoke-20260525-184743` is clean, reports `Found 30 SavedSpireFields`, and matches the current ZIP/DLL/PCK/manifest/README package hashes. Installed-PCK resource smoke under `.tools/runtime-evidence/current-package-smoke-20260514-015901` remains resource-only evidence. Live gameplay, save-load, death/failure-path, co-op, and clicked Ancient UI remain pending.
...
| Runtime smoke and Mod Settings UI | The latest smoke at `.tools/runtime-evidence/beta16-loader-smoke-20260525-184743` loaded BaseLib plus Spire Plus under technical id `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, audited clean, and matched the current beta.16 package hash. The refreshed Mod Settings UI list screenshot at `.tools/runtime-evidence/current-spire-plus-modsettings-20260513-111342/02-mod-config-list.png` shows `Spire Plus`. | Pass for current-package startup and visible mod list; gameplay/UI-click verification pending |
...
- Current normal Steam-client startup/log verification reports `Found 30 SavedSpireFields` for the current beta.16 package hash, and the refreshed Mod Settings UI list capture now shows `Spire Plus`; gameplay, clicked UI, save-load, and co-op rows remain pending.
