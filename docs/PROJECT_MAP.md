...
| M3 | Historical v0.106.1 source context remains available; current `v0.107.1` / STS2-RitsuLib `v0.4.31` beta.96 build, publish, package, installed parity, source-workspace preflight, RitsuLib settings UI evidence, and Off loader proof are refreshed; previous beta.93 AdditiveBatch1 loader evidence is retained only as previous-package context, and gameplay live smoke remains pending for beta.96. |
