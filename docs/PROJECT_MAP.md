...
| `docs/archive/legacy-planning/m5-revision-n-*-20260619.md` | Archive | Historical beta.88/previous package Revision N owner packet, validation replay, runtime evidence plan, and final report; active stubs under `docs/goals/` route current work to beta.99 RitsuLib-only docs. |
...
| `docs/goals/m5-revision-n-*.md` | Current boundary stubs | Compact pointers preserving the historical Revision N document names while routing current migration/runtime/owner work to beta.99 RitsuLib-only docs and archived full records. |
...
| M3 | Historical v0.106.1 source context remains available; current `v0.107.1` / STS2-RitsuLib `v0.4.32` beta.99 build, publish, package, runtime preflight, source-workspace validation, installed parity, and RitsuLib settings UI proof are refreshed; beta.99 direct Off loader proof remains pending; previous beta.96/beta.93 loader evidence is retained only as previous-package context, and gameplay live smoke remains pending for beta.99. |
