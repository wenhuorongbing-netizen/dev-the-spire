...
| M3 | Historical v0.106.1 source context remains available; current `v0.107.1` / STS2-RitsuLib `v0.4.34` beta.115 build, publish, package, runtime preflight, source-workspace validation, and installed parity are refreshed; previous beta.99/beta.96/beta.93 loader/settings evidence is retained only as previous-package context, and enabled-mode/gameplay live smoke remains pending for beta.115. |
