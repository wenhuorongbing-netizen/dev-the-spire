...
| `docs/features/sts1-events/` | Current planning | StS1 events migration: public 52-event baseline tracked; 54 canonical rows, 50 registry identities, 48 model files, and 47 compiling models reconciled. RegisterAll is now 56 calls; AdditiveBatch1 is 10 event types / 13 calls after Big Fish and Golden Idol moved to Act 1 bucket registration. Historical `v0.106.1` loader proof exists, while current `v0.107.0` beta.84 loader proof failed clean audit. |
