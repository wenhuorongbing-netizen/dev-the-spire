...
| M3 | Historical v0.106.1 source context remains available; current `v0.107.1` / STS2-RitsuLib `v0.4.32` beta.99 build, publish, package, runtime preflight, source-workspace validation, installed parity, RitsuLib settings UI proof, and direct Off loader proof are refreshed; previous beta.96/beta.93 loader evidence is retained only as previous-package context, and enabled-mode/gameplay live smoke remains pending for beta.99. |
