...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. Current package target remains `publish/SpirePlus-v0.1.0-private-beta.17.zip`. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
...
- The latest Steam-client loader smoke under `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` reported `Found 30 SavedSpireFields` with only BaseLib plus Spire Plus loaded, clean log audit, startup completion, game stop, restored mod isolation, and current beta.17 package hash parity. Older beta.13, `20260524-161744`, 22-field, and `20260523-current` loader smokes remain historical evidence only.
