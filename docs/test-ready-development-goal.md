...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.99.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the settings-page I18N resource migration plus STS2-RitsuLib `v0.4.32` update. Beta.96 RitsuLib-only Off loader proof is previous-package startup/loading evidence, and previous beta.93 direct RitsuLib-only AdditiveBatch1 proof remains historical loader/registration evidence only. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
...
- Historical loader evidence exists for beta.19 package startup and for RitsuLib diagnostic Off, CanaryOnly, and AdditiveBatch1 modes. The beta.85 Off/CanaryOnly, beta.86 AdditiveBatch1, beta.87 AdditiveBatch1 `v0.107.0`, beta.88 previous package `v0.107.1`, and beta.90 RitsuLib-only packets remain previous-package or previous package-context loader evidence. Current beta.99 RitsuLib-only Off proof under `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/` reached main menu on `v0.107.1` with exactly `STS2-RitsuLib` and `EZMicroBalance` loaded, audited clean, applied 25/25 Spire Plus patches, logged StS1Events disabled with 0 registration lines, passed Off verifier 21 / 0, and passed packet verification 43 / 0; this is startup/default-Off proof only. Previous beta.96 Off proof remains previous-package startup context. The previous beta.93 package has AdditiveBatch1 proof under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`: it reached main menu on `v0.107.1`, audited clean, applied 25/25 Spire Plus patches, and passed retained log/packet verifiers with 10 event types / 14 registration calls and exact act/shared tuple parity for the previous package only. The earlier beta.86 Steam-client attempt under `.tools/runtime-evidence/v01070-beta86-additive-batch1-20260618-031043/` is diagnostic only because StS1 stayed disabled when the already-running Steam client did not propagate the transient environment. Closing only the direct Golden Idol localization key remains a localization unblocker; it does not prove gameplay or replace verifier reports. Older beta.17, beta.13, `20260524-161744`, 22-field, and `20260523-current` loader smokes remain historical evidence only.
