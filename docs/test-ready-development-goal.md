...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. Current package target remains `publish/SpirePlus-v0.1.0-private-beta.16.zip`. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
...
- The latest Steam-client loader smoke under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` reported `Found 30 SavedSpireFields` with only BaseLib plus Spire Plus loaded, clean log audit, startup completion, game stop, and restored mod isolation for the previous beta.13 package. After the beta.16 Seedbed strength/planting refresh, fresh loader proof is pending again. Older `20260524-161744`, 22-field, and `20260523-current` loader smokes remain historical evidence only.
