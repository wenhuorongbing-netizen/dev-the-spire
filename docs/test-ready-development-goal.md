...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.128.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.34` target. beta.128 still needs game-launch/runtime patch-count and clicked UI proof. Beta.123 clicked UI smoke, beta.99 settings/Off proof, beta.96 RitsuLib-only Off loader proof, and previous beta.93 direct RitsuLib-only AdditiveBatch1 proof remain previous-package or historical loader/registration evidence only. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
