...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.135.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.34` target. Previous beta.128 forced clicked Ancient UI smoke, beta.99 settings/Off proof, beta.96 RitsuLib-only Off loader proof, and previous beta.93 direct RitsuLib-only AdditiveBatch1 proof remain previous-package or historical loader/registration evidence only. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
...
- Final browser GPTimage2 small art generated this pass is the current small-art baseline. No `generic_temporary` or `final_required_before_release` art blockers remain. Event backgrounds are active middle-draft resources. Beta.128 smoke-level Ancient clicked UI proof exists for forced Urda, Morvi, Lotha, and normal Vakuu; live art/readability review for hovers, relic-bar follow-through, and gated Vakuu fight-option remains unresolved.
- Historical loader evidence exists for beta.19 package startup and for RitsuLib diagnostic Off, CanaryOnly, and AdditiveBatch1 modes. The beta.85 Off/CanaryOnly, beta.86 AdditiveBatch1, beta.87 AdditiveBatch1 `v0.107.0`, beta.88 previous package `v0.107.1`, beta.90 RitsuLib-only packets, beta.93 AdditiveBatch1, beta.96 Off, beta.99 settings/Off rows, and beta.123 clicked UI rows remain previous-package or previous package-context evidence. Current beta.135 proof includes package parity, runtime preflight, and source-workspace validation. Latest clicked UI smoke remains previous beta.128 proof under `.tools/runtime-evidence/monkey-stability-20260623-062913/`; it covered Urda, Morvi, Lotha, and normal Vakuu with 4 / 4 iterations, 152/152 default runtime Spire Plus patches applied, and packet verification 1621 / 0. This is previous-package smoke-level clicked UI proof only. It does not prove beta.135 gameplay, save-load, replacement behavior, co-op, or release readiness. Older beta.17, beta.13, `20260524-161744`, 22-field, and `20260523-current` loader smokes remain historical evidence only.
...
Internal broad diagnostics are not a player-facing mod setting. Use `SPIREPLUS_ENABLE_DEBUG_LOGS=1` or legacy `EZMB_ENABLE_DEBUG_LOGS=1` only for focused source/debug sessions; preview-tool diagnostics remain the localized `ShowPreviewDebugLogs` setting.
## Required Reading
...
- relic-bar visibility, hover readability, and gameplay follow-through for Urda, Morvi, Lotha, Vakuu normal, and Vakuu fight; smoke-level clicked Ancient UI screenshots are captured for Urda/Morvi/Lotha/normal Vakuu only;
