...
- The latest Steam-client loader smoke under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` reported `Found 30 SavedSpireFields` with only BaseLib plus Spire Plus loaded, clean log audit, startup completion, game stop, and restored mod isolation for the current beta.13 package. Older `20260524-161744`, 22-field, and `20260523-current` loader smokes remain historical evidence only.
