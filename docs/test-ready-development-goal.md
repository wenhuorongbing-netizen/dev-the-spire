...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.92.zip`; build, publish, package refresh, installed-package check, runtime preflight, and direct RitsuLib-only Off/AdditiveBatch1 loader proof have run, but this only proves package/source-shape loader behavior. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
