...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. Current package target remains `publish/SpirePlus-v0.1.0-private-beta.10.zip`. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
...
- The latest Steam-client loader smoke under `.tools/runtime-evidence/manual-test-handoff-20260524-161744/release/fresh-current-package-loader-smoke` reported `Found 30 SavedSpireFields` with only BaseLib plus Spire Plus loaded and a clean log audit for an earlier package hash. The current Seedbed / Rootblight package changed DLL, PCK, manifest, ZIP, and README hashes, so it needs a fresh loader row. Older 22-field and `20260523-current` loader smokes remain historical evidence only.
