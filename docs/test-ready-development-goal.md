...
Current stop line: Codex should not chase release-ready evidence in this pass. The user will run live/manual testing. This is not a release-ready claim. Current package target is `publish/SpirePlus-v0.1.0-private-beta.85.zip`. The stable technical manifest id remains `EZMicroBalance`; do not edit it in place.
## Current State
...
- Historical loader evidence exists for beta.19 package startup and for RitsuLib diagnostic Off, CanaryOnly, and AdditiveBatch1 modes. The current beta.85 package has fresh `v0.107.0` Off loader proof under `.tools/runtime-evidence/v01070-beta85-current-package-runtime-fix-20260611-0510`; it proves startup and patch application only. Current StS1 CanaryOnly and AdditiveBatch1 enabled-mode proof is still missing; before StS1 gameplay or handoff claims, CanaryOnly must prove 4 event types / 6 registration calls and AdditiveBatch1 must prove 10 event types / 14 registration calls on beta.85 / `v0.107.0`. Closing only the direct Golden Idol localization key remains a localization unblocker; it does not close O25/O33 enabled-mode proof or replace verifier reports. Older beta.17, beta.13, `20260524-161744`, 22-field, and `20260523-current` loader smokes remain historical evidence only.
