...
internal static class AscensionMapService
{
    ...
    private static bool HasA11InsertedColumnRouteChoice(ActMap map)
    {
        try
        {
            var saved = SerializableActMap.FromActMap(map);
            return HasA11InsertedColumnRouteChoice(saved, BuildSerializableLookup(saved));
        }
        catch (Exception ex)
        {
            MainFile.Logger.Warn($"[EZMicroBalance] Ascension A11 route-choice diagnostic failed: {ex.Message}");
            return false;
        }
    }
