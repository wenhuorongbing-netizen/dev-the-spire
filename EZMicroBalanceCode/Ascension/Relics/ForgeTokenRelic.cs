...
internal sealed class ForgeTokenRelic : CustomRelicModel, ILocalizationProvider
{
    ...
    public override List<(string, string)>? Localization => LocManager.Instance.Language == "zhs"
        ? new RelicLoc(
            "铸令",
            "击败[gold]火印精英[/gold]后获得。下一个[gold]休息点[/gold]：[gold]休息[/gold]会随机升级[blue]1[/blue]张可升级的[gold]普通[/gold]或[gold]罕见[/gold]牌；[gold]锻造[/gold]会回复[blue]7[/blue]点生命。若休息时没有目标，改为回复[blue]5[/blue]点生命。特殊休息行动暂不消耗铸令。最多持有[blue]1[/blue]枚。",
