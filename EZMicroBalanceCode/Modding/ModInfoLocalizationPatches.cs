...
internal static class ModInfoLocalizationPatches
{
    ...
    private const string SimplifiedChineseDescription =
        "Spire Plus 是用于私测的《杀戮尖塔 2》单体玩法扩展。启用这一个 Mod，就能测试可悬停查看的先古之民遗物奖励、部分原版遗物调整、A11-A20 测试进阶和预览工具：根芽、根蚀、火印精英、战旗房、首领专属能力、王烙印、水晶球预知和变换预览。奖励可以更强，但代价、触发时机和后续结算必须能读懂。苗床是最清楚的例子：它给 8/12 点格挡，打出时先种下牌堆污染，留下格子截住后续污染，每种下 1 张牌都会给 1 张枯壳。种下不是消耗诅咒，也不是打出或弃牌，不会触发这些联动，也不会删除永久诅咒。根芽被种下后按已处理结算，战后不会生成根蚀 I。根蚀被种下后只在本战冻结，仍按同等级留在主牌组，战后不变好也不恶化。需要 BaseLib。";
