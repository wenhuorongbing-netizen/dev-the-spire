...
| `check-spire-plus-runtime-monkey-packet.ps1` | No-launch verifier for launched runtime monkey evidence folders. It checks `monkey-plan.json`, `monkey-summary.json`, expected iteration folders, `iteration-result.json`, copied `godot.log.after-launch`, `godot-log-audit.json`, retained `sts1-mode-log-check.json`, main-menu/runtime observation telemetry, no process exit, no hung window, no raw `Responding=false` probe samples, no pre-main-menu log-growth timeout, audit/restore/pass flags, expected package version, exact Spire Plus patch-count markers, and probe sample path containment; pass `-FailOnMismatch` only for launched packets, not dry-run plans. |
