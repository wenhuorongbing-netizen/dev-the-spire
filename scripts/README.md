...
| Script | Purpose |
| --- | --- |
| `analyze-spire-plus-runtime-failure.ps1` | No-launch triage analyzer for retained runtime monkey evidence. It reads `monkey-summary.json`, per-iteration `iteration-result.json`, copied `godot.log.after-launch`, and `godot-log-audit.json`, then maps hang signals, command acknowledgement failures, audit signatures, StS1 mode drift, package/API drift, restore failures, and feature-specific text to likely owner areas and next investigation steps. Use after a failed launched monkey packet; it does not prove or rerun gameplay. |
...
| `check-local-godot-source-workspace.ps1` | No-launch checker for the local recovered Godot source workspace. It verifies `source code\project.godot`, source versus installed-game `release_info.json`, Godot 4.5.1 Mono executable presence, GDRE export-log summary, ignored/untracked source/tooling roots, installed RitsuLib manifest/variant DLL compatibility and hash, package manifest parity, and records that the RitsuLib viewer is a log viewer rather than an unpacker or monkey runner. When run with `-OutFile`, its JSON report includes `OpenProjectCommand` and `EvidenceUsePolicy` so future agents can open the local project deliberately while using only the owner-authorized local install, keeping original game source/resources untracked, rejecting third-party dumps, and not treating source inspection as runtime proof. Stale recovered source and imperfect GDRE export are warnings by default; add `-RequireCurrentSourceSnapshot` or `-RequireCleanGdreExport` with `-FailOnMismatch` only when those must block the task. |
