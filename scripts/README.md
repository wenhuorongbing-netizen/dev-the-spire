...
| `spire-plus-package-evidence.ps1` | Shared helper for release-evidence scripts. It derives the current `publish\SpirePlus-v...` zip and versioned package artifact paths from `EZMicroBalance.json` so evidence collectors do not duplicate the package version in multiple places. Dot-source it from other scripts; do not run it directly. |
...
The verifier also computes the SHA256 of `publish\SpirePlus-v0.1.0-private-beta.31.zip` by default; use `-PackagePath` only when intentionally auditing a different zip. `-WritePassMarker` writes `release-evidence-verifier-pass.json` only after all rows pass or are accepted deferrals.
