...
| Script | Purpose |
| --- | --- |
| `analyze-spire-plus-runtime-failure.ps1` | No-launch triage analyzer for retained runtime monkey evidence. It reads `monkey-summary.json`, per-iteration `iteration-result.json`, copied `godot.log.after-launch`, sliced `godot.log.current-iteration`, and `godot-log-audit.json`, then maps hang signals, command acknowledgement failures, trusted audit signatures, StS1 mode drift, package/API drift, restore failures, and feature-specific text to likely owner areas and next investigation steps. Audit signatures only route ownership after the audit `Path`/`Length`/`Sha256` bind to `godot.log.current-iteration` and an analyzer-side audit recomputation matches. Use after a failed launched monkey packet; it does not prove or rerun gameplay. |
...
| `check-spire-plus-autoslay-packet.ps1` | No-launch verifier for future game-native AutoSlay batch evidence folders. It checks `autoslay-plan.json`, `autoslay-summary.json`, retained `check-local-godot-source-workspace.ps1` output and SHA256, source-workspace policy flags, `GameNativeAutoSlay` runner identity, the launcher/mod-hook `AutoSlayer.Start(seed, logFile)` invocation record, required package/game/Ritsu target switches, per-seed AutoSlay log paths and SHA256 hashes, start/completion/failure markers, retained in-evidence `godot.log.current-iteration` AutoSlay markers, and event-room traversal markers such as `Entering Event room` plus `Selecting event option:`. Use it only after a launched AutoSlay-backed packet exists; it does not launch the game or make source-only AutoSlay checks runtime proof. |
