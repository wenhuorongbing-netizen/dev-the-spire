...
| `check-installed-ezmb-package.ps1` | Check installed `EZMicroBalance` artifact hashes and the copied game-root `SpirePlus` ZIP against the current handoff docs. Defaults are Windows-oriented; pass `-ModDirectory` or `-GameRootZipPath` explicitly for non-default paths, or `-SkipGameRootZipCheck` when checking only an installed mod folder. |
| `check-installed-ezmb-package.sh` | macOS/Linux equivalent for checking installed `EZMicroBalance` artifact hashes with `shasum -a 256` or `sha256sum`. Pass the installed mod directory as the first argument. |
...
| `collect-future-peek-evidence.ps1` | Compatibility wrapper for older local commands. It forwards to `collect-preview-tools-evidence.ps1`; current work should use the preview-tools name because these tools now ship inside `EZMicroBalance`. |
...
The generated handoff includes `release/`, `ancient-ui/`, `vakuu/`, `preview-tools/`, and `coop/`. It does not launch the game and does not mark rows as passed.
