...
[void]$builder.AppendLine('# RitsuLib Patch Inventory')
[void]$builder.AppendLine()
[void]$builder.AppendLine("Generated: $generatedAt")
[void]$builder.AppendLine()
[void]$builder.AppendLine('Purpose: keep every RitsuLib ModPatcher registration visible, owned, and batch-labeled while retaining raw HarmonyPatch counts as a regression signal. Regenerate after adding, moving, or deleting patch classes.')
[void]$builder.AppendLine()
...
[void]$builder.AppendLine()
[void]$builder.AppendLine('Risk labels apply only to raw class-level `[HarmonyPatch]` declarations. Current migrated `IPatchMethod` classes are tracked by batch and explicit RitsuLib registry ownership below.')
...
[void]$builder.AppendLine('`SpirePlusMigratedPatchRegistry.RegisterAll(...)`. `RitsuLibBootstrap` applies them through')
[void]$builder.AppendLine('`RitsuLibFramework.ApplyRequiredPatcher(...)`; they are NOT picked up by raw `Harmony.PatchAll()`.')
[void]$builder.AppendLine()
