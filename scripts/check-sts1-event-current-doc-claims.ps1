...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.107 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, and clicked Ancient UI smoke passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD/worktree validation refreshed after the beta.107 RitsuLib default-key localization and clicked UI pass; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'ritsu_monthly_routes_batch4c_to_proposal' -Text $ritsuMonthlyDevSpec -Needle '`batch-4c-candidates.md` for the proposal-only patch list.'
