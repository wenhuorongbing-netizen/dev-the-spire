...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-21 beta.99 handoff summary; older May notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'agents_current_dependency_ritsulib_only' -Text $agents -Needle 'Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.32` installed'
Add-ContainsCheck -Name 'agents_current_off_not_live_ready' -Text $agents -Needle 'Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, current beta.99 enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff.'
...
Add-ContainsCheck -Name 'goal_migration_20260622_ritsu_latest_recheck' -Text $goalMigrationDoc -Needle '2026-06-22: NuGet flat-container and `dotnet list package --outdated --include-transitive` show `STS2.RitsuLib` `0.4.32` as the latest package.'
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, co-op, current beta.99 enabled-mode registration/gameplay proof, independent QA rerun, and tester-package handoff decisions remain pending.'
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'Current beta.99 RitsuLib Mod Settings clicked UI proof is captured under `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`: the normal Steam-client session temporarily isolated every mod except `STS2-RitsuLib` and `EZMicroBalance`, opened Settings -> `Mod Settings (RitsuLib)`, showed only `RitsuLib` and `Spire Plus`, rendered the Spire Plus Migration Status / `STS2-RitsuLib >= 0.4.32` runtime dependency / evidence-boundary / Preview Tools page, retained same-session `godot.log`, audited clean, and passed StS1 Off runtime shape verification 21 / 0.'
...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'Current beta.99 RitsuLib-only Off proof reached main menu with exactly `STS2-RitsuLib` and `EZMicroBalance` loaded, clean audit, 25/25 Spire Plus patches, StS1Events disabled with 0 registration lines, Off verifier 21 / 0, and packet verifier 43 / 0; previous beta.93 AdditiveBatch1 registered 10 event types through 14 calls with verifier 31 / 0 and packet 61 / 0 for the previous package only.'
Add-ContainsCheck -Name 'root_readme_manual_rows_pending' -Text $rootReadme -Needle 'Manual feature verification, gameplay, clicked Ancient UI, save-load, preview-tools live behavior, Vakuu, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.99` on Slay'
...
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.99`'
Add-ContainsCheck -Name 'beta_compat_ritsulib_only_dependency' -Text $betaCompatibility -Needle 'only `STS2-RitsuLib >= 0.4.32` as the shared runtime dependency'
...
Add-ContainsCheck -Name 'beta_compat_loader_settings_nonclaim' -Text $betaCompatibility -Needle 'Treat loader and settings evidence as scoped proof only: enabled-mode proof, gameplay, save-load, replacement, multiplayer, independent QA, package handoff, and release-ready compatibility proof remain pending.'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta96_settings_boundary' -Text $docsReadme -Needle 'beta.99 settings-page proof is current UI visibility evidence'
...
Add-ContainsCheck -Name 'migration_doc_beta96_boundary' -Text $migrationDoc -Needle 'Spire Plus is on the beta.99 RitsuLib-only target:'
...
Add-ContainsCheck -Name 'goal_migration_beta85_package_runtime_clean' -Text $goalMigrationDoc -Needle '`EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.32`'
...
Add-ContainsCheck -Name 'goal_event_beta99_current_loader_boundary' -Text $goalEventDoc -Needle 'As of 2026-06-21, current package truth is beta.99 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.32`. Beta.99 clicked settings UI proof is captured under `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`; beta.99 direct Off loader proof is captured under `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/`'
...
Add-ContainsCheck -Name 'goal_event_latest_beta94_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.99 governance checkpoint after the RitsuLib settings-page I18N resource migration: static/source docs must be recaptured for the current package before any handoff.'
...
Add-ContainsCheck -Name 'goal_event_source_workspace_checkpoint_beta93' -Text $goalEventDoc -Needle 'source workspace: 57 checks / 0 mismatches against installed v0.107.1, Spire Plus v0.1.0-private-beta.99, and STS2-RitsuLib 0.4.32 direct NuGet runtime layout'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.99.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the settings-page I18N resource migration plus STS2-RitsuLib `v0.4.32` update.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'beta.99 build, publish/package refresh, installed-package parity, source-workspace check, Mod Settings UI proof, and direct Off loader proof are the current package target.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.99`'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.99 enabled-mode'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.99'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.99`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.99 package parity and clicked RitsuLib Mod Settings'
...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.99 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, RitsuLib settings UI proof, and direct Off loader proof passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD validation refreshed after the beta.99 RitsuLib-only settings page I18N pass; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.99 package and STS2-RitsuLib `v0.4.32` direct NuGet runtime are installed and recapture current-package enabled-mode proof'
...
Add-ContainsCheck -Name 'sts1_goal_compile_package_current_split' -Text $sts1FeatureGoal -Needle 'Events must compile against the current repository package reference: `STS2.RitsuLib` `0.4.32`; Spire Plus must not require previous package.'
Add-ContainsCheck -Name 'sts1_goal_runtime_ritsu_variant_split' -Text $sts1FeatureGoal -Needle 'Current runtime reproof targets the installed official `STS2-RitsuLib` `v0.4.32` direct NuGet runtime layout.'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o22_ritsulib_requirement' -Text $gateLedgerCheckerScript -Needle "GateId = 'O22'; Field = 'requirement'; Needle = 'STS2-RitsuLib v0.4.32 direct NuGet runtime layout installed'"
...
Add-ContainsCheck -Name 'current_validation_20260622_ritsu_latest' -Text $currentValidation -Needle 'NuGet flat-container `https://api.nuget.org/v3-flatcontainer/sts2.ritsulib/index.json` still reports `0.4.32` as the latest `STS2.RitsuLib` package across 163 listed versions.'
...
Add-ContainsCheck -Name 'release_evidence_status_beta99_off_nonclaim' -Text $releaseEvidenceStatus -Needle 'beta.99 Mod Settings page proof is current UI visibility evidence and beta.99 direct Off loader proof is current startup/default-Off evidence. Gameplay, clicked Ancient UI, save-load, preview-tools live behavior, current enabled-mode proof, Vakuu, co-op, and full release-evidence packaging rows remain pending.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for beta.99 startup/default-Off loader; gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.99 RitsuLib-only package refresh: 2026-06-21; previous beta.96 Off loader proof and previous beta.93 AdditiveBatch1 evidence remain historical loader/registration proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.99 RitsuLib-only settings-page I18N package refresh: 2026-06-21; previous beta.96 Off loader proof and previous beta.93 AdditiveBatch1 evidence remain historical loader/registration proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_beta85_loader_nonclaim' -Text $testReadyCompletionAudit -Needle 'These hashes and earlier loader proof do not close beta.99 loader proof, live gameplay, save-load, death/failure path, or co-op verification.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.99.zip'
