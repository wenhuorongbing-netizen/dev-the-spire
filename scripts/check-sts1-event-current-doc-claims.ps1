...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_982' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*982 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_requires_event_markers' -Text $runtimeMonkeyDocs -Needle 'lack event-room traversal markers'
...
Add-ContainsCheck -Name 'status_board_pause_safe_static_v20_autoslay_982' -Text $statusBoard -Needle 'the runtime-monkey AutoSlay boundary/source-contract and packet-verifier follow-up later raised the active current-doc guard to 982 / 0 and static-file hygiene to 12 / 0 while preserving the same static-only boundary'
