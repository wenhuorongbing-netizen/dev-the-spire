...
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'The beta.85 Off and CanaryOnly smokes remain previous-package loader context; the beta.85 AdditiveBatch1 mismatch remains root-cause history for stale package/source shape.'
...
Add-ContainsCheck -Name 'goal_event_latest_pause_safe_static_checkpoint_20260617' -Text $goalEventDoc -Needle 'Latest pause-safe static checkpoint after this beta.86 doc-alignment pass: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 962 checks / 0 mismatches after the v20 subagent coverage'
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_962' -Text $goalEventDoc -Needle 'current-doc-claims: 962 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_962' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*962 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'dev_environment_sts1_enabled_smokes_current_split' -Text $devEnvironment -Needle 'Current beta.86 AdditiveBatch1 proof passed retained verifiers with 10 event types / 14 registration calls and exact tuple parity.'
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'Fresh beta.86 AdditiveBatch1 direct proof under `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/` reached main menu'
...
Add-ContainsCheck -Name 'ritsu_integration_beta86_additive_nonclaim' -Text $ritsuIntegrationDoc -Needle 'Current beta.86 AdditiveBatch1 proof is `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/`'
...
Add-ContainsCheck -Name 'v20_overlay_o81_pause_boundary' -Text $v20FinalGateOverlay -Needle 'O81,O76-O84,no unsupported commit/push,current-pending,Coordination pause forbids commit/push from this thread; package/runtime baseline remains eaaeb5a1 and latest pushed governance/test follow-up is d2ff20f5'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o0_beta86_baseline_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O0'; Field = 'evidence'; Needle = 'PROJECT_STATE.md records beta.86 package/runtime baseline at eaaeb5a1 and newer pushed governance/test follow-up through d2ff20f5'"
...
Add-ContainsCheck -Name 'hard_stop_v20_current_head_d2ff20f5' -Text $hardStopV20 -Needle '| Current HEAD observed in this thread | `d2ff20f5 (HEAD -> main, origin/main, origin/HEAD) Split release evidence template guards` |'
Add-ContainsCheck -Name 'hard_stop_v20_current_dirty_exact_recapture' -Text $hardStopV20 -Needle '| Current worktree observed in this thread | `git status --short` after the `d2ff20f5` recapture reported dirty event docs/guards plus separate `docs/features/ritsulib-migration/next-overnight-run.md`, `docs/goals/migration.md`, `docs/review.md`, `docs/worktree-cleanup-audit.md`, `docs/testing/runtime-monkey-stability.md`, `scripts/analyze-spire-plus-runtime-failure.ps1`, `scripts/check-spire-plus-runtime-monkey-packet.ps1`, `tests/EZMicroBalance.Tests/DocumentationCompactnessGuardTests.cs`, `tests/EZMicroBalance.Tests/EngineeringGovernanceGuardTests.cs`, `tests/EZMicroBalance.Tests/RuntimeMonkeyStabilityGuardTests.cs`, and untracked `tests/EZMicroBalance.Tests/DocumentationCompactnessGuardTests.ReleaseJournal.cs` plus `tests/EZMicroBalance.Tests/EngineeringGovernanceGuardTests.FeatureModuleMetadata.cs`; treat dirty files as separate scope requiring exact recapture before commit or handoff. |'
...
Add-ContainsCheck -Name 'release_evidence_status_beta85_off_nonclaim' -Text $releaseEvidenceStatus -Needle 'the current beta.86 `v0.107.0` AdditiveBatch1 direct loader smoke audited clean for loader/registration proof, while gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, and full release-evidence packaging rows remain pending.'
