...
Add-ContainsCheck -Name 'goal_event_latest_beta91_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.91 governance checkpoint after the gate-ledger/current-doc cleanup: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 1321 checks / 0 mismatches'
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_1321' -Text $goalEventDoc -Needle 'current-doc-claims: 1321 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_1321' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*1321 checks / 0 mismatches'
