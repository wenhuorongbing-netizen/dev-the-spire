...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-22 beta.114 handoff summary; older May/June package notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because beta.114 clicked UI, gameplay, save-load, co-op, current enabled-mode registration/gameplay proof, independent QA rerun, and tester-package handoff decisions remain pending.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle 'Installed beta.114 package artifacts are current; beta.108 clicked Ancient UI smoke and beta.99 or older rows are previous-package evidence.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.114` on Slay'
...
Add-ContainsCheck -Name 'doc_restructure_spec_beta105_ritsulib_boundary' -Text $docRestructureSpec -Needle 'The RitsuLib integration lane is current for beta.114: compile package,'
...
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.114`'
...
Add-ContainsCheck -Name 'migration_doc_current_boundary_stub' -Text $migrationDoc -Needle 'Current boundary: Spire Plus is RitsuLib-only for beta.114'
...
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.114 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.34`.'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.114.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.34` target. Smoke-level clicked Ancient UI proof remains the previous beta.108 packet.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'beta.114 package parity, runtime preflight 28 / 0, and source-workspace validation 58 / 0 are current; previous beta.108 clicked UI smoke packet verification passed 1621 / 0 for the previous 64-patch source state.'
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.114 enabled-mode gameplay proof.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.114 build, publish/package refresh, installed-package parity, runtime preflight, and source-workspace check are the current package target; beta.108 clicked Ancient UI smoke is previous-package smoke-level UI context.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.114`'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.114 enabled-mode'
...
Add-ContainsCheck -Name 'historical_revision_boundaries_revision_n_current_truth' -Text $historicalRevisionBoundaries -Needle 'Current migration truth is beta.114 RitsuLib-only'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.114'
Add-ContainsCheck -Name 'runtime_monkey_docs_runtime_current_package_target' -Text $runtimeMonkeyDocs -Needle '.\scripts\check-spire-plus-runtime-monkey-packet.ps1 `
  ...
  -ExpectedPackageVersion v0.1.0-private-beta.114'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.114`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.114 package parity, runtime preflight,'
...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.114 package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace checks passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.114 package and STS2-RitsuLib `v0.4.34` direct NuGet runtime are installed and recapture current-package enabled-mode proof.'
...
Add-ContainsCheck -Name 'ritsu_batch4c_requires_69_runtime_recapture' -Text $ritsuBatch4cCandidates -Needle 'Before citing this source migration as current runtime coverage, rebuild/package and capture a fresh loader packet that proves the current 78 migrated Spire Plus patch classes apply in the installed game.'
...
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.114` on Slay the Spire 2 `v0.107.1`.'
...
Add-ContainsCheck -Name 'current_validation_smoke_boundary' -Text $currentValidation -Needle 'This is previous-package smoke-level clicked Ancient UI migration proof; recapture it for beta.114 before tester handoff or release claims.'
...
Add-ContainsCheck -Name 'handoff_sts1_enabled_smokes_current_split' -Text $privateBetaHandoff -Needle 'Retained AdditiveBatch1 enabled-mode proof remains previous-package loader/registration evidence only and does not prove beta.114 gameplay.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.114 RitsuLib-only package refresh: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.114 RitsuLib-only package refresh: 2026-06-22; previous beta.99, beta.96, and beta.93 evidence remains historical loader/registration/settings proof only.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.114.zip'
