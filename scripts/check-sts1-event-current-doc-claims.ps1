...
Add-ContainsCheck -Name 'v20_overlay_o81_pause_boundary' -Text $v20FinalGateOverlay -Needle 'O81,O76-O84,no unsupported commit/push,current-pending,Coordination pause forbids commit/push from this thread; package/runtime baseline remains eaaeb5a1 and validated governance/test follow-up baseline is 254a6f41'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o0_beta86_baseline_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O0'; Field = 'evidence'; Needle = 'PROJECT_STATE.md records beta.86 package/runtime baseline at eaaeb5a1 and newer pushed governance/test follow-up through 254a6f41'"
...
Add-ContainsCheck -Name 'hard_stop_v20_current_head_254a6f41' -Text $hardStopV20 -Needle '| Current HEAD observed in this thread | `254a6f41 (HEAD -> main, origin/main, origin/HEAD) Refresh migration governance validation` |'
Add-ContainsCheck -Name 'hard_stop_v20_current_dirty_exact_recapture' -Text $hardStopV20 -Needle '| Current worktree observed in this thread | `git status --short` after the `254a6f41` recapture reported a clean pushed baseline; treat any later dirty files as separate scope requiring exact recapture before commit or handoff. |'
