...
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_1214' -Text $goalEventDoc -Needle 'current-doc-claims: 1214 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_1214' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*1214 checks / 0 mismatches'
...
Add-RegexCheck -Name 'dev_environment_sts1_enabled_smokes_current_split' -Text $devEnvironment -Pattern 'retained `v0\.107\.0` beta\.85/beta\.86/beta\.87 direct smokes remain previous-package/game-version loader context\.\s+The first `v0\.107\.1` beta\.87 recapture at .*? failed clean-loader proof because BaseLib `v3\.2\.1` logged 2 patch failures against `v0\.107\.1`\.\s+The beta\.88 BaseLib `v3\.3\.0` recapture at .*? reached main menu'
