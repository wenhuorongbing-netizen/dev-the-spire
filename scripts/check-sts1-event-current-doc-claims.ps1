...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-22 beta.108 handoff summary; older May/June package notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'agents_20260622_ritsu_latest_recheck' -Text $agents -Needle 'The 2026-06-22 NuGet flat-container and `dotnet list package --outdated --include-transitive` recheck found no newer `STS2.RitsuLib` package'
Add-ContainsCheck -Name 'goal_migration_20260622_ritsu_latest_recheck' -Text $goalMigrationDoc -Needle '2026-06-22: NuGet flat-container and `dotnet list package --outdated --include-transitive` show `STS2.RitsuLib` `0.4.33` as the latest package.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle 'beta.107 clicked Ancient UI smoke and beta.99 or older proof rows are previous-package context.'
...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'previous beta.107 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`; 4 / 4 `AncientUiSmoke` iterations passed'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.108` on Slay'
...
Add-ContainsCheck -Name 'doc_restructure_spec_beta105_ritsulib_boundary' -Text $docRestructureSpec -Needle 'The RitsuLib integration lane is current for beta.108: compile package,'
...
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.108`'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta96_settings_boundary' -Text $docsReadme -Needle 'beta.108 is the current package/source proof; beta.107 is previous-package clicked-Ancient-UI proof'
...
Add-ContainsCheck -Name 'migration_doc_current_boundary_stub' -Text $migrationDoc -Needle 'Current boundary: Spire Plus is RitsuLib-only for beta.108'
...
Add-ContainsCheck -Name 'goal_migration_beta85_package_runtime_clean' -Text $goalMigrationDoc -Needle '`EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.33`'
Add-ContainsCheck -Name 'goal_migration_beta87_current_override' -Text $goalMigrationDoc -Needle 'Beta.107 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`'
...
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.108 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.33`.'
Add-ContainsCheck -Name 'goal_event_ritsulib_only_dependency' -Text $goalEventDoc -Needle 'Spire Plus is RitsuLib-only. `EZMicroBalance.csproj` references `STS2.RitsuLib` `0.4.33`, and `EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.33` as the runtime dependency.'
...
Add-ContainsCheck -Name 'goal_event_clicked_ui_smoke_boundary' -Text $goalEventDoc -Needle 'Previous beta.107 clicked Ancient UI smoke is retained at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`: 4 / 4 Urda, Morvi, Lotha, and normal Vakuu iterations'
...
Add-ContainsCheck -Name 'goal_event_previous_packets_context_only' -Text $goalEventDoc -Needle 'Previous beta.99 settings/Off proof, beta.96 Off proof, beta.93 AdditiveBatch1 proof, beta.88 previous-package proof, and beta.85-beta.90 rows are retained previous-package or previous-game-version context only.'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.108.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.33` target.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'Beta.108 package parity, runtime preflight 28 / 0, and source-workspace validation 58 / 0 are current; previous beta.107 clicked UI smoke packet verification passed 1620 / 0 for the earlier 46-patch source state.'
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.108 enabled-mode gameplay proof.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.108 build, publish/package refresh, installed-package parity, runtime preflight, and source-workspace check are the current package target; beta.107 clicked Ancient UI smoke is previous-package context.'
Add-ContainsCheck -Name 'toreview_direct_localization_nonproof' -Text $toReview -Needle 'Previous beta.107 clicked UI proof is `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/` with 4 / 4 iterations, clean audit, StS1 Off verifier pass, exact game/Ritsu/package markers, and packet verifier 1620 / 0.'
...
Add-ContainsCheck -Name 'ritsu_integration_beta99_latest_recheck' -Text $ritsuIntegrationDoc -Needle 'The NuGet flat-container index reports `STS2.RitsuLib` latest `0.4.33`'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.108`'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.108 enabled-mode'
...
Add-ContainsCheck -Name 'historical_revision_boundaries_revision_n_current_truth' -Text $historicalRevisionBoundaries -Needle 'Current migration truth is beta.108 RitsuLib-only'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.108'
Add-ContainsCheck -Name 'runtime_monkey_docs_runtime_current_package_target' -Text $runtimeMonkeyDocs -Needle '.\scripts\check-spire-plus-runtime-monkey-packet.ps1 `
  ...
  -ExpectedPackageVersion v0.1.0-private-beta.108'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.108`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.108 package parity, runtime preflight, and'
...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.108 package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace checks passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current source now has 64 migrated RitsuLib patch classes after the source-only Batch 4c localization and visual-hover UI migrations. The retained beta.107 smoke applied 46 migrated patch classes and must not be stretched to cover the new source state.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.108 package and STS2-RitsuLib `v0.4.33` direct NuGet runtime are installed and recapture current-package enabled-mode proof.'
...
Add-ContainsCheck -Name 'sts1_goal_compile_package_current_split' -Text $sts1FeatureGoal -Needle 'Events must compile against the current repository package reference: `STS2.RitsuLib` `0.4.33`; Spire Plus must not require previous package.'
Add-ContainsCheck -Name 'sts1_goal_runtime_ritsu_variant_split' -Text $sts1FeatureGoal -Needle 'Current runtime reproof targets the installed official `STS2-RitsuLib` `v0.4.33` direct NuGet runtime layout.'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o22_ritsulib_requirement' -Text $gateLedgerCheckerScript -Needle "GateId = 'O22'; Field = 'requirement'; Needle = 'STS2-RitsuLib v0.4.33 direct NuGet runtime layout installed'"
...
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.108` on Slay the Spire 2 `v0.107.1`.'
Add-ContainsCheck -Name 'current_validation_ritsu_only_dependency' -Text $currentValidation -Needle '`EZMicroBalance.csproj` references `STS2.RitsuLib` `0.4.33`; `EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.33` as the runtime dependency.'
...
Add-ContainsCheck -Name 'current_validation_20260622_ritsu_latest' -Text $currentValidation -Needle 'NuGet flat-container `https://api.nuget.org/v3-flatcontainer/sts2.ritsulib/index.json` still reports `0.4.33` as the latest `STS2.RitsuLib` package across 164 listed versions.'
...
Add-ContainsCheck -Name 'current_validation_clicked_ui_smoke_current' -Text $currentValidation -Needle 'Previous beta.107 clicked Ancient UI smoke proof passed at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`: 4 / 4 `AncientUiSmoke` iterations for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`'
Add-ContainsCheck -Name 'current_validation_smoke_boundary' -Text $currentValidation -Needle 'This closes smoke-level clicked Ancient UI migration proof only for the previous beta.107 package.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for previous beta.107 startup/UI smoke; beta.108 live recapture pending; gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.108 RitsuLib-only package refresh: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.108 RitsuLib-only package refresh: 2026-06-22; previous beta.99, beta.96, and beta.93 evidence remains historical loader/registration/settings proof only.'
...
Add-ContainsCheck -Name 'test_ready_completion_audit_manual_rows_pending' -Text $testReadyCompletionAudit -Needle 'Live gameplay, save-load, death/failure-path, and co-op remain pending; beta.107 clicked Ancient UI smoke is recorded separately.'
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.108.zip'
...
Add-AutoSlayProofCommandSwitchValuesCheck -Name 'autoslay_packet_proof_commands_pin_current_target_values' -Paths $autoSlayProofCommandFiles -SwitchValues @{
    ...
    ExpectedRitsuLibVersion = '0.4.33'
