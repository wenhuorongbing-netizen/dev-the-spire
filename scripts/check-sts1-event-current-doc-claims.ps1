...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-22 beta.107 handoff summary; older May/June package notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle 'beta.99 and older proof rows are previous-package context.'
...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'current beta.107 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`; 4 / 4 `AncientUiSmoke` iterations passed'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.107` on Slay'
...
Add-ContainsCheck -Name 'doc_restructure_spec_beta105_ritsulib_boundary' -Text $docRestructureSpec -Needle 'The RitsuLib integration lane is current for beta.107: compile package,'
...
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.107`'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta96_settings_boundary' -Text $docsReadme -Needle 'beta.107 is the current package/source/clicked-Ancient-UI proof'
...
Add-ContainsCheck -Name 'migration_doc_current_boundary_stub' -Text $migrationDoc -Needle 'Current boundary: Spire Plus is RitsuLib-only for beta.107'
...
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.107 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.33`.'
...
Add-ContainsCheck -Name 'goal_event_clicked_ui_smoke_boundary' -Text $goalEventDoc -Needle 'Beta.107 clicked Ancient UI smoke is current at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`: 4 / 4 Urda, Morvi, Lotha, and normal Vakuu iterations'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.107.zip`; build, publish, package refresh, installed-package check, runtime preflight, source-workspace check, and smoke-level clicked Ancient UI proof have run for the STS2-RitsuLib `v0.4.33` target.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'Beta.107 package parity, runtime preflight 28 / 0, source-workspace validation 58 / 0, and clicked UI smoke are current; packet verification passed 1620 / 0.'
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.107 enabled-mode gameplay proof.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.107 build, publish/package refresh, installed-package parity, runtime preflight, source-workspace check, and clicked Ancient UI smoke are the current package target.'
Add-ContainsCheck -Name 'toreview_direct_localization_nonproof' -Text $toReview -Needle 'Current beta.107 clicked UI proof is `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/` with 4 / 4 iterations, clean audit, StS1 Off verifier pass, exact game/Ritsu/package markers, and packet verifier 1620 / 0.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.107`'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.107'
Add-ContainsCheck -Name 'runtime_monkey_docs_runtime_current_package_target' -Text $runtimeMonkeyDocs -Needle '.\scripts\check-spire-plus-runtime-monkey-packet.ps1 `
  ...
  -ExpectedPackageVersion v0.1.0-private-beta.107'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.107`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.107 package parity, runtime preflight, source-workspace'
...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.107 package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, and clicked Ancient UI smoke passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current source now has 52 migrated RitsuLib patch classes after the source-only Batch 4c localization migration. The retained beta.107 smoke applied 46 migrated patch classes and must not be stretched to cover the new source state.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.107 package and STS2-RitsuLib `v0.4.33` direct NuGet runtime are installed and recapture current-package enabled-mode proof.'
...
Add-ContainsCheck -Name 'ritsu_batch4c_requires_52_runtime_recapture' -Text $ritsuBatch4cCandidates -Needle 'Before citing this source migration as current runtime coverage, rebuild/package and capture a fresh loader packet that proves the 52 migrated Spire Plus patch classes apply in the installed game.'
...
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.107` on Slay the Spire 2 `v0.107.1`.'
...
Add-ContainsCheck -Name 'current_validation_clicked_ui_smoke_current' -Text $currentValidation -Needle 'Clicked Ancient UI smoke proof passed at `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`: 4 / 4 `AncientUiSmoke` iterations for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`'
Add-ContainsCheck -Name 'current_validation_smoke_boundary' -Text $currentValidation -Needle 'This closes smoke-level clicked Ancient UI migration proof only.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for beta.107 startup/UI smoke; gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.107 RitsuLib-only package refresh and clicked UI smoke: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.107 RitsuLib-only package refresh and clicked UI smoke: 2026-06-22; previous beta.99, beta.96, and beta.93 evidence remains historical loader/registration/settings proof only.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.107.zip'
