...
Add-ContainsCheck -Name 'v20_overlay_o81_pause_boundary' -Text $v20FinalGateOverlay -Needle 'O81,O76-O84,no unsupported commit/push,current-pending,Coordination pause forbids commit/push from this thread; package/runtime baseline remains eaaeb5a1 and latest pushed governance/test follow-up is 6d888ac8'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o0_beta86_baseline_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O0'; Field = 'evidence'; Needle = 'PROJECT_STATE.md records beta.86 package/runtime baseline at eaaeb5a1 and newer pushed governance/test follow-up through 6d888ac8'"
...
Add-ContainsCheck -Name 'hard_stop_v20_current_head_6d888ac8' -Text $hardStopV20 -Needle '| Current HEAD observed in this thread | `6d888ac8 (HEAD -> main, origin/main, origin/HEAD) Harden runtime validation ownership guards` |'
Add-ContainsCheck -Name 'hard_stop_v20_current_dirty_exact_recapture' -Text $hardStopV20 -Needle '| Current worktree observed in this thread | `git status -sb` before this doc/guard alignment reported dirty `docs/review.md`, `docs/worktree-cleanup-audit.md`, `tests/EZMicroBalance.Tests/ReleaseEvidenceGateTests.cs`, `tests/EZMicroBalance.Tests/RuntimeMonkeyStabilityGuardTests.cs`, and untracked `tests/EZMicroBalance.Tests/ReleaseEvidenceGateTests.ManualEvidenceTemplates.cs`; treat those pre-existing dirty files as separate scope requiring exact recapture before commit or handoff. |'
