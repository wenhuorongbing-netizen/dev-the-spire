...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because beta.95 loader, gameplay, UI, save-load, co-op, independent QA rerun, and tester-package handoff decisions remain pending.'
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'Current installed game is `v0.107.1`. Current beta.95 package parity is the active package evidence. Previous beta.93 RitsuLib-only Off and AdditiveBatch1 proof is previous-package loader/registration evidence. Older beta.85/beta.86/beta.87/beta.88/beta.90 rows are previous-package or previous-game-version context only. Gameplay, clicked UI, save-load, replacement, multiplayer, QA, and handoff claims still need separate evidence.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.95` on Slay'
...
Add-ContainsCheck -Name 'migration_doc_current_off_canary_loader_only' -Text $migrationDoc -Needle 'The retained beta.85-beta.90 proof rows remain previous-context evidence; previous `v0.107.1` beta.93 proof is clean at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` for loader/patch application and AdditiveBatch1 registration shape.'
...
Add-ContainsCheck -Name 'migration_doc_remaining_runtime_blockers' -Text $migrationDoc -Needle 'Gameplay, Mod Settings UI page refresh, event screenshots, save-load, image/render, replacement functional proof, co-op/fail-closed proof, independent QA, beta.95 loader proof, and tester-package handoff remain pending; handoff must recapture HEAD and worktree status after any later edits.'
...
Add-ContainsCheck -Name 'goal_event_beta93_previous_loader_boundary' -Text $goalEventDoc -Needle 'previous loader truth is beta.93 on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus `v0.1.0-private-beta.93` loaded, only STS2-RitsuLib as the shared runtime dependency, 25/25 Spire Plus patches applied, clean audits, Off packet verifier 43 / 0, AdditiveBatch1 enabled-mode verifier 31 / 0, and AdditiveBatch1 packet verifier 61 / 0. Current beta.95 loader proof is pending.'
...
Add-ContainsCheck -Name 'goal_event_latest_beta94_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.95 governance checkpoint after the gate-ledger/current-doc cleanup: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 1331 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.95.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run'
...
Add-ContainsCheck -Name 'ritsu_monthly_current_off_nonclaim' -Text $ritsuMonthlyDevSpec -Needle 'Previous beta.93 Off loader smoke is clean with exactly STS2-RitsuLib and Spire Plus loaded.'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.95'
...
Add-ContainsCheck -Name 'next_overnight_beta94_totals' -Text $nextOvernight -Needle 'Latest beta.95 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace check passed for the current dependency target; previous beta.93 RitsuLib-only Off/AdditiveBatch1 packet verification remains historical loader/registration evidence only.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD validation refreshed after the beta.95 RitsuLib-only settings UI pass and runtime packet-checker hardening; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.95 package and STS2-RitsuLib `v0.4.31` / `lib\0.107.1` runtime are installed and recapture current-package enabled-mode proof'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.95 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.95 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.95.zip'
