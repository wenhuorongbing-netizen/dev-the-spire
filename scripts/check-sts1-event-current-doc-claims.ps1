...
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_1003' -Text $goalEventDoc -Needle 'current-doc-claims: 1003 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_1003' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*1003 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'scripts_readme_autoslay_packet_requires_ordered_dual_log_event_sequence' -Text $scriptsReadme -Needle 'ordered Ancient event-room traversal markers such as `Entering Event room`, `Detected Ancient event, clicking through dialogue`, and `Selecting event option:` in both the sidecar and current-iteration logs'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_requires_event_markers' -Text $runtimeMonkeyDocs -Needle '`Detected Ancient event, clicking through dialogue`, and'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_requires_ancient_id' -Text $runtimeMonkeyDocs -Needle '`EventKind: Ancient` / `AncientId`'
...
Add-ContainsCheck -Name 'status_board_pause_safe_static_v20_autoslay_1003' -Text $statusBoard -Needle 'the runtime-monkey AutoSlay boundary/source-contract, packet-verifier, and analyzer hardening follow-up later raised the active current-doc guard to 1003 / 0 and static-file hygiene to 12 / 0 while preserving the same static-only boundary'
