...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-23 beta.126 handoff summary; older May/June package notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because gameplay, save-load, co-op, current enabled-mode registration/gameplay proof, beta.126 runtime proof, independent QA rerun, and tester-package handoff decisions remain pending.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle 'Installed beta.126 package artifacts and no-launch validation are current; beta.123 clicked Ancient UI smoke and beta.99 or older rows are previous-package evidence.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.126` on Slay'
...
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.126`'
...
Add-ContainsCheck -Name 'migration_doc_current_boundary_stub' -Text $migrationDoc -Needle 'Current boundary: Spire Plus is RitsuLib-only for beta.126'
...
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.126 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.34`; package parity, runtime preflight, and source-workspace validation are current no-launch evidence.'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.126.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.34` target.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'beta.126 package parity, runtime preflight 28 / 0, and source-workspace validation 58 / 0 are current for the packaged 151/19 state; beta.126 still needs game-launch/runtime patch-count proof.'
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.126 enabled-mode gameplay proof.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.126 build, publish/package refresh, installed-package parity, runtime preflight, and source-workspace check are the current package target. beta.126 still needs game-launch/runtime patch-count and clicked UI proof.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.126`'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.126'
Add-ContainsCheck -Name 'runtime_monkey_docs_runtime_current_package_target' -Text $runtimeMonkeyDocs -Needle '.\scripts\check-spire-plus-runtime-monkey-packet.ps1 `
  ...
  -ExpectedPackageVersion v0.1.0-private-beta.126'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.126`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current beta.126 package parity, runtime preflight, and source-workspace'
...
Add-ContainsCheck -Name 'ritsu_batch4c_requires_69_runtime_recapture' -Text $ritsuBatch4cCandidates -Needle 'Before citing the current source migration as runtime coverage, capture a fresh beta.126 loader packet that proves the current 151 migrated Spire Plus patch classes apply in the installed game.'
...
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.126` on Slay the Spire 2 `v0.107.1`.'
...
Add-ContainsCheck -Name 'current_validation_smoke_boundary' -Text $currentValidation -Needle 'The retained beta.123 clicked Ancient UI proof covers the earlier packaged 127-patch state only; it does not prove beta.126 runtime patch registration.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.126.zip'
