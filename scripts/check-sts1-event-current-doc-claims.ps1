...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta92_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-21 current beta.92 handoff summary; older May notes below are retained only as historical context.'
...
Add-NoRegexCheck -Name 'agents_no_current_prefer_baselib_rule' -Paths @('AGENTS.md') -Pattern 'Prefer BaseLib|Install BaseLib `v3\.2\.1`|verify BaseLib and Spire Plus'
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because gameplay, UI, save-load, co-op, independent QA rerun, and tester-package handoff decisions remain pending.'
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'Current installed game is `v0.107.1`. Current `v0.107.0` beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context; beta.88 AdditiveBatch1 loader proof is clean for `v0.107.1` loader/registration only but belongs to the previous BaseLib-backed package, and beta.90 belongs to the previous RitsuLib-only package. Beta.92 now has current RitsuLib-only Off and AdditiveBatch1 loader/registration proof. Gameplay, clicked UI, save-load, replacement, multiplayer, QA, and handoff claims still need separate evidence.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle '.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/'
...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'The beta.92 RitsuLib-only Off proof reached main menu with exactly `STS2-RitsuLib` and `EZMicroBalance` loaded, clean audit, and packet verifier 43 / 0; beta.92 AdditiveBatch1 registered 10 event types through 14 calls with verifier 31 / 0 and packet 61 / 0.'
Add-ContainsCheck -Name 'root_readme_manual_rows_pending' -Text $rootReadme -Needle 'Manual feature verification, gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff are still pending.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.92` on Slay'
...
Add-ContainsCheck -Name 'doc_restructure_spec_beta92_nonclaim' -Text $docRestructureSpec -Needle 'current beta.92 proves only RitsuLib-only `v0.107.1` Off and AdditiveBatch1 loader/registration behavior, beta.85/beta.87/beta.88/beta.90 rows remain previous-context evidence, and current CanaryOnly gameplay/runtime, save-load, replacement, multiplayer, QA, and handoff proof remain pending or blocked.'
...
Add-ContainsCheck -Name 'patch_boundaries_sts1_beta88_additive_nonclaim' -Text $patchBoundaries -Needle 'beta.85 proves retained `v0.107.0` default-Off and CanaryOnly loader behavior, beta.87 proves retained `v0.107.0` AdditiveBatch1 loader/registration behavior, beta.88 proves previous BaseLib-backed `v0.107.1` AdditiveBatch1 loader/registration behavior, and current beta.92 proves `v0.107.1` RitsuLib-only Off plus AdditiveBatch1 loader/registration behavior with STS2-RitsuLib `0.4.29`, 25/25 patches, 10 event types / 14 calls, clean audit, enabled-mode verifier 31 / 0, and runtime packet verifier 61 / 0.'
...
Add-ContainsCheck -Name 'beta_compat_off_canary_loader_nonclaim' -Text $betaCompatibility -Needle 'Treat these as loader proof only: gameplay, save-load, replacement, multiplayer, independent QA, clean-worktree, package handoff, and release-ready compatibility proof remain pending or blocked.'
Add-ContainsCheck -Name 'remote_setup_current_boundary' -Text $remoteDevelopmentSetup -Needle '2026-06-21 Current Boundary'
Add-ContainsCheck -Name 'remote_setup_beta85_off_path' -Text $remoteDevelopmentSetup -Needle '.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta92_loader_only' -Text $docsReadme -Needle 'current beta.92 RitsuLib-only AdditiveBatch1 proof covers loader/registration only'
Add-ContainsCheck -Name 'docs_readme_sts1_additive_pending_counts' -Text $docsReadme -Needle 'current beta.92 RitsuLib-only AdditiveBatch1 proof covers loader/registration only with 10 event types / 14 registration calls, while gameplay, save-load, replacement, multiplayer, QA, and handoff proof remain unproven'
Add-ContainsCheck -Name 'root_test_plan_beta88_additive_path' -Text $rootTestPlan -Needle '.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621'
...
Add-ContainsCheck -Name 'root_test_plan_off_canary_loader_only' -Text $rootTestPlan -Needle 'The beta.85 Off/CanaryOnly smokes remain previous-package context, beta.87 direct proof remains previous-game-version context, and the beta.85 AdditiveBatch1 13/14 mismatch remains root-cause history only.'
Add-ContainsCheck -Name 'root_test_plan_additive_still_requires_clean_packet' -Text $rootTestPlan -Needle 'Installed beta.92 package parity is restored, and current beta.92 `v0.107.1` RitsuLib-only direct AdditiveBatch1 proof under `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` reached main menu'
Add-ContainsCheck -Name 'migration_doc_current_off_canary_loader_only' -Text $migrationDoc -Needle 'The retained `v0.107.0` beta.87 package proof, BaseLib-backed beta.88 proof, and previous RitsuLib-only beta.90 proof remain previous-context evidence; current `v0.107.1` beta.92 proof is clean at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` for loader/patch application and AdditiveBatch1 registration shape.'
Add-ContainsCheck -Name 'migration_doc_enabled_modes_historical_only' -Text $migrationDoc -Needle 'Treat the previous `v0.106.1` Off/CanaryOnly/AdditiveBatch1 smokes as historical loader evidence only.'
Add-ContainsCheck -Name 'migration_doc_remaining_runtime_blockers' -Text $migrationDoc -Needle 'Gameplay, Mod Settings UI page refresh, event screenshots, save-load, image/render, replacement functional proof, co-op/fail-closed proof, independent QA, and tester-package handoff remain pending; handoff must recapture HEAD and worktree status after any later edits.'
...
Add-ContainsCheck -Name 'goal_migration_beta87_current_override' -Text $goalMigrationDoc -Needle 'Current beta.92 RitsuLib-only AdditiveBatch1 proof is `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/`. The BaseLib `v3.2.1` patch failures in `.tools/runtime-evidence/v01071-beta87-additive-batch1-direct-20260619-102309/` are root-cause history.'
Add-ContainsCheck -Name 'goal_debug_beta85_off_loader_nonclaim' -Text $goalDebugDoc -Needle 'loader proof'
Add-ContainsCheck -Name 'goal_debug_beta87_current_override' -Text $goalDebugDoc -Needle 'Beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context, beta.88 remains previous BaseLib-backed context, and beta.92 is the current RitsuLib-only loader/registration proof.'
...
Add-ContainsCheck -Name 'goal_refactor_beta87_current_override' -Text $goalRefactorDoc -Needle 'Beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context, beta.88 remains previous BaseLib-backed context, and beta.92 is the current RitsuLib-only loader/registration proof.'
...
Add-ContainsCheck -Name 'goal_event_beta88_current_override' -Text $goalEventDoc -Needle 'current loader truth is beta.92 on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` reached main menu with exactly STS2-RitsuLib `v0.4.29` and Spire Plus `v0.1.0-private-beta.92` loaded, no Spire Plus BaseLib dependency, 25/25 Spire Plus patches applied, clean audits, Off packet verifier 43 / 0, AdditiveBatch1 enabled-mode verifier 31 / 0, and AdditiveBatch1 packet verifier 61 / 0.'
...
Add-ContainsCheck -Name 'goal_event_latest_beta92_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.92 governance checkpoint after the gate-ledger/current-doc cleanup: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 1328 checks / 0 mismatches'
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_1328' -Text $goalEventDoc -Needle 'current-doc-claims: 1328 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'goal_event_validation_matrix_runtime_preflight_27' -Text $goalEventDoc -Needle 'runtime-preflight: 27 checks / 0 mismatches (local v0.107.1 / beta.92 target; read-only source/prereq only)'
...
Add-ContainsCheck -Name 'goal_event_source_workspace_checkpoint_beta92' -Text $goalEventDoc -Needle 'source workspace: 60 checks / 0 mismatches against installed v0.107.1 and STS2-RitsuLib 0.4.29 / compat 0.107.1'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_1328' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*1328 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'goal_event_beta88_additive_loader_current_pass_section' -Text $goalEventDoc -Needle 'Current beta.92 RitsuLib-only AdditiveBatch1 loader registration proof can be treated as current-pass for `O33`.'
Add-ContainsCheck -Name 'goal_event_off_canary_loader_nonextension_boundary' -Text $goalEventDoc -Needle 'Retained beta.85 Off, retained beta.85 CanaryOnly, retained beta.87 AdditiveBatch1, previous BaseLib-backed beta.88 AdditiveBatch1, and current beta.92 AdditiveBatch1 loader proof must not be extended to:'
...
Add-ContainsCheck -Name 'test_ready_goal_beta86_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.92.zip`; build, publish, package refresh, installed-package check, runtime preflight, and direct RitsuLib-only Off/AdditiveBatch1 loader proof have run'
Add-ContainsCheck -Name 'dev_environment_sts1_enabled_smokes_current_split' -Text $devEnvironment -Needle 'beta.92 RitsuLib-only AdditiveBatch1 proof is clean current package context under `.tools\runtime-evidence\v01071-beta92-ritsulib0429-additivebatch1-direct-20260621`, with 10 event types / 14 registration calls, enabled-mode verifier 31 / 0, and packet verifier 61 / 0.'
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'current RitsuLib-only AdditiveBatch1 loader/registration proof exists at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/`'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'The beta.92 AdditiveBatch1 proof is `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` with retained verifier proof for 10 event types / 14 registration calls, clean audit, enabled-mode verifier 31 / 0, and packet verifier 61 / 0.'
...
Add-ContainsCheck -Name 'legacy_v5_monthly_spec_beta92_additive_nonclaim' -Text $legacyV5MonthlySpec -Needle 'AdditiveBatch1 loader/registration with STS2-RitsuLib `0.4.29`'
...
Add-ContainsCheck -Name 'ritsu_integration_beta87_additive_nonclaim' -Text $ritsuIntegrationDoc -Needle 'fresh current-package direct proof covers RitsuLib-only Off at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/` plus RitsuLib-only AdditiveBatch1 at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/`'
...
Add-ContainsCheck -Name 'ritsu_monthly_current_off_nonclaim' -Text $ritsuMonthlyDevSpec -Needle 'Current-runtime beta.92 Off loader smoke is clean with exactly STS2-RitsuLib and Spire Plus loaded.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_current_note' -Text $ritsuRuntimeHardBlock -Needle '2026-06-21 current note: this report is historical `v0.106.1` loader-gate evidence.'
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta85_off_path' -Text $ritsuRuntimeHardBlock -Needle '.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/'
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_current_enabled_pending' -Text $ritsuRuntimeHardBlock -Needle 'Current gameplay, save-load, replacement, multiplayer, QA, and release-ready proof remain pending or blocked.'
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_coordination_boundary' -Text $ritsuRuntimeHardBlock -Needle 'After coordination clears, capture gameplay, save-load, render, replacement, multiplayer, and QA evidence or record the exact blocker.'
...
Add-ContainsCheck -Name 'scripts_readme_analyzer_direct_smoke_baselib_drift' -Text $scriptsReadme -Needle 'Bound BaseLib dirty-audit signatures such as `BaseLib patch failure` and `[ERROR] [BaseLib]` route to `PackageRuntimeDrift`'
Add-ContainsCheck -Name 'scripts_readme_analyzer_direct_smoke_baselib_patch_details' -Text $scriptsReadme -Needle '`BaseLibPatchFailures` details such as `AdjustCustomMessageKeys::Fuckery()` undefined target-method failures'
Add-ContainsCheck -Name 'scripts_readme_analyzer_direct_smoke_baselib_relic_collection_detail' -Text $scriptsReadme -Needle '`NRelicCollectionCategory::LoadRelics` instruction matcher failures'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.92'
Add-ContainsCheck -Name 'runtime_monkey_docs_runtime_current_package_target' -Text $runtimeMonkeyDocs -Needle '.\scripts\check-spire-plus-runtime-monkey-packet.ps1 `
  ...
  -ExpectedPackageVersion v0.1.0-private-beta.92'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_analyzer_direct_smoke_baselib_drift' -Text $runtimeMonkeyDocs -Needle '`BaseLib patch failure` and `[ERROR] [BaseLib]` to `PackageRuntimeDrift`'
Add-ContainsCheck -Name 'runtime_monkey_docs_analyzer_direct_smoke_baselib_patch_details' -Text $runtimeMonkeyDocs -Needle '`BaseLibPatchFailures` array records'
Add-ContainsCheck -Name 'runtime_monkey_docs_analyzer_direct_smoke_baselib_relic_collection_detail' -Text $runtimeMonkeyDocs -Needle '`NRelicCollectionCategory::LoadRelics` instruction'
...
Add-ContainsCheck -Name 'runtime_failure_analyzer_routes_baselib_errors_to_package_drift' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'scripts\analyze-spire-plus-runtime-failure.ps1') -Raw -Encoding UTF8) -Needle 'BaseLib.*(?:HarmonyException|Patching exception|patch(?:ing)? exception|failed)'
Add-ContainsCheck -Name 'runtime_failure_analyzer_extracts_baselib_patch_failures' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'scripts\analyze-spire-plus-runtime-failure.ps1') -Raw -Encoding UTF8) -Needle 'function Get-BaseLibPatchFailureDetails'
Add-ContainsCheck -Name 'runtime_failure_analyzer_reports_baselib_patch_failures' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'scripts\analyze-spire-plus-runtime-failure.ps1') -Raw -Encoding UTF8) -Needle 'BaseLibPatchFailures = @($baseLibPatchFailures)'
...
Add-ContainsCheck -Name 'runtime_monkey_tests_cover_direct_smoke_baselib_patch_details' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'tests\EZMicroBalance.Tests\RuntimeMonkeyStabilityGuardTests.DirectSmokeAnalyzer.cs') -Raw -Encoding UTF8) -Needle 'BaseLibPatchFailures'
...
Add-RegexCheck -Name 'runtime_canary_current_pass' -Text $runtimeChecklist -Pattern '\| CanaryOnly \|[^\r\n]*\[PASS\][^\r\n]*v01070-beta85-canary-20260617-233621'
...
Add-RegexCheck -Name 'runtime_batch1_current_pass_14' -Text $runtimeChecklist -Pattern '\| AdditiveBatch1 \|[^\r\n]*14 registration calls / 10 event types[^\r\n]*CURRENT PASS[^\r\n]*beta92 v01071 RitsuLib-only proof'
...
Add-ContainsCheck -Name 'runtime_exit_status_off_canary_batch1_blocked' -Text $runtimeChecklist -Needle 'Current exit status: beta.92 Off and AdditiveBatch1 loader rows pass for `v0.107.1` with RitsuLib `v0.4.29` / `lib\0.107.1` and no Spire Plus BaseLib dependency; beta.85/beta.87/beta.88/beta.90 loader rows remain previous-package or previous-dependency context.'
Add-ContainsCheck -Name 'runtime_checklist_coordination_boundary' -Text $runtimeChecklist -Needle 'Coordination boundary: run this checklist''s launch, gameplay, build, publish, package, or release-evidence steps only when a controlled validation lane is assigned.'
Add-ContainsCheck -Name 'runtime_checklist_after_pause_controlled_lane' -Text $runtimeChecklist -Needle 'the 2026-06-21 beta.92 direct packets supersede beta.88/beta.90 for current RitsuLib-only loader/registration proof only.'
...
Add-ContainsCheck -Name 'runtime_enabled_log_verifier_tuple_check' -Text $runtimeChecklist -Needle 'It verifies registration-call count, event class set, and observed registration tuples parsed from `Registered act event` / `Registered shared event` log lines when those tuple details are present.'
Add-ContainsCheck -Name 'runtime_enabled_log_verifier_tuple_fallback_boundary' -Text $runtimeChecklist -Needle 'If future logs lose act/shared tuple detail, Act-bucket proof remains source-derived until gameplay evidence proves those targets directly.'
...
Add-ContainsCheck -Name 'runtime_evidence_packet_exact_allowed_mod_ids' -Text $runtimeChecklist -Needle 'AllowedModIds` exactly equal to STS2-RitsuLib and EZMicroBalance for the current beta.92 RitsuLib-only lane'
Add-ContainsCheck -Name 'runtime_checklist_enabled_log_verifier_outfile_canary' -Text $runtimeChecklist -Needle '.\scripts\check-sts1-enabled-mode-runtime-log.ps1 -Mode CanaryOnly -LogPath "<evidence>\godot.log.current-iteration" -AuditPath "<evidence>\godot-log-current-iteration-audit.json" -ExpectedPackageVersion v0.1.0-private-beta.92 -ExpectedRitsuCompatBranch 0.107.1 -ExpectedRitsuLibVersion 0.4.29 -ExpectedGameVersion 0.107.1 -OutFile "<evidence>\enabled-mode-log-check.json" -FailOnMismatch'
...
Add-ContainsCheck -Name 'runtime_evidence_packet_uses_current_slice' -Text $runtimeChecklist -Needle 'uses `godot.log.current-iteration` as canonical proof'
Add-ContainsCheck -Name 'runtime_evidence_packet_binds_retained_current_slice' -Text $runtimeChecklist -Needle 'retained current slices must byte-match `godot.log.after-launch` after the `godot.log.before` prefix'
...
Add-ContainsCheck -Name 'runtime_evidence_packet_rejects_stale_current_slice' -Text $runtimeChecklist -Needle 'rejects retained current slices that do not match `godot.log.after-launch` after the `godot.log.before` byte prefix'
...
Add-ContainsCheck -Name 'next_overnight_beta87_totals' -Text $nextOvernight -Needle 'Latest beta.92 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace check, and RitsuLib-only Off/AdditiveBatch1 packet verification passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD validation refreshed after the beta.92 RitsuLib-only dependency pass and runtime packet-checker hardening; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.92 package and STS2-RitsuLib `v0.4.29` / `lib\0.107.1` runtime are still installed and current'
...
Add-ContainsCheck -Name 'ritsu_monthly_enabled_smokes_boundary' -Text $ritsuMonthlyDevSpec -Needle 'Current beta.92 AdditiveBatch1 enabled-mode smoke is clean with 10 event types / 14 registration calls and retained verifier reports.'
...
Add-ContainsCheck -Name 'ritsu_monthly_batch1_smoke_before_gameplay' -Text $ritsuMonthlyDevSpec -Needle 'Use the current AdditiveBatch1 enabled-mode smoke at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` as the beta.92 / `v0.107.1` 10 event types / 14 registration-call proof; recapture it after any package/source change before using it for a new claim.'
...
Add-ContainsCheck -Name 'ritsu_batch4c_enabled_smokes_boundary' -Text $ritsuBatch4cCandidates -Needle 'Current beta.92 AdditiveBatch1 retained log and packet verifiers passed with 10 event types / 14 registration lines and exact tuple parity. This is loader/registration proof only and is not Batch 4c approval, gameplay proof, or handoff proof.'
...
Add-ContainsCheck -Name 'sts1_goal_revision_n_boundary' -Text $sts1FeatureGoal -Needle 'Current Boundary (Revision P / beta.92)'
Add-ContainsCheck -Name 'sts1_goal_not_complete' -Text $sts1FeatureGoal -Needle 'This goal is not complete.'
Add-ContainsCheck -Name 'sts1_goal_default_off_canary_additive_boundary' -Text $sts1FeatureGoal -Needle 'Current beta.92 proof under `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` covers AdditiveBatch1 loader/registration shape on Slay the Spire 2 `v0.107.1` with RitsuLib `v0.4.29` / `lib\0.107.1` and no BaseLib dependency: 25/25 Spire Plus patches, 10 event types / 14 registration calls, clean audit, retained enabled-mode verifier 31 / 0, and runtime packet verifier 61 / 0.'
...
Add-ContainsCheck -Name 'sts1_goal_compile_package_current_split' -Text $sts1FeatureGoal -Needle 'Events must compile against the current repository package reference: `STS2.RitsuLib` `0.4.29`; Spire Plus must not require BaseLib.'
Add-ContainsCheck -Name 'sts1_goal_runtime_ritsu_variant_split' -Text $sts1FeatureGoal -Needle 'Current runtime reproof targets the installed official `STS2-RitsuLib` `v0.4.29` variant pack with `lib\0.107.1`.'
...
Add-ContainsCheck -Name 'implementation_plan_revision_n_boundary' -Text $implementationPlan -Needle 'Current boundary, Revision P / beta.92'
...
Add-ContainsCheck -Name 'implementation_plan_current_counts' -Text $implementationPlan -Needle 'Current beta.92 proof under `.tools/runtime-evidence/v01071-beta92-ritsulib0429-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/` covers `v0.107.1` RitsuLib-only Off/AdditiveBatch1 loader/registration only with STS2-RitsuLib `v0.4.29`, 25/25 patches, clean audit, retained enabled-mode verifier 31 / 0, and AdditiveBatch1 runtime packet verifier 61 / 0.'
...
Add-ContainsCheck -Name 'closure_plan_runtime_batch1_beta92_10_14' -Text $localizationClosurePlan -Needle 'Current beta.92 `v0.107.1` AdditiveBatch1 enabled-mode smoke with 10 event types / 14 source registration calls, or a fresher package-matched recapture if the worktree/package changes.'
...
Add-ContainsCheck -Name 'sts1_event_engine_research_beta92_nonclaim' -Text $sts1EventEngineResearch -Needle 'Current beta.92 proves only RitsuLib-only Off and AdditiveBatch1 loader/registration behavior; beta.85/beta.87/beta.88/beta.90 rows remain previous-context evidence. CanaryOnly gameplay/runtime, save-load, replacement, multiplayer, QA, handoff, and release-ready proof still require fresh current evidence.'
...
Add-ContainsCheck -Name 'status_board_current_enabled_boundary' -Text $statusBoard -Needle 'Current beta.92 has clean RitsuLib-only `v0.107.1` Off and AdditiveBatch1 direct loader/registration proof with STS2-RitsuLib `v0.4.29`, 25/25 Spire Plus patches applied, 10 event types / 14 registration calls, and exact tuple parity.'
...
Add-ContainsCheck -Name 'status_board_build_beta92_boundary' -Text $statusBoard -Needle '| Build | beta.92 validated 0 errors / 0 warnings | `PROJECT_STATE.md` and `docs/dev-environment.md` record the beta.92 RitsuLib-only build as 0 errors / 0 warnings. This remains no-game build validation, not event gameplay proof. |'
Add-ContainsCheck -Name 'status_board_tests_beta92_boundary' -Text $statusBoard -Needle '| Tests | beta.92 current guard/focused lanes passed; retained split lane passed | `PROJECT_STATE.md` and `docs/reviews/current-validation.md` record the trusted split test strategy, migration-focused guard totals, current package/artifact validation status, and the latest runtime packet-checker recapture. This proves automated guard/package coverage only, not enabled-mode or gameplay proof. |'
Add-ContainsCheck -Name 'status_board_format_beta92_boundary' -Text $statusBoard -Needle '| Format | beta.92 post-migration format passed | `PROJECT_STATE.md` and `docs/reviews/current-validation.md` record format/diff-check/patch-inventory/batch-classifier checks passing after the beta.92 RitsuLib-only migration validation and current-doc alignment. This remains no-game validation and should be recaptured after future code, resource, package, or handoff changes. |'
...
Add-ContainsCheck -Name 'test_plan_manual_smoke_batch1_10_14' -Text $testPlan -Needle 'Treat `.tools\runtime-evidence\v01071-beta92-ritsulib0429-additivebatch1-direct-20260621` as current `v0.107.1` AdditiveBatch1 loader smoke only: 10 event types / 14 registration calls'
...
Add-ContainsCheck -Name 'gate_map_o0_o76_scope' -Text $gateMap -Needle 'Scope: `docs/goals/event.md` Mandatory Overnight Run v19/v21 carry-forward, current `v0.107.1` / beta.92 loader truth.'
...
Add-ContainsCheck -Name 'gate_map_o21_o24_off_pass' -Text $gateMap -Needle '| `O21-O24` | current beta.92 RitsuLib-only loader pass |'
...
Add-ContainsCheck -Name 'gate_map_enabled_log_verifier_non_claim' -Text $gateMap -Needle 'They do not close gameplay gates; `O33` is closed only as loader-registration proof by the current beta.92 AdditiveBatch1 packet'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o0_beta92_baseline_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O0'; Field = 'evidence'; Needle = 'PROJECT_STATE.md records beta.92 RitsuLib-only package/runtime baseline and requires exact HEAD/worktree recapture for later governance/test follow-up commits'"
...
Add-ContainsCheck -Name 'gate_ledger_checker_o4_zero_warning_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O4'; Field = 'evidence'; Needle = 'PROJECT_STATE.md records beta.92 zero-warning validation'"
...
Add-ContainsCheck -Name 'gate_ledger_checker_o9_beta92_zip_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O9'; Field = 'evidence'; Needle = 'ZIP SHA F02683DA621A1B116DD47AF13C529088433E4D6F66B0994503090EC23CEC8A57'"
...
Add-ContainsCheck -Name 'gate_ledger_checker_o22_ritsulib_requirement' -Text $gateLedgerCheckerScript -Needle "GateId = 'O22'; Field = 'requirement'; Needle = 'STS2-RitsuLib v0.4.29 with lib/0.107.1 installed'"
Add-ContainsCheck -Name 'gate_ledger_checker_o23_current_log_path_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O23'; Field = 'evidence'; Needle = '.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/godot.log.current-iteration'"
...
Add-ContainsCheck -Name 'gate_ledger_checker_o67_status_board_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O67'; Field = 'evidence'; Needle = 'status-board records beta.92 RitsuLib-only AdditiveBatch1 current proof plus beta.85/beta.87/beta.88 previous-context loader proof'"
...
Add-ContainsCheck -Name 'gate_ledger_package_sha' -Text $gateLedger -Needle 'ZIP SHA F02683DA621A1B116DD47AF13C529088433E4D6F66B0994503090EC23CEC8A57'
...
Add-ContainsCheck -Name 'hard_stop_v20_blocked_o25_o33' -Text $hardStopV20 -Needle '`O25`: retained `v0.107.0` CanaryOnly enabled-mode smoke has since been captured by the shared validation lane and is loader proof only; this thread did not create it. Current `v0.107.1` loader proof still needs recapture.'
...
Add-ContainsCheck -Name 'current_validation_historical_target_fix_old_shape_nonclaim' -Text $currentValidation -Needle 'Historical Off mode proved 0 StS1 registration lines; historical CanaryOnly proved exactly 4 canary content registrations for that source/runtime state. Retained beta.85 default-Off/CanaryOnly and beta.87 AdditiveBatch1 proof are previous `v0.107.0` loader context; current beta.92 RitsuLib-only Off/AdditiveBatch1 proof is the active loader/registration truth'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for beta.92 loader/registration; gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta92_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.92 RitsuLib-only evidence refresh: 2026-06-21.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta92_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.92 RitsuLib-only evidence refresh: 2026-06-21.'
Add-ContainsCheck -Name 'test_ready_completion_audit_beta85_loader_nonclaim' -Text $testReadyCompletionAudit -Needle 'These hashes and historical loader proof do not close live gameplay, save-load, death/failure path, or co-op verification.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.92.zip'
...
Add-NoRegexCheck -Name 'no_current_migration_residue_from_baselib_or_old_claim_counts' -Paths $currentClaimFiles -Pattern 'current-doc claims (1316|1320|1324|1325|1326) / 0|current-doc claims passed (1316|1320|1324|1325|1326) / 0|returned (1316|1320|1324|1325|1326) checks / 0 mismatches|BaseLib and RitsuLib runtime dependency|BaseLib \+ RitsuLib runtime dependency|manifest [^\r\n]*BaseLib[^\r\n]*RitsuLib runtime dependency'
...
Add-AutoSlayProofCommandSwitchValuesCheck -Name 'autoslay_packet_proof_commands_pin_current_target_values' -Paths $autoSlayProofCommandFiles -SwitchValues @{
    ...
    ExpectedRitsuLibVersion = '0.4.29'
