...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, co-op, direct beta.99 Off loader proof, current beta.99 enabled-mode registration/gameplay proof, independent QA rerun, and tester-package handoff decisions remain pending.'
...
Add-ContainsCheck -Name 'root_readme_manual_rows_pending' -Text $rootReadme -Needle 'Manual feature verification, gameplay, clicked Ancient UI, save-load, preview-tools live behavior, Vakuu, direct beta.99 Off loader proof, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.'
...
Add-ContainsCheck -Name 'goal_event_beta96_current_loader_boundary' -Text $goalEventDoc -Needle 'As of 2026-06-21, current package truth is beta.99 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.32`. Beta.99 clicked settings UI proof is captured under `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`; beta.99 direct Off loader proof remains pending after the RitsuLib settings-page I18N resource migration.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'beta.99 build, publish/package refresh, installed-package parity, source-workspace check, and Mod Settings UI proof are the current package target.'
...
Add-ContainsCheck -Name 'ritsu_monthly_current_off_nonclaim' -Text $ritsuMonthlyDevSpec -Needle 'Beta.99 direct Off loader smoke remains pending after the settings-page I18N resource migration; beta.99 clicked RitsuLib settings UI proof is current'
...
Add-ContainsCheck -Name 'next_overnight_beta94_totals' -Text $nextOvernight -Needle 'Latest beta.99 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, and RitsuLib settings UI proof passed for the current dependency target; beta.99 direct Off loader proof remains pending after the RitsuLib I18N settings resource migration.'
...
Add-ContainsCheck -Name 'release_evidence_status_beta85_off_nonclaim' -Text $releaseEvidenceStatus -Needle 'beta.96 RitsuLib-only Off direct loader smoke is previous-package context after beta.99, while beta.99 Mod Settings page proof is current UI visibility evidence. Gameplay, clicked Ancient UI, save-load, preview-tools live behavior, beta.99 direct Off loader proof, current enabled-mode proof, Vakuu, co-op, and full release-evidence packaging rows remain pending.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pending for beta.99 direct Off loader; gameplay evidence pending'
