...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'fresh beta.87 AdditiveBatch1 direct smoke under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` reached main menu'
...
Add-ContainsCheck -Name 'root_test_plan_beta85_off_path' -Text $rootTestPlan -Needle '.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531'
...
Add-ContainsCheck -Name 'goal_migration_beta87_current_override' -Text $goalMigrationDoc -Needle 'Current beta.85/beta.86 loader proof remains previous-package context, and current beta.87 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/`.'
Add-ContainsCheck -Name 'goal_debug_beta85_off_loader_nonclaim' -Text $goalDebugDoc -Needle 'loader proof'
Add-ContainsCheck -Name 'goal_debug_beta87_current_override' -Text $goalDebugDoc -Needle 'Current beta.85/beta.86 loader proof remains previous-package context, and current beta.87 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/`.'
...
Add-ContainsCheck -Name 'goal_refactor_beta87_current_override' -Text $goalRefactorDoc -Needle 'Current beta.85/beta.86 loader proof remains previous-package context, and current beta.87 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/`.'
...
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_1025' -Text $goalEventDoc -Needle 'current-doc-claims: 1025 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_1025' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*1025 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'test_ready_goal_sts1_enabled_smokes_boundary' -Text $testReadyGoal -Needle 'The beta.85 Off/CanaryOnly and beta.86 AdditiveBatch1 `v0.107.0` packets remain previous-package loader context.'
...
Add-ContainsCheck -Name 'test_ready_goal_beta86_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.87.zip`; build, publish, package refresh, installed-package check, runtime preflight, and direct AdditiveBatch1 loader proof have run'
Add-ContainsCheck -Name 'dev_environment_sts1_enabled_smokes_current_split' -Text $devEnvironment -Needle 'Current beta.85/beta.86 `v0.107.0` proof covers Off/CanaryOnly loader context only, and beta.87 AdditiveBatch1 proof covers 10 event types / 14 registration calls with exact tuple parity.'
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'Fresh beta.87 direct proof: AdditiveBatch1 reached main menu, audited clean, and passed retained mode/packet verifiers with 0 mismatches.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'The beta.87 AdditiveBatch1 proof is `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` with retained verifier proof for 10 event types / 14 registration calls'
...
Add-ContainsCheck -Name 'ritsu_integration_beta87_additive_nonclaim' -Text $ritsuIntegrationDoc -Needle 'Current beta.87 AdditiveBatch1 direct proof is clean for loader/registration only.'
...
Add-ContainsCheck -Name 'scripts_readme_autoslay_packet_requires_runtime_log_growth' -Text $scriptsReadme -Needle 'runtime `LogGrew=true` and main-menu/runtime no-log-growth timeout rejection'
Add-ContainsCheck -Name 'scripts_readme_autoslay_packet_requires_probe_phases' -Text $scriptsReadme -Needle 'both `main-menu` and `runtime` phases plus `run-result.json` ProcessId binding'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_requires_timestamp_order' -Text $runtimeMonkeyDocs -Needle 'parseable ordered run-result timestamps'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_analyzer_reports_probe_phases' -Text $runtimeMonkeyDocs -Needle 'missing `main-menu` / `runtime` probe phases'
...
Add-ContainsCheck -Name 'runtime_exit_status_off_canary_batch1_blocked' -Text $runtimeChecklist -Needle 'Current exit status: current beta.87 AdditiveBatch1 loader row passes for `v0.107.0`; beta.85 Off and CanaryOnly loader rows remain previous-package context.'
...
Add-ContainsCheck -Name 'next_overnight_latest_observed_head_recapture_boundary' -Text $nextOvernight -Needle 'The run-start pushed HEAD for the blank-path/log-growth runtime probe follow-up was `77d46f23`; any later commits or dirty edits must still be recaptured exactly before handoff.'
Add-ContainsCheck -Name 'goal_migration_latest_observed_head_recapture_boundary' -Text $goalMigrationDoc -Needle 'The run-start pushed HEAD for the blank-path/log-growth runtime probe follow-up was `77d46f23`; recapture `git log -1 --oneline --decorate` and `git status --short --branch` again before handoff.'
...
Add-ContainsCheck -Name 'next_overnight_current_batch1_success_closed' -Text $nextOvernight -Needle '- [x] Current `v0.107.0` beta.87 AdditiveBatch1 smoke proves 10 event types / 14 registration calls with retained verifier reports; beta.85 13/14 attempt remains root-cause history only.'
...
Add-ContainsCheck -Name 'ritsu_batch4c_enabled_smokes_boundary' -Text $ritsuBatch4cCandidates -Needle 'Current beta.87 AdditiveBatch1 enabled-mode smoke exists at `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/`; retained log and packet verifiers passed with 10 event types / 14 registration lines and exact tuple parity.'
...
Add-ContainsCheck -Name 'sts1_goal_compile_package_current_split' -Text $sts1FeatureGoal -Needle 'Events must compile against the current repository package references: `STS2.RitsuLib` `0.4.24` and BaseLib `3.2.1`.'
...
Add-ContainsCheck -Name 'implementation_plan_revision_n_boundary' -Text $implementationPlan -Needle 'Current boundary, Revision N / beta.87'
...
Add-ContainsCheck -Name 'implementation_plan_current_counts' -Text $implementationPlan -Needle 'beta.85 CanaryOnly 4 event types / 6 registration calls as previous-package context, plus beta.87 AdditiveBatch1 10 event types / 14 registration calls'
...
Add-ContainsCheck -Name 'status_board_current_enabled_boundary' -Text $statusBoard -Needle 'Current beta.87 has clean `v0.107.0` AdditiveBatch1 direct loader proof with 10 event types / 14 registration calls and exact tuple parity.'
...
Add-ContainsCheck -Name 'status_board_date_v20_static_alignment_20260617' -Text $statusBoard -Needle '> Last updated: 2026-06-18 (current beta.87 AdditiveBatch1 verifier packet captured and passed; latest CanaryOnly-mode packet remains beta.85 previous-package context; no gameplay or release-status change)'
...
Add-ContainsCheck -Name 'test_plan_manual_smoke_batch1_10_14' -Text $testPlan -Needle 'Use `.tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531` as the fresh current `v0.107.0` AdditiveBatch1 smoke: 10 event types / 14 registration calls'
...
Add-ContainsCheck -Name 'test_plan_runtime_evidence_packet_off' -Text $testPlan -Needle '.\scripts\check-sts1-runtime-evidence-packet.ps1 -Mode AdditiveBatch1 -EvidenceDir ".tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531" -ExpectedPackageVersion v0.1.0-private-beta.87 -ExpectedRitsuCompatBranch 0.107.0 -ExpectedRitsuLibVersion 0.4.24 -ExpectedGameVersion 0.107.0 -OutFile ".tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531\runtime-evidence-packet-check.json" -FailOnMismatch'
...
Add-ContainsCheck -Name 'gate_map_o0_o76_scope' -Text $gateMap -Needle 'Scope: `docs/goals/event.md` Mandatory Overnight Run v19/v21 carry-forward, current `v0.107.0` / beta.87 truth.'
...
Add-ContainsCheck -Name 'gate_map_o21_o24_off_pass' -Text $gateMap -Needle '| `O21-O24` | current beta.87 AdditiveBatch1 loader pass |'
...
Add-ContainsCheck -Name 'gate_map_enabled_log_verifier_non_claim' -Text $gateMap -Needle 'They do not close gameplay gates; `O33` is closed only as loader-registration proof by the retained beta.87 AdditiveBatch1 packet'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for beta.87 AdditiveBatch1 loader registration only; gameplay rows pending'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.87.zip'
