...
Add-ContainsCheck -Name 'agents_current_beta87_additive_path' -Text $agents -Needle 'current beta.87 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/`'
...
Add-ContainsCheck -Name 'docs_readme_sts1_default_off_canary_only' -Text $docsReadme -Needle 'beta.87 direct AdditiveBatch1 proof covers loader/registration only'
Add-ContainsCheck -Name 'docs_readme_sts1_additive_pending_counts' -Text $docsReadme -Needle 'beta.87 direct AdditiveBatch1 proof covers loader/registration only with 10 event types / 14 registration calls, while gameplay, save-load, replacement, multiplayer, QA, and handoff proof remain unproven'
...
Add-ContainsCheck -Name 'ritsu_monthly_current_off_nonclaim' -Text $ritsuMonthlyDevSpec -Needle 'Current-runtime Off loader smoke is clean for beta.85 as previous-package context.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_current_note' -Text $ritsuRuntimeHardBlock -Needle '2026-06-19 current note: this report is historical `v0.106.1` loader-gate evidence.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_current_enabled_pending' -Text $ritsuRuntimeHardBlock -Needle 'Current `v0.107.1` loader proof, gameplay, save-load, replacement, multiplayer, QA, and release-ready proof remain pending or blocked.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD validation refreshed after the beta.87 dependency-floor pass and runtime packet-checker hardening; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'ritsu_monthly_enabled_smokes_boundary' -Text $ritsuMonthlyDevSpec -Needle 'Current CanaryOnly enabled-mode smoke is clean under `.tools/runtime-evidence/v01070-beta85-canary-20260617-233621/`, with 4 event types / 6 registration calls and retained verifier reports as previous-package context.'
...
Add-ContainsCheck -Name 'ritsu_monthly_canary_smoke_before_gameplay' -Text $ritsuMonthlyDevSpec -Needle 'Use the current CanaryOnly enabled-mode smoke at `.tools/runtime-evidence/v01070-beta85-canary-20260617-233621/` as the beta.85 / `v0.107.0` 4 event types / 6 registration-call proof.'
Add-ContainsCheck -Name 'ritsu_monthly_batch1_smoke_before_gameplay' -Text $ritsuMonthlyDevSpec -Needle 'Use the current AdditiveBatch1 enabled-mode smoke at `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` as the beta.87 / `v0.107.0` 10 event types / 14 registration-call proof; recapture it after any package/source change before using it for a new claim.'
...
Add-ContainsCheck -Name 'ritsu_batch4c_enabled_smokes_boundary' -Text $ritsuBatch4cCandidates -Needle 'Retained beta.87 AdditiveBatch1 enabled-mode smoke exists at `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` for `v0.107.0`; retained log and packet verifiers passed with 10 event types / 14 registration lines and exact tuple parity. This is loader/registration proof only and is not Batch 4c approval, gameplay proof, current `v0.107.1` runtime proof, or handoff proof.'
