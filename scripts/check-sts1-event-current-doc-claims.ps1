...
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'Current installed game is `v0.107.1`. Current `v0.107.0` beta.85/beta.86 loader proof remains previous-package context; current beta.87 AdditiveBatch1 loader proof is clean. That beta.87 proof is retained `v0.107.0` loader/registration context only and must be recaptured for a current `v0.107.1` runtime claim.'
...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'beta.87 AdditiveBatch1 direct smoke under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` reached main menu on `v0.107.0`'
...
Add-ContainsCheck -Name 'patch_boundaries_sts1_beta86_additive_nonclaim' -Text $patchBoundaries -Needle 'beta.85 proves retained `v0.107.0` default-Off and CanaryOnly loader behavior, beta.87 proves retained `v0.107.0` AdditiveBatch1 loader/registration behavior, the local install is now `v0.107.1` and needs recapture before current-runtime claims'
...
Add-ContainsCheck -Name 'migration_doc_current_off_canary_loader_only' -Text $migrationDoc -Needle 'retained `v0.107.0` beta.87 package proof clean for loader/patch application and AdditiveBatch1 registration shape; current `v0.107.1` proof still needs recapture'
...
Add-ContainsCheck -Name 'dev_environment_sts1_enabled_smokes_current_split' -Text $devEnvironment -Needle 'Retained beta.85/beta.86 `v0.107.0` proof covers Off/CanaryOnly loader context only, and beta.87 `v0.107.0` AdditiveBatch1 proof covers 10 event types / 14 registration calls with exact tuple parity. These retained rows are previous-game-version context after the local `v0.107.1` update.'
...
Add-ContainsCheck -Name 'runtime_exit_status_off_canary_batch1_blocked' -Text $runtimeChecklist -Needle 'Current exit status: retained beta.87 AdditiveBatch1 loader row passes for `v0.107.0`; beta.85 Off and CanaryOnly loader rows remain previous-package context. Current `v0.107.1` loader, Mod Settings UI, gameplay, save-load, and multiplayer rows remain pending.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, recapture current-version loader/registration proof; `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` is retained `v0.107.0` context only.'
...
Add-ContainsCheck -Name 'status_board_current_enabled_boundary' -Text $statusBoard -Needle 'Retained beta.87 has clean `v0.107.0` AdditiveBatch1 direct loader proof with 10 event types / 14 registration calls and exact tuple parity. The local game is now `v0.107.1`, so current-runtime loader proof must be recaptured.'
...
Add-RegexCheck -Name 'status_board_batch1_current_pass' -Text $statusBoard -Pattern 'AdditiveBatch1 runtime launch.*retained previous-version pass'
...
Add-ContainsCheck -Name 'status_board_date_v20_static_alignment_20260617' -Text $statusBoard -Needle '> Last updated: 2026-06-19 (retained beta.87 `v0.107.0` AdditiveBatch1 verifier packet captured and passed; latest CanaryOnly-mode packet remains beta.85 previous-package context; current `v0.107.1` loader recapture, gameplay, and release proof remain pending)'
...
Add-ContainsCheck -Name 'status_board_o15_source_identity_static' -Text $statusBoard -Needle '| AdditiveBatch1 source identity (O15) | **static-pass / retained runtime pass** |'
...
Add-ContainsCheck -Name 'test_plan_manual_smoke_batch1_10_14' -Text $testPlan -Needle 'Treat `.tools\runtime-evidence\v01070-beta87-additive-batch1-direct-20260618-152531` as retained `v0.107.0` AdditiveBatch1 smoke only: 10 event types / 14 registration calls'
