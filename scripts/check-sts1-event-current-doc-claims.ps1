...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-23 beta.133 handoff summary; older May/June package notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'agents_current_beta133_package_state' -Text $agents -Needle 'Current beta.133 build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace validation are current for the packaged 165/4 source state.'
Add-ContainsCheck -Name 'agents_current_patch_inventory_165_4' -Text $agents -Needle 'The source inventory is 165 migrated RitsuLib patch classes and 4 raw Harmony declarations; the remaining raw declarations are high-risk lifecycle patches, not clicked UI.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle 'Installed beta.133 package artifacts and no-launch validation are current; beta.128 forced clicked Ancient UI smoke and beta.99 or older rows are previous-package evidence.'
...
Add-ContainsCheck -Name 'root_readme_manual_rows_pending' -Text $rootReadme -Needle 'beta.133 runtime smoke, manual feature gameplay, save-load, preview-tools live behavior, gated Vakuu fight-option/victory return, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.133` on Slay'
...
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.133`'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta96_settings_boundary' -Text $docsReadme -Needle 'Current beta.133 RitsuLib-only package state, source-workspace proof, package parity, and forced clicked UI smoke boundary'
...
Add-ContainsCheck -Name 'migration_doc_current_boundary_stub' -Text $migrationDoc -Needle 'Current boundary: Spire Plus is RitsuLib-only for beta.133'
...
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.133 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.34`; package parity, runtime preflight, and source-workspace validation are current no-launch evidence.'
...
Add-ContainsCheck -Name 'test_ready_goal_direct_localization_nonproof' -Text $testReadyGoal -Needle 'This is previous-package smoke-level clicked UI proof only. It does not prove beta.133 gameplay, save-load, replacement behavior, co-op, or release readiness.'
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.133.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.34` target.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'beta.133 package parity, runtime preflight 28 / 0, and source-workspace validation 57 / 0 are current for the packaged 165/4 source state.'
...
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.133 enabled-mode gameplay proof.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.133 build, publish/package refresh, installed-package parity, runtime preflight, and source-workspace check are the current package target; previous beta.128 forced clicked Ancient UI smoke is retained separately.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.133`'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.133 enabled-mode'
...
Add-ContainsCheck -Name 'historical_revision_boundaries_revision_n_current_truth' -Text $historicalRevisionBoundaries -Needle 'Current migration truth is beta.133 RitsuLib-only'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.133'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.133`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.133 package parity, runtime preflight, and'
...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.133 no-launch package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace checks passed for the current dependency target; previous beta.128 forced clicked UI smoke remains previous-package proof.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Previous beta.128 smoke applied all 152 default runtime RitsuLib patch classes from the packaged beta.128 state. Current beta.133 package has 165 migrated patch classes and 4 raw Harmony declarations after the multiplayer save-quit diagnostic migration; recapture beta.133 runtime proof before claiming current in-game coverage.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.133 package and STS2-RitsuLib `v0.4.34` direct NuGet runtime are installed and recapture current-package enabled-mode proof.'
...
Add-ContainsCheck -Name 'ritsu_monthly_routes_batch4c_to_record' -Text $ritsuMonthlyDevSpec -Needle 'completed low-risk localization migration'
...
Add-ContainsCheck -Name 'ritsu_batch4c_requires_69_runtime_recapture' -Text $ritsuBatch4cCandidates -Needle 'Before citing the current source migration as runtime coverage, capture a fresh package/runtime packet that proves the current 165 migrated Spire Plus patch classes apply in the installed game.'
Add-ContainsCheck -Name 'ritsu_batch4c_event_runtime_claim_requires_enabled_smokes' -Text $ritsuBatch4cCandidates -Needle 'Before any Batch 4c follow-up is cited as StS1 event runtime readiness, cite current enabled-mode evidence with retained verifier reports and add the missing gameplay evidence, or state that the Batch 4c claim is unrelated to StS1Events enabled modes.'
...
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.133` on Slay the Spire 2 `v0.107.1`.'
...
Add-ContainsCheck -Name 'current_validation_smoke_boundary' -Text $currentValidation -Needle 'The previous beta.128 clicked Ancient UI proof covers forced Urda, Morvi, Lotha, and normal Vakuu UI only for that package; it does not prove beta.133 gameplay, save-load, gated Vakuu fight-option/victory return, co-op, or release readiness.'
...
Add-ContainsCheck -Name 'handoff_sts1_enabled_smokes_current_split' -Text $privateBetaHandoff -Needle 'Retained AdditiveBatch1 enabled-mode proof remains previous-package loader/registration evidence only and does not prove beta.133 gameplay.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for previous beta.128 startup/UI smoke; beta.133 runtime smoke and gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.133 RitsuLib-only package refresh/no-launch validation: 2026-06-23; previous beta.128 clicked UI smoke: 2026-06-23; previous beta.123, beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.133 RitsuLib-only package refresh/no-launch validation: 2026-06-23; previous beta.128 clicked UI smoke: 2026-06-23; previous beta.123, beta.99, beta.96, and beta.93 evidence remains historical loader/registration/settings proof only.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.133.zip'
