...
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'current beta.105 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260622-025733/`; 4 / 4 `AncientUiSmoke` iterations passed'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.105` on Slay'
...
Add-ContainsCheck -Name 'doc_restructure_spec_beta105_ritsulib_boundary' -Text $docRestructureSpec -Needle 'The RitsuLib integration lane is current for beta.105: compile package,'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta96_settings_boundary' -Text $docsReadme -Needle 'beta.105 is the current package/source/clicked-Ancient-UI proof'
...
Add-ContainsCheck -Name 'migration_doc_current_boundary_stub' -Text $migrationDoc -Needle 'Current boundary: Spire Plus is RitsuLib-only for beta.105'
...
Add-ContainsCheck -Name 'goal_migration_beta87_current_override' -Text $goalMigrationDoc -Needle 'Beta.105 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260622-025733/`'
...
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.105 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.33`.'
...
Add-ContainsCheck -Name 'goal_event_clicked_ui_smoke_boundary' -Text $goalEventDoc -Needle 'Beta.105 clicked Ancient UI smoke is current at `.tools/runtime-evidence/monkey-stability-20260622-025733/`: 4 / 4 Urda, Morvi, Lotha, and normal Vakuu iterations'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.105.zip`; build, publish, package refresh, installed-package check, runtime preflight, source-workspace check, and smoke-level clicked Ancient UI proof have run for the STS2-RitsuLib `v0.4.33` target.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'Beta.105 package parity, runtime preflight 28 / 0, source-workspace validation 57 / 0, and clicked UI smoke are current; packet verification passed 1621 / 0.'
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.105 enabled-mode gameplay proof.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.105 build, publish/package refresh, installed-package parity, runtime preflight, source-workspace check, and clicked Ancient UI smoke are the current package target.'
Add-ContainsCheck -Name 'toreview_direct_localization_nonproof' -Text $toReview -Needle 'Current beta.105 clicked UI proof is `.tools/runtime-evidence/monkey-stability-20260622-025733/` with 4 / 4 iterations, clean audit, StS1 Off verifier pass, exact game/Ritsu/package markers, and packet verifier 1621 / 0.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.105`'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.105 enabled-mode'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.105'
Add-ContainsCheck -Name 'runtime_monkey_docs_runtime_current_package_target' -Text $runtimeMonkeyDocs -Needle '.\scripts\check-spire-plus-runtime-monkey-packet.ps1 `
  ...
  -ExpectedPackageVersion v0.1.0-private-beta.105'
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.105`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.105 package parity, runtime preflight, source-workspace'
...
Add-ContainsCheck -Name 'next_overnight_beta99_totals' -Text $nextOvernight -Needle 'Latest beta.105 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, and clicked Ancient UI smoke passed for the current dependency target.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD/worktree validation refreshed after the beta.105 RitsuLib default-key localization and clicked UI pass; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.105 package and STS2-RitsuLib `v0.4.33` direct NuGet runtime are installed and recapture current-package enabled-mode proof.'
...
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.105` on Slay the Spire 2 `v0.107.1`.'
...
Add-ContainsCheck -Name 'current_validation_clicked_ui_smoke_current' -Text $currentValidation -Needle 'Clicked Ancient UI smoke proof passed at `.tools/runtime-evidence/monkey-stability-20260622-025733/`: 4 / 4 `AncientUiSmoke` iterations for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for beta.105 startup/UI smoke; gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.105 RitsuLib-only package refresh and clicked UI smoke: 2026-06-22; previous beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.105 RitsuLib-only package refresh and clicked UI smoke: 2026-06-22; previous beta.99, beta.96, and beta.93 evidence remains historical loader/registration/settings proof only.'
...
Add-ContainsCheck -Name 'test_ready_completion_audit_manual_rows_pending' -Text $testReadyCompletionAudit -Needle 'Live gameplay, save-load, death/failure-path, and co-op remain pending; beta.105 clicked Ancient UI smoke is recorded separately.'
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.105.zip'
