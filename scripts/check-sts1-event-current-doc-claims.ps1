...
Add-ContainsCheck -Name 'goal_event_latest_pause_safe_static_checkpoint_20260618_autoslay' -Text $goalEventDoc -Needle 'Latest pause-safe static checkpoint after the runtime-monkey AutoSlay boundary/source-contract pass: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 974 checks / 0 mismatches after the v20 subagent coverage'
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_974' -Text $goalEventDoc -Needle 'current-doc-claims: 974 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_974' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*974 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'status_board_pause_safe_static_v20_autoslay_974' -Text $statusBoard -Needle 'the runtime-monkey AutoSlay boundary/source-contract follow-up later raised the active current-doc guard to 974 / 0 while preserving the same static-only boundary'
