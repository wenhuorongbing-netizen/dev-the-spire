...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-21 beta.96 handoff summary; older May notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, co-op, current beta.96 enabled-mode registration/gameplay proof, independent QA rerun, and tester-package handoff decisions remain pending.'
...
Add-ContainsCheck -Name 'migration_doc_beta96_off_loader_only' -Text $migrationDoc -Needle 'Current beta.96 RitsuLib-only Off proof is startup/loading evidence only.'
...
Add-ContainsCheck -Name 'goal_migration_beta96_off_loader_pass' -Text $goalMigrationDoc -Needle 'Current beta.96 Off proof under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`'
...
Add-ContainsCheck -Name 'goal_debug_beta87_current_override' -Text $goalDebugDoc -Needle 'Beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context, beta.88 and beta.93 AdditiveBatch1 remain previous-package context, and beta.96 Off is the current RitsuLib-only loader proof.'
...
Add-ContainsCheck -Name 'goal_event_beta96_current_loader_boundary' -Text $goalEventDoc -Needle 'current Off loader truth is beta.96 on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus `v0.1.0-private-beta.96` loaded, only STS2-RitsuLib as the shared runtime dependency, 25/25 Spire Plus patches applied, clean audit, StS1Events disabled with 0 registration lines, and Off packet verifier 43 / 0.'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.96.zip`; build, publish, package refresh, installed-package check, runtime preflight, source-workspace check, and beta.96 RitsuLib-only Off loader proof have run'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'Current beta.96 RitsuLib-only Off loader proof exists at `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`.'
...
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'beta.96 build, publish/package refresh, installed-package parity, runtime preflight, source-workspace checks, Mod Settings UI proof, and Off loader proof are the current package target.'
...
Add-ContainsCheck -Name 'ritsu_integration_beta96_off_nonclaim' -Text $ritsuIntegrationDoc -Needle 'Current beta.96 direct Off proof at `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/` reached main menu'
...
Add-ContainsCheck -Name 'ritsu_monthly_current_off_nonclaim' -Text $ritsuMonthlyDevSpec -Needle 'Current beta.96 Off loader smoke is clean with exactly STS2-RitsuLib and Spire Plus loaded, 25/25 patches applied, StS1Events disabled with 0 registration lines, and packet verification 43 / 0.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.96 enabled-mode'
...
Add-ContainsCheck -Name 'runtime_checklist_beta96_loader_pass' -Text $runtimeChecklist -Needle '[PASS] `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta96_pending' -Text $runtimeChecklist -Needle 'Current exit status: beta.96 package parity, clicked RitsuLib Mod Settings UI'
...
Add-ContainsCheck -Name 'runtime_checklist_after_pause_controlled_lane' -Text $runtimeChecklist -Needle 'The beta.96 Off packet is retained at'
...
Add-ContainsCheck -Name 'next_overnight_beta94_totals' -Text $nextOvernight -Needle 'Latest beta.96 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, source-workspace check, and beta.96 RitsuLib-only Off packet verification passed for the current dependency target; previous beta.93 RitsuLib-only AdditiveBatch1 packet verification remains historical loader/registration evidence only.'
...
Add-ContainsCheck -Name 'hard_stop_v20_blocked_o25_o33' -Text $hardStopV20 -Needle '`O25`: retained `v0.107.0` CanaryOnly enabled-mode smoke has since been captured by the shared validation lane and is loader proof only; this thread did not create it. Current beta.96 Off loader proof now exists on `v0.107.1`, but current CanaryOnly enabled-mode proof still needs recapture.'
...
Add-ContainsCheck -Name 'current_validation_historical_target_fix_old_shape_nonclaim' -Text $currentValidation -Needle 'Historical Off mode proved 0 StS1 registration lines; historical CanaryOnly proved exactly 4 canary content registrations for that source/runtime state. Retained beta.85 default-Off/CanaryOnly and beta.87 AdditiveBatch1 proof are previous `v0.107.0` loader context; current beta.96 RitsuLib-only Off proof is active startup evidence, while previous beta.93 AdditiveBatch1 proof is previous-package registration context only.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for beta.96 Off loader; gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.96 RitsuLib-only package refresh and Off loader proof: 2026-06-21; previous beta.93 AdditiveBatch1 evidence remains historical loader/registration proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.96 RitsuLib-only package refresh and Off loader proof: 2026-06-21; previous beta.93 AdditiveBatch1 evidence remains historical loader/registration proof only.'
