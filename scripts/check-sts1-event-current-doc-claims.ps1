...
Add-ContainsCheck -Name 'doc_restructure_spec_beta91_nonclaim' -Text $docRestructureSpec -Needle 'current beta.91 proves only RitsuLib-only `v0.107.1` Off and AdditiveBatch1 loader/registration behavior, beta.85/beta.87/beta.88/beta.90 rows remain previous-context evidence, and current CanaryOnly gameplay/runtime, save-load, replacement, multiplayer, QA, and handoff proof remain pending or blocked.'
...
Add-ContainsCheck -Name 'goal_event_latest_beta91_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.91 governance checkpoint after the gate-ledger/current-doc cleanup: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 1320 checks / 0 mismatches'
Add-ContainsCheck -Name 'goal_event_validation_matrix_current_doc_claims_1320' -Text $goalEventDoc -Needle 'current-doc-claims: 1320 checks / 0 mismatches'
...
Add-RegexCheck -Name 'goal_event_lower_audit_current_doc_claims_1320' -Text $goalEventDoc -Pattern '\| Current doc claims\s+\|[^\r\n]*1320 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'closure_plan_runtime_batch1_beta91_10_14' -Text $localizationClosurePlan -Needle 'Current beta.91 `v0.107.1` AdditiveBatch1 enabled-mode smoke with 10 event types / 14 source registration calls, or a fresher package-matched recapture if the worktree/package changes.'
...
Add-ContainsCheck -Name 'sts1_event_engine_research_beta91_nonclaim' -Text $sts1EventEngineResearch -Needle 'Current beta.91 proves only RitsuLib-only Off and AdditiveBatch1 loader/registration behavior; beta.85/beta.87/beta.88/beta.90 rows remain previous-context evidence. CanaryOnly gameplay/runtime, save-load, replacement, multiplayer, QA, handoff, and release-ready proof still require fresh current evidence.'
...
Add-ContainsCheck -Name 'status_board_build_beta91_boundary' -Text $statusBoard -Needle '| Build | beta.91 validated 0 errors / 0 warnings | `PROJECT_STATE.md` and `docs/dev-environment.md` record the beta.91 RitsuLib-only build as 0 errors / 0 warnings. This remains no-game build validation, not event gameplay proof. |'
Add-ContainsCheck -Name 'status_board_tests_beta91_boundary' -Text $statusBoard -Needle '| Tests | beta.91 current guard/focused lanes passed; retained split lane passed | `PROJECT_STATE.md` and `docs/reviews/current-validation.md` record the trusted split test strategy, migration-focused guard totals, current package/artifact validation status, and the latest runtime packet-checker recapture. This proves automated guard/package coverage only, not enabled-mode or gameplay proof. |'
Add-ContainsCheck -Name 'status_board_format_beta91_boundary' -Text $statusBoard -Needle '| Format | beta.91 post-migration format passed | `PROJECT_STATE.md` and `docs/reviews/current-validation.md` record format/diff-check/patch-inventory/batch-classifier checks passing after the beta.91 RitsuLib-only migration validation and current-doc alignment. This remains no-game validation and should be recaptured after future code, resource, package, or handoff changes. |'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o23_current_log_path_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O23'; Field = 'evidence'; Needle = '.tools/runtime-evidence/v01071-beta91-ritsulib0428-additivebatch1-direct-20260620/godot.log.current-iteration'"
...
Add-ContainsCheck -Name 'gate_ledger_checker_o67_status_board_evidence' -Text $gateLedgerCheckerScript -Needle "GateId = 'O67'; Field = 'evidence'; Needle = 'status-board records beta.91 RitsuLib-only AdditiveBatch1 current proof plus beta.85/beta.87/beta.88 previous-context loader proof'"
