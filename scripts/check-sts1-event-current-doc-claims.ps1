...
Add-ContainsCheck -Name 'agents_current_beta133_package_state' -Text $agents -Needle 'Current beta.135 build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace validation are current for the packaged 169/0 source state.'
Add-ContainsCheck -Name 'agents_current_patch_inventory_169_0' -Text $agents -Needle 'The source inventory is 169 migrated RitsuLib patch classes and 0 raw Harmony declarations; new or changed patches must be explicit RitsuLib `IPatchMethod` / `ModPatcher` registrations.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'beta.135 package parity, runtime preflight 28 / 0, and source-workspace validation 57 / 0 are current for the packaged 169/0 source state.'
...
Add-ContainsCheck -Name 'ritsu_localization_requires_169_runtime_recapture' -Text $ritsuLocalizationFallbackRecord -Needle 'Before citing the current source migration as runtime coverage, capture a fresh package/runtime packet that proves the current 169 migrated Spire Plus patch classes apply in the installed game.'
