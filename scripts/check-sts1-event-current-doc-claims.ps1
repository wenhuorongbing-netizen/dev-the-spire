...
Add-ContainsCheck -Name 'private_beta_handoff_current_beta93_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-21 previous beta.93 handoff summary; older May notes below are retained only as historical context.'
Add-ContainsCheck -Name 'agents_current_beta93_additive_path' -Text $agents -Needle 'previous beta.93 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`'
...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because beta.94 loader, gameplay, UI, save-load, co-op, independent QA rerun, and tester-package handoff decisions remain pending.'
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'Current installed game is `v0.107.1`. Current beta.94 package parity is the active package evidence. Previous beta.93 RitsuLib-only Off and AdditiveBatch1 proof is previous-package loader/registration evidence. Older beta.85/beta.86/beta.87/beta.88/beta.90 rows are previous-package or previous-game-version context only. Gameplay, clicked UI, save-load, replacement, multiplayer, QA, and handoff claims still need separate evidence.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.94` on Slay'
...
Add-ContainsCheck -Name 'docs_readme_sts1_beta93_loader_only' -Text $docsReadme -Needle 'previous beta.93 RitsuLib-only AdditiveBatch1 proof covers loader/registration only'
Add-ContainsCheck -Name 'docs_readme_sts1_additive_pending_counts' -Text $docsReadme -Needle 'previous beta.93 RitsuLib-only AdditiveBatch1 proof covers loader/registration only with 10 event types / 14 registration calls, while gameplay, save-load, replacement, multiplayer, QA, and handoff proof remain unproven'
...
Add-ContainsCheck -Name 'root_test_plan_additive_still_requires_clean_packet' -Text $rootTestPlan -Needle 'Installed beta.93 package parity is restored, and previous beta.93 `v0.107.1` RitsuLib-only direct AdditiveBatch1 proof under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` reached main menu'
...
Add-ContainsCheck -Name 'goal_migration_beta85_off_loader_pass' -Text $goalMigrationDoc -Needle 'Previous beta.93 Off proof under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/`'
...
Add-ContainsCheck -Name 'goal_event_beta93_previous_loader_boundary' -Text $goalEventDoc -Needle 'previous loader truth is beta.93 on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus `v0.1.0-private-beta.93` loaded, only STS2-RitsuLib as the shared runtime dependency, 25/25 Spire Plus patches applied, clean audits, Off packet verifier 43 / 0, AdditiveBatch1 enabled-mode verifier 31 / 0, and AdditiveBatch1 packet verifier 61 / 0. Current beta.94 loader proof is pending.'
...
Add-ContainsCheck -Name 'goal_event_latest_beta94_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.94 governance checkpoint after the gate-ledger/current-doc cleanup: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 1331 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.94.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'previous beta.93 RitsuLib-only AdditiveBatch1 loader/registration proof exists at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`'
...
Add-ContainsCheck -Name 'ritsu_integration_beta87_additive_nonclaim' -Text $ritsuIntegrationDoc -Needle 'Previous beta.93 direct proof covers RitsuLib-only Off at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` plus RitsuLib-only AdditiveBatch1 at `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.94'
...
Add-ContainsCheck -Name 'runtime_evidence_packet_exact_allowed_mod_ids' -Text $runtimeChecklist -Needle 'AllowedModIds` exactly equal to STS2-RitsuLib and EZMicroBalance for the previous beta.93 RitsuLib-only lane'
Add-ContainsCheck -Name 'runtime_checklist_enabled_log_verifier_outfile_canary' -Text $runtimeChecklist -Needle '.\scripts\check-sts1-enabled-mode-runtime-log.ps1 -Mode CanaryOnly -LogPath "<evidence>\godot.log.current-iteration" -AuditPath "<evidence>\godot-log-current-iteration-audit.json" -ExpectedPackageVersion v0.1.0-private-beta.94 -ExpectedRitsuCompatBranch 0.107.1 -ExpectedRitsuLibVersion 0.4.31 -ExpectedGameVersion 0.107.1 -OutFile "<evidence>\enabled-mode-log-check.json" -FailOnMismatch'
...
Add-ContainsCheck -Name 'next_overnight_beta94_totals' -Text $nextOvernight -Needle 'Latest beta.94 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace check passed for the current dependency target; previous beta.93 RitsuLib-only Off/AdditiveBatch1 packet verification remains historical loader/registration evidence only.'
...
Add-ContainsCheck -Name 'ritsu_monthly_enabled_smokes_boundary' -Text $ritsuMonthlyDevSpec -Needle 'Previous beta.93 AdditiveBatch1 enabled-mode smoke is clean with 10 event types / 14 registration calls and retained verifier reports.'
...
Add-ContainsCheck -Name 'current_validation_historical_target_fix_old_shape_nonclaim' -Text $currentValidation -Needle 'Historical Off mode proved 0 StS1 registration lines; historical CanaryOnly proved exactly 4 canary content registrations for that source/runtime state. Retained beta.85 default-Off/CanaryOnly and beta.87 AdditiveBatch1 proof are previous `v0.107.0` loader context; previous beta.93 RitsuLib-only Off/AdditiveBatch1 proof is the active loader/registration truth'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.94 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.94 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.94.zip'
