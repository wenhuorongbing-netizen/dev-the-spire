...
$retiredFrameworkName = -join ([char[]](66, 97, 115, 101, 76, 105, 98))
...
$repoSts2Skill = Read-RepoText 'docs\skills\sts2-godot-mod-development.md'
...
$ritsuLocalizationFallbackRecord = Read-RepoText 'docs\archive\feature-audits\ritsulib-migration\batch-4c-candidates-20260623.md'
...
Add-Check -Name 'current_claim_scan_excludes_archived_ritsu_localization_record' -Passed (-not ($currentClaimFiles -contains 'docs\features\ritsulib-migration\batch-4c-candidates.md')) -Detail 'archived RitsuLib localization record must not be in stale claim scan scope'
...
Add-ContainsCheck -Name 'active_review_latest_addendum_beta93' -Text $activeReview -Needle 'Date: 2026-05-26; latest addendum: 2026-06-23'
Add-ContainsCheck -Name 'private_beta_handoff_current_beta96_date_boundary' -Text $privateBetaHandoff -Needle 'Date: 2026-06-23 beta.135 handoff summary; older May/June package notes below are retained only as historical context.'
...
Add-ContainsCheck -Name 'agents_current_beta133_package_state' -Text $agents -Needle 'Current beta.135 build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace validation are current for the 169/0 source inventory and 168 default compiled patch classes.'
Add-ContainsCheck -Name 'agents_current_patch_inventory_169_0' -Text $agents -Needle 'The source inventory is 169 migrated RitsuLib patch classes and 0 raw Harmony declarations; default beta.135 builds compile 168 patch classes because `Sts1ReplacementPrototype` is behind `REPLACEMENT_PROTOTYPE_ENABLED`.'
Add-ContainsCheck -Name 'agents_current_dependency_ritsulib_only' -Text $agents -Needle 'Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.34` installed'
Add-ContainsCheck -Name 'agents_current_off_not_live_ready' -Text $agents -Needle 'Do not claim live-ready or release-ready because gameplay, save-load, current enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff.'
Add-NoRegexCheck -Name 'agents_no_current_beta128_package_claim' -Paths @('AGENTS.md') -Pattern 'Installed beta\.128 build/publish/package refresh|Current beta\.128 package refresh|source inventory remains 153 migrated patch classes'
Add-NoRegexCheck -Name 'agents_no_current_prefer_retired_dependency_rule' -Paths @('AGENTS.md') -Pattern 'Prefer (?:previous package|retired dependency framework)|Install (?:previous package|retired dependency framework) `?v?3\.[0-9.]+`?|verify (?:previous package|retired dependency framework) and Spire Plus'
foreach ($entryDoc in @(
    @{ Name = 'agents_retired_framework_literal_absent'; Text = $agents },
    @{ Name = 'project_state_retired_framework_literal_absent'; Text = $projectState },
...
Add-ContainsCheck -Name 'agents_20260623_ritsu_latest_recheck' -Text $agents -Needle 'The 2026-06-23 NuGet/GitHub plus `dotnet list package --outdated --include-transitive` recheck found `0.4.34` as the latest stable `STS2.RitsuLib` package; no newer `STS2.RitsuLib` update is available.'
Add-ContainsCheck -Name 'goal_migration_20260623_ritsu_latest_recheck' -Text $goalMigrationDoc -Needle '2026-06-23: NuGet flat-container and `dotnet list package --outdated --include-transitive` show `STS2.RitsuLib` `0.4.34` as the latest package.'
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because gameplay, save-load, co-op, current enabled-mode registration/gameplay proof, independent QA rerun, and tester-package handoff decisions remain pending.'
Add-ContainsCheck -Name 'project_state_sts1_enabled_modes_boundary' -Text $projectState -Needle 'Previous beta.128 clicked Ancient UI smoke proof is `.tools/runtime-evidence/monkey-stability-20260623-062913/`: 4 / 4 `AncientUiSmoke` iterations passed for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`, with command ACKs, screenshots, clean log audits, StS1 Off verifier pass, exact game/Ritsu/package markers, no stale processes, 152/152 default runtime Spire Plus ModPatcher patches applied, and packet verification 1621 / 0.'
...
Add-ContainsCheck -Name 'root_readme_beta88_additive_path' -Text $rootReadme -Needle 'Installed beta.135 package artifacts and no-launch validation are current; beta.128 forced clicked Ancient UI smoke and beta.99 or older rows are previous-package evidence.'
Add-ContainsCheck -Name 'root_readme_historical_enabled_modes_only' -Text $rootReadme -Needle 'Historical loader proof used `v0.106.1`; the current local install is `v0.107.1`'
Add-ContainsCheck -Name 'root_readme_sts1_enabled_smokes_current_split' -Text $rootReadme -Needle 'previous beta.128 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260623-062913/`; 4 / 4 `AncientUiSmoke` iterations passed'
...
Add-ContainsCheck -Name 'doc_restructure_spec_current_boundary' -Text $docRestructureSpec -Needle 'This is the current documentation cleanup rule set, not an old move plan.'
Add-ContainsCheck -Name 'doc_restructure_spec_current_reading_path' -Text $docRestructureSpec -Needle 'For migration work, use `docs/features/ritsulib-migration/README.md` as the'
...
Add-ContainsCheck -Name 'beta_compat_revision_m_boundary' -Text $betaCompatibility -Needle '2026-06-23 Current Compatibility Boundary'
Add-ContainsCheck -Name 'beta_compat_beta99_target' -Text $betaCompatibility -Needle 'Spire Plus `v0.1.0-private-beta.135`'
Add-ContainsCheck -Name 'beta_compat_ritsulib_only_dependency' -Text $betaCompatibility -Needle 'only `STS2-RitsuLib >= 0.4.34` as the runtime dependency'
...
Add-ContainsCheck -Name 'remote_setup_current_boundary' -Text $remoteDevelopmentSetup -Needle '2026-06-23 Current Boundary'
...
Add-ContainsCheck -Name 'remote_setup_off_only_nonclaim' -Text $remoteDevelopmentSetup -Needle 'That proof covers startup, settings, loader registration, or smoke-level clicked UI only; remote setup, beta.135 runtime smoke, enabled modes, gameplay, save-load, replacement, multiplayer, QA, package handoff, and release-ready compatibility still require their own target-machine evidence.'
...
Add-ContainsCheck -Name 'goal_migration_beta96_off_loader_pass' -Text $goalMigrationDoc -Needle 'Previous beta.123/beta.99 settings/off proof, beta.96 Off proof, and beta.93 AdditiveBatch1 proof remain previous-package context only.'
...
Add-ContainsCheck -Name 'goal_migration_beta85_package_runtime_clean' -Text $goalMigrationDoc -Needle '`EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.34`'
Add-ContainsCheck -Name 'goal_migration_beta87_current_override' -Text $goalMigrationDoc -Needle 'Previous beta.128 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260623-062913/`'
Add-ContainsCheck -Name 'goal_debug_beta85_off_loader_nonclaim' -Text $goalDebugDoc -Needle 'loader proof'
Add-ContainsCheck -Name 'goal_debug_beta87_current_override' -Text $goalDebugDoc -Needle 'Beta.85/beta.86/beta.87 loader proof remains previous-package/game-version context, and beta.88/beta.93/beta.96/beta.99 proof remains previous-package context.'
...
Add-ContainsCheck -Name 'goal_event_archived_full_record' -Text $goalEventDoc -Needle 'Full archived record: `docs/archive/feature-audits/event-goal-full-20260622.md`.'
Add-ContainsCheck -Name 'goal_event_current_package_truth' -Text $goalEventDoc -Needle 'Current package truth is beta.135 on Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.34`; package parity, runtime preflight, and source-workspace validation are current no-launch evidence.'
Add-ContainsCheck -Name 'goal_event_ritsulib_only_dependency' -Text $goalEventDoc -Needle 'Spire Plus is RitsuLib-only. `EZMicroBalance.csproj` references `STS2.RitsuLib` `0.4.34`, and `EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.34` as the runtime dependency.'
Add-ContainsCheck -Name 'goal_event_source_and_ritsu_entry' -Text $goalEventDoc -Needle 'Future StS1 event work must start from RitsuLib docs/XML, active StS1 feature docs, and the unpacked local game source under `source code/src/Core/`.'
Add-ContainsCheck -Name 'goal_event_default_off_boundary' -Text $goalEventDoc -Needle 'The StS1 event prototype remains default Off.'
Add-ContainsCheck -Name 'goal_event_clicked_ui_smoke_boundary' -Text $goalEventDoc -Needle 'Latest clicked Ancient UI smoke remains previous beta.128 evidence at `.tools/runtime-evidence/monkey-stability-20260623-062913/`: 4 / 4 Urda, Morvi, Lotha, and normal Vakuu iterations'
Add-ContainsCheck -Name 'goal_event_smoke_nonclaim' -Text $goalEventDoc -Needle 'This is Ancient clicked-UI smoke evidence only. It does not close StS1 event gameplay, game-native AutoSlay batch proof, save-load, EN/ZHS runtime render, image/license/render, replacement functional behavior, multiplayer/fail-closed behavior, independent QA, release, or tester handoff gates.'
Add-ContainsCheck -Name 'goal_event_previous_packets_context_only' -Text $goalEventDoc -Needle 'Previous beta.99 settings/Off proof, beta.96 Off proof, beta.93 AdditiveBatch1 proof, beta.88 smoke-level UI proof, and beta.85-beta.90 rows are retained previous-package or previous-game-version context only.'
Add-ContainsCheck -Name 'goal_event_go_nogo_canary_current_4_6' -Text $goalEventDoc -Needle '4 event types / 6 registration calls.'
Add-ContainsCheck -Name 'goal_event_additive_10_14_boundary' -Text $goalEventDoc -Needle '10 event types / 14 registration calls.'
Add-ContainsCheck -Name 'goal_event_gate_map_route' -Text $goalEventDoc -Needle 'Current StS1 event work routes through `docs/features/sts1-events/v19-gate-evidence-map.md`, `docs/features/sts1-events/v19-gate-ledger.csv`, `docs/features/sts1-events/v20-final-gate-overlay.csv`, `docs/features/sts1-events/hard-stop-blocker-report-v20-coordination-pause-20260617.md`, `PROJECT_STATE.md`, and `docs/test-ready-development-goal.md`.'
Add-ContainsCheck -Name 'goal_event_v20_overlay_static_only' -Text $goalEventDoc -Needle 'The O0-O76 gate map and ledger remain the current runtime/static split. The O76-O84 final overlay is static-only and does not close runtime or handoff gates.'
...
Add-ContainsCheck -Name 'goal_event_autoslay_expected_ancient_ids_required_for_proof_mode' -Text $goalEventDoc -Needle 'Proof-mode AutoSlay commands must include `-ExpectedAncientIds VAKUU,URDA,MORVI,LOTHA`'
Add-ContainsCheck -Name 'goal_event_autoslay_expected_ancient_ids_nonclaim' -Text $goalEventDoc -Needle 'A clean static suite, current-doc scan, v19 ledger, v20 overlay, or subagent coverage run is no-launch governance evidence only.'
Add-ContainsCheck -Name 'goal_event_validation_pause_boundary' -Text $goalEventDoc -Needle 'Do not start overlapping validation lanes when a long `dotnet`, package, or game-runtime validation lane is already active.'
Add-ContainsCheck -Name 'goal_event_current_open_proof' -Text $goalEventDoc -Needle 'Capture current-package CanaryOnly and AdditiveBatch1 runtime proof before claiming current StS1 enabled-mode status.'
Add-ContainsCheck -Name 'goal_event_no_go_wording' -Text $goalEventDoc -Needle 'Do not write: all tasks complete, all StS1 events complete, full parity, gameplay-ready, release-ready'
...
Add-ContainsCheck -Name 'test_ready_goal_sts1_enabled_smokes_boundary' -Text $testReadyGoal -Needle 'The beta.85 Off/CanaryOnly, beta.86 AdditiveBatch1, beta.87 AdditiveBatch1 `v0.107.0`, beta.88 previous package `v0.107.1`, beta.90 RitsuLib-only packets, beta.93 AdditiveBatch1, beta.96 Off, beta.99 settings/Off rows, and beta.123 clicked UI rows remain previous-package or previous package-context evidence.'
Add-ContainsCheck -Name 'test_ready_goal_direct_localization_nonproof' -Text $testReadyGoal -Needle 'This is previous-package smoke-level clicked UI proof only. It does not prove beta.135 gameplay, save-load, replacement behavior, co-op, or release readiness.'
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package target is `publish/SpirePlus-v0.1.0-private-beta.135.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run for the STS2-RitsuLib `v0.4.34` target.'
...
Add-ContainsCheck -Name 'root_issues_sts1_enabled_smokes_boundary' -Text $rootIssues -Needle 'beta.135 package parity, runtime preflight 28 / 0, and source-workspace validation 57 / 0 are current for the 169/0 source state and 168 default compiled patch classes.'
Add-ContainsCheck -Name 'root_issues_clicked_ui_previous_package_boundary' -Text $rootIssues -Needle 'Previous beta.128 forced clicked Ancient UI smoke remains previous-package evidence'
Add-ContainsCheck -Name 'active_review_sts1_enabled_smokes_boundary' -Text $activeReview -Needle 'Previous enabled-mode packets remain historical or previous-package context and do not close current beta.135 enabled-mode gameplay proof.'
Add-ContainsCheck -Name 'active_review_direct_localization_nonproof' -Text $activeReview -Needle 'Direct Golden Idol localization cleanup remains a localization-only note; it does not prove gameplay or replace verifier reports.'
Add-ContainsCheck -Name 'toreview_sts1_enabled_smokes_current_split' -Text $toReview -Needle 'Latest validation snapshot: beta.135 build, publish/package refresh, installed-package parity, runtime preflight, and source-workspace check are the current package target; previous beta.128 forced clicked Ancient UI smoke is retained separately.'
Add-ContainsCheck -Name 'toreview_direct_localization_nonproof' -Text $toReview -Needle 'Latest clicked UI proof remains previous beta.128 evidence at `.tools/runtime-evidence/monkey-stability-20260623-062913/` with 4 / 4 iterations, clean audit, StS1 Off verifier pass, exact game/Ritsu/package markers, 152/152 default runtime Spire Plus patches from that package applied, and packet verifier 1621 / 0.'
...
Add-ContainsCheck -Name 'ritsu_integration_localization_fallback_migrated_source_only' -Text $ritsuIntegrationDoc -Needle 'Ascension localization fallback: six patch classes now use `IPatchMethod`'
...
Add-ContainsCheck -Name 'ritsu_localization_owner_decision_recorded' -Text $ritsuLocalizationFallbackRecord -Needle 'Owner decision recorded: 2026-06-22 continuation goal approved migrating the remaining six localization fallback candidates.'
Add-ContainsCheck -Name 'ritsu_localization_release_blocked' -Text $ritsuLocalizationFallbackRecord -Needle 'Release-ready remains blocked by gameplay, screenshot, save-load, image/render, replacement, multiplayer, independent QA, and tester-package handoff evidence.'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_beta99_target' -Text $ritsuRuntimeHardBlock -Needle 'Spire Plus `v0.1.0-private-beta.135`'
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_settings_ui_path' -Text $ritsuRuntimeHardBlock -Needle '.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/'
...
Add-ContainsCheck -Name 'ritsu_runtime_hard_block_manual_gates_pending' -Text $ritsuRuntimeHardBlock -Needle 'current beta.135 enabled-mode'
...
Add-ContainsCheck -Name 'historical_revision_boundaries_revision_n_current_truth' -Text $historicalRevisionBoundaries -Needle 'Current migration truth is beta.135 RitsuLib-only'
...
Add-ContainsCheck -Name 'scripts_readme_analyzer_direct_smoke_dependency_framework_drift' -Text $scriptsReadme -Needle 'Bound previous package dirty-audit signatures such as `dependency patch failure` and `[ERROR] [previous package]` route to `PackageRuntimeDrift`'
Add-ContainsCheck -Name 'scripts_readme_analyzer_direct_smoke_dependency_framework_patch_details' -Text $scriptsReadme -Needle '`dependency patch failures` details such as `AdjustCustomMessageKeys::Fuckery()` undefined target-method failures'
Add-ContainsCheck -Name 'scripts_readme_analyzer_direct_smoke_dependency_framework_relic_collection_detail' -Text $scriptsReadme -Needle '`NRelicCollectionCategory::LoadRelics` instruction matcher failures'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.135'
...
Add-ContainsCheck -Name 'current_validation_autoslay_expected_ancient_ids_plan_summary' -Text $currentValidation -Needle 'retained `autoslay-plan.json` `ExpectedAncientIds`, retained `autoslay-summary.json`, and sidecar-plus-current-log event traversal must bind to the same Ancient id'
Add-ContainsCheck -Name 'review_autoslay_expected_ancient_ids_plan_summary' -Text $activeReview -Needle 'Proof-mode packets still require `-ExpectedAncientIds` to match retained plan/summary/traversal state'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_analyzer_direct_smoke_dependency_framework_drift' -Text $runtimeMonkeyDocs -Needle '`dependency patch failure` and `[ERROR] [previous package]` to `PackageRuntimeDrift`'
Add-ContainsCheck -Name 'runtime_monkey_docs_analyzer_direct_smoke_dependency_framework_patch_details' -Text $runtimeMonkeyDocs -Needle '`dependency patch failures` array records'
Add-ContainsCheck -Name 'runtime_monkey_docs_analyzer_direct_smoke_dependency_framework_relic_collection_detail' -Text $runtimeMonkeyDocs -Needle '`NRelicCollectionCategory::LoadRelics` instruction'
...
Add-ContainsCheck -Name 'runtime_failure_analyzer_routes_dependency_framework_errors_to_package_drift' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'scripts\analyze-spire-plus-runtime-failure.ps1') -Raw -Encoding UTF8) -Needle '(?:previous package|dependency framework).*(?:HarmonyException|Patching exception|patch(?:ing)? exception|failed)'
Add-ContainsCheck -Name 'runtime_failure_analyzer_extracts_dependency_framework_patch_failures' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'scripts\analyze-spire-plus-runtime-failure.ps1') -Raw -Encoding UTF8) -Needle 'function Get-DependencyFrameworkFailureDetails'
Add-ContainsCheck -Name 'runtime_failure_analyzer_reports_dependency_framework_patch_failures' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'scripts\analyze-spire-plus-runtime-failure.ps1') -Raw -Encoding UTF8) -Needle 'DependencyFrameworkFailures = @($dependencyFrameworkFailures)'
...
Add-ContainsCheck -Name 'runtime_monkey_tests_cover_direct_smoke_dependency_framework_patch_details' -Text (Get-Content -LiteralPath (Resolve-RepoPath 'tests\EZMicroBalance.Tests\RuntimeMonkeyStabilityGuardTests.DirectSmokeAnalyzer.cs') -Raw -Encoding UTF8) -Needle 'DependencyFrameworkFailures'
...
Add-ContainsCheck -Name 'static_file_hygiene_scans_archived_ritsu_localization_record' -Text $staticFileHygieneScript -Needle "'docs\archive\feature-audits\ritsulib-migration\batch-4c-candidates-20260623.md'"
...
Add-ContainsCheck -Name 'static_file_hygiene_guards_archived_ritsu_localization_scope' -Text $staticFileHygieneScript -Needle "sts1_hygiene_scans_archived_ritsu_localization_record"
...
Add-ContainsCheck -Name 'runtime_checklist_beta99_target' -Text $runtimeChecklist -Needle 'Spire Plus `v0.1.0-private-beta.135`'
Add-ContainsCheck -Name 'runtime_checklist_settings_ui_path' -Text $runtimeChecklist -Needle '.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/'
Add-ContainsCheck -Name 'runtime_checklist_previous_beta96_loader_retained' -Text $runtimeChecklist -Needle 'The previous beta.96 Off packet is retained at'
Add-Check -Name 'runtime_checklist_no_previous_package_setup' -Passed (-not ($runtimeChecklist -match '(?i)Install previous[- ]package|Enable previous[- ]package|requires previous[- ]package|old dependency package')) -Detail 'runtime smoke checklist must not route current validation through the old dependency package'
...
Add-ContainsCheck -Name 'runtime_exit_status_beta99_settings_pass' -Text $runtimeChecklist -Needle 'Current exit status: beta.135 package parity, runtime preflight, and'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.135 package and STS2-RitsuLib `v0.4.34` direct NuGet runtime are installed and recapture current-package enabled-mode proof.'
...
Add-ContainsCheck -Name 'ritsu_monthly_routes_localization_migration_to_record' -Text $ritsuMonthlyDevSpec -Needle 'completed low-risk localization migration'
Add-ContainsCheck -Name 'ritsu_monthly_open_proof_rows_remain_open' -Text $ritsuMonthlyDevSpec -Needle 'gameplay, enabled-mode, save-load, replacement, co-op, QA, release readiness,'
Add-Check -Name 'ritsu_monthly_no_copied_runtime_evidence_paths' -Passed (-not ($ritsuMonthlyDevSpec -match '\.tools[/\\]runtime-evidence[/\\]')) -Detail 'monthly-dev-spec.md must remain a routing stub, not a runtime evidence ledger'
Add-ContainsCheck -Name 'ritsu_localization_requires_169_runtime_recapture' -Text $ritsuLocalizationFallbackRecord -Needle 'Before citing the current source migration as runtime coverage, capture a fresh package/runtime packet that proves the current source inventory and the 168 default compiled Spire Plus patch classes apply in the installed game.'
Add-ContainsCheck -Name 'ritsu_localization_event_runtime_claim_requires_enabled_smokes' -Text $ritsuLocalizationFallbackRecord -Needle 'Before any localization-migration follow-up is cited as StS1 event runtime readiness, cite current enabled-mode evidence with retained verifier reports and add the missing gameplay evidence, or state that the localization fallback claim is unrelated to StS1Events enabled modes.'
...
Add-ContainsCheck -Name 'sts1_goal_compile_package_current_split' -Text $sts1FeatureGoal -Needle 'Events must compile against the current repository package reference: `STS2.RitsuLib` `0.4.34`; Spire Plus must not require previous package.'
Add-ContainsCheck -Name 'sts1_goal_runtime_ritsu_variant_split' -Text $sts1FeatureGoal -Needle 'Current runtime reproof targets the installed official `STS2-RitsuLib` `v0.4.34` direct NuGet runtime layout.'
...
Add-ContainsCheck -Name 'gate_ledger_checker_o22_ritsulib_requirement' -Text $gateLedgerCheckerScript -Needle "GateId = 'O22'; Field = 'requirement'; Needle = 'STS2-RitsuLib v0.4.34 direct NuGet runtime layout installed'"
...
Add-ContainsCheck -Name 'hard_stop_latest_addendum_date' -Text $hardStop -Needle 'Date: 2026-06-11; latest addendum: 2026-06-22'
Add-ContainsCheck -Name 'hard_stop_beta93_supersession_note' -Text $hardStop -Needle 'Supersession note, 2026-06-23: this report remains the historical v19 coordination-blocker trace.'
...
Add-ContainsCheck -Name 'hard_stop_monthly_localization_record_enabled_before_gameplay_scope' -Text $hardStop -Needle '`docs/features/ritsulib-migration/monthly-dev-spec.md` and `docs/features/ritsulib-migration/batch-4c-candidates.md` now keep fresh current CanaryOnly/AdditiveBatch1 enabled-mode smoke proof ahead of StS1 gameplay, handoff, or event-runtime claims.'
...
Add-ContainsCheck -Name 'current_validation_compact_status' -Text $currentValidation -Needle 'Status: compact active validation summary.'
Add-ContainsCheck -Name 'current_validation_archived_full_record' -Text $currentValidation -Needle 'Full archived record: `docs/archive/feature-audits/current-validation-full-20260622.md`.'
Add-ContainsCheck -Name 'current_validation_current_target_beta105' -Text $currentValidation -Needle 'Current package target is Spire Plus `v0.1.0-private-beta.135` on Slay the Spire 2 `v0.107.1`.'
Add-ContainsCheck -Name 'current_validation_ritsu_only_dependency' -Text $currentValidation -Needle '`EZMicroBalance.csproj` references `STS2.RitsuLib` `0.4.34`; `EZMicroBalance.json` declares only `STS2-RitsuLib >= 0.4.34` as the runtime dependency.'
Add-ContainsCheck -Name 'current_validation_source_workspace_entry' -Text $currentValidation -Needle 'The unpacked local game source under `source code/src/Core/` is the primary API authority for game behavior. RitsuLib docs/XML are the modding API authority.'
Add-ContainsCheck -Name 'current_validation_20260622_ritsu_latest' -Text $currentValidation -Needle 'NuGet flat-container `https://api.nuget.org/v3-flatcontainer/sts2.ritsulib/index.json` still reports `0.4.34` as the latest `STS2.RitsuLib` package across 165 listed versions.'
Add-ContainsCheck -Name 'current_validation_20260623_github_recheck' -Text $currentValidation -Needle 'GitHub release API reports `v0.4.34` / `0.4.34` published on 2026-06-22'
Add-ContainsCheck -Name 'current_validation_no_ritsu_update' -Text $currentValidation -Needle '`dotnet list EZMicroBalance.csproj package --outdated --include-transitive` found no newer `STS2.RitsuLib`; it reported only transitive `System.IO.Hashing 9.0.0 -> 10.0.9`.'
Add-ContainsCheck -Name 'current_validation_clicked_ui_smoke_current' -Text $currentValidation -Needle 'Previous beta.128 clicked Ancient UI smoke proof passed at `.tools/runtime-evidence/monkey-stability-20260623-062913/`: 4 / 4 `AncientUiSmoke` iterations for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`'
Add-ContainsCheck -Name 'current_validation_smoke_boundary' -Text $currentValidation -Needle 'The previous beta.128 clicked Ancient UI proof covers forced Urda, Morvi, Lotha, and normal Vakuu UI only for that package; it does not prove beta.135 gameplay, save-load, gated Vakuu fight-option/victory return, co-op, or release readiness.'
Add-ContainsCheck -Name 'current_validation_off_canary_proof_not_extended' -Text $currentValidation -Needle 'It does not prove event encounter gameplay, gated Vakuu fight-option/victory return, live gameplay, save-load, image rendering, replacement functional behavior, multiplayer fail-closed behavior, independent QA, release handoff, live-ready, or private-beta release readiness.'
Add-ContainsCheck -Name 'current_validation_previous_packets_context_only' -Text $currentValidation -Needle 'Previous beta.99 settings/Off proof, beta.96 Off proof, beta.93 AdditiveBatch1 proof, and beta.85-beta.90 rows are retained previous-package or previous-game-version context only.'
...
Add-ContainsCheck -Name 'hard_stop_v20_blocked_o25_o33' -Text $hardStopV20 -Needle '`O25`: retained `v0.107.0` CanaryOnly enabled-mode smoke has since been captured by the shared validation lane and is loader proof only; this thread did not create it. Previous beta.99/beta.96 loader/settings proof exists on `v0.107.1` as previous-package startup/settings proof, but current beta.135 CanaryOnly enabled-mode proof still needs recapture.'
...
Add-ContainsCheck -Name 'current_validation_static_suite_15' -Text $currentValidation -Needle 'expected summary is 15 static steps, 0 suite failures'
...
Add-ContainsCheck -Name 'current_validation_open_proof_rows' -Text $currentValidation -Needle 'Gameplay, gated Vakuu fight-option UI, Vakuu victory return/no-black-screen, save-load, replacement behavior, co-op/fail-closed proof, current enabled-mode gameplay, independent QA, and final tester handoff remain open.'
...
Add-ContainsCheck -Name 'handoff_sts1_enabled_smokes_current_split' -Text $privateBetaHandoff -Needle 'Retained AdditiveBatch1 enabled-mode proof remains previous-package loader/registration evidence only and does not prove beta.135 gameplay.'
...
Add-ContainsCheck -Name 'release_evidence_status_beta99_off_nonclaim' -Text $releaseEvidenceStatus -Needle 'beta.99 RitsuLib settings/off proof is previous-package context after beta.135. Current beta.135 covers package parity, runtime preflight, and source-workspace identity; previous beta.128 covers smoke-level clicked Ancient UI for Urda, Morvi, Lotha, and normal Vakuu. Gameplay, gated Vakuu fight-option/victory return, save-load, preview-tools live behavior, current enabled-mode proof, co-op, and full release-evidence packaging rows remain pending.'
...
Add-ContainsCheck -Name 'private_beta_release_audit_loader_only' -Text $privateBetaReleaseAudit -Needle 'Pass for previous beta.128 startup/UI smoke; beta.135 runtime smoke and gameplay evidence pending'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.135 RitsuLib-only package refresh/no-launch validation: 2026-06-23; previous beta.128 clicked UI smoke: 2026-06-23; previous beta.123, beta.99, beta.96, and beta.93 proof remains historical loader/registration/settings context only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.135 RitsuLib-only package refresh/no-launch validation: 2026-06-23; previous beta.128 clicked UI smoke: 2026-06-23; previous beta.123, beta.99, beta.96, and beta.93 evidence remains historical loader/registration/settings proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_beta85_loader_nonclaim' -Text $testReadyCompletionAudit -Needle 'These hashes and earlier loader proof do not close live gameplay, save-load, death/failure path, or co-op verification.'
Add-ContainsCheck -Name 'test_ready_completion_audit_manual_rows_pending' -Text $testReadyCompletionAudit -Needle 'Live gameplay, save-load, death/failure-path, and co-op remain pending; previous beta.128 clicked Ancient UI smoke is recorded separately.'
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.135.zip'
...
Add-NoRegexCheck -Name 'no_current_migration_residue_from_retired_dependency_or_old_claim_counts' -Paths $currentClaimFiles -Pattern 'current-doc claims (1316|1320|1324|1325|1326|1328) / 0|current-doc claims passed (1316|1320|1324|1325|1326|1328) / 0|returned (1316|1320|1324|1325|1326|1328) checks / 0 mismatches|(?:previous package|retired dependency framework) and RitsuLib runtime dependency|(?:previous package|retired dependency framework) \+ RitsuLib runtime dependency|manifest [^\r\n]*(?:previous package|retired dependency framework)[^\r\n]*RitsuLib runtime dependency'
...
Add-NoRegexCheck -Name 'no_next_runtime_untracked_localization_record_claim' -Paths @('docs\archive\feature-audits\ritsulib-migration\next-runtime-qa-run-20260623.md') -Pattern 'untracked `docs/features/ritsulib-migration/batch-4c-candidates\.md`'
