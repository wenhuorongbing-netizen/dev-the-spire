...
Add-ContainsCheck -Name 'project_state_current_off_non_claim' -Text $projectState -Needle 'Do not claim live-ready or release-ready because beta.96 loader, gameplay, clicked Ancient UI, save-load, co-op, independent QA rerun, and tester-package handoff decisions remain pending.'
...
Add-ContainsCheck -Name 'restructure_doc_current_ritsulib_only_target' -Text $restructureDoc -Needle 'Current package/runtime target is Spire Plus `v0.1.0-private-beta.96` on Slay'
...
Add-ContainsCheck -Name 'migration_doc_remaining_runtime_blockers' -Text $migrationDoc -Needle 'Gameplay, event screenshots, save-load, image/render, replacement functional proof, co-op/fail-closed proof, independent QA, beta.96 loader proof, and tester-package handoff remain pending; handoff must recapture HEAD and worktree status after any later edits.'
...
Add-ContainsCheck -Name 'goal_event_beta93_previous_loader_boundary' -Text $goalEventDoc -Needle 'previous loader truth is beta.93 on Slay the Spire 2 `v0.107.1`: `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` reached main menu with exactly STS2-RitsuLib `v0.4.31` and Spire Plus `v0.1.0-private-beta.93` loaded, only STS2-RitsuLib as the shared runtime dependency, 25/25 Spire Plus patches applied, clean audits, Off packet verifier 43 / 0, AdditiveBatch1 enabled-mode verifier 31 / 0, and AdditiveBatch1 packet verifier 61 / 0. Current beta.96 loader proof is pending.'
...
Add-ContainsCheck -Name 'goal_event_latest_beta94_governance_checkpoint' -Text $goalEventDoc -Needle 'Latest beta.96 governance checkpoint after the gate-ledger/current-doc cleanup: `scripts/check-sts1-event-current-doc-claims.ps1 -FailOnMismatch` returned 1331 checks / 0 mismatches'
...
Add-ContainsCheck -Name 'goal_event_source_workspace_checkpoint_beta93' -Text $goalEventDoc -Needle 'source workspace: 59 checks / 0 mismatches against installed v0.107.1 and STS2-RitsuLib 0.4.31 / compat 0.107.1'
...
Add-ContainsCheck -Name 'test_ready_goal_beta94_preflight_drift' -Text $testReadyGoal -Needle 'The current retained package/runtime evidence target is `publish/SpirePlus-v0.1.0-private-beta.96.zip`; build, publish, package refresh, installed-package check, runtime preflight, and source-workspace check have run'
...
Add-ContainsCheck -Name 'runtime_monkey_docs_autoslay_current_package_target' -Text $runtimeMonkeyDocs -Needle '-ExpectedPackageVersion v0.1.0-private-beta.96'
...
Add-ContainsCheck -Name 'next_overnight_beta94_totals' -Text $nextOvernight -Needle 'Latest beta.96 no-game/package validation is summarized in `PROJECT_STATE.md` and `docs/reviews/current-validation.md`: build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace check passed for the current dependency target; previous beta.93 RitsuLib-only Off/AdditiveBatch1 packet verification remains historical loader/registration evidence only.'
...
Add-ContainsCheck -Name 'next_overnight_head_validation_beta87' -Text $nextOvernight -Needle 'Current HEAD validation refreshed after the beta.96 RitsuLib-only settings UI pass and runtime packet-checker hardening; recheck again before handoff if any later edits appear.'
...
Add-ContainsCheck -Name 'next_overnight_batch1_smoke_step_before_gameplay' -Text $nextOvernight -Needle 'Before any AdditiveBatch1 gameplay claim, confirm the beta.96 package and STS2-RitsuLib `v0.4.31` / `lib\0.107.1` runtime are installed and recapture current-package enabled-mode proof'
...
Add-ContainsCheck -Name 'private_beta_release_audit_current_beta94_date_boundary' -Text $privateBetaReleaseAudit -Needle 'Original audit date: 2026-05-15; latest beta.96 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.'
Add-ContainsCheck -Name 'test_ready_completion_audit_current_beta94_date_boundary' -Text $testReadyCompletionAudit -Needle 'Original audit date: 2026-05-14; latest beta.96 RitsuLib-only package refresh: 2026-06-21; previous beta.93 loader evidence remains historical loader/registration proof only.'
...
Add-ContainsCheck -Name 'platform_testing_current_beta85_zip' -Text $platformTesting -Needle 'publish/SpirePlus-v0.1.0-private-beta.96.zip'
