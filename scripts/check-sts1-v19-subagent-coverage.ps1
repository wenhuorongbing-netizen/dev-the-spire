...
Add-ContainsCheck -Name 'coverage_additive_retained_loader_packet' -Text $coverageText -Needle 'Current beta.92 AdditiveBatch1 packet at `.tools/runtime-evidence/v01071-beta92-ritsulib0429-additivebatch1-direct-20260621/`'
...
Add-Check -Name 'additive_loader_gate_current_pass' -Passed ($additiveLoaderRow.Count -eq 1 -and $additiveLoaderRow[0].current_status -eq 'current-pass') -Detail 'O33 must remain current-pass for beta.92 loader/registration proof'
