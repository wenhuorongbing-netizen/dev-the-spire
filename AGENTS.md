...
- Current dependency configurations are aligned on STS2-RitsuLib `v0.4.32` and Slay the Spire 2 `v0.107.1`. Installed beta.99 package/hash parity, runtime preflight, and source-workspace validation are recorded on 2026-06-21; beta.99 RitsuLib Mod Settings clicked UI proof and Off loader proof remain pending.
  Previous beta.96 RitsuLib Mod Settings clicked UI proof is captured under `.tools/runtime-evidence/beta96-ritsulib-mod-settings-clicked-ui-20260621-160701/`, previous beta.96 RitsuLib-only Off proof is captured under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`, and beta.93 RitsuLib-only AdditiveBatch1 direct proof remains previous-package loader/registration evidence only.
...
- Controlled `--force-steam off` smoke loading exists for the previous beta.96 package only; beta.99 Off loader proof and normal Steam-client startup/log verification must be recaptured after the RitsuLib settings-page I18N resource migration.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.32` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` from the NuGet package deploy target, with root `STS2-RitsuLib.dll` SHA256 `1412F0148F6A52077CA26617F9FA5499517C5BFD5337EBA8DBF216FA1E517F47`. The previous `v0.4.31` variant-pack install is backed up under `%TEMP%\codex-sts2-ritsulib-backup-20260621-212257`. Current beta.99 package parity, runtime preflight, and source-workspace validation are recorded, but beta.99 RitsuLib Mod Settings clicked UI proof and beta.99 Off loader proof remain pending.
  Previous beta.96 RitsuLib Mod Settings clicked UI proof reached the game settings screen with only `STS2-RitsuLib` and `EZMicroBalance` enabled, opened Settings -> `Mod Settings (RitsuLib)`, showed only `RitsuLib` and `Spire Plus` in the RitsuLib Mods tree, rendered the Spire Plus Migration Status / runtime dependency / evidence-boundary / Preview Tools page, retained the same-session `godot.log`, and audited clean for earlier settings-page visibility only. Previous beta.96 Off proof exists under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`: direct `--force-steam off` reached main menu with exactly RitsuLib and Spire Plus loaded, applied 25/25 Spire Plus patches, audited clean, logged StS1Events disabled with 0 registration lines, and passed packet verification 43 / 0. Previous beta.93 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` as previous-package loader/registration evidence only. Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, current beta.99 enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
