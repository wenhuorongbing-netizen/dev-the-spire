...
- Current dependency configurations are aligned on STS2-RitsuLib `v0.4.33` and Slay the Spire 2 `v0.107.1`. The 2026-06-22 NuGet flat-container and `dotnet list package --outdated --include-transitive` recheck found no newer `STS2.RitsuLib` package, and the current source-workspace recheck passed 58 checks / 0 mismatches including local RitsuLib XML/API marker coverage. Installed beta.105 package/hash parity, runtime preflight, source-workspace validation, and clicked Ancient UI smoke proof are current.
  Current beta.105 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260622-025733/`: 4 / 4 `AncientUiSmoke` iterations passed for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`, with command ACKs, screenshots, clean log audits, StS1 Off verifier pass, exact game/Ritsu/package markers, and packet verification 1621 / 0. Previous beta.99 RitsuLib Mod Settings clicked UI proof, previous beta.99 RitsuLib-only Off proof, and beta.93 RitsuLib-only AdditiveBatch1 direct proof remain previous-package loader/settings/registration evidence only.
...
- Current controlled runtime smoke proof is beta.105 clicked Ancient UI smoke for forced UI paths only; current enabled-mode and gameplay proof remain pending.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.33` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` from the NuGet package deploy target, with root `STS2-RitsuLib.dll` SHA256 `8575756BB53BA75015F8686361C9CD9E2FE47CFE20E0C6FAE291C8F429555D14`. The previous `v0.4.32` runtime install is backed up under `%TEMP%\codex-sts2-ritsulib-backup-0.4.32-20260622-025408`. Current beta.105 package parity, runtime preflight, source-workspace validation, and clicked Ancient UI smoke proof are recorded.
  Current beta.105 clicked Ancient UI smoke under `.tools/runtime-evidence/monkey-stability-20260622-025733/` reached the game UI with only `STS2-RitsuLib` and `EZMicroBalance` enabled, applied 25/25 Spire Plus patches in the installed package, audited clean, logged StS1Events disabled, passed the Off verifier, and passed packet verification 1621 / 0 after forcing Urda, Morvi, Lotha, and normal Vakuu UI selections. Later source-only RitsuLib UI/input migration records 46 migrated patch classes and 125 raw Harmony declarations; publish/package/runtime proof must be recaptured before using that HEAD for tester handoff. Previous beta.99 RitsuLib Mod Settings clicked UI proof and previous beta.99 Off loader proof remain previous-package context; previous beta.93 AdditiveBatch1 loader/registration proof remains previous-package loader/registration evidence only. Do not claim live-ready or release-ready because gameplay, save-load, current beta.105 enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
...
- Install STS2-RitsuLib `v0.4.33` under `<GameRoot>\mods\STS2-RitsuLib`. The current local lane uses the official NuGet package deploy target with a root runtime DLL; an official variant-pack layout is also acceptable when its manifest version matches the package target.
