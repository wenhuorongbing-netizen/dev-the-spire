...
- Current controlled runtime smoke proof is beta.107 clicked Ancient UI smoke for forced UI paths only; current enabled-mode and gameplay proof remain pending.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.33` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` from the NuGet package deploy target, with root `STS2-RitsuLib.dll` SHA256 `8575756BB53BA75015F8686361C9CD9E2FE47CFE20E0C6FAE291C8F429555D14`. The previous `v0.4.32` runtime install is backed up under `%TEMP%\codex-sts2-ritsulib-backup-0.4.32-20260622-025408`. Current beta.107 package parity, runtime preflight, source-workspace validation, and clicked Ancient UI smoke proof are recorded.
  Current beta.107 clicked Ancient UI smoke under `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/` reached the game UI with only `STS2-RitsuLib` and `EZMicroBalance` enabled, applied all 46 Spire Plus patches in the installed package, audited clean, logged StS1Events disabled, passed the Off verifier, and passed packet verification 1620 / 0 after forcing Urda, Morvi, Lotha, and normal Vakuu UI selections. The current source records 46 migrated RitsuLib `IPatchMethod` patch classes and 125 raw Harmony declarations; this package/runtime recapture covers that current migration state. Previous beta.99 RitsuLib Mod Settings clicked UI proof and previous beta.99 Off loader proof remain previous-package context; previous beta.93 AdditiveBatch1 loader/registration proof remains previous-package loader/registration evidence only. Do not claim live-ready or release-ready because gameplay, save-load, current beta.107 enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
