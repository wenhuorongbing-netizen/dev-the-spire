...
- Controlled `--force-steam off` smoke loading exists for the current beta.96 package; normal Steam-client startup/log verification remains earlier-package context only unless recaptured.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.31` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` with `lib\0.107.1\STS2-RitsuLib.dll`. Current beta.96 RitsuLib Mod Settings clicked UI proof reached the game settings screen with only `STS2-RitsuLib` and `EZMicroBalance` enabled, opened Settings -> `Mod Settings (RitsuLib)`, showed only `RitsuLib` and `Spire Plus` in the RitsuLib Mods tree, rendered the Spire Plus Migration Status / runtime dependency / evidence-boundary / Preview Tools page, retained the same-session `godot.log`, and audited clean. Current beta.96 Off proof exists under `.tools/runtime-evidence/v01071-beta96-ritsulib0431-off-direct-20260621-185056/`: direct `--force-steam off` reached main menu with exactly RitsuLib and Spire Plus loaded, applied 25/25 Spire Plus patches, audited clean, logged StS1Events disabled with 0 registration lines, and passed packet verification 43 / 0. Previous beta.93 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` as previous-package loader/registration evidence only. Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, current beta.96 enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
