...
- Current dependency configurations are aligned on STS2-RitsuLib `v0.4.32` and Slay the Spire 2 `v0.107.1`. The 2026-06-22 NuGet flat-container and `dotnet list package --outdated --include-transitive` recheck found no newer `STS2.RitsuLib` package, and the source-workspace recheck passed 57 checks / 0 mismatches. Installed beta.99 package/hash parity, runtime preflight, source-workspace validation, beta.99 RitsuLib Mod Settings clicked UI proof, and beta.99 direct Off loader proof are recorded.
  Current beta.99 RitsuLib Mod Settings clicked UI proof is captured under `.tools/runtime-evidence/mod-settings-beta99-ritsulib-click-20260621-223210/`, current beta.99 RitsuLib-only Off proof is captured under `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/`, and beta.93 RitsuLib-only AdditiveBatch1 direct proof remains previous-package loader/registration evidence only.
...
- Controlled beta.99 `--force-steam off` smoke loading is current for startup/loading and default-Off StS1Events behavior only; current enabled-mode and gameplay proof remain pending.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.32` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` from the NuGet package deploy target, with root `STS2-RitsuLib.dll` SHA256 `1412F0148F6A52077CA26617F9FA5499517C5BFD5337EBA8DBF216FA1E517F47`. The previous `v0.4.31` variant-pack install is backed up under `%TEMP%\codex-sts2-ritsulib-backup-20260621-212257`. Current beta.99 package parity, runtime preflight, source-workspace validation, RitsuLib Mod Settings clicked UI proof, and direct Off loader proof are recorded.
  Current beta.99 RitsuLib Mod Settings clicked UI proof reached the game settings screen with only `STS2-RitsuLib` and `EZMicroBalance` enabled, opened Settings -> `Mod Settings (RitsuLib)`, showed only `RitsuLib` and `Spire Plus` in the RitsuLib Mods tree, rendered the Spire Plus Migration Status / `STS2-RitsuLib >= 0.4.32` runtime dependency / evidence-boundary / Preview Tools page, retained the same-session `godot.log`, audited clean, and passed StS1 Off runtime shape verification 21 / 0. Current beta.99 Off proof exists under `.tools/runtime-evidence/v01071-beta99-ritsulib0432-off-direct-20260621-234221/`: direct `--force-steam off` reached main menu with exactly RitsuLib and Spire Plus loaded, applied 25/25 Spire Plus patches, audited clean, logged StS1Events disabled with 0 registration lines, passed Off verifier 21 / 0, and passed packet verification 43 / 0. Previous beta.93 AdditiveBatch1 loader/registration proof exists under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/` as previous-package loader/registration evidence only. Do not claim live-ready or release-ready because gameplay, clicked Ancient UI, save-load, current beta.99 enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
...
- Install STS2-RitsuLib `v0.4.32` under `<GameRoot>\mods\STS2-RitsuLib`. The current local lane uses the official NuGet package deploy target with a root runtime DLL; an official variant-pack layout is also acceptable when its manifest version matches the package target.
