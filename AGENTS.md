...
- Current local source was refreshed from Slay the Spire 2 `v0.106.1`; BaseLib runtime/project package are aligned on `v3.1.4`. The current installed game is `v0.107.0`; official `STS2-RitsuLib` `v0.4.16` with `lib\0.107.0` is installed. Installed beta.85 package parity passed on 2026-06-11, and fresh beta.85 Off loader proof is clean for startup/default-Off patch application.
