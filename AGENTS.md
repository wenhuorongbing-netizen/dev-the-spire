...
- Current dependency configurations are aligned on STS2-RitsuLib `v0.4.34` and Slay the Spire 2 `v0.107.1`. The 2026-06-23 NuGet/GitHub plus `dotnet list package --outdated --include-transitive` recheck found `0.4.34` as the latest stable `STS2.RitsuLib` package; no newer `STS2.RitsuLib` update is available. Current beta.135 build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace validation are current for the 169/0 source inventory and 168 default compiled patch classes.
  Previous beta.128 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260623-062913/`: 4 / 4 `AncientUiSmoke` iterations passed for `URDA`, `MORVI`, `LOTHA`, and normal `VAKUU`, with command ACKs, screenshots, clean log audits, StS1 Off verifier pass, exact game/Ritsu/package markers, 152/152 default runtime Spire Plus ModPatcher patches applied from that package, and packet verification 1621 / 0. This is previous-package smoke-level clicked UI proof only. Previous beta.99 RitsuLib Mod Settings clicked UI proof, previous beta.99 RitsuLib-only Off proof, and beta.93 RitsuLib-only AdditiveBatch1 direct proof remain previous-package loader/settings/registration evidence only.
...
- Current retained runtime smoke proof is previous beta.128 clicked Ancient UI smoke for forced UI paths only; current beta.135 runtime smoke, enabled-mode proof, and gameplay proof remain pending.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.34` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` from the NuGet package deploy target, with root `STS2-RitsuLib.dll` SHA256 `47FED8A7071E71497C4C2AB3357D2B779B864B1EB7BC5CFB3E73BB8C4123B088` and `STS2-RitsuLib.xml` SHA256 `E5CB7922E69806639B60D7E6040D0B9B0F127ED6A7579479271E695A5C3C3D01`. Current beta.135 package refresh is recorded.
  Current beta.135 package refresh is recorded in `PROJECT_STATE.md`, `docs/reviews/current-validation.md`, and `docs/private-beta-verification-handoff.md`. The source inventory is 169 migrated RitsuLib patch classes and 0 raw Harmony declarations; default beta.135 builds compile 168 patch classes because `Sts1ReplacementPrototype` is behind `REPLACEMENT_PROTOTYPE_ENABLED`. New or changed patches must be explicit RitsuLib `IPatchMethod` / `ModPatcher` registrations. Previous beta.128 clicked Ancient UI smoke under `.tools/runtime-evidence/monkey-stability-20260623-062913/` reached the game UI with only `STS2-RitsuLib` and `EZMicroBalance` enabled, applied 152/152 default runtime Spire Plus patches in that installed package, audited clean, logged StS1Events disabled, passed the Off verifier, and passed packet verification 1621 / 0 after forcing Urda, Morvi, Lotha, and normal Vakuu UI selections. Previous beta.123/beta.108 clicked UI, beta.99 RitsuLib Mod Settings clicked UI, beta.99 Off loader proof, and beta.93 AdditiveBatch1 loader/registration proof remain previous-package context. Do not claim live-ready or release-ready because gameplay, save-load, current enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
