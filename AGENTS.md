...
- Current dependency configurations are aligned on STS2-RitsuLib `v0.4.34` and Slay the Spire 2 `v0.107.1`. The 2026-06-23 NuGet/GitHub plus `dotnet list package --outdated --include-transitive` recheck found `0.4.34` as the latest stable `STS2.RitsuLib` package; no newer `STS2.RitsuLib` update is available. Current beta.133 build/publish/package refresh, installed-package parity, runtime preflight, and source-workspace validation are current for the packaged 165/4 source state.
...
- Current retained runtime smoke proof is previous beta.128 clicked Ancient UI smoke for forced UI paths only; current beta.133 runtime smoke, enabled-mode proof, and gameplay proof remain pending.
- Current runtime dependency setup has official `STS2-RitsuLib` `v0.4.34` installed at `E:\Steam\steamapps\common\Slay the Spire 2\mods\STS2-RitsuLib` from the NuGet package deploy target, with root `STS2-RitsuLib.dll` SHA256 `47FED8A7071E71497C4C2AB3357D2B779B864B1EB7BC5CFB3E73BB8C4123B088` and `STS2-RitsuLib.xml` SHA256 `E5CB7922E69806639B60D7E6040D0B9B0F127ED6A7579479271E695A5C3C3D01`. Current beta.133 package refresh is recorded.
  Current beta.133 package refresh is recorded in `PROJECT_STATE.md`, `docs/reviews/current-validation.md`, and `docs/private-beta-verification-handoff.md`. The source inventory is 165 migrated RitsuLib patch classes and 4 raw Harmony declarations; the remaining raw declarations are high-risk lifecycle patches, not clicked UI. Previous beta.128 clicked Ancient UI smoke under `.tools/runtime-evidence/monkey-stability-20260623-062913/` reached the game UI with only `STS2-RitsuLib` and `EZMicroBalance` enabled, applied 152/152 default runtime Spire Plus patches in that installed package, audited clean, logged StS1Events disabled, passed the Off verifier, and passed packet verification 1621 / 0 after forcing Urda, Morvi, Lotha, and normal Vakuu UI selections. Previous beta.123/beta.108 clicked UI, beta.99 RitsuLib Mod Settings clicked UI, beta.99 Off loader proof, and beta.93 AdditiveBatch1 loader/registration proof remain previous-package context. Do not claim live-ready or release-ready because gameplay, save-load, current enabled-mode registration/gameplay proof, replacement functional proof, multiplayer fail-closed, independent QA rerun, and versioned tester-package handoff remain pending; recapture current HEAD/worktree before any later tester handoff. Live gameplay feature verification remains pending.
