...
- Latest validation 2026-05-25 after the Seedbed / planting text, Rootblight hold, and manifest overview package refresh: `dotnet build`, default `dotnet test` (281 passed / 20 skipped), `dotnet publish`, package refresh, opt-in artifact tests (301 passed / 0 skipped), website syntax checks, `dotnet format`, `git diff --check`, installed package check, and package hash sync all succeeded. The manifest description is readable bilingual UTF-8 without BOM, and `scripts/package-spire-plus.ps1` reads manifests as UTF-8. The current package Steam-client loader smoke is pending again because no game was opened for the latest package hash; the verifier fails closed until live/manual rows are filled.
- Historical guard context remains archived from 2026-05-24 after the Sere Talon `NRelic` fallback package refresh: focused Sere Talon/release-evidence/documentation/website guards passed, the current-package loader row is pending again, and there are 20 pending live/manual rows. Manual feature results are pending.
- The most recent Steam-client loader smoke under `.tools/runtime-evidence/manual-test-handoff-20260524-161744/release/fresh-current-package-loader-smoke` loaded exactly BaseLib plus Spire Plus, registered config for `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, and audited clean for a prior package hash. Treat it as startup/log context only until the current package is launched; Vakuu Sere Talon event-option, relic-bar, inspect-screen, hover, gameplay, save-load, and co-op rows remain pending.
- The latest script-packaged artifacts were hash-parity synced on 2026-05-25 after the Seedbed / planting text, Rootblight hold, and manifest overview pass. Current package hashes are recorded in `docs/issues.md`, `docs/toreview.md`, and `docs/review.md`.
