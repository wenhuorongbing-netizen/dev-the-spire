...
- Latest pushed cleanup/package guard commit before the current beta.37 validation batch: `5be5c51 Refresh beta35 package guards`.
...
- Release artifact checks are opt-in with `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1`. The latest beta.37 run passed after the real installed folder and game-root zip were refreshed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` still works.
- Latest in-progress validation target 2026-05-26 is the beta.37 Soul Tide / Fission Exhaust / Elite Root refresh: Soul Tide now counts Beckons before their turn-end in-hand damage moves them out of hand, waits through Soul Fysh's enemy turn, and converts them into capped Block at the next player side start; Fission text now states that the card enters the Exhaust pile and triggers Exhaust effects normally; the Urda Elite Root option icon was refreshed with updated GPTimage2 provenance. `publish/SpirePlus-v0.1.0-private-beta.37.zip`, `.tools\publish-game-root\mods\EZMicroBalance`, `D:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and the game-root zip were refreshed and hash-checked. The beta.19 Steam-client loader smoke under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` is still useful historical startup proof for the same 30-field source family, but beta.37 loader/gameplay proof was not recaptured in this pass. Gameplay, clicked UI, save-load, route traversal, preview-tools, Vakuu, and co-op rows remain pending.
