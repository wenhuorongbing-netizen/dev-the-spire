...
- Release artifact checks are opt-in with `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1`. The latest beta.32 run passed after the real installed folder and game-root zip were refreshed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` still works.
- Latest in-progress validation target 2026-05-26 is the beta.32 Soul Tide timing refresh: Soul Fysh now converts unanswered Beckons into capped Block at enemy turn end, so the Block is already visible when the next player turn begins. The player-turn-start path remains a fallback if an unusual turn flow leaves pending Block. `publish/SpirePlus-v0.1.0-private-beta.32.zip`, `.tools\publish-game-root\mods\EZMicroBalance`, `D:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and the game-root zip were refreshed and hash-checked. The beta.19 Steam-client loader smoke under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` is still useful historical startup proof for the same 30-field source family, but beta.32 loader/gameplay proof was not recaptured in this pass. Gameplay, clicked UI, save-load, route traversal, preview-tools, Vakuu, and co-op rows remain pending.
