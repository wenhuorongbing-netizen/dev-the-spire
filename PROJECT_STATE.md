...
- Latest in-progress validation target 2026-05-25 is the beta.14 Seedbed strength/planting clarity refresh: Seedbed is strengthened to 8/12 Block, immediately plants 1/2 eligible draw/discard cards, and the mod/site text now explains that Planting removes a card from this combat without playing, discarding, or exhausting it. The previous current package Steam-client loader smoke remains beta.13 historical startup context; fresh loader proof is pending again for beta.14, so the verifier should fail closed with 20 pending live/manual rows until user testing fills them.
...
- The previous Steam-client loader smoke under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` loaded exactly BaseLib plus Spire Plus, registered config for `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, audited clean, stopped the game, and restored 24 isolated mod entries for beta.13. This proves historical loader/startup only; beta.14 loader proof, Vakuu Sere Talon event-option, relic-bar, inspect-screen, hover, gameplay, save-load, and co-op rows remain pending.
