...
- Latest pushed cleanup/package evidence baseline: current beta.78 Ascension card-event dispatch split package sync; refresh the exact pushed HEAD with `git log -1 --oneline --decorate` before release packaging or final handoff.
...
- Release artifact checks are opt-in with `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1`. The latest beta.78 run must pass after the installed folder and game-root zip are refreshed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` still works.
- Latest in-progress validation target 2026-05-26 is the beta.78 Ascension card-event dispatch split package sync: `AscensionCombatModifierService.CardEvents.cs` now owns Ascension card-play, energy-spend, and card-entered-hand dispatch. `AscensionCombatModifierService.CombatEvents.cs` keeps HP, damage, power-amount, and death dispatch. `RootBudCombatHook.CardFlow.cs` still owns the hook forwarding entry points, and public service method names, hook callers, saved state, test names, and player-visible mod behavior are unchanged. `publish/SpirePlus-v0.1.0-private-beta.78.zip`, `D:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and the game-root zip were refreshed and hash-checked before handoff. `.tools\publish-game-root` is tooling/isolated-publish context only, not the current package-parity source. The beta.19 Steam-client loader smoke under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` is still useful historical startup proof for the same 30-field source family, but beta.78 loader/gameplay proof was not recaptured in this pass. Gameplay, clicked UI, save-load, route traversal, preview-tools, Vakuu, and co-op rows remain pending.
