...
- Latest pushed cleanup/package guard commit before the current beta.36 validation batch: `5be5c51 Refresh beta35 package guards`.
...
- Release artifact checks are opt-in with `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1`. The latest beta.36 run passed after the real installed folder and game-root zip were refreshed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` still works.
- Latest in-progress validation target 2026-05-26 is the beta.36 Soul Tide / Fission Exhaust / Elite Root refresh: Soul Tide now counts Beckons before their turn-end in-hand damage moves them out of hand, waits through Soul Fysh's enemy turn, and converts them into capped Block at the next player side start; Fission text now states that the card enters the Exhaust pile and triggers Exhaust effects normally; the Urda Elite Root option icon was refreshed with updated GPTimage2 provenance. `publish/SpirePlus-v0.1.0-private-beta.36.zip`, `.tools\publish-game-root\mods\EZMicroBalance`, `D:\Steam\steamapps\common\Slay the Spire 2\mods\EZMicroBalance`, and the game-root zip were refreshed and hash-checked. The beta.19 Steam-client loader smoke under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` is still useful historical startup proof for the same 30-field source family, but beta.36 loader/gameplay proof was not recaptured in this pass. Gameplay, clicked UI, save-load, route traversal, preview-tools, Vakuu, and co-op rows remain pending.
...
- The previous Steam-client loader smoke under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` remains beta.13 historical startup context. The beta.17 and beta.19 packages have historical loader/startup evidence; beta.36 startup remains pending. Vakuu Sere Talon event-option, relic-bar, inspect-screen, hover, gameplay, save-load, and co-op rows remain pending.
- The latest script-packaged artifacts were hash-parity synced on 2026-05-26 after the Soul Tide / Fission Exhaust / Elite Root refresh and manifest overview pass. Current package hashes are recorded in `docs/issues.md`, `docs/toreview.md`, and `docs/review.md`.
