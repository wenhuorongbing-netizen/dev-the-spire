...
- Latest in-progress validation target 2026-05-25 is the beta.17 Seedbed strength/planting clarity refresh: Seedbed is strengthened to 8/12 Block, immediately plants 1/2 eligible draw/discard cards, and the mod/site text now explains that Planting removes a card from this combat without playing, discarding, or exhausting it. A fresh beta.17 Steam-client loader smoke under `.tools/runtime-evidence/beta17-loader-smoke-20260525-194311` loaded exactly BaseLib plus Spire Plus, matched the current package hash, reported `Found 30 SavedSpireFields`, reached main menu, audited clean, stopped the game, and restored the isolated mod setup. Gameplay, clicked UI, save-load, route traversal, preview-tools, Vakuu, and co-op rows remain pending.
...
- The previous Steam-client loader smoke under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` remains beta.13 historical startup context. The current beta.17 package has a newer loader/startup row; Vakuu Sere Talon event-option, relic-bar, inspect-screen, hover, gameplay, save-load, and co-op rows remain pending.
