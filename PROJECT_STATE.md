...
- Release artifact checks are opt-in with `SPIREPLUS_RUN_RELEASE_ARTIFACT_TESTS=1`. The latest run correctly failed on installed-folder parity because the game process kept the old `mods\EZMicroBalance` DLL/manifest locked; the isolated beta.29 package checker passed. Legacy `EZMB_RUN_RELEASE_ARTIFACT_TESTS=1` still works.
- Latest in-progress validation target 2026-05-26 is the beta.29 Seedbed planting rules and mod-overview refresh: Seedbed is strengthened to 8/12 Block, immediately plants 1/2 eligible draw/discard cards, and the mod/site text now explains Planting as combat-only isolation without playing, discarding, or exhausting the card. `publish/SpirePlus-v0.1.0-private-beta.29.zip` was rebuilt and copied to the game root. The beta.19 Steam-client loader smoke under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` is still useful historical startup proof for the same 30-field source family, but beta.29 loader/gameplay proof was not recaptured in this pass. Gameplay, clicked UI, save-load, route traversal, preview-tools, Vakuu, and co-op rows remain pending.
...
- The latest script-packaged artifacts were hash-parity synced on 2026-05-26 after the Seedbed planting rules and manifest overview pass. Current package hashes are recorded in `docs/issues.md`, `docs/toreview.md`, and `docs/review.md`.
