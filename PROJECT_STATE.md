...
- The current Steam-client loader smoke under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` loaded exactly BaseLib plus Spire Plus, registered config for `EZMicroBalance`, reported `Found 30 SavedSpireFields`, reached startup completion, audited clean, stopped the game, and restored 24 isolated mod entries. This proves loader/startup only; Vakuu Sere Talon event-option, relic-bar, inspect-screen, hover, gameplay, save-load, and co-op rows remain pending.
...
- Current source defines 30 SavedSpireFields. The latest Steam-client startup/log evidence under `.tools/runtime-evidence/release-ready-manual/fresh-current-package-loader-smoke` loaded exactly BaseLib plus Spire Plus, config registered, `Found 30 SavedSpireFields`, startup completion reached, 0 release-blocking log signatures were found, stopped the game, and helper restore completed. Previous `20260524-161744`, `20260523-current`, `Found 22 SavedSpireFields`, and 16-field loader logs remain historical context only.
