...
- Latest validation 2026-05-25 after the co-op gameplay fail-closed crash hardening package refresh: `dotnet build`, default `dotnet test` (281 passed / 20 skipped), `dotnet publish`, package refresh, opt-in artifact tests (301 passed / 0 skipped), website syntax checks, `dotnet format`, `git diff --check`, installed package check, and release verifier fail-closed check all succeeded in their expected modes. The current package Steam-client loader smoke is pending again because no game was opened for the latest package hash; the verifier fails closed until live/manual rows are filled.
...
- The latest script-packaged artifacts were hash-parity synced on 2026-05-25 after the co-op gameplay fail-closed crash hardening pass. Current package hashes are recorded in `docs/issues.md`, `docs/toreview.md`, and `docs/review.md`.
