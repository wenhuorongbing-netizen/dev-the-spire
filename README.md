...
- The latest local implementation state was refreshed from Slay the Spire 2 `v0.106.1` with BaseLib `v3.1.4`; the current installed game is now `v0.107.0`, and RitsuLib `v0.4.16` with `lib\0.107.0` is installed. The current packaged tester artifact is `publish/SpirePlus-v0.1.0-private-beta.85.zip`, with hashes recorded in `docs/issues.md` and `docs/private-beta-verification-handoff.md`.
- Runtime update after the RitsuLib target fix: diagnostic Off, CanaryOnly, and AdditiveBatch1 loader smokes remain clean historical `v0.106.1` evidence only. Current RitsuLib `v0.4.16` with `lib\0.107.0` is installed, beta.85 package parity passes, and the fresh beta.85 `v0.107.0` Off smoke under `.tools/runtime-evidence/v01070-beta85-current-package-runtime-fix-20260611-0510` reached main menu, applied 25/25 Spire Plus ModPatcher patches, and audited clean. Current StS1 enabled-mode proof is still pending: CanaryOnly must show 4 event types / 6 registration calls and AdditiveBatch1 must show 10 event types / 14 registration calls before gameplay or handoff claims. Manual feature verification is still pending; gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.0` with RitsuLib `v0.4.16` / `lib\0.107.0`. Installed beta.85 package parity passes, and fresh beta.85 `v0.107.0` Off smoke audited clean after the Spire Plus API-target drift fix.
