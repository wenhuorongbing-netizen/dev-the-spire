...
- The latest local implementation state targets Slay the Spire 2 `v0.107.1` with official STS2-RitsuLib `v0.4.34` installed from the NuGet package deploy target. The active manifest is `v0.1.0-private-beta.113`; the current package pass refreshed the zip, installed folder, game-root zip, and hash docs. Beta.108 clicked Ancient UI smoke and beta.99 or older proof rows are previous-package context.
- Current source has completed the clicked/input UI migration, visual/hover UI getter migration, rest-site Meat Cleaver UI migration, and Batch 4c localization fallback migration to RitsuLib `IPatchMethod` / `ModPatcher`; `docs/patch-inventory.md` records 69 migrated patch classes and 102 raw Harmony declarations remaining. beta.113 was rebuilt, published, and packaged after the RitsuLib `0.4.34` update; clicked UI and gameplay proof still need recapture after this source migration.
