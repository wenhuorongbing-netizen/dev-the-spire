...
- Local cleanup note: raw local `.tools` runtime-evidence folders may be pruned; those historical entries remain documentation records, not proof that raw evidence folders are currently present. Current beta.96 Off loader proof exists, but gameplay, clicked UI, save-load, preview-tools, Vakuu, current enabled-mode registration/gameplay, and co-op evidence must be captured before any release-ready claim; Off proof is loader evidence only and does not prove disable-mod gameplay inside a run.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with STS2-RitsuLib `v0.4.31` / `lib\0.107.1`. Installed beta.96 package parity and beta.96 Off loader proof are current package evidence; beta.93 RitsuLib-only AdditiveBatch1 loader proof is previous-package evidence. Gameplay/manual verification remains pending.
