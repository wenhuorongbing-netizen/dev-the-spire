...
- The latest implementation pass has been refreshed for Slay the Spire 2 `v0.106.0` with BaseLib `v3.1.4`: `source code/` was cleaned and regenerated from the current installed PCK/DLL, build succeeds with 0 warnings/errors, and publish/package refresh is on the `publish/SpirePlus-v0.1.0-private-beta.80.zip` line with `EZMicroBalance/` as the technical install folder. The beta.80 pass split Ancient behavior localization and documentation guard coverage into `AncientBehaviorGuardTests.LocalizationDocs.cs`, keeping v4.3/v4.2 doc-current checks, localization parity, Simplified Chinese text guards, release-checklist checks, and manual-matrix rows reviewable without changing test names, package evidence semantics, runtime hooks, saved state, or player-visible behavior. Release-artifact tests remain an opt-in gate after hash/doc refreshes. Historical package smoke/resource evidence under `.tools/runtime-evidence/current-package-smoke-20260514-015901` verified an earlier installed package with headless Ancient scene/texture loading and a normal Steam helper startup. The beta.19 Steam-client loader evidence under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` loaded only BaseLib plus Spire Plus, registered config for `EZMicroBalance`, reported `v0.1.0-private-beta.19`, `Found 30 SavedSpireFields`, reached main menu, and audited clean; beta.80 loader proof has not been recaptured yet. Gameplay, save-load, clicked UI, preview-tools, Vakuu, and co-op proof remain pending. A BaseLib-only plug-off startup/log pass under `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020` loaded 1 mod, did not initialize Spire Plus, and audited clean. A refreshed Mod Settings UI screenshot shows `Spire Plus` in the Mods list for the display-name package; full live gameplay/manual feature verification is still pending.
- Local cleanup note: raw local `.tools` runtime-evidence folders were pruned; those historical entries remain documentation records, not currently present raw evidence folders. Fresh beta.80 loader, gameplay, clicked UI, save-load, preview-tools, Vakuu, and co-op evidence must be recaptured before any release-ready claim; fresh plug-off evidence must be recaptured before any release-ready claim.
