...
- The latest local implementation state targets Slay the Spire 2 `v0.107.1` with official STS2-RitsuLib `v0.4.33` installed from the NuGet package deploy target. The active manifest is `v0.1.0-private-beta.105`; the current package pass refreshed the zip, installed folder, game-root zip, and hash docs. Beta.105 package parity, runtime preflight, source-workspace validation, and clicked Ancient UI smoke proof are current; beta.99 and older proof rows are previous-package context.
- Source after `f9d5335d` has completed the clicked/input UI migration to RitsuLib `IPatchMethod` / `ModPatcher`; `docs/patch-inventory.md` records 46 migrated patch classes and 125 raw Harmony declarations remaining. This source-only migration has not been published or runtime-smoked yet, so the beta.105 package proof remains installed-package context.
- Runtime update after the latest RitsuLib target fix: current beta.105 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260622-025733/`; 4 / 4 `AncientUiSmoke` iterations passed for Urda, Morvi, Lotha, and normal Vakuu, with screenshots, command ACKs, clean audits, StS1 Off verifier pass, exact game/Ritsu/package markers, and packet verification 1621 / 0. Manual feature gameplay, save-load, preview-tools live behavior, gated Vakuu fight-option/victory return, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.
...
- Local cleanup note: raw local `.tools` runtime-evidence folders may be pruned; those historical entries remain documentation records, not proof that raw evidence folders are currently present.
  beta.105 clicked Ancient UI smoke covers only the forced UI paths. Gameplay, save-load, current enabled-mode proof, co-op, and independent QA evidence are still required before any live-ready or release-ready claim. Preview-tools live behavior and gated Vakuu fight-option/victory-return evidence also remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with STS2-RitsuLib `v0.4.33` direct runtime layout. Installed beta.105 package parity and clicked Ancient UI smoke are current package evidence; beta.99 and older rows are previous-package evidence. Gameplay/manual verification remains pending.
