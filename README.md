...
- The latest local implementation state targets Slay the Spire 2 `v0.107.1` with STS2-RitsuLib `v0.4.31` / `lib\0.107.1`. The active manifest is `v0.1.0-private-beta.94`; the latest RitsuLib-only settings UI publish/package pass refreshed the zip, installed folder, game-root zip, and hash docs. Previous beta.93 RitsuLib-only Off and AdditiveBatch1 loader proof is clean under `.tools/runtime-evidence/v01071-beta93-ritsulib0431-off-direct-20260621/` and `.tools/runtime-evidence/v01071-beta93-ritsulib0431-additivebatch1-direct-20260621/`, but beta.94 loader proof still needs recapture.
- Runtime update after the latest RitsuLib target fix: diagnostic Off, CanaryOnly, and AdditiveBatch1 loader smokes remain clean historical `v0.106.1` evidence only. Current RitsuLib `v0.4.31` with `lib\0.107.1` is installed, beta.85 Off and CanaryOnly proof remain previous-package loader context, beta.87 AdditiveBatch1 proof remains clean `v0.107.0` loader context, and beta.88/beta.90 proof rows are previous-package contexts. The previous beta.93 RitsuLib-only Off proof reached main menu with exactly `STS2-RitsuLib` and `EZMicroBalance` loaded, clean audit, and packet verifier 43 / 0; beta.93 AdditiveBatch1 registered 10 event types through 14 calls with verifier 31 / 0 and packet 61 / 0. Manual feature verification, gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff are still pending.
...
- Local cleanup note: raw local `.tools` runtime-evidence folders may be pruned; those historical entries remain documentation records, not proof that raw evidence folders are currently present. Fresh beta.94 loader, gameplay, clicked UI, save-load, preview-tools, Vakuu, and co-op evidence must be recaptured before any release-ready claim; previous beta.93 Off proof is loader evidence only and does not prove disable-mod gameplay inside a run.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with STS2-RitsuLib `v0.4.31` / `lib\0.107.1`. Installed beta.94 package parity is current package evidence; beta.93 RitsuLib-only Off and AdditiveBatch1 loader proof is previous-package evidence. Gameplay/manual verification remains pending.
- .NET SDK 9.0.313 or compatible
- Godot .NET / Mono 4.5.1
- STS2-RitsuLib runtime `v0.4.31` or newer installed under `<GameRoot>\mods\STS2-RitsuLib`
