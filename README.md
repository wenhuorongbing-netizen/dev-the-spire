...
- The latest local implementation state was refreshed from historical Slay the Spire 2 `v0.106.1` source material, while the active installed-game target is Slay the Spire 2 `v0.107.1` with BaseLib `v3.3.0` and RitsuLib `v0.4.24` / `lib\0.107.0`. The current packaged tester artifact is `publish/SpirePlus-v0.1.0-private-beta.88.zip`, with hashes recorded in `docs/issues.md` and `docs/private-beta-verification-handoff.md`.
- Runtime update after the RitsuLib target fix: diagnostic Off, CanaryOnly, and AdditiveBatch1 loader smokes remain clean historical `v0.106.1` evidence only. Current RitsuLib `v0.4.24` with `lib\0.107.0` is installed, beta.85 Off and CanaryOnly proof remain previous-package loader context, and beta.87 AdditiveBatch1 proof remains clean `v0.107.0` loader context. The first `v0.107.1` beta.87 recapture exposed BaseLib `v3.2.1` patch failures; the current beta.88 pass upgraded BaseLib to `v3.3.0` and captured clean AdditiveBatch1 direct proof under `.tools/runtime-evidence/v01071-beta88-baselib330-additive-batch1-direct-cleanlog-20260619-103937/`: main menu reached, audit clean, enabled-mode verifier 31 / 0, packet verifier 0 mismatches, and 10 event types / 14 registration calls. Manual feature verification is still pending; gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with BaseLib `v3.3.0` and RitsuLib `v0.4.24` / `lib\0.107.0`. Installed beta.88 package parity is recorded after the BaseLib compatibility pass; beta.88 AdditiveBatch1 direct smoke audited clean on `v0.107.1` for loader/registration only.
- .NET SDK 9.0.313 or compatible
- Godot .NET / Mono 4.5.1
- BaseLib runtime `v3.3.0` installed under `<GameRoot>\mods\BaseLib`
