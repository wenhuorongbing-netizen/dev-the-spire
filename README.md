...
- The latest local implementation state targets Slay the Spire 2 `v0.107.1` with official STS2-RitsuLib `v0.4.33` installed from the NuGet package deploy target. The active manifest is `v0.1.0-private-beta.108`; the current package pass refreshed the zip, installed folder, game-root zip, and hash docs. Beta.108 package parity, runtime preflight, and source-workspace validation are current; beta.107 clicked Ancient UI smoke and beta.99 or older proof rows are previous-package context.
- Current source has completed the clicked/input UI migration, visual/hover UI getter migration, and Batch 4c localization fallback migration to RitsuLib `IPatchMethod` / `ModPatcher`; `docs/patch-inventory.md` records 64 migrated patch classes and 107 raw Harmony declarations remaining. Beta.108 was rebuilt, published, and packaged after those migrations; live/UI runtime proof for the 64-patch source state still needs recapture.
- Runtime update after the latest RitsuLib target fix: previous beta.107 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-beta107-rerun-20260622-144051/`; 4 / 4 `AncientUiSmoke` iterations passed for Urda, Morvi, Lotha, and normal Vakuu, with screenshots, command ACKs, clean audits, StS1 Off verifier pass, exact game/Ritsu/package markers, and packet verification 1620 / 0. Manual feature gameplay, save-load, preview-tools live behavior, gated Vakuu fight-option/victory return, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.
...
- Local cleanup note: raw local `.tools` runtime-evidence folders may be pruned; those historical entries remain documentation records, not proof that raw evidence folders are currently present.
  beta.107 clicked Ancient UI smoke covers only the forced UI paths for that previous package. Manual feature verification, gameplay, save-load, current enabled-mode proof, co-op, and independent QA evidence are still pending before any live-ready or release-ready claim. Preview-tools live behavior and gated Vakuu fight-option/victory-return evidence also remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with STS2-RitsuLib `v0.4.33` direct runtime layout. Installed beta.108 package parity is current package evidence; beta.107 clicked Ancient UI smoke and beta.99 or older rows are previous-package evidence. Gameplay/manual verification remains pending.
- .NET SDK 9.0.313 or compatible
- Godot .NET / Mono 4.5.1
- STS2-RitsuLib runtime `v0.4.33` or newer installed under `<GameRoot>\mods\STS2-RitsuLib`
