...
- The latest local implementation state was refreshed from Slay the Spire 2 `v0.106.1` with BaseLib `v3.1.4`; the current installed game is now `v0.107.0`, and RitsuLib `v0.4.16` with `lib\0.107.0` is installed. At HEAD `f32c6767` plus dirty source/docs changes, the solution build passes with 0 errors and 0 warnings after installed-game API compatibility fixes and expanded Sts1Events owner guards. The current migration validation lane passed the full test project and exact solution-level `dotnet test EZMicroBalance.sln --no-build` with 464 passed / 21 skipped / 0 failed / 485 total. The last packaged tester artifact is still `publish/SpirePlus-v0.1.0-private-beta.84.zip`; it must not be treated as a package of the current dirty source until version, publish, package, hashes, and handoff docs are refreshed.
- Runtime update after the RitsuLib target fix: diagnostic Off, CanaryOnly, and AdditiveBatch1 loader smokes remain clean historical `v0.106.1` evidence only. Current RitsuLib `v0.4.16` with `lib\0.107.0` is installed, and beta.84 package parity passes after the 2026-06-10 DLL restore. The fresh beta.84 `v0.107.0` Off smoke reached main menu but failed clean audit on stale Spire Plus API targets. Gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, current-source package decision, and versioned tester-package handoff remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.0` with RitsuLib `v0.4.16` / `lib\0.107.0`. Installed beta.84 package parity passes after the 2026-06-10 DLL restore, but the fresh beta.84 `v0.107.0` Off smoke failed clean audit on stale Spire Plus API targets.
