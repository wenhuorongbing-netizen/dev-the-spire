...
- The latest local implementation state was refreshed from Slay the Spire 2 `v0.106.1` with BaseLib `v3.1.4`; the current installed game is now `v0.107.0`, and RitsuLib `v0.4.16` with `lib\0.107.0` is installed. The current packaged tester artifact is `publish/SpirePlus-v0.1.0-private-beta.86.zip`, with hashes recorded in `docs/issues.md` and `docs/private-beta-verification-handoff.md`.
- Runtime update after the RitsuLib target fix: diagnostic Off, CanaryOnly, and AdditiveBatch1 loader smokes remain clean historical `v0.106.1` evidence only. Current RitsuLib `v0.4.16` with `lib\0.107.0` is installed, beta.85 Off and CanaryOnly proof remain previous-package loader context, and the fresh beta.86 AdditiveBatch1 direct smoke under `.tools/runtime-evidence/v01070-beta86-additive-batch1-direct-20260618-031254/` reached main menu, applied 25/25 Spire Plus ModPatcher patches, audited clean, and passed retained log/packet verifiers with 10 event types / 14 registration calls including `Sts1TheCleric` in both Act 1 buckets. Manual feature verification is still pending; gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.0` with RitsuLib `v0.4.16` / `lib\0.107.0`. Installed beta.86 package parity is being revalidated after the package/source alignment pass; beta.86 AdditiveBatch1 direct smoke audited clean after the Spire Plus API-target drift fix.
