...
- The latest local implementation state targets Slay the Spire 2 `v0.107.1` with official STS2-RitsuLib `v0.4.34` installed from the NuGet package deploy target. The active manifest is `v0.1.0-private-beta.116`; the current package pass refreshed the zip, installed folder, game-root zip, and hash docs. Beta.108 clicked Ancient UI smoke and beta.99 or older proof rows are previous-package context.
- Current source has completed the clicked/input UI migration, visual/hover UI getter migration, rest-site Meat Cleaver UI migration, Preview transform prediction source/lifetime migration, Ascension selection/lobby UI migration, Neow/Vakuu event-option UI migration, A20 courtyard portrait migration, Batch 4c localization fallback migration, and core inline-localization fallback migration to RitsuLib `IPatchMethod` / `ModPatcher`; `docs/patch-inventory.md` records 91 migrated patch classes and 78 raw Harmony declarations remaining. beta.116 was rebuilt, published, and packaged after the RitsuLib `0.4.34` update; clicked UI and gameplay proof still need recapture after this source migration.
- Runtime update after the latest RitsuLib target fix: previous beta.108 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-beta108-20260622-172312/`; 4 / 4 `AncientUiSmoke` iterations passed for Urda, Morvi, Lotha, and normal Vakuu, with screenshots, command ACKs, clean audits, StS1 Off verifier pass, exact game/Ritsu/package markers, all 64 migrated Spire Plus ModPatcher patches applied, and packet verification 1621 / 0. Manual feature gameplay, save-load, preview-tools live behavior, gated Vakuu fight-option/victory return, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.
...
- Local cleanup note: raw local `.tools` runtime-evidence folders may be pruned; those historical entries remain documentation records, not proof that raw evidence folders are currently present.
  previous beta.108 clicked Ancient UI smoke covers only the forced UI paths. Manual feature verification, gameplay, save-load, current enabled-mode proof, co-op, and independent QA evidence are still pending before any live-ready or release-ready claim. Preview-tools live behavior and gated Vakuu fight-option/victory-return evidence also remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with STS2-RitsuLib `v0.4.34` direct runtime layout. Installed beta.116 package artifacts are current; beta.108 clicked Ancient UI smoke and beta.99 or older rows are previous-package evidence. Gameplay/manual verification remains pending.
