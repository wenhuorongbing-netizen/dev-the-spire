...
The original project and manifest id are `EzDailyContent`. That id must not be renamed in-place. The architecture decision for the private beta is to keep a new independent mod project with stable id `EZMicroBalance`, so players can enable or disable Spire Plus separately from later mods. The player-facing display name is `Spire Plus`; the manifest id, package folder, env vars, saved-field prefixes, and namespaces remain `EZMicroBalance` for compatibility this cycle.
...
- The latest implementation pass has been refreshed for Slay the Spire 2 `v0.105.0` with BaseLib `v3.1.2`: build succeeds with 0 warnings/errors and publish/package refresh succeeded for the current DLL/PCK/package as `publish/SpirePlus-v0.1.0-private-beta.0.zip` with `EZMicroBalance/` as the install folder. Normal tests pass in default mode, and release-artifact tests are run as an opt-in gate after hash/doc refreshes. Historical package smoke/resource evidence under `.tools/runtime-evidence/current-package-smoke-20260514-015901` verified an earlier installed package with headless Ancient scene/texture loading and a normal Steam helper startup. current source now defines 25 SavedSpireFields after later static fixes, so fresh live loader parity for the current package is still pending. A BaseLib-only plug-off startup/log pass under `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020` loaded 1 mod, did not initialize Spire Plus / `EZMicroBalance`, and audited clean. A refreshed Mod Settings UI screenshot shows `Spire Plus` in the Mods list for the current display-name package; full live gameplay/manual feature verification is still pending.
