...
- Runtime update after the RitsuLib target fix: diagnostic Off, CanaryOnly, and AdditiveBatch1 loader smokes remain clean historical `v0.106.1` evidence only. Current RitsuLib `v0.4.24` with `lib\0.107.0` is installed, beta.85 Off and CanaryOnly proof remain previous-package loader context, and the fresh beta.87 AdditiveBatch1 direct smoke under `.tools/runtime-evidence/v01070-beta87-additive-batch1-direct-20260618-152531/` reached main menu on `v0.107.0`, applied 25/25 Spire Plus ModPatcher patches, audited clean, and passed retained log/packet verifiers with 10 event types / 14 registration calls including `Sts1TheCleric` in both Act 1 buckets. The local game is now `v0.107.1`, so recapture loader proof before using it as current runtime evidence. Manual feature verification is still pending; gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with RitsuLib `v0.4.24` / `lib\0.107.0`. Installed beta.87 package parity is recorded after the dependency-floor pass; beta.87 AdditiveBatch1 direct smoke audited clean on `v0.107.0` after the Spire Plus API-target drift fix and verifier hardening, and must be recaptured for a current `v0.107.1` runtime claim.
