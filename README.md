...
- The latest implementation pass has been refreshed for Slay the Spire 2 `v0.106.1` with BaseLib `v3.1.4`: `source code/` was cleaned and regenerated from the current installed PCK/DLL, current no-game validation at HEAD `6b149ba0` passes with 464 passed / 21 skipped / 0 failed / 485 total tests and 89 tracked Sts1Events nullable warnings, and publish/package refresh is on the `publish/SpirePlus-v0.1.0-private-beta.84.zip` line with `EZMicroBalance/` as the technical install folder. Official `STS2-RitsuLib` `v0.3.10` is now installed at the E-drive game-root mods path with a `0.106.1` runtime variant. Current best loader evidence under `.tools/runtime-evidence/sts1-events-v15-loader-20260531-231135/` reaches main menu with BaseLib, RitsuLib, and Spire Plus loaded, but audit is not clean: 11 Godot ERROR hits remain, including `ritsulib-variants.json` manifest parsing and optional Spire Plus ModPatcher failures. The beta.84 pass resolved a NullReferenceException in the Urda Seedbed drawing hooks and incremented the package version. Release-artifact tests remain an opt-in gate after hash/doc refreshes. Historical package smoke/resource evidence under `.tools/runtime-evidence/current-package-smoke-20260514-015901` verified an earlier installed package with headless Ancient scene/texture loading and a normal Steam helper startup. The beta.19 Steam-client loader evidence under `.tools/runtime-evidence/beta19-loader-smoke-20260525-213336` loaded only BaseLib plus Spire Plus, registered config for `EZMicroBalance`, reported `v0.1.0-private-beta.19`, `Found 30 SavedSpireFields`, reached main menu, and audited clean; current beta.84 loader proof is error-blocked, not clean. Gameplay, save-load, clicked UI, preview-tools, Vakuu, and co-op proof remain pending. A BaseLib-only plug-off startup/log pass under `.tools/runtime-evidence/live-spire-plus-disabled-session-20260513-143020` loaded 1 mod, did not initialize Spire Plus, and audited clean. A refreshed Mod Settings UI screenshot shows `Spire Plus` in the Mods list for the display-name package; full live gameplay/manual feature verification is still pending.
- Runtime update after the RitsuLib target fix: diagnostic Off and CanaryOnly loader smokes under `.tools/runtime-evidence/ritsulib-off-after-target-fix-20260531-2325/` and `.tools/runtime-evidence/ritsulib-canary-after-target-fix-20260531-2327/` reached main menu with BaseLib, RitsuLib, and Spire Plus, clean audits, and 25/25 Spire Plus RitsuLib patches. This supersedes the earlier v15 error-blocked loader note for diagnostic loader proof only; gameplay, clicked UI, save-load, preview-tools, Vakuu, co-op, independent QA rerun, and versioned tester-package handoff remain pending.
...
- Slay the Spire 2 public beta, current local target `v0.106.1`
