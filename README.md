...
- The latest local implementation state targets Slay the Spire 2 `v0.107.1` with official STS2-RitsuLib `v0.4.34` installed from the NuGet package deploy target. The active manifest is `v0.1.0-private-beta.128`; the current package pass refreshed the zip, installed folder, game-root zip, hash docs, installed-package parity, runtime preflight, source-workspace validation, and forced clicked Ancient UI smoke. Beta.99 or older proof rows are previous-package context.
- Current source has completed the clicked/input UI migration, visual/hover UI getter migration, rest-site Meat Cleaver UI migration, Preview transform prediction source/lifetime migration, Ascension selection/lobby UI migration, Neow/Vakuu event-option UI migration, Act Ancient unlock-list UI migration, Vakuu event-state UI migration, A20 courtyard portrait migration, Batch 4c localization fallback migration, core inline-localization fallback migration, Ancient reward getter/relic hook migration, low-risk Ancient reward hook migration, Aeonglass intent UI migration, and Enemy Damage polish getter migration to RitsuLib `IPatchMethod` / `ModPatcher`; `docs/patch-inventory.md` records 153 migrated source patch classes and 17 raw Harmony declarations remaining. The default beta.128 runtime registers 152 Spire Plus RitsuLib patches because the migrated StS1 replacement prototype is compile-symbol gated behind `REPLACEMENT_PROTOTYPE_ENABLED`.
- Runtime update after the latest RitsuLib target fix: current beta.128 clicked Ancient UI smoke proof is captured under `.tools/runtime-evidence/monkey-stability-20260623-062913/`; 4 / 4 `AncientUiSmoke` iterations passed for Urda, Morvi, Lotha, and normal Vakuu, with screenshots, command ACKs, clean audits, StS1 Off verifier pass, exact game/Ritsu/package markers, 152/152 default runtime Spire Plus ModPatcher patches applied, and packet verification 1621 / 0. Manual feature gameplay, save-load, preview-tools live behavior, gated Vakuu fight-option/victory return, current enabled-mode registration/gameplay, co-op, independent QA rerun, and versioned tester-package handoff are still pending.
...
- Local cleanup note: raw local `.tools` runtime-evidence folders may be pruned; those historical entries remain documentation records, not proof that raw evidence folders are currently present.
  current beta.128 clicked Ancient UI smoke covers only the forced UI paths. Manual feature verification, gameplay, save-load, current enabled-mode proof, co-op, and independent QA evidence are still pending before any live-ready or release-ready claim. Preview-tools live behavior and gated Vakuu fight-option/victory-return evidence also remain pending.
...
- Slay the Spire 2 public beta. Historical loader proof used `v0.106.1`; the current local install is `v0.107.1` with STS2-RitsuLib `v0.4.34` direct runtime layout. Installed beta.128 package artifacts, no-launch validation, and forced clicked Ancient UI smoke are current; beta.99 or older rows are previous-package evidence. Gameplay/manual verification remains pending.
