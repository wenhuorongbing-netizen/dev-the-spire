...
| `AncientBehaviorGuardTests.cs` / `AncientBehaviorGuardTests.LocalizationDocs.cs` / `AncientBehaviorGuardTests.SereTalonRoutes.cs` / `AncientBehaviorGuardTests.DistinguishedCape.cs` / `AncientBehaviorGuardTests.SovereignBlade.cs` / `AncientBehaviorGuardTests.PrismaticGem.cs` / `AncientBehaviorGuardTests.VelvetChoker.cs` / `AncientBehaviorGuardTests.DirectDeckGainFeedback.cs` / `AncientBehaviorGuardTests.JeweledMask.cs` / `AncientBehaviorGuardTests.JewelryBox.cs` / `AncientBehaviorGuardTests.PaelsTooth.cs` / `AncientBehaviorGuardTests.DebtAndFolly.cs` / `AncientBehaviorGuardTests.TemporaryGeneratedCards.cs` / `AncientBehaviorGuardTests.MeatCleaver.cs` | Ancient reward behavior, package shape, patch targets, Sere Talon/Tanx Claws, Distinguished Cape, Sovereign Blade, Prismatic Gem, Velvet Choker, direct deck-gain feedback, Jeweled Mask, Jewelry Box, Pael's Tooth, Debt/Folly, temporary generated cards, Meat Cleaver, localization parity, documentation currency, release-checklist, and manual matrix guards. |
...
| `AncientHighRiskSourceGuardTests.cs` / `AncientHighRiskSourceGuardTests.RootSight.cs` | High-risk source patterns for Ancient patches; the Root Sight map-preview/no-consume guard is in the focused Root Sight partial. |
...
Previous beta.93 validation uses a split no-build lane when the one-shot solution
