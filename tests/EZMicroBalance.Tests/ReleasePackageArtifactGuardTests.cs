...
public sealed class ReleasePackageArtifactGuardTests
{
    ...
    public void CurrentDocsMatchReleaseHashesAndAvoidPinnedStaleTestTotals()
    {
        ...
        Assert.Contains("Fresh loader smoke for the current beta.19 package hash is pending", docsByPath["docs/release-checklist.md"], StringComparison.Ordinal);
        Assert.Contains("This is historical loader/startup evidence, not current beta.19 proof and not gameplay proof", docsByPath["docs/release-checklist.md"], StringComparison.Ordinal);
