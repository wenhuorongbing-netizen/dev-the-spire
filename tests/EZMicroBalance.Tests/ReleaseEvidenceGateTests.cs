...
public sealed class ReleaseEvidenceGateTests
{
    ...
    private static void AssertPackageHashesUseVersionedArtifacts(string packageHashesPath)
    {
        ...
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.10.zip", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.10\\EZMicroBalance\\EZMicroBalance.dll", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.10\\EZMicroBalance\\EZMicroBalance.pck", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.10\\EZMicroBalance\\EZMicroBalance.json", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.10\\EZMicroBalance\\README_INSTALL.txt", paths);
        ...
        foreach (var file in files.Where(file => file.GetProperty("Path").GetString()?.StartsWith("publish\\SpirePlus-v0.1.0-private-beta.10", StringComparison.Ordinal) == true))
        {
    ...
    public void CurrentManualTestHandoffScriptCreatesAllPendingEvidenceSections()
    {
        ...
        try
        {
            ...
            Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.10.zip", summary.GetProperty("PackagePath").GetString());
            Assert.Equal("37E6EE714BE16601A0A55BBA912A27F69E2AF978A8F464E4F9C4CF86D14BB07A", summary.GetProperty("PackageSha256").GetString());
            ...
            Assert.Contains("ZIP: `publish\\SpirePlus-v0.1.0-private-beta.10.zip`.", startHere, StringComparison.Ordinal);
            Assert.Contains("ZIP SHA256: `37E6EE714BE16601A0A55BBA912A27F69E2AF978A8F464E4F9C4CF86D14BB07A`.", startHere, StringComparison.Ordinal);
    ...
    public void CurrentManualTestHandoffArchivesStaleLoaderFilesWhenHashCannotBePreserved()
    {
        ...
        try
        {
            ...
            var packageRow = packageHashes["Files"]!.AsArray()
                .Single(row => (string?)row?["Path"] == "publish\\SpirePlus-v0.1.0-private-beta.10.zip")!
                .AsObject();
        ...
        })
        {
            ...
            try
            {
                ...
                if (scriptName == "collect-release-evidence.ps1")
                {
                    ...
                    Assert.Equal("37E6EE714BE16601A0A55BBA912A27F69E2AF978A8F464E4F9C4CF86D14BB07A", manifest.GetProperty("PackageSha256").GetString());
                    Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.10.zip", manifest.GetProperty("PackagePath").GetString());
    ...
    public void ReleaseVerifierRejectsStalePackageHashRows()
    {
        ...
        try
        {
            ...
            foreach (var fileNode in packageHashesNode["Files"]!.AsArray())
            {
                ...
                if (path != "publish\\SpirePlus-v0.1.0-private-beta.10\\EZMicroBalance\\EZMicroBalance.dll")
                {
            ...
            Assert.Contains("SpirePlus-v0.1.0-private-beta.10\\\\EZMicroBalance\\\\EZMicroBalance.dll", verifyResult.Output, StringComparison.Ordinal);
        }
        finally
        {
            if (Directory.Exists(evidenceDir))
    ...
    public void AncientUiEvidenceHelperCreatesPendingPreparePlansWithoutLaunching()
    {
        ...
        })
        {
            ...
            try
            {
                ...
                Assert.Contains(files, file => file.GetProperty("Path").GetString() == "publish\\SpirePlus-v0.1.0-private-beta.10.zip");
