...
public sealed class ReleaseEvidenceGateTests
{
    ...
    private static void AssertPackageHashesUseVersionedArtifacts(string packageHashesPath)
    {
        ...
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.28.zip", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.28\\EZMicroBalance\\EZMicroBalance.dll", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.28\\EZMicroBalance\\EZMicroBalance.pck", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.28\\EZMicroBalance\\EZMicroBalance.json", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.28\\EZMicroBalance\\README_INSTALL.txt", paths);
        ...
        foreach (var file in files.Where(file => file.GetProperty("Path").GetString()?.StartsWith("publish\\SpirePlus-v0.1.0-private-beta.28", StringComparison.Ordinal) == true))
        {
    ...
    public void CurrentManualTestHandoffScriptCreatesAllPendingEvidenceSections()
    {
        ...
        try
        {
            ...
            Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.28.zip", summary.GetProperty("PackagePath").GetString());
            Assert.Equal("E58B6204BFDCCBE9CA8132AD286D936D30E576FD69A8A9256CB2A08335B503C2", summary.GetProperty("PackageSha256").GetString());
            ...
            Assert.Contains("ZIP: `publish\\SpirePlus-v0.1.0-private-beta.28.zip`.", startHere, StringComparison.Ordinal);
            Assert.Contains("ZIP SHA256: `E58B6204BFDCCBE9CA8132AD286D936D30E576FD69A8A9256CB2A08335B503C2`.", startHere, StringComparison.Ordinal);
    ...
    public void CurrentManualTestHandoffArchivesStaleLoaderFilesWhenHashCannotBePreserved()
    {
        ...
        try
        {
            ...
            var packageRow = packageHashes["Files"]!.AsArray()
                .Single(row => (string?)row?["Path"] == "publish\\SpirePlus-v0.1.0-private-beta.28.zip")!
                .AsObject();
        ...
        })
        {
            ...
            try
            {
                ...
                if (scriptName == "collect-release-evidence.ps1")
                {
                    ...
                    Assert.Equal("E58B6204BFDCCBE9CA8132AD286D936D30E576FD69A8A9256CB2A08335B503C2", manifest.GetProperty("PackageSha256").GetString());
                    Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.28.zip", manifest.GetProperty("PackagePath").GetString());
    ...
    public void ReleaseVerifierRejectsStalePackageHashRows()
    {
        ...
        try
        {
            ...
            foreach (var fileNode in packageHashesNode["Files"]!.AsArray())
            {
                ...
                if (path != "publish\\SpirePlus-v0.1.0-private-beta.28\\EZMicroBalance\\EZMicroBalance.dll")
                {
            ...
            Assert.Contains("SpirePlus-v0.1.0-private-beta.28\\\\EZMicroBalance\\\\EZMicroBalance.dll", verifyResult.Output, StringComparison.Ordinal);
        }
        finally
        {
            if (Directory.Exists(evidenceDir))
    ...
    public void AncientUiEvidenceHelperCreatesPendingPreparePlansWithoutLaunching()
    {
        ...
        })
        {
            ...
            try
            {
                ...
                Assert.Contains(files, file => file.GetProperty("Path").GetString() == "publish\\SpirePlus-v0.1.0-private-beta.28.zip");
