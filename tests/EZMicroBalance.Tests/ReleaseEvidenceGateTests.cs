...
public sealed class ReleaseEvidenceGateTests
{
    ...
    private static void AssertPackageHashesUseVersionedArtifacts(string packageHashesPath)
    {
        ...
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.7.zip", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.7\\EZMicroBalance\\EZMicroBalance.dll", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.7\\EZMicroBalance\\EZMicroBalance.pck", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.7\\EZMicroBalance\\EZMicroBalance.json", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.7\\EZMicroBalance\\README_INSTALL.txt", paths);
        ...
        foreach (var file in files.Where(file => file.GetProperty("Path").GetString()?.StartsWith("publish\\SpirePlus-v0.1.0-private-beta.7", StringComparison.Ordinal) == true))
        {
    ...
    public void CurrentManualTestHandoffScriptCreatesAllPendingEvidenceSections()
    {
        ...
        try
        {
            ...
            Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.7.zip", summary.GetProperty("PackagePath").GetString());
            Assert.Equal("4E838C7EE1A790E65C045A3D72CE572450E66CA54E31616F578D342AEF9F5DC6", summary.GetProperty("PackageSha256").GetString());
            ...
            Assert.Contains("ZIP: `publish\\SpirePlus-v0.1.0-private-beta.7.zip`.", startHere, StringComparison.Ordinal);
            Assert.Contains("ZIP SHA256: `4E838C7EE1A790E65C045A3D72CE572450E66CA54E31616F578D342AEF9F5DC6`.", startHere, StringComparison.Ordinal);
    ...
    public void CurrentManualTestHandoffArchivesStaleLoaderFilesWhenHashCannotBePreserved()
    {
        ...
        try
        {
            ...
            var packageRow = packageHashes["Files"]!.AsArray()
                .Single(row => (string?)row?["Path"] == "publish\\SpirePlus-v0.1.0-private-beta.7.zip")!
                .AsObject();
        ...
        })
        {
            ...
            try
            {
                ...
                if (scriptName == "collect-release-evidence.ps1")
                {
                    ...
                    Assert.Equal("4E838C7EE1A790E65C045A3D72CE572450E66CA54E31616F578D342AEF9F5DC6", manifest.GetProperty("PackageSha256").GetString());
                    Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.7.zip", manifest.GetProperty("PackagePath").GetString());
    ...
    public void ReleaseVerifierRejectsStalePackageHashRows()
    {
        ...
        try
        {
            ...
            foreach (var fileNode in packageHashesNode["Files"]!.AsArray())
            {
                ...
                if (path != "publish\\SpirePlus-v0.1.0-private-beta.7\\EZMicroBalance\\EZMicroBalance.dll")
                {
            ...
            Assert.Contains("SpirePlus-v0.1.0-private-beta.7\\\\EZMicroBalance\\\\EZMicroBalance.dll", verifyResult.Output, StringComparison.Ordinal);
        }
        finally
        {
            if (Directory.Exists(evidenceDir))
    ...
    public void AncientUiEvidenceHelperCreatesPendingPreparePlansWithoutLaunching()
    {
        ...
        })
        {
            ...
            try
            {
                ...
                Assert.Contains(files, file => file.GetProperty("Path").GetString() == "publish\\SpirePlus-v0.1.0-private-beta.7.zip");
