...
public sealed class ReleaseEvidenceGateTests
{
    ...
    private static void AssertPackageHashesUseVersionedArtifacts(string packageHashesPath)
    {
        ...
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.20.zip", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.20\\EZMicroBalance\\EZMicroBalance.dll", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.20\\EZMicroBalance\\EZMicroBalance.pck", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.20\\EZMicroBalance\\EZMicroBalance.json", paths);
        Assert.Contains("publish\\SpirePlus-v0.1.0-private-beta.20\\EZMicroBalance\\README_INSTALL.txt", paths);
        ...
        foreach (var file in files.Where(file => file.GetProperty("Path").GetString()?.StartsWith("publish\\SpirePlus-v0.1.0-private-beta.20", StringComparison.Ordinal) == true))
        {
    ...
    public void CurrentManualTestHandoffScriptCreatesAllPendingEvidenceSections()
    {
        ...
        try
        {
            ...
            Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.20.zip", summary.GetProperty("PackagePath").GetString());
            Assert.Equal("D15A0B21CB97AC498AE015116D230601F7F42E3EFB8CDF37D3226258FB1E52AD", summary.GetProperty("PackageSha256").GetString());
            ...
            Assert.Contains("ZIP: `publish\\SpirePlus-v0.1.0-private-beta.20.zip`.", startHere, StringComparison.Ordinal);
            Assert.Contains("ZIP SHA256: `D15A0B21CB97AC498AE015116D230601F7F42E3EFB8CDF37D3226258FB1E52AD`.", startHere, StringComparison.Ordinal);
    ...
    public void CurrentManualTestHandoffArchivesStaleLoaderFilesWhenHashCannotBePreserved()
    {
        ...
        try
        {
            ...
            var packageRow = packageHashes["Files"]!.AsArray()
                .Single(row => (string?)row?["Path"] == "publish\\SpirePlus-v0.1.0-private-beta.20.zip")!
                .AsObject();
        ...
        })
        {
            ...
            try
            {
                ...
                if (scriptName == "collect-release-evidence.ps1")
                {
                    ...
                    Assert.Equal("D15A0B21CB97AC498AE015116D230601F7F42E3EFB8CDF37D3226258FB1E52AD", manifest.GetProperty("PackageSha256").GetString());
                    Assert.Equal("publish\\SpirePlus-v0.1.0-private-beta.20.zip", manifest.GetProperty("PackagePath").GetString());
    ...
    public void ReleaseVerifierRejectsStalePackageHashRows()
    {
        ...
        try
        {
            ...
            foreach (var fileNode in packageHashesNode["Files"]!.AsArray())
            {
                ...
                if (path != "publish\\SpirePlus-v0.1.0-private-beta.20\\EZMicroBalance\\EZMicroBalance.dll")
                {
            ...
            Assert.Contains("SpirePlus-v0.1.0-private-beta.20\\\\EZMicroBalance\\\\EZMicroBalance.dll", verifyResult.Output, StringComparison.Ordinal);
        }
        finally
        {
            if (Directory.Exists(evidenceDir))
    ...
    public void AncientUiEvidenceHelperCreatesPendingPreparePlansWithoutLaunching()
    {
        ...
        })
        {
            ...
            try
            {
                ...
                Assert.Contains(files, file => file.GetProperty("Path").GetString() == "publish\\SpirePlus-v0.1.0-private-beta.20.zip");
