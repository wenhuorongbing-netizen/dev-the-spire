...
public sealed partial class ReleaseEvidenceGateTests
{
    ...
    public void DefaultModeDoesNotRequireLiveEvidenceButBlocksReleaseReadyClaims()
    {
        ...
        Assert.Contains("Older beta.85/beta.86/beta.87/beta.88/beta.90/beta.123 rows are previous-package or previous-game-version context only.", projectState, StringComparison.Ordinal);
        Assert.Contains("Previous beta.96 RitsuLib-only Off proof", projectState, StringComparison.Ordinal);
        ...
        Assert.Contains("Found 22 previous saved-state registrations", projectState, StringComparison.OrdinalIgnoreCase);
    }
}
