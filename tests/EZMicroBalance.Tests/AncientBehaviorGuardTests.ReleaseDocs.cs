...
public sealed partial class AncientBehaviorGuardTests
{
    ...
    public void ReleaseChecklistKeepsPendingRuntimeGatesAndManualRowsExplicit()
    {
        ...
        Assert.Contains("Status: automated gates passed for the current RitsuLib-only beta.134 no-launch package shape; previous beta.128 clicked UI smoke is retained separately.", manualMatrix, StringComparison.Ordinal);
        Assert.Contains("Previous beta.128 clicked Ancient UI smoke is captured under `.tools/runtime-evidence/monkey-stability-20260623-062913/`", manualMatrix, StringComparison.Ordinal);
